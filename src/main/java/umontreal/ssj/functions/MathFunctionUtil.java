...
public class MathFunctionUtil {
   ...
   /**
    * Returns the first derivative of the function `func` evaluated at `x`. If the
    * given function implements
    * 
    * @ref MathFunctionWithFirstDerivative, this method calls
    *      {@link MathFunctionWithFirstDerivative.derivative() derivative(double)}.
    *      Otherwise, if the function implements
    * @ref MathFunctionWithDerivative, this method calls
    *      {@link MathFunctionWithDerivative.derivative() derivative(double, int)}.
    *      If the function does not implement any of these two interfaces, the
    *      method uses {@link #finiteCenteredDifferenceDerivative()
   ...
   /**
    * Returns the @f$n@f$th derivative of function `func` evaluated at `x`.
    * If @f$n=0@f$, this returns @f$f(x)@f$. If @f$n=1@f$, this calls
    * {@link #derivative() derivative(MathFunction, double)} and returns the
    * resulting first derivative. Otherwise, if the function implements @ref
    * MathFunctionWithDerivative, this method calls
    * {@link MathFunctionWithDerivative.derivative() derivative(double, int)}. If
    * the function does not implement this interface, the method uses
    * {@link #finiteCenteredDifferenceDerivative()
    * finiteCenteredDifferenceDerivative(MathFunction, double, int, double)}
    * if @f$n@f$ is even, or {@link #finiteDifferenceDerivative()
    * finiteDifferenceDerivative(MathFunction, double, int, double)} if
    * 
    * @f$n@f$ is odd, to obtain a numerical approximation of the derivative.
    * @param func the function to derivate.
    * @param x    the evaluation point.
    ...
    * function @f$f(x)@f$. This method estimates @f[ \frac{d^nf(x)}{dx^n}, @f]
    * the @f$n@f$th derivative of @f$f(x)@f$ evaluated at @f$x@f$. This method
    * first computes @f$f_i=f(x+i\epsilon)@f$, for @f$i=0,…,n@f$,
    * with @f$\epsilon=h^{1/n}@f$. The estimate is then given by
    * 
    * @f$\Delta^nf_0/h@f$, where @f$\Delta^nf_i=\Delta^{n-1}f_{i+1} -
    *                      \Delta^{n-1}f_i@f$, and @f$\Delta f_i = f_{i+1} -
    *                      f_i@f$.
    * @param func the function to derivate.
    * @param x    the evaluation point.
    * @param n    the order of the derivative.
   ...
   /**
    * Returns @f$(f(x + h) - f(x - h))/(2h)@f$, an estimate of the first derivative
    * of @f$f(x)@f$ using centered differences.
    * 
    * @param func the function to derivate.
    * @param x    the evaluation point.
    * @param h    the error.
   ...
   /**
    * Computes and returns an estimate of the @f$n@f$th derivative of the
    * function @f$f(x)@f$ using finite centered differences. If @f$n@f$ is even,
    * this method returns
    * {@link #finiteDifferenceDerivative(MathFunction,double,int,double)
    * finiteDifferenceDerivative(func, x - \epsilon*n/2, n, h)}, with
    * 
    * @f$h=\epsilon^n@f$. @param func the function to derivate.
    * @param x the evaluation point.
    * @param n the order of the derivative.
    * @param h the error.
    * @return the estimate of the derivative.
   ...
   /**
    * Removes any point `(NaN, y)` or `(x, NaN)` from `x` and `y`, and returns a 2D
    * array containing the filtered points. This method filters each pair
    * (<tt>x[i]</tt>, <tt>y[i]</tt>) containing at least one NaN element. It
    * constructs a 2D array containing the two filtered arrays, whose size is
    * smaller than or equal to `x.length`.
    * 
   ...
   /**
    * Returns the integral of the function `func` over @f$[a, b]@f$. If the given
    * function implements @ref MathFunctionWithIntegral, this returns
    * {@link umontreal.ssj.functions.MathFunctionWithIntegral.integral()
    * integral(double, double)}. Otherwise, this calls {@link #simpsonIntegral()
    * simpsonIntegral(MathFunction, double, double, int)} with #NUMINTERVALS
    * intervals.
    * 
   ...
   /**
    * Computes and returns an approximation of the integral of `func` over @f$[a,
    * b]@f$, using the Simpson’s @f$1/3@f$ method with `numIntervals` intervals.
    * This method estimates @f[ \int_a^b f(x)dx, @f] where @f$f(x)@f$ is the
    * function defined by `func` evaluated at
    * 
    * @f$x@f$, by dividing @f$[a, b]@f$ in @f$n=@f$&nbsp;`numIntervals` intervals
    *          of length @f$h=(b - a)/n@f$. The integral is estimated by @f[
    *          \frac{h}{3}(f(a)+4f(a+h)+2f(a+2h)+4f(a+3h)+\cdots+f(b)) @f] This
    *          method assumes that @f$a\le b<\infty@f$, and @f$n@f$ is even.
    ...
    * 
    * @f$f(x)@f$ over @f$[a, b]@f$, using Gauss-Lobatto adaptive quadrature with 5
    *            nodes, with tolerance `tol`. This method estimates @f[ \int_a^b
    *            f(x)dx, @f] where @f$f(x)@f$ is the function defined by `func`.
    *            Whenever the estimated error is larger than `tol`, the
    *            interval @f$[a, b]@f$ will be halved in two smaller intervals, and
    *            the method will recursively call itself on the two smaller
    *            intervals until the estimated error is smaller than `tol`.
    * @param func the function being integrated.
    * @param a    the left bound
    * @param b    the right bound.
    * @param tol  error.
   ...
   /**
    * Similar to method {@link #gaussLobatto() gaussLobatto(MathFunction, double,
    * double, double)}, but also returns in `T[0]` the subintervals of integration,
    * and in `T[1]`, the partial values of the integral over the corresponding
    * subintervals. Thus `T[0][0]` @f$= x_0 = a@f$ and `T[0][n]` @f$=x_n =b@f$;
    * `T[1][i]` contains the value of the integral over the
    * subinterval @f$[x_{i-1}, x_i]@f$; we also have `T[1][0]` @f$=0@f$. The sum
    * over all `T[1][i]`, for @f$i=1, …, n@f$ gives the value of the integral
    * over @f$[a,b]@f$, which is the value returned by this method. *WARNING:* The
    * user *must reserve* the 2 elements of the first dimension (<tt>T[0]</tt> and
    * <tt>T[1]</tt>) before calling this method.
    * 
    * @param func function being integrated
    * @param a    left bound of interval
    * @param b    right bound of interval
    * @param tol  error
    * @param T    @f$(x,y)@f$ = (values of partial intervals,partial values of
    *             integral)
