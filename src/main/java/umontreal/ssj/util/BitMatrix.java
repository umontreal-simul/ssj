...
public class BitMatrix implements Serializable, Cloneable {
   ...
   /**
    * Creates a new `BitMatrix` with `r` rows and `c` columns using the data in
    * `data`. Note that the orders of the bits for the rows are using the same
    * order than for the @ref BitVector. This does mean that the first bit is the
    * lowest order bit of the last `int` in the row and the last bit is the highest
    * order bit of the first `int` int the row.
    * 
   ...
   /**
    * Verifies that `this` and `that` are strictly identical. They must both have
    * the same dimensions and data.
    * 
   ...
   /**
    * Creates a String containing all the data of the `BitMatrix`. The result is
    * displayed in a matrix form, with each row being put on a different line. Note
    * that the bit at (0,0) is at the upper left of the matrix, while the bit at
    * (0) in a @ref BitVector is the least significant bit.
    * 
   ...
   /**
    * Creates a String containing all the data of the `BitMatrix`. The data is
    * displayed in the same format as are the `int[][]` in `Java` code. This allows
    * the user to print the representation of a `BitMatrix` to be put, directly in
    * the source code, in the constructor `BitMatrix(int[][], int, int)`. The
    * output is not designed to be human-readable.
    * 
    * @return the content of the `BitMatrix`
    */
   ...
   /**
    * Returns the value of the bit in the specified row and column. If the value is
    * 1, return `true`. If it is 0, return `false`.
    * 
    * @param row    the row of the selected bit
    * @param column the column of the selected bit
    * @return the value of the bit as a boolean
    *
    * @exception IndexOutOfBoundsException if the selected bit would be outside the
    *                                      `BitMatrix`
    */
   ...
   /**
    * Changes the value of the bit in the specified row and column. If `value` is
    * `true`, changes it to 1. If `value` is `false` changes it to 0.
    * 
    * @param row    the row of the selected bit
    * @param column the column of the selected bit
   ...
   /**
    * Returns the `BitMatrix` resulting from the application of the `not` operator
    * on the original `BitMatrix`. The effect is to swap all the bits of the
    * `BitMatrix`, turning all 0 into 1 and all 1 into 0.
    * 
   ...
   /**
    * Returns the `BitMatrix` resulting from the application of the `and` operator
    * on the original `BitMatrix` and `that`. Only bits which were at 1 in both
    * `BitMatrix` are set at 1 in the result. All others are set to 0.
    * 
    * @param that the second operand of the `and` operator
    * @return the result of the `and` operator
    *
    * @exception IncompatibleDimensionException if the two `BitMatrix` are not of
    *                                           the same dimensions
    */
   ...
   /**
    * Returns the `BitMatrix` resulting from the application of the `or` operator
    * on the original `BitMatrix` and `that`. Only bits which were at 0 in both
    * `BitMatrix` are set at 0 in the result. All others are set to 1.
    * 
   ...
   /**
    * Returns the `BitMatrix` resulting from the application of the `xor` operator
    * on the original `BitMatrix` and `that`. Only bits which were at 1 in only one
    * of the two `BitMatrix` are set at 1 in the result. All others are set to 0.
    * 
   ...
   /**
    * Multiplies the column @ref BitVector by a `BitMatrix` and returns the result.
    * The result is @f$A \times v@f$, where @f$A@f$ is the `BitMatrix`, and @f$v@f$
    * is the @ref BitVector.
    * 
   ...
   /**
    * Multiplies `vect`, seen as a column @ref BitVector, by a `BitMatrix`.
    * (See @ref BitVector to see the conversion between `int` and @ref BitVector.)
    * The result is @f$A \times v@f$, where
    * 
   ...
   /**
    * Multiplies two <tt>BitMatrix</tt>’s together. The result is @f$A \times
    * B@f$, where @f$A@f$ is the `this BitMatrix` and @f$B@f$ is the `that
    * BitMatrix`.
    * 
    * @param that the other `BitMatrix` to multiply
    * @return the result of the multiplication
    *
    * @exception IncompatibleDimensionException if the number of columns of the
    *                                           first `BitMatrix` is not equal to
    ...
    *
    * @exception IncompatibleDimensionException if the `BitMatrix` is not square
    ...
    * 
    * @param e the exponent of the power up to which to raise the `BitMatrix`
    * @return the power of the `BitMatrix`
    *
    * @exception IncompatibleDimensionException if the `BitMatrix` is not square
    */
   ...
   /**
    * Runtime exception raised when the dimensions of the `BitMatrix` are not
