...
public class TextDataReader {
   ...
   /**
    * Reads an array of double-precision values from the reader `input`. For each
    * line of text obtained from the given reader, this method trims whitespaces,
    * and parses the remaining text as a double-precision value. This method
    * ignores every character other than the digits, the plus and minus signs, the
    * period (<tt>.</tt>), and the letters `e` and `E`. Moreover, lines starting
    * with a pound sign (<tt>#</tt>) are considered as comments and thus skipped.
    * The method returns an array containing all the parsed values.
    * 
    ...
    * #readDoubleData(Reader) to obtain an array of double-precision values from
    * the resource.
    * 
   ...
   /**
    * Opens the file with name `file`, and calls #readDoubleData(Reader) to obtain
    * an array of double-precision values from the file.
    * 
   ...
   public static int[] readIntData(Reader input) throws IOException {
      ...
      while ((li = inb.readLine()) != null) {
         ...
         while (index < li.length()
               && (li.charAt(index) == '+' || li.charAt(index) == '-' || Character.isDigit(li.charAt(index))))
            ++index;
   ...
   /**
    * Connects to the URL referred to by the URL object `url`, and calls
    * #readIntData(Reader) to obtain an array of integers from the resource.
    * 
    * @param url the URL object representing the resource to read.
    * @return the obtained array of integers.
    *
   ...
   /**
    * Reads an array of strings from the reader `input`. For each line of text
    * obtained from the given reader, this method trims leading and trailing
    * whitespaces, and stores the remaining string. Lines starting with a pound
    * sign (<tt>#</tt>) are considered as comments and thus skipped. The method
    * returns an array containing all the read strings.
    * 
    * @param input the reader to obtain data from.
   ...
   /**
    * Connects to the URL referred to by the URL object `url`, and calls
    * #readStringData(Reader) to obtain an array of integers from the resource.
    * 
    * @param url the URL object representing the resource to read.
    * @return the obtained array of strings.
    *
   ...
   /**
    * Uses the reader `input` to obtain a 2-dimensional array of double-precision
    * values. For each line of text obtained from the given reader, this method
    * trims whitespaces, and parses the remaining text as an array of
    * double-precision values. Every character other than the digits, the plus
    * (<tt>+</tt>) and minus (<tt>-</tt>) signs, the period (<tt>.</tt>), and the
    * letters `e` and `E` are ignored and can be used to separate numbers on a
    * line. Moreover, lines starting with a pound sign (<tt>#</tt>) are considered
    * as comments and thus skipped. The lines containing only a semicolon sign
    * (<tt>;</tt>) are considered as empty lines. The method returns a 2D array
    * containing all the parsed values. The returned array is not always
    * rectangular.
    * 
    * @param input the reader to obtain data from.
      ...
      while ((li = inb.readLine()) != null) {
         ...
         if (li.equals(";")) {
            ...
            while (index < li.length() && (!end)) {
               while (index < li.length() && (li.charAt(index) == '+' || li.charAt(index) == '-'
                     || li.charAt(index) == 'e' || li.charAt(index) == 'E' || li.charAt(index) == '.'
   ...
   /**
    * Connects to the URL referred to by the URL object `url`, and calls
    * #readDoubleData2D(Reader) to obtain a matrix of double-precision values from
    * the resource.
    * 
    * @param url the URL object representing the resource to read.
   ...
   /**
    * Opens the file with name `file`, and calls #readDoubleData2D(Reader) to
    * obtain a matrix of double-precision values from the file.
    * 
    * @param file the name of the file to read.
    * @return the obtained matrix of double-precision values.
    *
   ...
   /**
    * Connects to the URL referred to by the URL object `url`, and calls
    * #readDoubleData(Reader) to obtain a matrix of integers from the resource.
    * 
    * @param url the URL object representing the resource to read.
   ...
   /**
    * Reads comma-separated values (CSV) from reader `input`, and returns a 2D
    * array of strings corresponding to the read data. Lines are delimited using
    * line separators <tt>\\r</tt>, <tt>\\n</tt>, and <tt>\\r\\n</tt>. Each line
    * contains one or more values, separated by the column delimiter `colDelim`. If
    * a string of characters is surrounded with the string delimiter `stringDelim`,
    * any line separator and column separator appear in the string. The string
    * delimiter can be inserted in such a string by putting it twice. Usually, the
    * column delimiter is the comma, and the string delimiter is the quotation
    * mark. The following example uses these default delimiters. <tt>
    * <pre>
    *          "One","Two","Three"
    *           1,2,3
    ...
    * 
    * @f$3\times3@f$. The first row contains the strings `One`, `Two`, and `Three`
    *                 while the second row contains the strings `1`, `2`, and `3`.
    *                 The first column of the last row contains the string `String
    *                 with " delimiter`.
    * @param input       the reader to obtain data from.
   ...
   /**
    * Connects to the URL referred to by the URL object `url`, and calls
    * #readCSVData(Reader,char,char) to obtain a matrix of strings from the
    * resource.
    * 
    * @param url         the URL object representing the resource to read.
    * @param colDelim    the column delimiter.
