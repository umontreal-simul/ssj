...
public class Num {
   ...
   /**
    * Largest `int` @f$x@f$ such that @f$10^x@f$ is representable (approximately)
    * as a `double`.
    */
   ...
   /**
    * Largest integer @f$n_0 = 2^{53}@f$ such that any integer @f$n \le n_0@f$ is
   ...
   /**
    * Contains the precomputed positive powers of 2. One has <tt>TWOEXP[j]</tt>@f$
    * = 2^j@f$, for @f$j=0,…,64@f$.
    */
   ...
   /**
    * Returns the value of @f$\binom{n}{s}@f$, the number of different combinations
    * of @f$s@f$ objects amongst
    * 
    * @f$n@f$. @param n total number of objects
    ...
    * 
    * @f$\binom{n}{s}@f$, the number of different combinations of @f$s@f$ objects
    *                     amongst
    * @f$n@f$. @param n total number of objects
    * @param s number of chosen objects on a combination
    ...
    * 
    * @anchor REF_util_Num_Stirling2 @f[ M[i,j] = \left\{\begin{matrix} j \\ i
    *         \end{matrix}\right\} \quad\mbox{ for $0\le i\le m$ and $0\le i\le
    *         j\le n$}. \tag{Stirling2} @f] See @cite iKNU73a&thinsp; (Section
    *         1.2.6). The matrix @f$M@f$ is the transpose of Knuth’s (1973).
   ...
   /**
    * Returns the natural logarithm of the gamma function @f$\Gamma(x)@f$ evaluated
    * at `x`. Gives 16 decimals of precision, but is implemented only
    * for @f$x>0@f$.
    * 
    ...
    * is defined in terms of the Gamma function as @f[ B(\lambda, \nu) =
    * \frac{\Gamma(\lambda)\Gamma(\nu)}{\Gamma(\lambda+ \nu)} @f] with
    * `lam` @f$=\lambda@f$ and `nu` @f$=\nu@f$.
    */
   ...
   /**
    * Returns the value of the trigamma function @f$d\psi(x)/dx@f$, the derivative
    * of the digamma function, evaluated at @f$x@f$.
    */
   ...
   /**
    * Returns the value of the tetragamma function @f$d^2\psi(x)/d^2x@f$, the
    * second derivative of the digamma function, evaluated at @f$x@f$.
    */
   ...
   /**
    * Returns the value of the ratio @f$\Gamma(x+1/2)/\Gamma(x)@f$ of two gamma
    ...
    * 
    * @f$n@f$ elements of @f$A@f$ and returns the sum. This algorithm is more
    *         precise than the naive algorithm. See
   ...
   /**
    * Computes the sum @f[ \sideset{}’\sum_{-n/2<j\le n/2}\; \frac{1}{|j|}, @f]
   ...
   /**
    * Returns the volume @f$V@f$ of a sphere of radius 1 in @f$t@f$ dimensions
    * using the norm @f$L_p@f$. It is given by the formula @f[ V = \frac{\left[2
    * \Gamma(1 + 1/p)\right]^t}{\Gamma\left(1 + t/p\right)}, \qquad p > 0, @f]
    ...
    * 
    * @f$x@f$. Only degrees @f$n\le8@f$ are programmed for now. The first Bernoulli
    *          polynomials of even degree are:
    * @anchor REF_util_Num_bernoulli
    * @f{align}{ B_0(x) & = 1 \nonumber \\ B_2(x) & = x^2-x+1/6 \nonumber \\ B_4(x)
    *            & = x^4-2x^3+x^2-1/30 \tag{bernoulli} \\ B_6(x) & =
   ...
   /**
    * Evaluates a series of Chebyshev polynomials @f$T_j@f$ at @f$x@f$ over the
    * basic interval @f$[-1, \;1]@f$, using the method of Clenshaw @cite
    * mCLE62a&thinsp;, i.e., computes and returns @f[ y = \frac{a_0}{2} +
    * \sum_{j=1}^n a_j T_j (x). @f]
    * 
    ...
    *         Clenshaw @cite mCLE62a&thinsp;, i.e., computes and returns @f[ y =
    *         \frac{a_0}{2} + \sum_{j=1}^n a_j T_j^* (x). @f]
    * @param a coefficients of the polynomials
    * @param n largest degree of polynomials
    * @param x the parameter of the @f$T_j^*@f$ functions
    * @return the value of a series of Chebyshev polynomials @f$T_j^*@f$.
    ...
    * 
    * @f$K_{\nu}@f$ is the modified Bessel’s function of the second kind. The
    *               relative error on the returned value is less
    *               than @f$0.5\times10^{-6}@f$ for
    * @f$x > 10^{-300}@f$.
   ...
   /**
    * Returns the value of @f$e^x K_1(y)@f$, where @f$ K_1@f$ is the modified
    * Bessel function of the second kind of order 1. Restriction:
    * 
    * @f$y > 0@f$.
    */
   ...
   /**
    * Returns the value of <tt>erf</tt>(@f$x@f$), the error function. It is defined
    * as @f[ \mbox{erf}(x) = \frac{2}{\sqrt{\pi}}\int_0^x dt  e^{-t^2}. @f]
    * 
    * @param x value at which the function is calculated
    ...
    * function. It is defined as @f[ \mbox{erfc}(x) =
    * \frac{2}{\sqrt{\pi}}\int_x^{\infty}dt  e^{-t^2}. @f]
    * 
    * @param x value at which the function is calculated
   ...
   /**
    * Returns the value of <tt>erf</tt>@f${}^{-1}(u)@f$, the inverse of the error
    * function. If @f$u = @f$<tt>erf</tt>@f$(x)@f$, then @f$x =
    * 
    * @f$<tt>erf</tt>@f${}^{-1}(u)@f$.
   ...
   /**
    * Returns the value of <tt>erfc</tt>@f${}^{-1}(u)@f$, the inverse of the
    * complementary error function. If @f$u =
    * 
