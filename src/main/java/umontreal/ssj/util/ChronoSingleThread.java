...
/**
 * Extends the @ref AbstractChrono class to compute the CPU time for a single
 * thread. This uses the getThreadCpuTime function from a @ref ThreadMXBean
 * object, from @ref java.lang.management.ManagementFactory. If the associated
 * thread is dead, the returned time will be 0.
 *
...
public class ChronoSingleThread extends AbstractChrono {
   ...
   protected long getTime() {
      if (threadMXBean == null) {
         // We use lazy initialization to avoid a potential exception being wrapped into
         // a confusing
         // ExceptionInInitializerError. That would happen if this initialization was in
         // a static block instead of
         // in this method.
   ...
   /**
    * Constructs a `ChronoSingleThread` object associated with current thread and
   ...
   /**
    * Constructs a `ChronoSingleThread` object associated with the given Thread
