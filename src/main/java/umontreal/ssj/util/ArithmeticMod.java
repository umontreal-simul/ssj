 ...
 * different versions. These allow operations on `double`, `int` and `long`. The
 * `int` and `long` versions work exactly like the `double` ones.
 *
...
public class ArithmeticMod {
    ...
    * than @f$2^{35}@f$. Works also if `s` or `c` are negative. The result is
    * always positive (and thus always between 0 and `m` - 1).
    * 
   ...
   /**
    * Computes the result of @f$\mathtt{A} \times\mathtt{\mathbf{s}} \bmod m@f$ and
    * puts the result in `v`. Where `s` and `v` are both column vectors. This
    * method works even if `s` = `v`.
    * 
   ...
   /**
    * Computes @f$\mathtt{A} \times\mathtt{B} \bmod m@f$ and puts the result in
    * `C`. Works even if `A` = `C`, `B` = `C` or `A` = `B` = `C`.
    * 
    * @param A the first factor of the multiplication
   ...
   /**
    * Computes @f$\mathtt{A}^{2^{\mathtt{e}}} \bmod m@f$ and puts the result in
    * `B`. Works even if `A` = `B`.
    * 
   ...
   /**
    * Computes @f$\mathtt{A}^c \bmod m@f$ and puts the result in `B`. Works even if
    * `A` = `B`.
    * 
    * @param A the matrix to raise to a power
   ...
   /**
    * Computes @f$(a \times s + c) \bmod m@f$. Works also if `s` or `c` are
    * negative. The result is always positive (and thus always between 0 and `m` -
    * 1).
    * 
    * @param a the first factor of the multiplication
    * @param s the second factor of the multiplication
   ...
   /**
    * Exactly like #matMatModM(double[][],double[][],double[][],double) using
   ...
   /**
    * Exactly like #matVecModM(double[][],double[],double[],double) using `double`,
    * but with `long` instead of `double`.
    * 
    * @param A the multiplication matrix
    * @param s the multiplied vector
    * @param v the result of the multiplication
