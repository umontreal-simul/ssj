 ...
 * methods. It defines certain functionalities of the ANSI C `printf` function
 * that also can be accessed through static methods. The information given here
 * is strongly inspired from the `man` page of the C `printf` function.
 *
 * Most methods of this class format numbers for the English US locale only. One
 * can use the Java class Formatter for performing locale-independent
 * formatting.
 *
...
public class PrintfFormat implements CharSequence, Appendable {
   ...
   /**
    * Uses the #s(int,String) static method to append `str` to the buffer. A
    * minimum of `fieldwidth` characters will be used.
    * 
    * @param fieldwidth minimum number of characters that will be added to the
    *                   buffer
    * @param str        string to append to the buffer
    * @return this object
    */
   ...
   /**
    * Uses the #f(int,double) static method to append `x` to the buffer. A minimum
    * of `fieldwidth` characters will be used.
    * 
   ...
   /**
    * Uses the #f(int,int,double) static method to append `x` to the buffer. A
    * minimum of `fieldwidth` characters will be used with the given `precision`.
    * 
    * @param fieldwidth minimum length of the converted string to be appended
    * @param precision  number of digits after the decimal point of the converted
    *                   value
    * @param x          value to be appended to the buffer
    * @return this object
    */
   ...
   /**
    * Uses the #d(int,long) static method to append `x` to the buffer. A minimum of
    * `fieldwidth` characters will be used.
    * 
    * @param fieldwidth minimum length of the converted string to be appended
    * @param x          value to be appended to the buffer
    * @return this object
   ...
   /**
    * Uses the #format(int,int,int,double) static method with the same four
    * arguments to append `x` to the buffer.
    * 
    * @param fieldwidth minimum length of the converted string to be appended
   ...
   /**
    * Same as {@link #s(int,String) s(0, str)}. If the string `str` is null, it
   ...
   /**
    * Formats the string `str` like the <tt>%s</tt> in the C `printf` function. The
    * `fieldwidth` argument gives the minimum length of the resulting string. If
    * `str` is shorter than `fieldwidth`, it is left-padded with spaces. If
    * `fieldwidth` is negative, the string is right-padded with spaces if
    * necessary. The String will never be truncated. If `str` is null, it calls
    * {@link #s(int,String) s(fieldwidth, "null")}. The `fieldwidth` argument has
    * the same effect for the other methods in this class.
    * 
   ...
   /**
    * Formats the long integer `x` into a string like <tt>%d</tt> in the C `printf`
    * function. It converts its argument to decimal notation, `precision` gives the
    * minimum number of digits that must appear; if the converted value requires
    * fewer digits, it is padded on the left with zeros. When zero is printed with
   ...
   /**
    * Formats a double-precision number `x` like <tt>%E</tt> in C <tt>printf</tt>.
    ...
    * digits after it is equal to the precision; if the precision is 0, no
    * decimal-point character appears. The exponent always contains at least two
    ...
    * `printf`. The argument is rounded and converted to decimal notation in the
    ...
    * explicitly 0, no decimal-point character appears. If a decimal point appears,
    ...
    * `printf`. The argument is converted in style <tt>%f</tt> or <tt>%E</tt>.
    * `precision` specifies the number of significant digits. If it is 0, it is
    * treated as 1. Style <tt>%E</tt> is used if the exponent from its conversion
    * is less than @f$-4@f$ or greater than or equal to `precision`. Trailing zeros
    * are removed from the fractional part of the result; a decimal point appears
   ...
   /**
    * Returns a String containing `x`. Uses a total of at least `fieldwidth`
    * positions (including the sign and point when they appear), `accuracy` digits
    * after the decimal point and at least `precision` significant digits.
    * `accuracy` and `precision` must be strictly smaller than `fieldwidth`. The
    * number is rounded if necessary. If there is not enough space to format the
    * number in decimal notation with at least `precision` significant digits
    * (<tt>accuracy</tt> or `fieldwidth` is too small), it will be converted to
    * scientific notation with at least `precision` significant digits. In that
    * case, `fieldwidth` is increased if necessary.
    * 
    * @param fieldwidth minimum length of the returned string
    * @param accuracy   number of digits after the decimal point
    * @param precision  number of significant digits
   ...
   /**
    * This method is equivalent to #format(int,int,int,double), except it formats
    * the given value for the locale `locale`.
    * 
   ...
   /**
    * Converts @f$x@f$ to a String representation in base @f$b@f$ using formatting
    * similar to the @f$f@f$ methods. Uses a total of at least `fieldwidth`
    * positions (including the sign and point when they appear) and `accuracy`
    * digits after the decimal point. If `fieldwidth` is negative, the number is
    * printed left-justified, otherwise right-justified. Restrictions: @f$2 \le b
    * \le10@f$ and @f$|x| < 2^{63}@f$.
    * 
    * @param fieldwidth minimum length of the returned string
    * @param accuracy   number of digits after the decimal point
    * @param b          base
   ...
   /**
    * Stores a string containing `x` into `res[0]`, and a string containing `error`
    * into `res[1]`, both strings being formatted with the same notation. Uses a
    * total of at least `fieldwidth` positions (including the sign and point when
    * they appear) for `x`, `fieldwidtherr` positions for `error`, `accuracy`
    * digits after the decimal point and at least `precision` significant digits.
    * `accuracy` and `precision` must be strictly smaller than `fieldwidth`. The
    * numbers are rounded if necessary. If there is not enough space to format `x`
    * in decimal notation with at least `precision` significant digits
    * (<tt>accuracy</tt> or `fieldwidth` are too small), it will be converted to
    * scientific notation with at least `precision` significant digits. In that
    * case, `fieldwidth` is increased if necessary, and the error is also formatted
    * in scientific notation.
    * 
    * @param fieldwidth    minimum length of the value string
    * @param fieldwidtherr minimum length of the error string
    * @param accuracy      number of digits after the decimal point for the value
    *                      and error
    * @param precision     number of significant digits for the value
    * @param x             the value to be processed
    * @param error         the error on the value to be processed
    * @param res           an array that will be filled with the formatted value
    *                      and formatted error
    */
   ...
   /**
    * Stores a string containing `x` into `res[0]`, and a string containing `error`
    * into `res[1]`, both strings being formatted with the same notation. This
    * calls #formatWithError(int,int,int,int,double,double,String[]) with the
    * minimal accuracy for which the formatted string for `error` is non-zero. If
    * `error` is 0, the accuracy is 0. If this minimal accuracy causes the strings
    * to be formatted using scientific notation, this method increases the accuracy
    * until the decimal notation can be used.
    * 
    * @param fieldwidth    minimum length of the value string
    * @param fieldwidtherr minimum length of the error string
    * @param precision     number of significant digits for the value
    ...
    * #formatWithError(int,int,int,double,double,String[]), except that it formats
    * the given value and error for the locale `locale`.
    * 
