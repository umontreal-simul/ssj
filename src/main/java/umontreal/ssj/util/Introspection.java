...
public class Introspection {
   ...
   /**
    * Returns all the methods declared and inherited by a class. This is similar to
    * java.lang.Class.getMethods except that it enumerates non-public methods as
    * well as public ones. This method uses java.lang.Class.getDeclaredMethods to
    * get the declared methods of `c`. It also gets the declared methods of
    * superclasses. If a method is defined in a superclass and overriden in a
    * subclass, only the overridden method will be in the returned array.
    *
    * Since this method uses java.lang.Class.getDeclaredMethods, it can throw a
   ...
   private static void removeByNameAndSignature(List<Method> methods, Method m) {
      for (ListIterator<Method> it = methods.listIterator(); it.hasNext();) {
         ...
         if (tst == null)
            continue;
         if (tst.getName().equals(m.getName()) && tst.getReturnType() == m.getReturnType() && sameSignature(tst, m))
            it.set(null);
      }
   }
   ...
   /**
    * Returns all the fields declared and inherited by a class. This is similar to
    * java.lang.Class.getFields except that it enumerates non-public fields as well
    * as public ones. This method uses java.lang.Class.getDeclaredFields to get the
    * declared fields of `c`. It also gets the declared fields of superclasses and
    * implemented interfaces.
    *
    * Note that since this method uses java.lang.Class.getDeclaredFields, it can
    * throw a SecurityException if a security manager is present.
    * 
    * @param c the class being processed.
   ...
   private static void processInterfaceFields(final Class<?> c, final List<Field> lst,
         final Set<Class<?>> traversedInterfaces) {
      Class<?>[] iface = c.getInterfaces();
      for (int i = 0; i < iface.length; i++) {
   ...
   /**
    * This is like java.lang.Class.getMethod, except that it can return non-public
    * methods.
    * 
    * @param c    the class being processed.
   ...
   public static Method getMethod(Class<?> c, String name, Class[] pt) throws NoSuchMethodException {
      ...
      throw new NoSuchMethodException("Cannot find method " + name + " in class " + c.getName());
   }

   /**
    * This is like java.lang.Class.getField, except that it can return non-public
    * fields.
    *
    * Note that since this method uses java.lang.Class.getDeclaredField, it can
    * throw a SecurityException if a security manager is present.
    * 
    * @param c    the class being processed.
    * @param name the name of the method.
   ...
   public static Field getField(Class<?> c, String name) throws NoSuchFieldException {
      ...
      throw new NoSuchFieldException("Cannot find field " + name + " in " + c.getName());
   }

   /**
    * Returns the field name corresponding to the value of an enumerated type
    * `val`. This method gets the class of `val` and scans its fields to find a
    * public static and final field containing `val`. If such a field is found, its
   ...
   /**
    * Returns the field of class `cls` corresponding to the name `name`. This
    * method looks for a public, static, and final field with name `name` and
    * returns its value. If no appropriate field can be found, an
    ...
    *
    * @exception IllegalArgumentException if `name` does not correspond to a valid
    *                                     field name.
    */
   ...
   /**
    * Similar to \ref #valueOf(Class<T>,String), with case insensitive field name
    * look-up. If `cls` defines several fields with the same case insensitive name
    * `name`, an IllegalArgumentException is thrown.
    * 
    * @param cls  the class to look for a field in.
    * @param name the name of field.
    * @return the object in the field.
    *
    * @exception IllegalArgumentException if `name` does not correspond to a valid
    *                                     field name, or if the class defines
   ...
   public static <T> T valueOfIgnoreCase(Class<T> cls, String name) {
      ...
      for (int i = 0; i < fields.length; i++) {
         ...
         if (field.getName().equalsIgnoreCase(name) && Modifier.isStatic(field.getModifiers())
               && Modifier.isFinal(field.getModifiers()) && cls.isAssignableFrom(field.getType()))
            try {
               T res2 = (T) field.get(null);
               if (res != null && res2 != null)
                  throw new IllegalArgumentException("Found more than one field with the same name in class "
                        + cls.getName() + " if case is ignored");
               res = res2;
            } catch (IllegalAccessException iae) {
            }
      }
