...
public class DMatrix {
   ...
   /**
    * Creates a new `DMatrix` with `r` rows and `c` columns using the data in
    * `data`.
    * 
   ...
   /**
    * Given a symmetric positive-definite matrix @f$M@f$, performs the Cholesky
    * decomposition of @f$M@f$ and returns the result as a lower triangular
    * matrix @f$L@f$, such that @f$M = L L^T@f$.
    * 
    ...
    * 
    * @f$U\Lambda U^{\mathsf{t}}@f$ by using the singular value decomposition of
    *             matrix @f$M@f$. Puts the eigenvalues, which are the diagonal
    *             elements of matrix @f$\Lambda@f$, sorted by decreasing size, in
    *             vector `lambda`, and puts matrix @f$A = U\sqrt{\Lambda}@f$ in
    *             `A`.
    ...
    *             matrix @f$M@f$. Puts the eigenvalues, which are the diagonal
    *             elements of matrix @f$\Lambda@f$, sorted by decreasing size, in
    *             vector `lambda`. Returns matrix @f$A = U\sqrt{\Lambda}@f$.
    * @param M      input matrix
    * @param lambda the eigenvalues
    ...
    * 
    * @f$A@f$ is a square matrix, @f$b@f$ and @f$x@f$ are vectors. Returns the
    *         solution @f$x@f$.
    ...
    * 
    * @f$U@f$ is a square upper triangular matrix. @f$B@f$ and @f$X@f$ must have
   ...
   /**
    * Returns @f$e^A@f$, the exponential of the square matrix @f$A@f$. The scaling
    * and squaring method @cite mHIG09a is used with Padé approximants to compute
    * the exponential.
    * 
    * @param A input matrix
    * @return the exponential of @f$A@f$
    ...
    * 
    * @f$A@f$. The only non-zero elements of @f$A@f$ are on the diagonal and on the
    *          first superdiagonal. This method is faster than \ref #exp(final
    *          DoubleMatrix2D) because of the special form of
    * @f$A@f$. @param A bidiagonal matrix
    * @return @f$e^A@f$
    */
    ...
    * 
    * @f$A@f$ are on the diagonal and on the first superdiagonal. Uses the scaling
    *         and squaring method @cite mHIG09a with Padé approximants.
    *         Returns @f$c@f$.
    ...
    * 
    * @f$A@f$ are on the diagonal and on the first superdiagonal. Uses the scaling
    *         and squaring method @cite mSKA09a, @cite mHIG09a&thinsp; with Padé
    *         approximants. Returns @f$e^A - I@f$.
    * @param A bidiagonal matrix
    * @return @f$(e^A - I)b@f$
    */
   ...
   /**
    * Computes @f$c = (e^A - I)b@f$, where @f$e^A@f$ is the exponential of the
    * *bidiagonal* square matrix @f$A@f$. The only non-zero elements of @f$A@f$ are
    * on the diagonal and on the first superdiagonal. Uses the scaling and squaring
    * method @cite mSKA09a, @cite mHIG09a&thinsp; with a Taylor expansion.
    * Returns @f$c@f$.
    * 
    * @param A bidiagonal matrix
    * @param b vector
    ...
    * @f$B@f$ which has a bandwidth of `sb + 1`. The resulting matrix @f$C@f$ has
    *         `sa + sb` non-zero superdiagonals, and a bandwidth of `sa + sb + 1`.
    * @param A  input matrix
   ...
   /**
    * Multiplication of the matrix @f$A@f$ by the scalar @f$h@f$. @f$A@f$ is a
    * square banded upper triangular matrix. It has a non-zero diagonal, `sa`
    * superdiagonals, and thus a bandwidth of `sa + 1`. The result of the
    * multiplication is put back in @f$A@f$.
    * 
    ...
    * scalars @f$g@f$ and @f$h@f$. @f$A@f$ is a square banded upper triangular
    * matrix. It has a non-zero diagonal, `sa` superdiagonals, and thus a bandwidth
    * of `sa + 1`. Similarly for @f$B@f$ which has a bandwidth of `sb + 1`. The
    * result is put back in @f$A@f$.
    * 
   ...
   /**
    * Returns matrix @f$M@f$ as a string. It is displayed in matrix form, with each
    * row on a line.
    * 
   ...
   /**
    * Creates a String containing all the data of the `DMatrix`. The result is
    * displayed in matrix form, with each row on a line.
    * 
    ...
    *
    * @exception IndexOutOfBoundsException if the selected element would be outside
    *                                      the `DMatrix`
    */
