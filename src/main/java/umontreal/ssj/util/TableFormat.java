...
/**
 * This class provides methods to format arrays and matrices into String s in
 * different styles. This could be useful for printing arrays and subarrays, or
 * for putting them in files for further treatment by other softwares such as
...
public class TableFormat {
   ...
   /**
    * Formats a String containing the elements `n1` to `n2` (inclusive) of table
    * `V`, `k` elements per line, `p` positions per element. If `k` = 1, the array
    * index will also appear on the left of each element, i.e., each line `i` will
    * have the form `i V[i]`.
    * 
    ...
    * Gives at least `p1` positions per element, `p2` digits after the decimal
    * point, and at least `p3` significant digits.
    * 
    * @param V  array to be formated
    * @param n1 index of the first element being formated
    * @param n2 index of the last element being formated
   ...
   private static void fixeDelim(int style) {
      /*
       * Fixe les delimiteurs pour imprimer une matrice selon un format approprie
       */
      Style = style;
      switch (style) {
      case MATHEMATICA:
    ...
    * `style`. The elements are formated in `w` positions each, with a precision of
    * `p` digits. `Name` provides an identifier for the submatrix. To be treated by
    * `Matlab`, the returned string must be copied to a file with extension `.m`.
    * If the file is named `poil.m`, for example, it can be accessed by calling
    * `poil` in `Matlab`. For `Mathematica`, if the file is named `poil`, it will
    * be read using `<< poil;`.
    * 
    ...
    * @param style formating style of the submatrix, being one of #MATHEMATICA,
    *              #MATLAB, or #PLAIN
