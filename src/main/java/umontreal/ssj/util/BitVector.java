 ...
 * The vectors can be of arbitrary length. The operations provided are all the
 * binary operations available to the `int` and `long` primitive types in Java.
 * In the implementation, the vector is split in blocks of 32 bits and each
 * block is represented as an integer.
 *
 * All bit operations are present in two forms: a normal form and a `self` form.
 * The normal form returns a newly created object containing the result, while
 * the `self` form puts the result in the calling object (<tt>this</tt>). The
 * return value of the `self` form is the calling object itself. This is done to
 * allow easier manipulation of the results, making it possible to chain
 * operations.
 *
...
public class BitVector implements Serializable, Cloneable {
    ...
    * Component `vect[0]` makes the 32 lowest order bits, with `vect[1]` being the
    * 32 next lowest order bits, and so on. The normal bit order is then used to
    * fill the 32 bits (the first bit is the lowest order bit and the last bit is
    * largest order bit). Note that the sign bit is used as the largest order bit.
    * 
    * @param vect   the bits data
    * @param length the length of the vector
    * @exception IllegalArgumentException when the length of `vect` is not
    *                                     compatible with the `length` provided
    */
   ...
   /**
    * Creates a new `BitVector` using the data in `vect`. The length of the
    * `BitVector` is always equals to 32 times the length of `vect`.
    * 
    * @param vect the bits data
    */
   ...
   /**
    * Verifies if two <tt>BitVector</tt>’s have the same length and the same
    * data.
    * 
   ...
   /**
    * Resizes the `BitVector` so that its length is equal to `size`. If the
    * `BitVector` is enlarged, then the newly added bits are given the value 1 if
    * `filling` is set to `true` and 0 otherwise.
    * 
   ...
   /**
    * Resizes the `BitVector` so that its length is equal to `size`. Any new bit
    * added is set to 0.
    * 
    * @param size the new size of the `BitVector`
    */
   ...
   /**
    * Gives the value of the bit in position `pos`. If the value is 1, returns
    * `true`; otherwise, returns `false`.
    * 
    * @param pos the position of the checked bit
    * @return the value of the bit as a boolean
    *
    * @exception ArrayIndexOutOfBoundsException if `pos` is outside the range of
    *                                           the `BitVector`
    */
   ...
   /**
    * Sets the value of the bit in position `pos`. If `value` is equal to `true`,
   ...
   /**
    * Returns a string containing all the bits of the `BitVector`, starting with
    * the highest order bit and finishing with the lowest order bit. The bits are
    * grouped by groups of 8 bits for ease of reading.
    * 
   ...
   /**
    * Returns a `BitVector` which is the result of the `not` operator on the
    * current `BitVector`. The `not` operator is equivalent to the <code>~</code>
    * operator in Java and thus swap all bits (bits previously set to 0 become 1
    * and bits previously set to 1 become 0).
    * 
   ...
   /**
    * Returns a `BitVector` which is the result of the `xor` operator applied on
    * `this` and `that`. The `xor` operator is equivalent to the <code>^</code>
    * operator in Java. All bits which were set to 0 in one of the vector and to 1
    * in the other vector are set to 1. The others are set to 0. This is equivalent
    * to the addition in modulo 2 arithmetic.
    * 
   ...
   /**
    * Applies the `xor` operator on `this` with `that`. Stores the result in `this`
    * and returns it.
    * 
    * @param that the second operand to the `xor` operator
    * @return `this`
   ...
   /**
    * Returns a `BitVector` which is the result of the `and` operator with both the
    * `this` and `that` <tt>BitVector</tt>’s. The `and` operator is equivalent to
    * the <code>&</code> operator in Java. Only bits which are set to 1 in both
    * `this` and `that` are set to 1 in the result, all the others are set to 0.
    * 
   ...
   /**
    * Applies the `and` operator on `this` with `that`. Stores the result in `this`
    * and returns it.
    * 
    * @param that the second operand to the `and` operator
    * @return `this`
   ...
   /**
    * Returns a `BitVector` which is the result of the `or` operator with both the
    * `this` and `that` <tt>BitVector</tt>’s. The `or` operator is equivalent to
    * the `|` operator in Java. Only bits which are set to 0 in both `this` and
    * `that` are set to to 0 in the result, all the others are set to 1.
    * 
   ...
   /**
    * Applies the `or` operator on `this` with `that`. Stores the result in `this`
    * and returns it.
    * 
    * @param that the second operand to the `or` operator
    * @return `this`
   ...
   /**
    * Returns a `BitVector` equal to the original with all the bits shifted `j`
    * positions to the right if `j` is positive, and shifted `j` positions to the
    * left if `j` is negative. The new bits that appears to the left or to the
    * right are set to 0. If `j` is positive, this operation is equivalent to the
   ...
   /**
    * Shift all the bits of the current `BitVector` `j` positions to the right if
    * `j` is positive, and `j` positions to the left if `j` is negative. The new
    * bits that appears to the left or to the rigth are set to 0. Returns `this`.
    * 
    * @param j the size of the shift
    * @return `this`
    */
   ...
   /**
    * Returns the scalar product of two <tt>BitVector</tt>’s modulo 2. It returns
    * `true` if there is an odd number of bits with a value of 1 in the result of
    * the `and` operator applied on `this` and `that`, and returns `false`
    * otherwise.
    * 
