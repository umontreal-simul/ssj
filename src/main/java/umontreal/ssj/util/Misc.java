...
/**
 * This class provides miscellaneous functions that are hard to classify. Some
 * may be moved to another class in the future.
 *
...
public class Misc {
    ...
    * Let @f$t_0\le\cdots\le t_n@f$ be simulation times stored in a subset of
    * `times`. This method uses binary search to determine the smallest
    * value @f$i@f$ for which @f$t_i\le t < t_{i+1}@f$, and returns @f$i@f$. The
    * value of @f$t_i@f$ is stored in `times[start+i]` whereas @f$n@f$ is defined
    * as `end - start`. If
    * 
    * @f$t<t_0@f$, this returns @f$-1@f$. If @f$t\ge t_n@f$, this returns
    * @f$n@f$. Otherwise, the returned value is greater than or equal to 0, and
    *          smaller than or equal to @f$n-1@f$. `start` and `end` are only used
    *          to set lower and upper limits of the search in the `times` array;
    *          the index space of the returned value always starts at 0. Note that
    *          if the elements of `times` with indices `start`, …, `end` are not
    *          sorted in non-decreasing order, the behavior of this method is
    *          undefined.
    ...
    * @exception IllegalArgumentException       if `start` is negative, or if `end`
    *                                           is smaller than `start`.
    * @exception ArrayIndexOutOfBoundsException if `start + end` is greater than or
   ...
   /**
    * Computes the Newton interpolating polynomial. Given the @f$n+1@f$ real
    * distinct points @f$(x_0, y_0),@f$ @f$(x_1, y_1),…, (x_n, y_n)@f$, with
    * `X[i]` @f$= x_i@f$, `Y[i]` @f$= y_i@f$, this function computes the @f$n+1@f$
    * coefficients `C[i]` @f$= c_i@f$ of the Newton interpolating
    * polynomial @f$P(x)@f$ of degree @f$n@f$ passing through these points, i.e.
    * such that @f$y_i= P(x_i)@f$, given by
    * 
    * @anchor REF_util_Misc_eq_newton_interpol @f[ \qquad P(x) = c_0 + c_1(x-x_0) +
    *         c_2(x-x_0)(x-x_1) + \cdots+ c_n(x-x_0)(x-x_1) \cdots(x-x_{n-1}).
    *         \tag{eq.newton.interpol} @f]
    ...
    * {@link #interpol(int,double[],double[],double[]) interpol(n, X, Y, C)}, this
    * function returns the value of the interpolating polynomial
    * 
    * @f$P(z)@f$ evaluated at @f$z@f$ (see eq.
    *            {@link REF_util_Misc_eq_newton_interpol eq.newton.interpol} ).
    * @param n degree of the interpolating polynomial
    * @param X @f$x@f$-coordinates of points
    * @param C Coefficients of the interpolating polynomial
    ...
    * 
    * @anchor REF_util_Misc_eq_horner @f[ \qquad P(x) = c_0 + c_1 x + c_2 x^2 +
    *         \cdots+ c_n x^n \tag{eq.horner} @f]
