 ...
 *       integer coordinates: The subcube with coordinates @f$(i_1,…,i_d)@f$
 *       is defined as @f$\prod_{j=0}^{d-1} [i_j 2^{-m},  (i_j+1) 2^{-m})@f$.
 *       Note that each interval is open on the right. That is, if we multiply
 *       the coordinates of a point in the subcube by @f$2^m@f$ and truncate
 *       them to integers, we obtain the integer coordinates of the subcube. For
...
public class HilbertCurveSort implements MultiDimSort01<MultiDim01> {
    ...
    *         constructor will initialize a @ref HilbertCurveMap with arguments `d`
    *         and @f$m@f$. This map can be accessed with #getHilbertCurveMap.
   ...
   /**
    * Constructs a @ref HilbertCurveSort object with a given mapping of the Hilbert
    * curve in the unit hypercube.
    * 
