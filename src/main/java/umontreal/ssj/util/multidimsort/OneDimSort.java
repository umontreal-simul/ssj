 ...
 * objects according to a given sorting coordinate @f$j \ge0@f$ specified in the
 * constructor. One must have @f$j \in\{0,…,d-1\}@f$ where @f$d@f$ is the
 * dimension of the @ref MultiDimComparable<T> objects to be sorted. The sort
 * uses the method java.util.Arrays.sort from class Arrays. For example, to sort
 * objects based on the first coordinate, one should use `OneDimSort(0)`. It is
 * also possible to specify in the constructor the dimension @f$d@f$
 * (<tt>dimState</tt>) of the objects to be sorted.
 * 
...
public class OneDimSort<T extends MultiDimComparable<? super T>> implements MultiDimSortComparable<T> {
   ...
   /**
    * Constructs a OneDimSort that will sort the objects with respect to their
    * coordinate @f$j@f$. The dimension returned by #dimension() will then
    * be @f$d=j+1@f$.
    */
   ...
   public String toString() {
      return this.getClass().getSimpleName() + ": d = " + d + ", sort coordinate (starts at 0) = " + j;
   }
