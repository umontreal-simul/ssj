 ...
 *
 *                     This map (conceptually) divides the unit
 *                     hypercube @f$[0,1)^d@f$ in
 ...
 *              (arbitrary) order.
 *
 *              For the implementation of sorts based on the Hilbert curve (or
 *              Hilbert index), we identify and sort the subcubes by their
 *              Hilbert index, but it is also convenient to identify them
 *              (alternatively) with @f$m@f$-bit integer coordinates: The
 *              subcube with coordinates @f$(i_1,…,i_d)@f$ is defined
 *              as @f$\prod_{j=0}^{d-1} [i_j 2^{-m},  (i_j+1) 2^{-m})@f$. Note
 *              that each interval is open on the right. That is, if we multiply
 *              the coordinates of a point in the subcube by @f$2^m@f$ and
 *              truncate them to integers, we obtain the integer coordinates of
 *              the subcube. For example, if @f$d=2@f$ and @f$m=4@f$, we
 *              have @f$2^8 = 256@f$ subcubes, whose integer coordinates go from
 *              0 to 15, and the point @f$(0.1, 0.51)@f$ belongs to the subcube
 *              with integer coordinates @f$(1, 8)@f$.
 *
 *              For given @f$d@f$ and @f$m@f$, this class offers methods to
 *              compute the integer coordinates of the corresponding subcube
 *              from the real-valued coordinates of a point in @f$[0,1)^d@f$, as
 *              well as the Hilbert index of a subcube from its integer
 *              coordinates, and vice versa. The code that computes the latter
 *              correspondences is taken (with slight adaptations) from the
 *              `hilbert.c` program of Spencer W. Thomas, University of
 *              Michigan, 1991.
 *
...
public class HilbertCurveMap {
    ...
    * 
    * @f$m@f$ bits of each of the first `d` coordinates to sort the points. In this
    *         implementation, one must have @f$md \le63@f$.
    ...
    * parameter @f$m = \lfloor63/d\rfloor@f$. In this implementation, one must
    * have @f$md \le63@f$.
    * 
    * @param d maximum dimension
    */
   ...
   /**
    * Returns the number of bits @f$m@f$ that is used to divide the axis of each
    * coordinate.
    */
   public int getM() {
      return m;
   ...
   /**
    * Takes as input in `a[]` the integer coordinates of a subcube and returns the
    * corresponding Hilbert index @f$r@f$.
    */
   ...
   /**
    * Takes in `p[]` a point with real-valued coordinates and places in `a[]` the
    * integer coordinates of the corresponding subcube. The two arrays are assumed
    * to have length @f$d@f$.
    */
