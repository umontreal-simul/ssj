 ...
 *                                           function of the number @f$n@f$ of
 *                                           objects to be sorted. This number
 * @f$n@f$ is first rounded up the nearest power of 2, say @f$p = 2^e \ge n@f$.
 *         (Often, it should already be a power of 2.) Then the @f$e_j@f$ are
 *         defined as @f$e_0 = \lceil e \alpha_0 \rceil@f$ and @f$e_j = \lceil(e
 *         - e_0 - \cdots- e_{j-1}) (\alpha_0 + \cdots+ \alpha_j) \rceil@f$
 *         for @f$1\le j < d@f$. This gives @f$e_0 + \cdots+ e_{d-1} = e@f$.
 *
...
public class BatchSortPow2<T extends MultiDimComparable<? super T>> extends BatchSort<T> {
   ...
   /**
    * Constructs a @ref BatchSortPow2 that will use proportion exponents in
   ...
   /**
    * For a number @f$n@f$ of objects and a predefined vector of proportion
    * exponents @f$\alpha_0,\alpha_1,…,\alpha_{d-1}@f$, computes and sets the
    * corresponding vector of batch numbers, for which each batch number is a power
    * of 2 and their product is @f$p = 2^e \ge n@f$.
    * 
