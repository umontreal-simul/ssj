 ...
 * sort on multivariate arrays. It separates the objects in @f$n_0@f$ batches of
 * (approximately) equal size such that each object in a batch is smaller or
 * equal, in the first coordinate, to the objects in the next batches. Then,
 * each batch is separated in @f$n_1@f$ batches in the same way but using the
 ...
 * exceed the dimension of the objects to be sorted, and the product @f$p = n_0
 * n_1 \cdots n_{d-1}@f$ must normally be equal to the number @f$n@f$ of objects
 * to be sorted, which is `iMax - iMin` when doing a partial sort, and the total
 * number of objects otherwise. This means that one should normally use always
 * the same @f$n@f$ after the BatchSort object has been constructed in this way,
 * with a fixed @f$p@f$. The sorting method always uses batch sizes at each
 * level as if the number of objects was exactly @f$p@f$. The batch size at
 * level @f$j@f$ is @f$n_{j+1} \cdots n_{d-1}@f$. If there are less than @f$p@f$
 * objects, some batches (the last ones) can have fewer objects or be empty.
 * What is done is conceptually equivalent to adding dummy objects at the end of
 * the array so their total number is exactly @f$p@f$. For example, if @f$n_0 =
 * 3@f$, @f$p=15@f$, and @f$n=13@f$, then the batch size will be @f$15/3 = 5@f$
 * but the last batch will have only 3 objects. The value of @f$n_{d-1}@f$ is
 ...
 *
 * A second way of constructing a BatchSort object is by specifying an array of
 * non-negative doubles @f$\alpha_0,\alpha_1,…,\alpha_{d-1}@f$, called
 * *proportion exponents*, such that @f$\alpha_0+\cdots+\alpha_{d-1} = 1@f$. For
 * each number @f$n@f$ of objects to be sorted, the batch numbers will be
 * recomputed as (approximately) @f$n_j = n^{\alpha_j}@f$. With this
 * construction, one can easily handle very different (arbitrary) values
 * of @f$n@f$ with the same BatchSort object. The vector of batch numbers is
 * recomputed each time we sort with a new value of @f$n@f$, as follows: @f$n_j
 * = \lceil n^{\alpha_j} \rceil@f$, for @f$j = 0,\dots,d-1@f$. These values are
 * saved in case we use the same @f$n@f$ the next time. They give @f$p = n_0
 * \cdots n_{d-1} \ge n@f$. The batch size at level @f$j@f$ is again @f$n_{j+1}
 * \cdots n_{d-1}@f$. When @f$n < p@f$, some batches (the last ones) have fewer
...
public class BatchSort<T extends MultiDimComparable<? super T>> implements MultiDimSortComparable<T> {
   ...
   /**
    * Constructs a BatchSort that will always use the (fixed) batch numbers given
    * in `batchNumbers`. These batch numbers can be changed only by creating new
    * object, of by using proportion exponents via `setBatchExponents`. The number
    * of objects to sort should not exceed the product of the numbers in
    * `batchNumbers`.
    * 
   ...
   /**
    * Constructs a BatchSort that will use the proportion exponents in
    * `batchExponents`, which must contain non-negative numbers that sum to 1.
    * 
    * @param batchExponents proportion exponents to compute the batches sizes
    */
   ...
   /**
    * Resets the corresponding vector of batch numbers for the given number @f$n@f$
    * of objects. This method can be invoked only when proportion exponents are
    * used and have been defined previously.
    * 
   ...
   /**
    * Sets the vector of proportion
    * exponents @f$\alpha_0,\alpha_1,…,\alpha_{d-1}@f$ to the values in
    * `batchExponents`. These values must be non-negative and sum to 1. From now
    * on, this BatchSort object will use proportion exponents.
    * 
