 ...
 * a @ref MultiDimComparable<T> array based on its first @f$d@f$ dimensions. The
 * sort first separates the array in two, such that the objects in the first
 * part are smaller or equal to those in the second part for their coordinate 0.
 * If the number of objects is even, the two parts will contain the same number
 * of objects, otherwise there will be one more in the second part. Then each
 * part is recursively separated in two based on coordinate 1, and so on, until
 * each part contains a single object. If not done yet after splitting on
 * coordinate @f$d-1@f$, the procedure cycles back to coordinate 0, and
 * continues. The resulting order is the result of the sort.
 *
 * <div class="SSJ-bigskip"></div>
 */
public class SplitSort<T extends MultiDimComparable<? super T>> implements MultiDimSortComparable<T> {
