...
public class FDist {
   ...
   /**
    * Similar to @ref umontreal.ssj.probdist.KolmogorovSmirnovPlusDist but for the
    * case where the distribution function @f$F@f$ has a jump of size
    * 
    * @f$a@f$ at a given point @f$x_0@f$, is zero at the left of @f$x_0@f$, and is
    *         continuous at the right of @f$x_0@f$. The Kolmogorov-Smirnov
    *         statistic is defined in that case as
    * @anchor REF_gof_FDist_eq_KSPlusJumpOne @f[ D_N^+(a) = \sup_{a\le u\le1}
    *         \left(\hat{F}_N (F^{-1}(u)) - u\right) =
    *         \max_{\Rule{0.0pt}{7.0pt}{0.0pt} \lfloor1+aN \le j \le N} \left(j/N -
    *         F (V_{(j)})\right). \tag{KSPlusJumpOne} @f]
    *         where @f$V_{(1)},…,V_{(N)}@f$ are the observations sorted by
    *         increasing order. The method returns an approximation
    *         of @f$P[D_N^+(a) \le x]@f$ computed via
    * @anchor REF_gof_FDist_DistKSJ1
    * @anchor REF_gof_FDist_DistKSJ2
    * @f{align}{ P[D_N^+(a) \le x] & = 1 - x \sum_{i=0}^{\lfloor N (1-a-x)\rfloor}
    *            \binom{N}{i} \left(\frac{i}{N} + x \right)^{i-1} \left(1 -
    *            \frac{i}{N} - x \right)^{N-i}. \tag{DistKSJ1} \\ & = x
    *            \sum_{j=0}^{\lfloor N (a+x) \rfloor} \binom{N}{j}
    *            \left(\frac{j}{N} - x \right)^j \left(1 - \frac{j}{N} + x
    *            \right)^{N-j-1}. \tag{DistKSJ2}
    * @f} The current implementation uses formula ( {@link REF_gof_FDist_DistKSJ2
    *     DistKSJ2} ) when @f$N (x+a) < 6.5@f$ and @f$x+a < 0.5@f$, and uses (
    *     {@link REF_gof_FDist_DistKSJ1 DistKSJ1} ) when @f$Nx \ge6.5@f$ or @f$x+a
    *     \ge0.5@f$. Restriction: @f$0 < a < 1@f$.
    * @param N sample size
   ...
   /**
    * Returns @f$F (m)@f$, the distribution function of the scan statistic with
    * parameters @f$N@f$ and @f$d@f$, evaluated at @f$m@f$. For a description of
    * this statistic and its distribution, see FBar.scan(int,double,int), which
    * computes its complementary distribution @f$\bar{F} (m) = 1 - F (m-1)@f$.
    * 
