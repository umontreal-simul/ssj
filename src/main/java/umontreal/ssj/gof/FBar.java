...
/**
 * This class is similar to @ref FDist, except that it provides static methods
 * to compute or approximate the complementary distribution function of @f$X@f$,
 * which we define as @f$\bar{F} (x) = P[X\ge x]@f$, instead of
 * 
 * @f$F (x)=P[X\le x]@f$. Note that with our definition of @f$\bar{F}@f$, one
 *      has @f$\bar{F} (x) = 1 - F (x)@f$ for continuous distributions and
 * @f$\bar{F} (x) = 1 - F (x-1)@f$ for discrete distributions over the integers.
 *
...
public class FBar {
    ...
    * defined as
    * 
    * @anchor REF_gof_FBar_eq_scan @f[ S_N (d) = \sup_{0\le y\le1-d}
    *         \eta[y, y+d], \tag{scan} @f] where @f$d@f$ is a constant in @f$(0,
    *         1)@f$, @f$\eta[y, y+d]@f$ is the number of observations falling
    *         inside the interval @f$[y, y+d]@f$, from a sample of @f$N@f$
    *         i.i.d. @f$U (0,1)@f$ random variables. One has (see @cite
    *         tAND95b&thinsp;),
    * @anchor REF_gof_FBar_DistScan1
    * @anchor REF_gof_FBar_DistScan2
    * @f{align}{ P[S_N (d) \ge m] & \approx \left(\frac{m}{d}-N-1\right) b (m) + 2
    *            \sum_{i=m}^N b (i) \tag{DistScan1} \\ & \approx 2
    *            (1-\Phi(\theta\kappa)) + \theta\kappa\frac{\exp(-\theta^2\kappa^2
    *            /2)}{d \sqrt{2\pi}} \tag{DistScan2}
    * @f} where @f$\Phi@f$ is the standard normal distribution function.
    * @f{align*}{ b (i) & = \binom{N}{i} d^i (1-d)^{N-i}, \\ \theta & =
    *             \sqrt{\frac{d}{1-d}}, \\ \kappa & = \frac{m}{d \sqrt{N}} -
    *             \sqrt{N}.
    * @f} For @f$d \le1/2@f$, ( {@link REF_gof_FBar_DistScan1 DistScan1} ) is exact
    *     for @f$m > N/2@f$, but only an approximation otherwise. The approximation
    *     ( {@link REF_gof_FBar_DistScan2 DistScan2} ) is good when @f$N d^2@f$ is
    *     large or when @f$d > 0.3@f$ and
    * @f$N>50@f$. In other cases, this implementation sometimes use the
    *             approximation proposed by Glaz @cite tGLA89a&thinsp;. For more
    *             information, see @cite tAND95b, @cite tGLA89a, @cite
    *             tWAL87a&thinsp;. The approximation returned by this function is
    *             generally good when it is close to 0, but is not very reliable
    *             when it exceeds, say, 0.4. If @f$m \le(N + 1)d@f$, the method
    *             returns 1. Else, if @f$Nd \le10@f$, it returns the approximation
    *             given by Glaz @cite tGLA89a&thinsp;. If @f$Nd > 10@f$, it
    *             computes ( {@link REF_gof_FBar_DistScan2 DistScan2} ) or (
    *             {@link REF_gof_FBar_DistScan1 DistScan1} ) and returns the result
    *             if it does not exceed 0.4, otherwise it computes the
    *             approximation from @cite tGLA89a&thinsp;, returns it if it is
    *             less than 1.0, and returns 1.0 otherwise. The relative error can
    *             reach 10% when @f$Nd \le10@f$ or when the returned value is less
    *             than 0.4. For @f$m > Nd@f$ and @f$Nd > 10@f$, a returned value
    *             that exceeds @f$0.4@f$ should be regarded as unreliable. For @f$m
    *             = 3@f$, the returned values are totally unreliable. (There may be
    *             an error in the original formulae in
    * @cite tGLA89a&thinsp;). Restrictions: @f$N \ge2@f$ and @f$d \le1/2@f$.<br>
    * @param n sample size (@f$\ge2@f$)
    * @param d length of the test interval (@f$\in(0,1)@f$)
    * @param m scan statistic
    * @return the complementary distribution function of the statistic evaluated at
    *         `m`
    */
