...
/**
 * This static class provides methods to compute a kernel density estimator from
 * a set of @f$n@f$ individual observations @f$x_0, …, x_{n-1}@f$, which
 * define an empirical distribution. The methods return the estimated density
 * either at a given point or on a grid or set of @f$m@f$ predefined points.
 * Some details on how a kernel density estimator is defined and how to select
...
public class KernelDensity {
   ...
   /**
    * Given an empirical distribution `dist`, this method computes and returns the
    * kernel density estimate at a single point @f$y@f$. The kernel density is
    * `kern`, and the bandwidth is @f$h@f$.
    */
   ...
   /**
    * Given the empirical distribution `dist`, this method computes the kernel
    * density estimate at each of the @f$m@f$ evaluation points
    * <tt>evalPoints[</tt>@f$j@f$<tt>]</tt>, @f$j= 0, 1, …, (m-1)@f$,
    * where @f$m@f$ is the length of `EvalPoints`, the kernel is `kern.density(x)`,
    * and the bandwidth is @f$h@f$. Returns the estimates as an array of @f$m@f$
    * values. Note that the evaluation points do not have to cover the entire
    * support of the density; they can cover only a small a small interval.
    * 
   ...
   /**
    * Similar to @f$computeDensity (dist, kern, h, evalPoints)@f$, but much more
    * efficient for very large n. We assume that the kernel is unimodal
    * (increasing, then decreasing). For each evaluation point j, in the sum that
    * defines the density estimator, we add only the terms that contribute more
    * than epsilon0 to the sum. We stop adding as soon as the new term is <
    * epsilon0. One can try epsilon0 = E-10 for example. We have also replaced dist
    * by a sorted array @f$double[] data@f$ that contains the raw data.
    * 
    */
