 ...
 *        transformations @cite tSTE86a&thinsp;. Here, @f$U_{(0)}, …,
 *        U_{(n-1)}@f$ stand for @f$n@f$ observations @f$U_0,…,U_{n-1}@f$
 *        sorted by increasing order, where @f$0\le U_i\le1@f$.
 *
...
public class GofStat {
    ...
    *      represents the @f$n@f$ observations contained in `data`, and @f$U@f$,
    *      the returned transformed observations. If `data` contains random
    *      variables from the distribution function `dist`, then the result will
    *      contain uniform random variables over @f$[0,1]@f$.
   ...
   /**
    * Applies the transformation @f$U_i = F (V_i)@f$ for @f$i = 0, 1, …, n-1@f$,
    * where @f$F@f$ is a *discrete* distribution function, and returns the result
    * as an array of length @f$n@f$. @f$V@f$ represents the @f$n@f$ observations
    * contained in `data`, and @f$U@f$, the returned transformed observations.
    *
    * Note: If @f$V@f$ are the values of random variables with distribution
    * function `dist`, then the result will contain the values of *discrete* random
    * variables distributed over the set of values taken by `dist`, not uniform
    * random variables over
    * 
    * @f$[0,1]@f$. @param data array of observations to be transformed
    * @param dist assumed distribution of the observations
    * @return the array of transformed observations
    */
   ...
   /**
    * Assumes that the real-valued observations @f$U_0,…,U_{n-1}@f$ contained in
    * `sortedData` are already sorted in increasing order and computes the
    * differences between the successive observations. Let
    * 
    * @f$D@f$ be the differences returned in `spacings`. The difference
    * @f$U_i - U_{i-1}@f$ is put in @f$D_i@f$ for `n1 < i <= n2`, whereas @f$U_{n1}
    *        - a@f$ is put into @f$D_{n1}@f$ and @f$b - U_{n2}@f$ is put
    *        into @f$D_{n2+1}@f$. The number of observations must be greater or
    *        equal than `n2`, we must have `n1 < n2`, and `n1` and `n2` are greater
    *        than 0. The size of `spacings` will be at least @f$n+1@f$ after the
    *        call returns.
    * @param sortedData array of sorted observations
    * @param spacings   pointer to an array object that will be filled with
    *                   spacings
    * @param n1         starting index, in `sortedData`, of the processed
    *                   observations
    * @param n2         ending index, in `sortedData` of the processed observations
    ...
    * diff(IntArrayList,IntArrayList,int,int,int,int)}, but for the continuous
    * case.
    * 
    * @param sortedData array of sorted observations
    * @param spacings   pointer to an array object that will be filled with
    ...
    * 
    * @cite rKNU98a, @cite tSTE86a&thinsp;. Let @f$U@f$ be the @f$n@f$ observations
    *       contained into `data`, and let @f$S@f$ be the spacings contained into
    *       `spacings`, Assumes that @f$S[0..n]@f$ contains the *spacings*
    *       between @f$n@f$ real numbers @f$U_0,…,U_{n-1}@f$ in the
    *       interval @f$[0,1]@f$. These spacings are defined by @f[ S_i = U_{(i)} -
    *       U_{(i-1)}, \qquad1\le i < n, @f]
    *       where @f$U_{(0)}=0@f$, @f$U_{(n-1)}=1@f$,
    *       and @f$U_{(0)},…,U_{(n-1)}@f$, are the @f$U_i@f$ sorted in increasing
    *       order. These spacings may have been obtained by calling
    *       #diff(DoubleArrayList,DoubleArrayList,int,int,double,double). This
    *       method transforms the spacings into new spacings, by a variant of the
    *       method described in section 11 of @cite rMAR85a&thinsp; and also by
    *       Stephens @cite tSTE86a&thinsp;: it sorts @f$S_0,…,S_n@f$ to
    *       obtain @f$S_{(0)} \le S_{(1)} \le S_{(2)} \le\cdots\le S_{(n)}@f$,
    *       computes the weighted differences
    * @f{align*}{ S_0 & = (n+1) S_{(0)}, \\ S_1 & = n (S_{(1)}-S_{(0)}), \\ S_2 & =
    *             (n-1) (S_{(2)}-S_{(1)}), \\ & \vdots \\ S_n & =
    *             S_{(n)}-S_{(n-1)},
    * @f} and computes @f$V_i = S_0 + S_1 + \cdots+ S_i@f$ for @f$0\le i < n@f$. It
    *     then returns @f$S_0,…,S_n@f$ in `S[0..n]` and
    * @f$V_1,…,V_n@f$ in `V[1..n]`.
    *
    *                   Under the assumption that the @f$U_i@f$ are i.i.d. @f$U
    *                   (0,1)@f$, the new @f$S_i@f$ can be considered as a new set
    *                   of spacings having the same distribution as the original
    *                   spacings, and the @f$V_i@f$ are a new sample of i.i.d. @f$U
    *                   (0,1)@f$ random variables, sorted by increasing order.
    *
    *                   This transformation is useful to detect *clustering* in a
    *                   data set: A pair of observations that are close to each
    *                   other is transformed into an observation close to zero. A
    *                   data set with unusually clustered observations is thus
    *                   transformed to a data set with an accumulation of
    *                   observations near zero, which is easily detected by the
    *                   Anderson-Darling GOF test.
    * @param data     array of observations
    * @param spacings spacings between the observations, will be filled with the
    *                 new spacings
    */
   ...
   /**
    * Applies the *power ratios* transformation @f$W@f$ described in section 8.4 of
    * Stephens @cite tSTE86a&thinsp;. Let @f$U@f$ be the
    * 
    * @f$n@f$ observations contained into `sortedData`. Assumes that
    * @f$U@f$ contains @f$n@f$ real numbers @f$U_{(0)},…,U_{(n-1)}@f$ from the
    *         interval @f$[0,1]@f$, already sorted in increasing order, and
    *         computes the transformations: @f[ U’_i = (U_{(i)} /
    *         U_{(i+1)})^{i+1}, \qquad i=0,…,n-1, @f] with @f$U_{(n)} = 1@f$.
    *         These @f$U’_i@f$ are sorted in increasing order and put back in
    *         `U[1...n]`. If the @f$U_{(i)}@f$ are i.i.d.
    * @f$U (0,1)@f$ sorted by increasing order, then the @f$U’_i@f$ are also
    *      i.i.d. @f$U (0,1)@f$.
    *
    *      This transformation is useful to detect clustering, as explained in
    *      #iterateSpacings(DoubleArrayList,DoubleArrayList), except that here a
    *      pair of observations close to each other is transformed into an
    *      observation close to 1. An accumulation of observations near 1 is also
    *      easily detected by the Anderson-Darling GOF test.
    * @param sortedData sorted array of real-valued observations in the
    *                   interval @f$[0,1]@f$ that will be overwritten with the
   ...
   /**
    * This class helps managing the partitions of possible outcomes into categories
    * for applying chi-square tests. It permits one to automatically regroup
    * categories to make sure that the expected number of observations in each
    * category is large enough. To use this facility, one must first construct an
    * `OutcomeCategoriesChi2` object by passing to the constructor the expected
    * number of observations for each original category. Then, calling the method
    * #regroupCategories will regroup categories in a way that the expected number
    * of observations in each category reaches a given threshold `minExp`. Experts
    * in statistics recommend that `minExp` be always larger than or equal to 5 for
    * the chi-square test to be valid. Thus, `minExp` = 10 is a safe value to use.
    * After the call, `nbExp` gives the expected numbers in the new categories and
    * `loc[i]` gives the relocation of category @f$i@f$, for each @f$i@f$. That is,
    * `loc[i] = j` means that category @f$i@f$ has been merged with
    * category @f$j@f$ because its original expected number was too small, and
    * `nbExp[i]` has been added to `nbExp[j]` and then set to zero. In this case,
    * all observations that previously belonged to category @f$i@f$ are redirected
    * to category @f$j@f$. The variable `nbCategories` gives the final number of
    * categories, `smin` contains the new index of the lowest category, and `smax`
   ...
   public static class OutcomeCategoriesChi2 {
      ...
      /**
       * Constructs an `OutcomeCategoriesChi2` object using the array `nbExp` for the
       * number of expected observations in each category. The `smin` and `smax`
       * fields are set to 0 and
       * 
       * @f$(n-1)@f$ respectively, where @f$n@f$ is the length of array `nbExp`. The
       *             `loc` field is set such that `loc[i]=i` for each `i`. The field
       *             `nbCategories` is set to @f$n@f$.
       ...
       * observations array. Only the expected numbers from the `smin` to `smax`
       * (inclusive) indices will be considered valid. The `loc` field is set such
       * that `loc[i]=i` for each `i` in the interval `[smin, smax]`. All `loc[i]` for
       * <tt>i </tt>@f$\le@f$<tt> smin</tt> are set to `smin`, and all `loc[i]` for
       * <tt>i </tt>@f$\ge@f$<tt> smax</tt> are set to `smax`. The field
       * `nbCategories` is set to (<tt>smax - smin +
       * 1</tt>).
       * 
       * @param nbExp array of expected observations for each category
      ...
      /**
       * Constructs an `OutcomeCategoriesChi2` object. The field `nbCategories` is set
       * to `nbCat`.
       * 
       * @param nbExp array of expected observations for each category
       * @param smin  Minimum index for valid expected number of observations
       * @param smax  Maximum index for valid expected number of observations
       * @param loc   array for which `loc[i]` gives the relocation of the category
       *              `i`
       */
      ...
      /**
       * Regroup categories as explained earlier, so that the expected number of
       * observations in each category is at least `minExp`. We usually choose
       * `minExp` = 10.
       * 
    ...
    * 
    * @f$o_i@f$ in `count[smin...smax]`, for which the corresponding expected
    *           values @f$e_i@f$ are in `nbExp[smin...smax]`. Assuming that
    * @f$i@f$ goes from 1 to @f$k@f$, where @f$k =@f$ `smax-smin+1` is the number
    *         of categories, the chi-square statistic is defined as
    * @anchor REF_gof_GofStat_eq_chi_square @f[ X^2 = \sum_{i=1}^k \frac{(o_i -
    *         e_i)^2}{e_i}. \tag{chi-square} @f] Under the hypothesis that
    *         the @f$e_i@f$ are the correct expectations and if these @f$e_i@f$ are
    *         large enough, @f$X^2@f$ follows approximately the chi-square
    *         distribution with @f$k-1@f$ degrees of freedom. If some of
    *         the @f$e_i@f$ are too small, one can use `OutcomeCategoriesChi2` to
    *         regroup categories.
    * @param nbExp numbers expected in each category
    * @param count numbers observed in each category
    * @param smin  index of the first valid data in `count` and `nbExp`
    * @param smax  index of the last valid data in `count` and `nbExp`
    ...
    * @f$e_i@f$ are in `cat`. This assumes that `cat.regroupCategories` has been
    *           called before to regroup categories in order to make sure that the
    *           expected numbers in each category are large enough for the
    *           chi-square test.
   ...
   /**
    * Computes and returns the chi-square statistic for the observations stored in
    * `data`, assuming that these observations follow the discrete distribution
    * `dist`. For `dist`, we assume that there is one set @f$S=\{a, a+1,…, b-1,
    * b\}@f$, where @f$a<b@f$ and
    * 
    * @f$a\ge0@f$, for which @f$p(s)>0@f$ if @f$s\in S@f$ and
    * @f$p(s)=0@f$ otherwise.
    *
    *              Generally, it is not possible to divide the integers in
    *              intervals satisfying @f$nP(a_0\le s< a_1)=nP(a_1\le s<
    *              a_2)=\cdots=nP(a_{j-1}\le s< a_j)@f$ for a discrete
    *              distribution, where @f$n@f$ is the sample size, i.e., the number
    *              of observations stored into `data`. To perform a general
    *              chi-square test, the method starts from `smin` and finds the
    *              first non-negligible probability
    * @f$p(s)\ge\epsilon@f$, where @f$\epsilon=@f$ DiscreteDistributionInt.EPSILON.
    *                        It uses `smax` to allocate an array storing the number
    *                        of expected observations (@f$np(s)@f$) for each
    * @f$s\ge@f$ `smin`. Starting from @f$s=@f$ `smin`, the @f$np(s)@f$ terms are
    *            computed and the allocated array grows if required until a
    ...
    *            to get the chi-square test statistic. If `numCat` is not `null`,
    *            the number of categories after regrouping is returned in
    *            `numCat[0]`. The number of degrees of freedom is equal to
    ...
    * @param smin   estimated minimum value of @f$s@f$ for which @f$p(s)>0@f$
    * @param smax   estimated maximum value of @f$s@f$ for which @f$p(s)>0@f$
    * @param minExp minimum number of expected observations in each interval
   ...
   /**
    * Similar to #chi2(double[],int[],int,int), except that the expected number of
    * observations per category is assumed to be the same for all categories, and
    * equal to `nbExp`.
    * 
   ...
   /**
    * Computes the chi-square statistic for a continuous distribution. Here, the
    * equiprobable case can be used. Assuming that `data` contains observations
    * coming from the uniform distribution, the
    * 
    * @f$[0,1]@f$ interval is divided into @f$1/p@f$ subintervals, where
    * @f$p=@f$ <tt>minExp</tt>@f$/n@f$, @f$n@f$ being the sample size, i.e., the
    *          number of observations stored in `data`. For each subinterval, the
    *          method counts the number of contained observations and the
    *          chi-square statistic is computed using
    ...
    * {@link REF_gof_FBar_eq_scan scan} ). Let @f$U@f$ be the @f$n@f$ observations
    * contained into `sortedData`. The @f$n@f$ observations in @f$U[0..n-1]@f$ must
    * be real numbers in the interval
    * 
    * @f$[0,1]@f$, sorted in increasing order. (See FBar.scan for the distribution
    *              function of @f$S_n (d)@f$).
    * @param sortedData sorted array of real-valued observations in the
    *                   interval @f$[0,1]@f$
    ...
    * @anchor REF_gof_GofStat_eq_CraMis @f[ W_n^2 = \frac{1}{ 12n} +
    *         \sum_{j=0}^{n-1} \left(U_{(j)} - \frac{(j+0.5) }{ n}\right)^2,
    *         \tag{CraMis} @f] assuming that `sortedData`
    *         contains @f$U_{(0)},…,U_{(n-1)}@f$ sorted in increasing order.
    ...
    * @f{align}{ G_n & = \sqrt{n} \max_{\Rule{0.0pt}{7.0pt}{0.0pt} 0\le j \le n-1}
    *            \left\{ (j+1)/n - U_{(j)} + \overline{U}_n - 1/2 \right\}
    *            \tag{WatsonG} \\ & = \sqrt{n}\left(D_n^+ + \overline{U}_n -
    *            1/2\right), \nonumber
    * @f} where @f$\overline{U}_n@f$ is the average of the observations
    * @f$U_{(j)}@f$, assuming that `sortedData` contains the
    *                sorted @f$U_{(0)},…,U_{(n-1)}@f$.
    * @param sortedData array of sorted real-valued observations in the
    *                   interval @f$[0,1]@f$
    ...
    * @f{align}{ W_n^2 & = \frac{1}{ 12n} + \sum_{j=0}^{n-1} \left\{U_{(j)} -
    *            \frac{(j + 0.5)}{ n} \right\}^2, \\ U_n^2 & = W_n^2 -
    *            n\left(\overline{U}_n - 1/2\right)^2. \tag{WatsonU}
    * @f} where @f$\overline{U}_n@f$ is the average of the observations
    * @f$U_{(j)}@f$, assuming that `sortedData` contains the
    *                sorted @f$U_{(0)},…,U_{(n-1)}@f$.
    * @param sortedData array of sorted real-valued observations in the
   ...
   /**
    * Used by #andersonDarling(DoubleArrayList). <tt>Num.DBL_EPSILON</tt> is
    * usually @f$2^{-52}@f$.
    */
   public static double EPSILONAD = Num.DBL_EPSILON / 2;

   /**
    * Computes and returns the Anderson-Darling statistic @f$A_n^2@f$ (see method
    * #andersonDarling(double[]) ).
    * 
    * @param sortedData array of sorted real-valued observations in the
    *                   interval @f$[0,1]@f$
    ...
    * @f{align*}{ A_n^2 & = -n -\frac{1}{ n} \sum_{j=0}^{n-1} \left\{
    *             (2j+1)\ln(U_{(j)}) + (2n-1-2j) \ln(1-U_{(j)}) \right\},
    *             \tag{Andar}
    * @f} assuming that `sortedData` contains @f$U_{(0)},…,U_{(n-1)}@f$ sorted in
    *     increasing order. When computing @f$A_n^2@f$, all observations
    * @f$U_i@f$ are projected on the interval @f$[\epsilon, 1-\epsilon]@f$ for
    *           some @f$\epsilon> 0@f$, in order to avoid numerical overflow when
    *           taking the logarithm of @f$U_i@f$ or
    * @f$1-U_i@f$. The variable `EPSILONAD` gives the value of
    * @f$\epsilon@f$. @param sortedData array of sorted real-valued observations in
    ...
    * corresponding @f$p@f$-value @f$p@f$. The @f$n@f$ (unsorted) observations in
    * `data` are assumed to be independent and to come from the continuous
    * distribution `dist`. Returns the 2-elements array [@f$A_n^2@f$, @f$p@f$].
    * 
    * @param data array of observations
    * @param dist assumed distribution of the observations
    ...
    * 
    * @f$D_n^-@f$, and @f$D_n@f$ (see method #kolmogorovSmirnov(DoubleArrayList) ).
    *              Returns the array [@f$D_n^+@f$, @f$D_n^-@f$, @f$D_n@f$].
    * @param sortedData array of sorted real-valued observations in the
    *                   interval @f$[0,1]@f$
    ...
    * @f{align}{ D_n^+ & = \max_{0\le j\le n-1} \left((j+1)/n - U_{(j)}\right),
    *            \tag{DNp} \\ D_n^- & = \max_{0\le j\le n-1} \left(U_{(j)} -
    *            j/n\right), \tag{DNm} \\ D_n & = \max (D_n^+, D_n^-). \tag{DN}
    * @f} and returns an array of length 3 that contains
    *     [@f$D_n^+@f$, @f$D_n^-@f$, @f$D_n@f$]. These statistics compare the
    *     empirical distribution of @f$U_{(1)},…,U_{(n)}@f$, which are assumed to
    *     be in `sortedData`, with the uniform distribution over @f$[0,1]@f$.
    * @param sortedData array of sorted real-valued observations in the
    *                   interval @f$[0,1]@f$
    * @return the array [@f$D_n^+@f$, @f$D_n^-@f$, @f$D_n@f$]
    */
    ...
    *                 (unsorted) observations in `data` with the theoretical
    *                 distribution `dist`. The KS
    *                 statistics @f$D_n^+@f$, @f$D_n^-@f$ and @f$D_n@f$ are
    *                 returned in `sval[0]`, `sval[1]`, and `sval[2]` respectively,
    *                 and their corresponding @f$p@f$-values are returned in
    *                 `pval[0]`, `pval[1]`, and `pval[2]`.
   ...
   /**
    * Compute the KS statistics @f$D_n^+(a)@f$ and @f$D_n^-(a)@f$ defined in the
    * description of the method FDist.kolmogorovSmirnovPlusJumpOne, assuming
    * that @f$F@f$ is the uniform distribution over @f$[0,1]@f$ and
    * that @f$U_{(1)},…,U_{(n)}@f$ are in `sortedData`. Returns the array
    * [@f$D_n^+@f$, @f$D_n^-@f$].
    * 
    * @param sortedData array of sorted real-valued observations in the
    *                   interval @f$[0,1]@f$
   ...
   /**
    * Computes a variant of the @f$p@f$-value @f$p@f$ whenever a test statistic has
    * a *discrete* probability distribution. This
    * 
    * @f$p@f$-value is defined as follows:
    * @f{align*}{ p_L & = P[Y \le y] \\ p_R & = P[Y \ge y] \\ p & = \left\{
    *             \begin{array}{l@{qquad}l} p_R, & \mbox{if } p_R < p_L \\ 1 - p_L,
    *             \mbox{if } p_R \ge p_L \mbox{ and } p_L < 0.5 \\ 0.5 &
    *             \mbox{otherwise.} \end{array} \right.
    * @f} @f[ \begin{array}{rll} p = & p_R, & \qquad\mbox{if } p_R < p_L, \\ p = &
    *     1 - p_L, & \qquad\mbox{if } p_R \ge p_L \mbox{ and } p_L < 0.5, \\ p = &
    *     0.5 & \qquad\mbox{otherwise.} \end{array} @f] The function
    *     takes @f$p_L@f$ and @f$p_R@f$ as input and returns
    * @f$p@f$. @param pL left @f$p@f$-value
