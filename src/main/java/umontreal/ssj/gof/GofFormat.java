...
/**
 * This class contains methods used to format results of GOF test statistics, or
 * to apply a series of tests simultaneously and format the results. It is in
 * fact a translation from C to Java of a set of functions that were specially
 * written for the implementation of TestU01, a software package for testing
 * uniform random number generators @cite iLEC01t&thinsp;.
 *
 ...
 *                least one* @f$p@f$-value less than 0.01, say, is larger than
 *                0.01. One must therefore be careful with the interpretation of
 *                these @f$p@f$-values (one could use, e.g., the Bonferroni
 *                inequality @cite sLAW00a&thinsp;). Applying simultaneous tests
 *                is convenient in some situations, such as in screening
 *                experiments for detecting statistical deficiencies in random
 *                number generators. In that context, rejection of the null
 *                hypothesis typically occurs with extremely
 *                small @f$p@f$-values (e.g., less than @f$10^{-15}@f$), and the
 *                interpretation is quite obvious in this case.
 *
 *                The class also provides tools to plot an empirical or
 *                theoretical distribution function, by creating a data file
 *                that contains a graphic plot in a format compatible with the
 *                software specified by the environment variable #graphSoft.
...
public class GofFormat {
   ...
   /**
    * Environment variable that selects the type of software to be used for
    * plotting the graphs of functions. The data files produced by #graphFunc and
    * #graphDistUnif will be in a format suitable for this selected software. The
    * default value is `GNUPLOT`. To display a graphic in file `f` using `gnuplot`,
    * for example, one can use the command
    * "<tt>plot f with steps, x with lines</tt>" in `gnuplot`. `graphSoft` can take
    * the values #GNUPLOT or #MATHEMATICA.
    */
...
// Renommer drawCDF en fixant mono = 1 et éliminant mono.
   ...
   /**
    * Formats data to plot the graph of the distribution function @f$F@f$ over the
    * interval @f$[a,b]@f$, and returns the result as a String. The method
    * `dist.cdf(x)` returns the value of @f$F@f$ at @f$x@f$. The String `desc`
    * gives a short caption for the graphic plot. The method computes the @f$m+1@f$
    * points @f$(x_i,  F (x_i))@f$, where
    * 
    * @f$x_i = a + i (b-a)/m@f$ for @f$i=0,1,…,m@f$, and formats these points
    *        into a `String` in a format suitable for the software specified by
    *        #graphSoft. NOTE: see also the more recent class
    ...
    * interval @f$[a,b]@f$, and returns the result as a String. The method
    * `dist.density(x)` returns the value of @f$f(x)@f$ at @f$x@f$. The String
    * `desc` gives a short caption for the graphic plot. The method computes
    * the @f$m+1@f$ points @f$(x_i,  f(x_i))@f$, where
    * 
    * @f$x_i = a + i (b-a)/m@f$ for @f$i=0,1,…,m@f$, and formats these points
    *        into a `String` in a format suitable for the software specified by
    *        #graphSoft. NOTE: see also the more recent class
   ...
   /**
    * Formats data to plot the empirical distribution of @f$U_{(1)},…,U_{(N)}@f$,
    * which are assumed to be in `data[0...N-1]`, and to compare it with the
    * uniform distribution. The @f$U_{(i)}@f$ must be sorted. The two
    * endpoints @f$(0, 0)@f$ and @f$(1, 1)@f$ are always included in the plot. The
    * string `desc` gives a short caption for the graphic plot. The data is printed
    * in a format suitable for the software specified by #graphSoft. NOTE: see also
    * the more recent class @ref umontreal.ssj.charts.EmpiricalChart.
    * 
   ...
   public static String graphDistUnif(DoubleArrayList data, String desc) {
      ...
      case GNUPLOT:
         ...
         for (i = 0; i < n; i++)
            sb.append(
                  PrintfFormat.g(16, 8, u[i]) + "  " + PrintfFormat.g(16, 8, (i + 1) * unSurN) + PrintfFormat.NEWLINE);

         sb.append(PrintfFormat.g(16, 8, 1.0) + "  " + PrintfFormat.g(16, 8, 1.0) + PrintfFormat.NEWLINE
               + PrintfFormat.NEWLINE);
         break;
      case MATHEMATICA:
         sb.append("(*----------------------------------*)" + PrintfFormat.NEWLINE);
         sb.append("(* " + PrintfFormat.s(-70, desc) + PrintfFormat.NEWLINE + " *)" + PrintfFormat.NEWLINE
               + PrintfFormat.NEWLINE + "points = { " + PrintfFormat.NEWLINE);

         sb.append(formatMath2(0.0, 0.0) + "," + PrintfFormat.NEWLINE);
         for (i = 0; i < n; i++)
            sb.append(formatMath2(u[i], (i + 1) * unSurN) + "," + PrintfFormat.NEWLINE);
    ...
    * @f$\epsilon@f$ or larger than @f$1-\epsilon@f$ is *not* written explicitly;
    ...
    * 
    * @f$p@f$-values should be marked as suspect when printing test results. If
    *                `SUSPECTP` @f$= \alpha@f$, then any @f$p@f$-value less
    *                than @f$\alpha@f$ or larger than @f$1-\alpha@f$ is considered
    *                suspect and is "singled out" by `formatp1`. The default value
    *                is 0.01.
    */
   public static double SUSPECTP = 0.01;

   /**
    * Returns the @f$p@f$-value @f$p@f$ of a test, in the format "@f$1-p@f$"
    * if @f$p@f$ is close to 1, and @f$p@f$ otherwise. Uses the environment
    * variable #EPSILONP and replaces @f$p@f$ by
    * 
   ...
   /**
    * Returns the string "<tt>p-value of test : </tt>", then calls #formatp0 to
    * print @f$p@f$, and adds the marker "<tt>****</tt>" if
    * 
    * @f$p@f$ is considered suspect (uses the environment variable `SUSPECTP` for
    *         this).
    * @param p the @f$p@f$-value to be formated
   ...
   public static String formatp2(double x, double p) {
      ...
      return PrintfFormat.format(8, 2, 1, x) + PrintfFormat.NEWLINE + formatp1(p);
   }

   /**
    * Formats the test statistic `x` for a test named `testName` with
    * 
    * @f$p@f$-value `p`. The first line of the returned string contains the name of
    *               the test and the statistic whereas the second line contains its
    *               p-value. The formated values of `x` and `p` are aligned.
   ...
   /**
    * Computes the @f$p@f$-value of the chi-square statistic `chi2` for a test with
    * `k` intervals. Uses @f$d@f$ decimal digits of precision in the calculations.
    * The result of the test is returned as a string. The @f$p@f$-value is computed
    * using GofStat.pDisc.
    * 
   ...
   public static String formatChi2(int k, int d, double chi2) {
      ...
      sb.append(PrintfFormat.NEWLINE + "p-value                               : "
            + formatp0(GofStat.pDisc(ChiSquareDist.cdf(k - 1, d, chi2), ChiSquareDist.barF(k - 1, d, chi2))));
      sb.append(PrintfFormat.NEWLINE + PrintfFormat.NEWLINE);
      return sb.toString();
   }
    ...
    * statistics @f$D_N^+@f$, @f$D_N^-@f$, and @f$D_N@f$, whose values are in `dp,
    * dm, d`, respectively, assuming a sample of size `n`. Then formats these
    * statistics and their @f$p@f$-values using #formatp2 for each one.
    * 
    * @param n  sample size
    * @param dp value of the @f$D_N^+@f$ statistic
    * @param dm value of the @f$D_N^-@f$ statistic
    * @param d  value of the @f$D_N@f$ statistic
    * @return the string representation of the Kolmogorov-Smirnov statistics and
    *         their p-values
    */
   public static String formatKS(int n, double dp, double dm, double d) {
      // Prints the results of a Kolmogorov-Smirnov test
      return "Kolmogorov-Smirnov+ statistic = D+    :" + formatp2(dp, KolmogorovSmirnovPlusDist.barF(n, dp))
            + "Kolmogorov-Smirnov- statistic = D-    :" + formatp2(dm, KolmogorovSmirnovPlusDist.barF(n, dm))
            + "Kolmogorov-Smirnov statistic = D      :" + formatp2(d, KolmogorovSmirnovDistQuick.barF(n, d))
            + PrintfFormat.NEWLINE + PrintfFormat.NEWLINE;
   }

   /**
    * Computes the KS test statistics to compare the empirical distribution of the
    * observations in `data` with the theoretical distribution `dist` and formats
    * the results. See also method
    ...
    * statistic @f$D_N^+(a)@f$ defined in ( {@link REF_gof_FDist_eq_KSPlusJumpOne
    * KSPlusJumpOne} ). Writes a header, computes the @f$p@f$-value and calls
    * #formatp2.
    * 
    * @param n  sample size
    * @param a  size of the jump
    * @param dp value of @f$D_N^+(a)@f$
    * @return the string representation of the Kolmogorov-Smirnov statistic and its
    *         p-value
    */
   public static String formatKSJumpOne(int n, double a, double dp) {
      double d = 1.0 - FDist.kolmogorovSmirnovPlusJumpOne(n, a, dp);

      return PrintfFormat.NEWLINE + "Kolmogorov-Smirnov+ statistic = D+    : " + PrintfFormat.g(8, 2, dp)
            + PrintfFormat.NEWLINE + formatp1(d) + PrintfFormat.NEWLINE;
   }

   /**
    * Similar to #formatKS(DoubleArrayList,ContinuousDistribution), but
    * for @f$D_N^+(a)@f$ defined in ( {@link REF_gof_FDist_eq_KSPlusJumpOne
    * KSPlusJumpOne} ).
    * 
    * @param data array of observations to be tested
    * @param dist assumed distribution of the data
    * @param a    size of the jump
    * @return string representation of the Kolmogorov-Smirnov statistic and its
    *         p-value
    */
   ...
   /**
    * Name of each `testType` test. Could be used for printing the test results,
    * for example.
    */
   public static final String[] TESTNAMES = { "KolmogorovSmirnovPlus", "KolmogorovSmirnovMinus", "KolmogorovSmirnov",
         "Anderson-Darling", "CramerVon-Mises", "Watson G", "Watson U", "Mean", "Correlation" };

   /**
    * The set of EDF tests that are to be performed when calling the methods
    * #activeTests, #formatActiveTests, etc. By default, this set contains `KSP`,
    * `KSM`, and `AD`. Note: `MEAN` and `COR` are *always excluded* from this set
    * of active tests. The valid indices for this array are #KSP, #KSM, #KS, #AD,
   ...
   /**
    * Computes all EDF test statistics enumerated above (except <tt>COR</tt>) to
    * compare the empirical distribution of @f$U_{(0)},…,U_{(N-1)}@f$ with the
    * uniform distribution, assuming that these sorted observations are in
    * `sortedData`. If @f$N > 1@f$, returns `sVal` with the values of the KS
    * statistics @f$D_N^+@f$,
    * 
    ...
    *      `sVal[KSP]`. Calling this method is more efficient than computing these
    *      statistics separately by calling the corresponding methods in
    * @ref GofStat.
   ...
   /**
    * The observations @f$V@f$ are in `data`, not necessarily sorted, and their
    * empirical distribution is compared with the continuous distribution `dist`.
    * If @f$N = 1@f$, only puts `data.get (0)` in `sVal[MEAN]`, and @f$1 -
    * {}@f$<tt>dist.cdf (data.get (0))</tt> in `sVal[KSP]`.
    * 
   ...
   /**
    * Computes the EDF test statistics by calling #tests(DoubleArrayList,double[]),
    * then computes the @f$p@f$-values of those that currently belong to
    * `activeTests`, and return these quantities in `sVal` and `pVal`,
    * respectively. Assumes that @f$U_{(0)},…,U_{(N-1)}@f$ are in `sortedData`
    * and that we want to compare their empirical distribution with the uniform
    * distribution. If @f$N = 1@f$, only puts @f$1 -
    * {}@f$<tt>sortedData.get (0)</tt> in `sVal[KSP], pVal[KSP]`, and `pVal[MEAN]`.
    * 
    * @param sortedData array of sorted observations
    * @param sVal       array that will be filled with the results of the tests
   ...
   /**
    * The observations are in `data`, not necessarily sorted, and we want to
    * compare their empirical distribution with the distribution `dist`. If @f$N =
    * 1@f$, only puts `data.get(0)` in `sVal[MEAN]`, and
    * 
    * @f$1 - {}@f$<tt>dist.cdf (data.get (0))</tt> in `sVal[KSP], pVal[KSP]`, and
    *      `pVal[MEAN]`.
    * @param data array of observations to test
    * @param dist assumed distribution of the observations
    * @param sVal array that will be filled with the results of the tests
   ...
   /**
    * Gets the @f$p@f$-values of the *active* EDF test statistics, which are in
    * `activeTests`. It is assumed that the values of these statistics and
    * their @f$p@f$-values are *already computed*, in `sVal` and `pVal`, and that
    * the sample size is `n`. These statistics and @f$p@f$-values are formated
    * using #formatp2 for each one. If `n=1`, prints only `pVal[KSP]` using
    * #formatp1.
    * 
    * @param n    sample size
   ...
   public static String formatActiveTests(int n, double[] sVal, double[] pVal) {
      ...
      if (activeTests[KSP])
         sb.append("Kolmogorov-Smirnov+ statistic = D+    :" + formatp2(sVal[KSP], pVal[KSP]));
      if (activeTests[KSM])
         sb.append("Kolmogorov-Smirnov- statistic = D-    :" + formatp2(sVal[KSM], pVal[KSM]));
      if (activeTests[KS])
         sb.append("Kolmogorov-Smirnov statistic  = D     :" + formatp2(sVal[KS], pVal[KS]));
      if (activeTests[AD])
         sb.append("Anderson-Darling statistic = A2       :" + formatp2(sVal[AD], pVal[AD]));
      if (activeTests[CM])
         sb.append("Cramer-von Mises statistic = W2       :" + formatp2(sVal[CM], pVal[CM]));
      if (activeTests[WG])
         sb.append("Watson statistic = G                  :" + formatp2(sVal[WG], pVal[WG]));
      if (activeTests[WU])
         sb.append("Watson statistic = U2                 :" + formatp2(sVal[WU], pVal[WU]));
      sb.append(PrintfFormat.NEWLINE);
      return sb.toString();
   }
    ...
    * transformation to the @f$U_{(0)},…,U_{(N-1)}@f$, assuming that these
    * observations are in `sortedData`, then computes the EDF test statistics and
    * calls #activeTests(DoubleArrayList,double[],double[]) after each
    * transformation. The function returns the *original* array `sortedData` (the
    * transformations are applied on a copy of <tt>sortedData</tt>). If `printval =
    * true`, stores all the values into the returned String after each iteration.
    * If `graph = true`, calls #graphDistUnif after each iteration to print to
    * stream `f` the data for plotting the distribution function of the @f$U_i@f$.
    * 
    * @param sortedData array containing the sorted observations
    * @param k          number of times the tests are applied
    * @param printval   if `true`, stores all the values of the observations at
    *                   each iteration
    * @param graph      if `true`, the distribution of the @f$U_i@f$ will be
    *                   plotted after each iteration
    * @param f          stream where the plots are written to
    * @return a string representation of the test results
    */
   public static String iterSpacingsTests(DoubleArrayList sortedData, int k, boolean printval, boolean graph,
         PrintWriter f) {
      ...
      for (j = 1; j <= k; j++) {
         sb.append("-----------------------------------" + PrintfFormat.NEWLINE
               + "EDF Tests after \"iterateSpacings\", level : " + PrintfFormat.d(2, j) + PrintfFormat.NEWLINE);
    ...
    * @return a string representation of the test results
    */
   public static String iterPowRatioTests(DoubleArrayList sortedData, int k, boolean printval, boolean graph,
         PrintWriter f) {
      ...
      for (i = 1; i <= k; i++) {
         ...
         sb.append("-----------------------------------" + PrintfFormat.NEWLINE
               + "EDF Tests after \"powerRatios\", level : " + PrintfFormat.d(2, i) + PrintfFormat.NEWLINE);
