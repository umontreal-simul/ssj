 ...
 * should be defined as a subclass of the class `Event`, and should provide an
 * implementation of the method #actions which is executed when an event of this
 * type occurs. The instances of these subclasses are the actual events.
 *
 * Each event is linked to a simulator represented by an instance of
 * 
 * @ref Simulator before it can be scheduled and processed. A default simulator,
 *      given by `Simulator.getDefaultSimulator`, is used if no simulator is
 *      linked explicitly with an event. When an event is constructed, it is not
 *      scheduled. It must be scheduled separately by calling one of the
 *      scheduling methods #schedule, #scheduleNext, #scheduleBefore, etc. An
 *      event can also be cancelled before it occurs.
 *
 ...
 *      priority. For example, if events `e1` and `e2` occur at the same time
 *      with priority 2 and 1 respectively, then `e2` will occur before `e1`.
 *      Events with the same time and priority occur in the order they were
 *      scheduled.
 *
...
public abstract class Event implements Comparable<Event> {
   ...
   /**
    * Constructs a new event instance, which can be placed afterwards into the
    * event list of the default simulator by calling one of the `schedule...`
    * variants. For example, if `Bang` is an `Event` subclass, the statement
    * "<tt>new Bang().scheduleNext();</tt>" creates a new `Bang` event and places
    ...
    *       the past."); }\end{hide}
    *
    *       @endcode Constructs a new event and inserts it in the event list of the
    *       default simulator. If `delay >= 0.0`, the event is scheduled to happen
    *       in `delay` units of simutated time. If two or more events are scheduled
    *       to happen at the same time, events with the highest priorities (lowest
    *       value of the `priority` field) occur first. If two or more events are
    *       schedule to the same time, with the same priority, they are placed in
    *       the event list (and executed) in the same order as they have been
    *       scheduled.
    *
    *       We recall that such constructors with parameters are not inherited
    *       automatically by the subclasses in Java, but they can be invoked using
    *       `super`. For example, one can have
    *
    * @code
    *
    ...
    * `sim.time() + delay`, by inserting it in the event list. When two or more
    * events are scheduled to happen at the same time and with the same priority,
    * they are placed in the event list (and executed) in the same order as they
    * have been scheduled. Note that the priority of this event should be adjusted
    * using #setPriority *before* it is scheduled.
    * 
    * @param delay simulation time that must pass before the event happens
    */
   ...
   /**
    * Schedules this event as the *first* event in the event list, to be executed
   ...
   /**
    * Schedules this event to happen just before, and at the same time, as the
    * event `other`. For example, if `Bing` and `Bang` are `Event` subclasses,
    ...
    *
    *       @endcode the event list contains two new events scheduled to happen in
    *       12 units of time: a `Bing` event, followed by a `Bang` called `bigOne`.
   ...
   /**
    * Schedules this event to happen just after, and at the same time, as the event
    * `other`.
    * 
   ...
   /**
    * Cancels this event before it occurs. Returns `true` if cancellation succeeds
    * (this event was found in the event list), `false` otherwise.
    * 
   ...
   /**
    * Finds the first occurence of an event of class "type" in the event list, and
    * cancels it. Returns `true` if cancellation succeeds, `false` otherwise.
    * 
   ...
   /**
    * Sets the simulator associated with this event to `sim`. This method should
    * not be called while this event is in an event list.
    * 
   ...
   /**
    * Sets the (planned) time of occurence of this event to `time`. This method
    * should never be called after the event was scheduled, otherwise the events
    * would not execute in ascending time order anymore.
    * 
   ...
   /**
    * Sets the priority of this event to `inPriority`. This method should never be
    * called after the event was scheduled, otherwise the events would not execute
    * in ascending priority order anymore.
    * 
    ...
    * Returns @f$-1@f$ or @f$+1@f$ as this event occurs before or after the
    * specified event `e`, respectively. If the two events occur at the same time,
    * then returns @f$-1@f$, @f$0@f$, or @f$+1@f$ as this event has a smaller,
   ...
   /**
    * This is the method that is executed when this event occurs. Every subclass of
    * `Event` that is to be instantiated must provide an implementation of this
    * method.
    */
