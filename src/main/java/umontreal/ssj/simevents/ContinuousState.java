 ...
 * continuous-time simulation. Any simulator, including the default static one,
 * can have an associate continuous state which is obtained using the
 * `continuousState()` method of the @ref Simulator class. This state includes
 * all active integration variables as well as the current integration method.
 *
 * One of the methods #selectEuler, #selectRungeKutta2 or #selectRungeKutta4
 * must be called before starting any integration. These methods permit one to
 * select the numerical integration method and the step size `h` (in time units)
 * that will be used for *all* continuous-time variables linked to the
 * simulator. For all the methods, an integration step at time @f$t@f$ changes
 * the values of the variables from their old values at time @f$t-h@f$ to their
 * new values at time @f$t@f$.
 *
...
public class ContinuousState {
   ...
   /**
    * Creates a new @ref ContinuousState object linked to the given simulator.
    * Usually, the user should not call this constructor directly since a new
    * object is created automatically by the `continuousState()` method of
    * class @ref Simulator.
    */
   ...
   /**
    * Returns the list of continuous-time variables currently integrated by the
    * simulator. The returned list is updated automatically as variables are added
    * or removed, but it cannot be modified directly. One must instead use
    * `startInteg` or `stopInteg` in class
    * 
   ...
   /**
    * Stops the integration process for @ref Continuous variable. The variable
