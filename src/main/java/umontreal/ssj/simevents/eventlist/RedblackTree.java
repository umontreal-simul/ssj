 ...
 * black. When modifying the structure, the tree is reorganized for the colors
 * to satisfy rules that give an average @f$O(\log(n))@f$ time for removing the
 * first event or inserting a new event, where @f$n@f$ is the number of elements
 * in the structure. However, adding or removing events imply reorganizing the
 * tree and requires more overhead than a binary search tree.
 *
 * The present implementation uses the Java 2 TreeMap class which implements a
 * red black tree for general usage. This event list implementation is not
 * efficient.
 *
...
public class RedblackTree implements EventList {
	...
	public Event getFirst() {
		return isEmpty() ? null : tree.get(tree.firstKey()).events.get(0);
	}
	...
	public String toString() {
		...
		for (Node node : tree.values()) {
			for (Event ev : node.events)
				sb.append(PrintfFormat.NEWLINE + PrintfFormat.g(12, 7, ev.time()) + ", "
						+ PrintfFormat.g(8, 4, ev.priority()) + " : " + ev.toString());
		}
		return sb.toString();
	}
	...
	private static class Node {
		...
		/**
		 * Remove an event from a node. Returns true if the node becomes empty.
		 */
		public boolean remove(Event ev) {
			Iterator<Event> itr = events.iterator();
			while (itr.hasNext()) {
	...
	private class RBItr implements ListIterator<Event> {
		...
		RBItr() {
			...
			prevNode = null;
			nextNode = tree.isEmpty() ? null : (Node) tree.get(tree.firstKey());
			prevNodeIndex = 0;
			nextNodeIndex = 0;
			nextIndex = 0;
		...
		public boolean hasNext() {
			...
			return nextNode != null && nextNodeIndex < nextNode.events.size();
		}

		public boolean hasPrevious() {
			if (modCount != expectedModCount)
				throw new ConcurrentModificationException();
			return prevNode != null && prevNodeIndex >= 0;
		}
