...
/**
 * An implementation of @ref EventList using a doubly linked linear list. Each
 * event is stored into a list node that contains a pointer to its following and
 * preceding events. Adding an event requires a linear search to keep the event
 * list sorted by event time and priority. Removing the first event is done in
 * constant time because it simply removes the first list node. List nodes are
 * recycled for increased memory management efficiency.
 *
