 ...
 *       the worst case of the linked list. This gives a @f$O(\log(n))@f$
 *       average time for adding or removing an event, where @f$n@f$ is the size
 *       of the event list.
 *
...
public class SplayTree implements EventList {
	 ...
	 * 
	 * Procedure : On fait d'abord un splay sur l'entree a enlever pour la faire
	 * monter a la racine. Une fois a la racine, on l'enleve. On cherche ensuite
	 * l'element minimal du sous-arbre droit (qui contient les elements qui sont
	 * plus grand que e) et, avec un splay, on le remonte a la racine de son
	 * sous-arbre. Cet element, puisqu'il est le minimum de son sous-arbre, n'a pas
	 * de fils gauche. On lui rattache donc comme fils gauche le sous-arbre gauche.
	 * 
	 * La raison de cette procedure plus compliquee que l'equivalent dans un arbre
	 * binaire ordinaire est que, pour que la garantie de temps d'acces amorti en
	 * O(log n) du splay tree s'applique, il faut que chaque acces (qui necessite
	 * une recherche) d'un element s'accompagne du splay de cet element a la racine.
	 */
