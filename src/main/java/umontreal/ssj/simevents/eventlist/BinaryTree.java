...
/**
 * An implementation of @ref EventList using a binary search tree. Every event
 * is stored into a tree node which has left and right children. Using the event
 * time as a comparator the left child is always smaller than its parent whereas
 * the right is greater or equal. This allows an average
 * 
 * @f$O(\log(n))@f$ time for adding an event and searching the first event,
 *                  where @f$n@f$ is the number of events in the structure.
 *                  There is less overhead for adding and removing events than
 *                  splay tree or red black tree. However, in the worst case,
 *                  adding or removing could be done in time proportional
 *                  to @f$n@f$ because the binary search tree can be turned into
 *                  a linked list.
 *
...
public class BinaryTree implements EventList {
   ...
   public void addFirst(Event ev) {
      /**
       * Ajoute "ev" comme premier evenement dans l'arbre. Donc completement a gauche
       * On met l'ancien premier evenement a droite de ev. (Necessaire quand on a des
       * evenements simultanes)
       */
      Entry cursor = root;
   ...
   /**
    * fonction qui trouve le noeud (Entry) d'un evenement dans l'arbre
    */
