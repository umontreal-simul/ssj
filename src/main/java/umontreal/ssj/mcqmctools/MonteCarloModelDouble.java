 ...
 *      is a real-valued random variable @f$X@f$. We could have asked `simulate`
 *      to return the performance to avoid a separate call to `getPerformance`,
 *      but we decided not, because in some situations one may not always need
 *      the performance computed by `getPerformance`, which might be costly to
 *      compute for nothing, but only some other output information. This also
 *      applies to @ref MonteCarloModel.
 */
...
public interface MonteCarloModelDouble {
   ...
   /**
    * Recovers and returns the realization of the performance measure, of type
    * `double`.
    */
