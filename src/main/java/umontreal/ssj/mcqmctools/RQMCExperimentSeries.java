...
/**
 * This class offers facilities to perform experiments on the convergence of the
 * variance when estimating a mean (expectation) with a series of RQMC point
 * sets usually of the same type, but different sizes @f$n@f$. The series of
 * RQMC point sets of different sizes can be passed in an array to the
 * constructor. The method @ref testVarianceRate performs an experiment with a
 * given @ref MonteCarloModelDouble and the series of point sets. In this
 * experiment, for each size @f$n@f$ of point set, @f$m@f$ independent
 * replicates of the RQMC estimator (which is an average over RQMC @f$n@f$
 * points) are computed. One can then recover the average and the empirical
 * variance of these @f$m@f$ replicates of the @f$n@f$-point average, their logs
 * in any base, etc., in arrays, as well as the estimated linear regression of
 * log(variance) as a function of log(n).
 * 
 * One can also perform experiments with many such series of point sets for the
 * same model, and display the results for the different series on the same
 * plot. Likewise, one can also perform an experiment with many different
 * estimators for the same model and same point sets.
 */
...
public class RQMCExperimentSeries {
   ...
   /**
    * When set to true, a real-time display of execution results and CPU times will
    * be printed on the default output.
    */
   ...
   /**
    * Returns the vector of variances from the last experiment. Each variance in
    * the vector is the empirical variance of the m replicates of the RQMC
    * estimator. To obtain the variance per run, it must be multiplied by n.
    */
    ...
    * coefficients (constant and slope) in two-dimensional vector. The first
    * numSkip values in the array are skipped (not used) to make the regression.
   ...
   /**
    * Takes the regression coefficients of log(variance) in #regCoeff and returns a
    * two-line string that reports on these coefficients.
    * 
    ...
    * format to produce a plot with the pgfplot package.
    * 
    * @return Report as a string.
    */
   ...
   /**
    * Performs an experiment (testVarianceRate) for each point set series in the
    * given list, and returns a report as a string.
    * 
    * @param model
    * @param list
    * @param m
