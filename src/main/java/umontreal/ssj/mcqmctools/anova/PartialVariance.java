...
/**
 * Represents the partial variance of a function with respect to a given
 * coordinate set.
 *
 */
public class PartialVariance extends Tally implements Comparable<PartialVariance> {
   ...
   /**
    * Returns the sensitivity index (fraction of the total variance) of the
    * coordinate set under consideration.
    *
    * Throws an IllegalStateException if the total variance is not set. Should be
    * overridden in deriving classes.
    */
   public double sensitivityIndex() {

      if (totalVar == null)
         throw new IllegalStateException(
               "trying to access the sensitivity index without" + " a reference to the total variance");

      return average() / totalVar.average();
   }

   /**
    * Returns 1, 0 or -1 if the current partial variance is larger, equal or
    * smaller than the partial variance \c var, respectively.
    *
    */
