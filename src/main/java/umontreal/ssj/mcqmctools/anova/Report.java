...
public class Report {
   ...
   /**
    * Set to <0 for left-aligned label with given width. Set to 0 for
    * label-adjusted width. Set to >0 for right-aligned value with given width.
    *
    */
   ...
   /**
    * Set to <0 for left-aligned value with given width. Set to 0 for
   ...
   public Report(String label, String value) {
      this.label = label;
      this.value = (value == null) ? null : String.format("%" + (valueWidth == 0 ? "" : valueWidth) + "s", value);
   ...
   public String toString(int baseIndentation) {
      ...
      if (label != null) {
         ...
         if (value != null)
            s += String.format("%" + (labelWidth == 0 ? "" : labelWidth) + "s %s", label + ":", value);
         else
            s += "==> " + label;
         s += NEWLINE;
      }
