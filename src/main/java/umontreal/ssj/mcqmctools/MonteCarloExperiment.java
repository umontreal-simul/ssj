 ...
 *      The `RandomStream` used for the experiment is reset to a new substream
 *      after each run.
 */
...
public class MonteCarloExperiment {
    ...
    * for a model of type @ref MonteCarloModelDoubleArray. Consequently, the
   ...
   /**
    * Given statistics collected in `statX` and `statC` as with @ref
    * simulateRunsCV, this method computes the mean and variance of the estimators
    * with and without the CV and returns them in the two-dimensional vectors
    * `mean` and `variance` (mean[0] is the value without CV, mean[1] the value
    * with CV, and similarly for the variance.
    */
   ...
   /**
    * Performs `n` simulation runs to estimate the difference in performance
    * between `model2` and `model1`, divided by `delta`, using common random
    * numbers (CRN) across the two models. One substream is used for each run and
    * the same `n` substreams are used for the two models. Returns the statistics
    * on the `n` differences in `statDiff`. By taking `delta` = 1, this just
    * estimates the difference. By taking `delta` > 0 very small, and if the two
    * models are in fact the same model but with a parameter that differs by
    * `delta`, this gives a finite-difference estimator of the derivative of the
    * performance with respect to this parameter.
    */
   ...
   public static String simulateRunsDefaultReportStudent(MonteCarloModelDouble model, int n, RandomStream stream,
      ...
      str.append(10, d, d - 1, statValue.variance());
      str.append("\n");
      // str.append("\n");
   ...
   /**
    * In this version, there is no need to provide a `Chrono`; it is created
    * inside.
    */
   public static String simulateRunsDefaultReportStudent(MonteCarloModelDouble model, int n, RandomStream stream,
   ...
   public static String simulateRunsDefaultReportCV(MonteCarloModelCV model, int n, RandomStream stream,
      ...
      statWithCV.varianceWithCV(varCV);   //  Is this implemented for more than one CV ?  ***********
   ...
   /**
    * In this one the timer is created internally (and cannot be accessed
    * externally).
    */
   public static String simulateRunsDefaultReportCV(MonteCarloModelCV model, int n, RandomStream stream,
    ...
    */
   public static String simulateRunsDefaultReportCV(MonteCarloModelCV model, int n, RandomStream stream, double[] mean,
         double[] variance, double level, int d, Chrono timer) {
      PrintfFormat str = new PrintfFormat();
      timer.init();
