 ...
 * sets as in class  @ref LCGPointSet, but implemented differently. The
 * parameters are the modulus @f$n@f$ and the multiplier @f$a@f$, for an
 * arbitrary integer @f$1 \le a < n@f$. [When @f$a@f$ is outside the interval
 * @f$[1,n)@f$, then we replace @f$a@f$ by (@f$a \bmod n@f$) in all
 * calculations.] The number of points is @f$n@f$, their dimension is
 * @f$s@f$, and they are defined by
 * @f[
 ...
 * @f$t@f$ coordinates rejected. The @f$s@f$-dimensionnal points are then
 * defined as
 * @f[
 *   \mathbf{u}_i = (i/n)(a^t, a^{t+1}, a^{t+2}, …, a^{t+s-1}) \bmod1
 * @f]
 * for @f$i=0,…,n-1@f$ and fixed @f$t@f$.
 *
...
public class KorobovLattice extends Rank1Lattice {
    ...
    * @f$t@f$ coordinates of a standard Korobov lattice are dropped as
    * described above. The case @f$t=0@f$ corresponds to the standard
    * Korobov lattice.
    */
   ...
   /**
    * Returns the multiplier @f$a@f$ of the lattice. (The original one
    * before it is reset to @f$a \bmod n@f$).
    */
   public int getA() {
      return genA;
   }
