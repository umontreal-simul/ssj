 ...
 * matrices @f$\mathbf{C}_j@f$ are stored in a large array of size @f$sk@f$. The @f$c@f$-th column
 * of @f$\mathbf{C}_j@f$, for @f$c=0,\dots,k-1@f$, is stored at position @f$jk + c@f$ of that array,
 * as a 32-bit integer. For all derived classes, the above 32-bit integer must be of the form @f$
 * [0\, 0\, \cdots\, C_0\, C_1 \cdots \, C_{r-1}]@f$. The value of @f$k@f$ cannot exceed 31 (32 is
 * not allowed because Java does not have 32-bit unsigned integers). The value of @f$w@f$ is always
 * 31. In this class, the random digital shift in base 2 generated by `addRandomShift` corresponds
 * to a random XOR.
 * 
