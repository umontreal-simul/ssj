 ...
 * nature of computers and is much more efficient than for the general case. Binary expansions are
 * easy to obtain because the computer already uses them internally. The generator matrices
 * 
 * @f$\mathbf{C}_j@f$ are stored in a large array of size @f$sk@f$. The
 * @f$c@f$-th column of @f$\mathbf{C}_j@f$, for @f$c=0,…,k-1@f$, is stored at position @f$jk + c@f$
 *            of that array, as a 32-bit integer. For all derived classes, the above 32-bit integer
 *            must be of the form @f$ [0 0 \cdots C_0 C_1 \cdots C_{r-1}]@f$. The value of @f$k@f$
 *            cannot exceed 31 (32 is not allowed because Java does not have 32-bit unsigned
 *            integers). The value of
 * @f$w@f$ is always 31.
 * @remark **Pierre:** In this implementation, @f$w@f$ is always used in place of @f$r@f$ so the
 *         value of @f$r@f$ is not used.
 *
 *         The random digital shift in base 2 corresponds to a random XOR. It can be applied via the
 *         method #addRandomShift.
 *
...
public class DigitalNetBase2 extends DigitalNet {
	 ...
	 * Each integer corresponds to a column of bits.
	 */
