...
/**
 * Extends the class @ref ContinuousDistribution2Dim for the *bivariate normal*
 * distribution @cite tJOH72a&thinsp; (page 84). It has means @f$E[X]
 * =\mu_1@f$, @f$E[Y] =\mu_2@f$, and variances var@f$[X] = \sigma_1^2@f$,
 * var@f$[Y] = \sigma_2^2@f$ such that @f$\sigma_1 > 0@f$ and @f$\sigma_2 >
 * 0@f$. The correlation between @f$X@f$ and @f$Y@f$ is @f$\rho@f$. Its density
 * function is
 * 
 * @anchor REF_probdistmulti_BiNormalDist_eq_f1binormal @f[ f (x, y) =
 *         \frac{1}{2\pi\sigma_1\sigma_2\sqrt{1-\rho^2}}e^{-T}
 *         \tag{f1binormal} @f] @f[ T =
 *         \frac{1}{2(1-\rho^2)}\left[\left(\frac{x-\mu_1}{\sigma_1}\right)^2 -
 *         2\rho\left(\frac{x-\mu_1}{\sigma_1}\right)
 *         \left(\frac{y-\mu_2}{\sigma_2}\right) +
 *         \left(\frac{y-\mu_2}{\sigma_2}\right)^2\right] @f] and the
 *         corresponding distribution function is (the `cdf` method)
 * @anchor REF_probdistmulti_BiNormalDist_eq_cdf1binormal @f[ \Phi(\mu_1,
 *         \sigma_1, x, \mu_2, \sigma_2, y, \rho) =
 *         \frac{1}{2\pi\sigma_1\sigma_2\sqrt{1-\rho^2}} \int_{-\infty}^x dx
 *         \int_{-\infty}^y dy  e^{-T}. \tag{cdf1binormal} @f] We also define
 *         the upper distribution function (the `barF` method) as
 * @anchor REF_probdistmulti_BiNormalDist_eq_cdf3binormal @f[
 *         \overline{\Phi}(\mu_1, \sigma_1, x, \mu_2, \sigma_2, y, \rho) =
 *         \frac{1}{2\pi\sigma_1\sigma_2\sqrt{1-\rho^2}} \int^{\infty}_x dx
 *         \int^{\infty}_y dy  e^{-T}. \tag{cdf3binormal} @f]
 *         When @f$\mu_1=\mu_2=0@f$ and @f$\sigma_1=\sigma_2=1@f$, we have the
 *         *standard binormal* distribution, with corresponding distribution
 *         function
 * @anchor REF_probdistmulti_BiNormalDist_eq_cdf2binormal @f[ \Phi(x, y, \rho) =
 *         \frac{1}{2\pi\sqrt{1-\rho^2}} \int_{-\infty}^x dx \int_{-\infty}^y
 *         dy  e^{-S} \tag{cdf2binormal} @f] @f[ S = \frac{x^2 - 2\rho x y +
 *         y^2}{2(1-\rho^2)}. @f] <div class="SSJ-bigskip"></div>
 *
...
public class BiNormalDist extends ContinuousDistribution2Dim {
   ...
   /**
    * Constructs a `BiNormalDist` object with default parameters @f$\mu_1 = \mu_2 =
    * 0@f$, @f$\sigma_1 = \sigma_2 = 1@f$ and correlation
    * 
    * @f$\rho= @f$<tt> rho</tt>.
    */
    ...
    * {@link REF_probdistmulti_BiNormalDist_eq_f1binormal f1binormal} )
    * with @f$\mu_1 = \mu_2 = 0@f$ and @f$\sigma_1 = \sigma_2 = 1@f$.
    */
    ...
    * {@link REF_probdistmulti_BiNormalDist_eq_f1binormal f1binormal} ) with
    * parameters @f$\mu_1@f$ = `mu1`, @f$\mu_2@f$ = `mu2`, @f$\sigma_1@f$ =
    * `sigma1`, @f$\sigma_2@f$ = `sigma2` and
    * 
    * @f$\rho@f$ = `rho`.
    */
    ...
    * {@link REF_probdistmulti_BiNormalDist_eq_cdf2binormal cdf2binormal} ) using
    * the fast Drezner-Wesolowsky method described in
    * 
...
//          if (h7 > 0.  && -h3 < 500.0)
    ...
    * {@link REF_probdistmulti_BiNormalDist_eq_cdf1binormal cdf1binormal} ) with
    * parameters @f$\mu_1@f$ = `mu1`, @f$\mu_2@f$ = `mu2`, @f$\sigma_1@f$ =
    * `sigma1`, @f$\sigma_2@f$ = `sigma2` and
    * 
   ...
   /**
    * Computes the standard upper *binormal* distribution with @f$\mu_1 = \mu_2 =
    * 0@f$ and @f$\sigma_1 = \sigma_2 = 1@f$. Uses the fast Drezner-Wesolowsky
    * method described in @cite tDRE90a&thinsp;. The absolute error is expected to
    * be smaller than @f$2 \cdot10^{-7}@f$.
    */
    ...
    * {@link REF_probdistmulti_BiNormalDist_eq_cdf3binormal cdf3binormal} ) with
    * parameters @f$\mu_1@f$ = `mu1`, @f$\mu_2@f$ = `mu2`, @f$\sigma_1@f$ =
    * `sigma1`, @f$\sigma_2@f$ = `sigma2` and
    * 
    * @f$\rho@f$ = `rho`. Uses the fast Drezner-Wesolowsky method described in
