 ...
 * density is
 * 
 * @anchor REF_probdistmulti_DirichletDist_eq_fDirichlet @f[ f(x_1,…, x_d) =
 *         \frac{\Gamma(\alpha_0) \prod_{i=1}^d x_i^{\alpha_i -
 *         1}}{\prod_{i=1}^d \Gamma(\alpha_i)} \tag{fDirichlet} @f] where @f$x_i
 *         \ge0@f$, @f$\sum_{i=1}^d x_i = 1@f$, @f$\alpha_0 = \sum_{i=1}^d
 *         \alpha_i@f$, and @f$\Gamma@f$ is the Gamma function.
 *
...
public class DirichletDist extends ContinuousDistributionMulti {
   ...
   /**
    * Computes the density ( {@link REF_probdistmulti_DirichletDist_eq_fDirichlet
    * fDirichlet} ) of the Dirichlet distribution with parameters (@f$\alpha_1@f$,
    * …, @f$\alpha_d@f$).
    */
   ...
   /**
    * Estimates the parameters [@f$\hat{\alpha_1},…,\hat{\alpha_d}@f$] of the
    * Dirichlet distribution using the maximum likelihood method. It uses
    * the @f$n@f$ observations of @f$d@f$ components in table
    * 
    * @f$x[i][j]@f$, @f$i = 0, 1, …, n-1@f$ and @f$j = 0, 1, …, d-1@f$. The
    *                equations of the maximum likelihood are defined in
    * @cite ccAVR04a&thinsp; (Technical appendix)
    * @f{align*}{ L(\hat{\alpha}_1,\hat{\alpha}_2,…,\hat{\alpha}_k) & = n \left(
    *             G(\alpha_0) - \sum_{i=1}^k G(\hat{\alpha}_i) \right) +
    *             \sum_{i=1}^k (\hat{\alpha}_i - 1) Z_i
    * @f} where @f$G@f$ is the logarithm of the gamma function and
    * @f{align*}{ \alpha_0 & = \sum_{i=1}^k \hat{\alpha}_i \\ Z_i & = \sum_{j=1}^n
    *             \ln(X_{i,j}) \qquad\mbox{for }i=1,…,k.
    * @f}
    * @param x the list of observations to use to evaluate parameters
    * @param n the number of observations to use to evaluate parameters
    * @param d the dimension of each vector
    * @return returns the parameter [@f$\hat{\alpha_1},…,\hat{\alpha_d}@f$]
    */
   ...
   /**
    * Returns the parameters (@f$\alpha_1@f$, …, @f$\alpha_d@f$) of this object.
    */
   ...
   /**
    * Sets the parameters (@f$\alpha_1@f$, …, @f$\alpha_d@f$) of this object.
    */
