 ...
 * 
 * @anchor REF_probdistmulti_MultiNormalDist_eq_fMultinormal @f[ f(\mathbf{x}) =
 ...
 *         \tag{fMultinormal} @f] where @f$\mathbf{x}= (x_1,…,x_d)@f$.
 *
...
public class MultiNormalDist extends ContinuousDistributionMulti {
    ...
    * {@link REF_probdistmulti_MultiNormalDist_eq_fMultinormal fMultinormal} ) of
    * the multinormal distribution with parameters
    * 
   ...
   /**
    * Returns the mean @f$E[\mathbf{X}] = \boldsymbol{\mu}@f$ of the multinormal
    * distribution with parameters @f$\boldsymbol{\mu}@f$ and
    * 
   ...
   /**
    * Estimates the parameters @f$\boldsymbol{\Sigma}@f$ of the multinormal
    * distribution using the maximum likelihood method. It uses the @f$n@f$
    * observations of @f$d@f$ components in table
    * 
