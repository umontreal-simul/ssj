 ...
 * probabilities for a random vector @f$X@f$ with a discrete distribution over
 * the integers.
 *
...
public abstract class DiscreteDistributionIntMulti {
   ...
   /**
    * Computes the cumulative probability function @f$F@f$ of the distribution
    * evaluated at `x`, assuming the lowest values start at 0, i.e. computes @f[ F
    * (x_1, x_2, …, x_d) = \sum_{s_1=0}^{x_1} \sum_{s_2=0}^{x_2}
    * \cdots\sum_{s_d=0}^{x_d} p(s_1, s_2, …, s_d). @f] Uses the naive
    * implementation, is very inefficient and may underflows.
    */
