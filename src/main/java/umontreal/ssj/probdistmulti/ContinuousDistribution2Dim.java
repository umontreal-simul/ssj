 ...
 * function @f$f(x, y)@f$; thus the signature of a `density` method is supplied
 * here. This class also provides a default implementation of
 * 
 * @f$\overline{F}(x, y)@f$, the upper `CDF`. The inverse function
 * @f$F^{-1}(u)@f$ represents a curve @f$y = h(x)@f$ of constant @f$u@f$ and it
 *                 is not implemented.
 *
...
public abstract class ContinuousDistribution2Dim extends ContinuousDistributionMulti {
    ...
    * distribution function, but there is *no guarantee* that this target is always
    * attained.
    */
   ...
   /**
    * Returns @f$f(x, y)@f$, the density of @f$(X, Y)@f$ evaluated at @f$(x, y)@f$.
    * 
    ...
    * 
    * @param x point @f$(x[0], x[1])@f$ at which the density is evaluated
   ...
   /**
    * Computes the distribution function @f$F(x, y)@f$: @f[ F(x, y) = P[X\le x, Y
    * \le y] = \int_{-\infty}^x ds \int_{-\infty}^y dt  f(s, t). @f]
    * 
    * @param x value @f$x@f$ at which the distribution function is evaluated
    * @param y value @f$y@f$ at which the distribution function is evaluated
    ...
    * 
    * @f$\overline{F}(x, y)@f$: @f[ \overline{F}(x, y) = P[X\ge x, Y \ge y] =
    *                    \int^{\infty}_x ds \int^{\infty}_y dt  f(s, t). @f]
    * @param x value @f$x@f$ at which the upper distribution is evaluated
    * @param y value @f$y@f$ at which the upper distribution is evaluated
   ...
   /**
    * Computes the cumulative probability in the square region @f[ P[a_1 \le X \le
    * b_1,\: a_2 \le Y \le b_2] = \int_{a_1}^{b_1} dx \int_{a_2}^{b_2} dy  f(x,
    * y). @f]
    * 
