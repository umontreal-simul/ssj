 ...
 * 
 * @anchor REF_probdistmulti_BiStudentDist_eq_pdf1bit @f[ f (x, y) =
 *         \frac{1}{2\pi\sqrt{1-\rho^2}}\left[1 + \frac{x^2 - 2\rho x y +
 *         y^2}{\nu(1-\rho^2)}\right]^{-(\nu+ 2)/2} , \tag{pdf1bit} @f] and the
 *         corresponding distribution function (the <tt>cdf</tt>) is
 * @anchor REF_probdistmulti_BiStudentDist_eq_cdf1bit @f[ T_{\nu}(x, y, \rho) =
 *         \frac{1}{2\pi\sqrt{1-\rho^2}} \int_{-\infty}^x dx \int_{-\infty}^y
 *         dy  f (x, y). \tag{cdf1bit} @f] We also define the upper
 *         distribution function called `barF` as
 * @anchor REF_probdistmulti_BiStudentDist_eq_cdf2bit @f[ \overline{T}_{\nu}(x,
 *         y, \rho) = \frac{1}{2\pi\sqrt{1-\rho^2}} \int^{\infty}_x dx
 *         \int^{\infty}_y dy  f(x,y). \tag{cdf2bit} @f]
...
public class BiStudentDist extends ContinuousDistribution2Dim {
    ...
    * {@link REF_probdistmulti_BiStudentDist_eq_pdf1bit pdf1bit} ) with
    * correlation @f$\rho@f$ = `rho` and @f$\nu@f$ = `nu` degrees of freedom.
    */
    ...
    * {@link REF_probdistmulti_BiStudentDist_eq_cdf1bit cdf1bit} ) using the method
    * described in @cite tGEN04a&thinsp;. The code for the `cdf` was translated
    * directly from the Matlab code written by Alan Genz and available from his web
    * page at
    * [http://www.math.wsu.edu/faculty/genz/homepage](http://www.math.wsu.edu/faculty/genz/homepage)
    * (the code is copyrighted by Alan Genz and is included in this package with
    * the kind permission of the author). The correlation is
    * 
...
// This function is translated from Alan Genz's Matlab code.
    ...
    * {@link REF_probdistmulti_BiStudentDist_eq_cdf2bit cdf2bit} ).
    */
   ...
   /**
    * Sets the parameters @f$\nu@f$ = `nu` and @f$\rho@f$ = `rho` of this object.
    */
