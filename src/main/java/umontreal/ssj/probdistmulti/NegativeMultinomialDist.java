 ...
 * 
 * @anchor REF_probdistmulti_NegativeMultinomialDist_eq_fNegativeMultinomial @f[
 *         P[X = (x_1, …,x_d)] = \frac{\Gamma\left(n + \sum_{i=1}^d x_i\right)
 *         p_0^n}{\Gamma(n)} \prod_{i=1}^d \frac{p_i^{x_i}}{x_i!}
 *         \tag{fNegativeMultinomial} @f] where @f$p_0 = 1 - \sum_{i=1}^d
 *         p_i@f$.
 *
...
public class NegativeMultinomialDist extends DiscreteDistributionIntMulti {
   ...
   /**
    * Creates a `NegativeMultinomialDist` object with parameters @f$n@f$ and
    * (@f$p_1@f$, …, @f$p_d@f$) such that @f$\sum_{i=1}^d p_i < 1@f$, as
    * described above. We have @f$p_i = @f$ `p[i-1]`.
    */
   ...
   /**
    * Computes the covariance matrix of the negative multinomial distribution with
   ...
   /**
    * Computes the correlation matrix of the negative multinomial distribution with
    * parameters @f$n@f$ and (@f$p_1@f$, …, @f$p_d@f$).
    */
    ...
    *                  distribution using the maximum likelihood method. It uses
    *                  the
    ...
    * @f{align*}{ \sum_{s=1}^M \frac{F_s}{(\hat{n} + s - 1)} & = \ln\left(1 +
    *             \frac{1}{\hat{n} m} \sum_{j=1}^m \Upsilon_j \right) \\ p_i & =
    *             \frac{\lambda_i}{1 + \sum_{j=1}^d \lambda_j} \qquad\mbox{for }
    *             i=1, …,d
    * @f} where
    * @f{align*}{ \lambda_i & = \frac{\sum_{j=1}^m X_{i,j}}{\hat{n} m}
    *             \qquad\mbox{for } i=1, …,d \\ \Upsilon_j & = \sum_{i=1}^d
    *             X_{i,j} \qquad\mbox{for } j=1, …,m \\ F_s & = \frac{1}{m}
    *             \sum_{j=1}^m \mbox{\textbf{1}}\{\Upsilon_j \ge s\}
    *             \qquad\mbox{for } s=1, …,M \\ M & = \max_j \{\Upsilon_j\}
    * @f}
    ...
    * multinomial distribution using the maximum likelihood method. It uses
    * the @f$m@f$ observations of @f$d@f$ components in table
    * <tt>x[</tt>@f$i@f$<tt>][</tt>@f$j@f$<tt>]</tt>, @f$i = 0, 1, …, m-1@f$
    * and @f$j = 0, 1, …, d-1@f$. The equation of maximum likelihood is defined
    * as: @f[ \sum_{s=1}^M \frac{\nu F_s}{(1 + \nu(s - 1))} = \ln\left(1 +
    * \frac{\nu}{m} \sum_{j=1}^m \Upsilon_j \right)\\ @f] where the symbols are
    * defined as in #getMLE(int[][],int,int).
    * 
    * @param x the list of observations used to evaluate parameters
    * @param m the number of observations used to evaluate parameters
    * @param d the dimension of each vector
   ...
   /**
    * Sets the parameters @f$n@f$ and (@f$p_1@f$, …, @f$p_d@f$) of this object.
    */
