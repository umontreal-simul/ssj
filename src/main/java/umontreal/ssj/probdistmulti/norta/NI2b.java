...
/**
 * Extends the class @ref NortaInitDisc and implements the algorithm NI2b. It is
 * a variant of NI2a. It uses the derivative, so it calls the method #deriv to
 * compute the function @f$g’_r@f$ given in (
 * {@link REF_probdistmulti_norta_NortaInitDisc_grp_M grp_M} ) and uses
 * numerical integration with Simpson’s rules as well. But the integration
 * grid is either in the interval @f$[0,1-\delta]@f$ or
 * 
 * @f$[-1+\delta, 0]@f$, depending on the sign of @f$r_X@f$. Here the number of
 *                subintervals of integration is fixed to @f$m@f$ and the
 *                algorithm stops at iteration @f$k@f$ if the root is in
 *                subinterval
...
public class NI2b extends NortaInitDisc {
    ...
    * `dist1` and `dist2`, the parameter for the truncation `tr` (see the
    * constructor of class @ref NortaInitDisc ), and the specific parameters `m`
    * and @f$\delta=@f$ `delta` for the algorithm NI2b, as described above.
    */
   ...
   /**
    * Computes and returns the correlation @f$\rho_Z@f$ using the algorithm NI2b.
    */
