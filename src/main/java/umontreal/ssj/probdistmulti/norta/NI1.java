 ...
 * root-bracketing, bisection and inverse quadratic interpolation. It calls the
 * method #integ to compute the function @f$g_r@f$ given in (
 * {@link REF_probdistmulti_norta_NortaInitDisc_gr_M gr_M} ). The search should
 * be done in the interval @f$[-1,0]@f$ if
 * 
 * @f$r_X\in[-1,0]@f$, or @f$[0,1]@f$ if @f$r_X\in[0,1]@f$. At each iteration,
 *                     the algorithm halves the interval length and uses an
 *                     accuracy
...
public class NI1 extends NortaInitDisc {
    ...
    * `dist1` and `dist2`, the parameter for truncation `tr` (see the constructor
    * of class @ref NortaInitDisc ) and the specific parameter @f$\epsilon=@f$
    * `tolerance` defined above for the algorithm NI1.
    */
   ...
   /**
    * Computes and returns the correlation @f$\rho_Z@f$ using the algorithm NI1.
    */
   public double computeCorr() {
      ...
      double tolerance1; /*
                          ...
                          */
      /**
       * a, b, c, fa, fb, fc, and tolerance correspond to a, b, c, f(a) f(b), f(c) and
       * epsilon, respectively, in the paper (paragraph "Method NI1" of section 3).
       */
