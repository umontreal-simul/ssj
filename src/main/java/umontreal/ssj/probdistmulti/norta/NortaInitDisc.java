...
/**
 * This abstract class defines the algorithms used for NORTA initialization when
 * the marginal distributions are discrete. Four algorithms are supported for
 * now, and they are defined as subclasses of the class `NortaInitDisc`.
 *
 ...
 * correlation @f$r_X=\mbox{Corr}(F_1(X_1),F_2(X_2))@f$, the parameters of the
 * marginal distributions and a parameter for truncation @f$tr@f$. For the
 * correlation matching, we must have finite supports for the two distributions.
 * Then if the support of each marginal is infinite, we have to upper-bound it
 * at the quantile of order @f$tr@f$. For example, if the marginals have their
 * support points in @f$[0,+\infty)@f$, the software will truncate
 * to @f$[0,F_l^{-1}(tr)]@f$, for @f$l=1,2@f$. The parameter
 * 
 * @f$tr@f$ must to be given by the user, depending on the type of the two
 *          distributions. If the marginals have finite supports, one can simply
 *          give
 * @f$tr=1@f$.
 *
 *             Each algorithm @ref NI1, @ref NI2a, @ref NI2b and @ref NI3 can be
 *             used to calculate the corresponding correlation
 * @f$\rho_Z=\mbox{Corr}(Z_1,Z_2)@f$, where @f$Z_1@f$ and @f$Z_2@f$ are standard
 *                                    normal random variables. These subclasses
 *                                    implement the specific methods for NORTA
 *                                    initialization presented in @cite
 *                                    sAVR06a&thinsp;.
 *
 *                                    Each type of algorithm should be defined
 *                                    as a subclass of
 ...
 *
 *       When creating a class representing an algorithm, the #toString method
 *       can be called to display information about the inputs.
 *
...
public abstract class NortaInitDisc {
   ...
   /**
    * Constructor with the target rank correlation `rX`, the two discrete marginals
    * `dist1` and `dist2` and the parameter for the truncation `tr`. This
    * constructor can be called only by the constructors of the subclasses.
    */
   ...
   /**
    * Computes and returns the correlation @f$\rho_Z@f$. Every subclass of @ref
    * NortaInitDisc must implement this method.
    */
    ...
    * <ul>
    * <li>The number of support points @f$m_1@f$ and @f$m_2@f$ for the two
    * distributions.</li>
    * <li>The means and standard deviations of @f$F_1(X_1)@f$ and
    * 
    * @f$F_2(X_2)@f$, respectively.</li>
    *                 <li>The vectors @f$p_1[i]@f$, @f$p_2[j]@f$,
    * @f$z_1[i]=\Phi^{-1}(f_1[i])@f$ and @f$z_2[j]=\Phi^{-1}(f_2[j])@f$,
    *                                where @f$f_1[i]@f$ and @f$f_2[j]@f$,
    *                                for @f$i=0,…,m_1-1@f$;
    * @f$j=0,…,m_2-1@f$, are the cumulative probability functions, and
    * @f$\Phi@f$ is the standard normal distribution function.</li>
    *            </ul>
    *            Every subclass of @ref NortaInitDisc must call this method.
    */
    ...
    * @f{align}{ \tag{gr_M} g_r(r)=\sum_{i=0}^{m_1-2} p_{1,i+1} \sum_{j=0}^{m_2-2}
    *            p_{2,j+1} \bar{\Phi}_r(z_{1,i },z_{2,j}),
    * @f} which involves the bivariate normal integral @f$ \bar{\Phi}_r(x,y)=
    *     \int_x^{\infty}\int_y^{\infty}\phi_r(z_1,z_2) dz_1 dz_2@f$. Method #barF
    *     of class
    ...
    * @f$\bar{\Phi}_r(x,y)@f$, with @f$m_1@f$, @f$m_2@f$, and the
    *                          vectors @f$p_1[i], i=1,…,m_1-1@f$; @f$z_1[i],
    *                          i=0,…,m_1-2@f$; @f$p_2[j],
    *                          j=1,…,m_2-1@f$; @f$z_2[j], j=0,…,m_2-2@f$. The
    *                          correlation parameter
    * @f$r@f$ must be in @f$[-1,1]@f$. This method may be called by subclasses
    *         of @ref NortaInitDisc.
    ...
    * @f{align}{ \tag{grp_M} g’_r(r)=\sum_{i=0}^{m_1-2} p_{1,i+1}
    *            \sum_{j=0}^{m_2-2} p_{2,j+1} \phi_r(z_{1,i },z_{2,j}),
    * @f} where @f$\phi_r@f$ is the bivariate standard binormal density. The method
    *     uses @f$m_1@f$, @f$m_2@f$, and the vectors @f$p_1[i],
