...
/**
 * Extends the class @ref NortaInitDisc and implements the algorithm NI2a. It
 * uses the derivative, so it calls the method #deriv to compute the
 * function @f$g’_r@f$ given in (
 * {@link REF_probdistmulti_norta_NortaInitDisc_grp_M grp_M} ). The double
 * integration in ( {@link REF_probdistmulti_norta_overview_gr gr} ) is
 ...
 * @f$1-2h<\rho_m<1@f$. The initial point is chosen as @f$\rho_0=2 \sin(\pi r_X
 *                      /6)@f$. The integration is done between @f$\rho_0@f$ and
 * @f$\rho_m=\pm(1-\delta)@f$, or between @f$\rho_0@f$ and @f$0@f$, depending on
 *                             the sign of @f$r_X@f$ and on whether the root is
 *                             to the left, or to the right of @f$\rho_0@f$. So
 *                             depending on the case, the worst-case integration
 *                             distance will be set to @f$d=|1-\delta-\rho_0|@f$
 *                             or
 * @f$d=|\rho_0|@f$. Then, the step size is readjusted to @f$h^*=d/(2m)@f$,
 *                   where @f$d@f$ is the maximum number of steps (iterations)
 *                   calculated based on the pre-defined step size @f$h@f$,
 *                   so @f$m=\lceil d / (2h) \rceil@f$. The algorithm stops at
 *                   iteration @f$k@f$ if the root is in a subinterval
 * @f$[\rho_{k-1},\rho_k]@f$, and a quadratic interpolation is used to compute
 *                            the solution. For this, the method #interpol of
 *                            class
...
public class NI2a extends NortaInitDisc {
    ...
    * `dist1` and `dist2`, the paramater for the truncation `tr` (see the
    * constructor of class @ref NortaInitDisc ), and the specific parameters `h`
    * and @f$\delta=@f$ `delta` for the algorithm NI2a, as described above.
    */
   ...
   /**
    * Computes and returns the correlation @f$\rho_Z@f$ using the algorithm NI2a.
    */
   public double computeCorr() {
      ...
      /**
       * d and m correspond to d and m given in the third paragraph of section 4 in
       * the paper.
       */
