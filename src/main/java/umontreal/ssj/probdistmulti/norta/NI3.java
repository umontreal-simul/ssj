 ...
 * {@link REF_probdistmulti_norta_NortaInitDisc_gr_M gr_M} ) and (
 * {@link REF_probdistmulti_norta_NortaInitDisc_grp_M grp_M} ) and uses an
 * adapted version of the Newton-Raphson algorithm. At any iteration, if the
 * solution falls outside the search interval, the algorithm uses bisection and
 * halves the interval length to guarantee convergence. The initial solution is
 * taken as @f$\rho_0=2 \sin(\pi r_X /6)@f$, and then at each
 * iteration @f$k@f$, @f$f_r(\rho_k)@f$ and
 * 
 * @f$f_r^’(\rho_k)@f$ are calculated and a solution is computed by the
 *                       recurrence formula: @f[
 *                       \rho_{k+1}=\rho_k-\frac{f_r(\rho_k)}{f_r^’(\rho_k)}. @f]
 *                       The algorithm stops at iteration @f$k@f$ if
 * @f$|\rho_{k-1}-\rho_k|\leq\epsilon@f$. The function @f$f_r@f$ is the one
 *                                        given in (
 *                                        {@link REF_probdistmulti_norta_overview_fr
 *                                        fr} ).
 *
 *                                        <div class="SSJ-bigskip"></div><div
...
public class NI3 extends NortaInitDisc {
    ...
    * constructor of class @ref NortaInitDisc ), and the specific
    * parameter @f$\epsilon=@f$ `tolerance` for the algorithm NI3, as defined
    * above.
    */
   ...
   /**
    * Computes and returns the correlation @f$\rho_Z@f$ using algorithm NI3.
    */
