...
public class BiNormalDonnellyDist extends BiNormalDist {
    ...
    * rho</tt> , and @f$d = @f$ `ndig` digits of accuracy (the absolute error is
    *             smaller than @f$10^{-d}@f$). Restriction: @f$d \le15@f$.
    */
   ...
   /**
    * Same as {@link #BiNormalDonnellyDist(double,int) BiNormalDonnellyDist(rho,
    * 15)}.
    */
   ...
   /**
    * Same as {@link #BiNormalDonnellyDist(double,double,double,double,double,int)
   ...
   /**
    * The following methods use the parameter `ndig` for the number of digits of
    * absolute accuracy. If the same methods are called without the `ndig`
    * parameter, a default value of `ndig` = 15 will be used.
    *
    * Computes the standard *binormal* distribution (
    * {@link REF_probdistmulti_BiNormalDist_eq_cdf2binormal cdf2binormal} ) with
    * the method described in @cite tDON73a&thinsp;, where `ndig` is the number of
    * decimal digits of accuracy provided (<tt>ndig</tt> @f$\le15@f$). The code was
    * translated from the Fortran program written by T. G. Donnelly and copyrighted
    * by the ACM (see
    ...
    * `sigma1`, @f$\sigma_2@f$ = `sigma2`, correlation @f$\rho@f$ = `rho` and
    ...
    * {@link REF_probdistmulti_BiNormalDist_eq_cdf3binormal cdf3binormal} ) with
    * parameters @f$\rho@f$ = `rho` and `ndig` decimal digits of accuracy.
    */
