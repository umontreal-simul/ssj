 ...
 * 
 * @anchor REF_probdistmulti_MultinomialDist_eq_fMultinomial @f[ P[X =
 *         (x_1,…,x_d)] = {n!} \prod_{i=1}^d\frac{p_i^{x_i}}{x_i!},
 *         \tag{fMultinomial} @f] where @f$\sum_{i=1}^d x_i = n@f$
 *         and @f$\sum_{i=1}^d p_i = 1@f$.
 *
...
public class MultinomialDist extends DiscreteDistributionIntMulti {
    ...
    * {@link REF_probdistmulti_MultinomialDist_eq_fMultinomial fMultinomial} ) of
    * the multinomial distribution with parameters @f$n@f$ and
    * (@f$p_1@f$,…,@f$p_d@f$) evaluated at @f$x@f$.
    */
   ...
   /**
    * Estimates and returns the parameters [@f$\hat{p_i}@f$,…,@f$\hat{p_d}@f$] of
    * the multinomial distribution using the maximum likelihood method. It uses
    * the @f$m@f$ observations of @f$d@f$ components in table @f$x[i][j]@f$, @f$i =
    * 0, 1, …, m-1@f$ and @f$j = 0, 1, …, d-1@f$. The equations of the maximum
    * likelihood are defined as
    * 
    * @f{align*}{ \hat{p}_i = \frac{\bar{X_i}}{N}.
    * @f}
