 ...
 * Its parameters are @f$\alpha>0@f$ (shape) and @f$\lambda>0@f$ (rate). Its
 * density function is
 * 
 * @anchor REF_randvar_GammaGen_eq_fgamma @f[ f(x) = \lambda^{\alpha}x^{\alpha-
 *         1}e^{-\lambda x} / \Gamma(\alpha) \qquad\mbox{ for } x>0,
 *         \tag{fgamma} @f] where @f$\Gamma@f$ is the gamma function defined by
 * @anchor REF_randvar_GammaGen_eq_Gamma @f[ \Gamma(\alpha) =
 *         \int_0^{\infty}x^{\alpha- 1} e^{-x} dx. \tag{Gamma} @f] The
...
public class GammaGen extends RandomVariateGen {
    ...
    * 
    * @f$&nbsp;`alpha` and @f$\lambda= @f$&nbsp;`lambda`, using stream `s`.
    */
