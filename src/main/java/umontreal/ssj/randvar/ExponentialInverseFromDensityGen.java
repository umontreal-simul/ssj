...
/**
 * This class implements *exponential* random variate generators using numerical
 * inversion of the *exponential* density as described in
 * 
...
public class ExponentialInverseFromDensityGen extends ExponentialGen {
    ...
    * 
    * @f$\lambda@f$ = `lambda`, using stream `stream`. It uses numerical inversion
    *               with precomputed tables. The @f$u@f$-resolution `ueps` is the
    *               desired absolute error in the `cdf`, and `order` is the degree
    ...
    * #getDistribution, after using one of the other constructors to create the
    * precomputed tables. This is useful when one needs many generators using the
    * same exponential distribution (same @f$\lambda@f$). Precomputing tables for
    * numerical inversion is costly; thus using only one set of tables for many
    * generators is more efficient. The first @ref ExponentialInverseFromDensityGen
    * generator using the other constructors creates the precomputed tables. Then
    * all other streams use this constructor with the same set of tables.
    */
