 ...
 * 
 * @anchor REF_randvar_BinomialGen_eq_fmass_binomial @f[ p(x) = {n \choose x}
 *         p^x (1-p)^{n-x} = \frac{n!}{x!(n-x)!}\; p^x (1-p)^{n-x}
 *         \qquad\mbox{for } x=0,1,2,…, n \tag{fmass-binomial} @f] where
...
public class BinomialGen extends RandomVariateGenInt {
   ...
   /**
    * Generates a new integer from the *binomial* distribution with parameters @f$n
    * = @f$&nbsp;`n` and @f$p = @f$&nbsp;`p`, using the given stream `s`.
    */
