...
/**
 * This class implements random variate generators with the *beta* distribution
 * with shape parameters @f$\alpha> 0@f$ and @f$\beta> 0@f$, over the
 * interval @f$(a,b)@f$, where @f$a < b@f$. The density function of this
 * distribution is @f[ f(x) = \frac{\Gamma(\alpha+\beta)}{
 * \Gamma(\alpha)\Gamma(\beta)(b - a)^{\alpha+\beta-1}} (x - a)^{\alpha- 1}(b -
 * x)^{\beta-1} \qquad\mbox{ for } a < x < b, @f] and @f$f(x)=0@f$ elsewhere,
...
public class BetaGen extends RandomVariateGen {
   ...
   /**
    * Creates a new beta generator with parameters @f$\alpha=@f$ `alpha`
    * and @f$\beta=@f$ `beta`, over the interval @f$(0,1)@f$, using stream `s`.
    */
