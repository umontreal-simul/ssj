...
/**
 * This class implements random variate generators for the *Erlang* distribution
 * with parameters @f$k > 0@f$ and @f$\lambda> 0@f$. This Erlang random variable
 * is the sum of @f$k@f$ exponentials with parameter
 * 
 * @f$\lambda@f$ and has mean @f$k/\lambda@f$.
 *
 *               The (non-static) `nextDouble` method simply calls `inverseF` on
 *               the distribution.
 *
...
public class ErlangGen extends GammaGen {
    ...
    * 
    * @f$k = @f$&nbsp;`k` and @f$\lambda= @f$&nbsp;`lambda`, using stream `s`.
    */
