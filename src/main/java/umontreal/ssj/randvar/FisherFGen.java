 ...
 * and @f$m@f$ are positive integers. The density function of this distribution
 * is
 * 
 * @anchor REF_randvar_FisherFGen_eq_FisherF @f[ f(x) = \frac{\Gamma({(n +
 *         m)}/2)n^{n/2}m^{m/2}}{\Gamma(n/2)\Gamma(m/2)} \frac{x^{{(n -
 *         2)}/2}}{(m + nx)^{{(n + m)}/2}}, \qquad\mbox{for } x > 0
 *         \tag{FisherF} @f] <div class="SSJ-bigskip"></div>
 *
...
public class FisherFGen extends RandomVariateGen {
   ...
   /**
    * Generates a variate from the *Fisher F* distribution with @f$n@f$ and @f$m@f$
