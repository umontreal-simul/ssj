 ...
 * inversion of the *normal* density as described in @cite rDER10a&thinsp;. It
 ...
 *      numerical inversion. This will be useful if one wants to generate a
...
public class NormalInverseFromDensityGen extends NormalGen {
    ...
    * 
    * @f$\mu=@f$ `mu` and @f$\sigma@f$ = `sigma`, using stream `stream`. It uses
    *            numerical inversion with precomputed tables. The
    * @f$u@f$-resolution `ueps` is the desired absolute error in the `cdf`, and
   ...
   public NormalInverseFromDensityGen(RandomStream stream, double mu, double sigma, double ueps, int order) {
      ...
      dist = new InverseDistFromDensity((ContinuousDistribution) dist, xc, ueps, order);
   }
   ...
   /**
    * Creates a new normal generator using the *normal* distribution `dist` and
    * stream `stream`. `dist` may be obtained by calling method #getDistribution,
    * after using one of the other constructors to create the precomputed tables.
    * This is useful when one needs many generators using the same normal
    * distribution. Precomputing tables for numerical inversion is costly; thus
    * using only one set of tables for many generators is more efficient. The first
    * 
    * @ref NormalInverseFromDensityGen generator using the other constructors
    *      creates the precomputed tables. Then all other streams use this
    *      constructor with the same set of tables.
    */
