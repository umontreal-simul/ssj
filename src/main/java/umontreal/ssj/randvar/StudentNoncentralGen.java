 ...
 * Student-t* distribution with @f$n>0@f$ degrees of freedom and noncentrality
 * parameter @f$\delta@f$. If @f$X@f$ is distributed according to a normal
 * distribution with mean @f$\delta@f$ and variance 1, and
 * 
 * @f$Y@f$ (statistically independent of @f$X@f$) is distributed according to a
 *         chi-square distribution with @f$n@f$ degrees of freedom, then @f[
 *         T’ = \frac{X}{\sqrt{Y/n}} @f] has a noncentral @f$t@f$-distribution
 *         with @f$n@f$ degrees of freedom and noncentrality
 *         parameter @f$\delta@f$.
 *
...
public class StudentNoncentralGen extends RandomVariateGen {
   ...
   /**
    * Creates a *noncentral-t* random variate generator using normal generator
    * `ngen` and chi-square generator `cgen`.
    */
