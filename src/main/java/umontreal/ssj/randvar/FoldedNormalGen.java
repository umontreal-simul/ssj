...
/**
 * This class implements methods for generating random variates from the *folded
 * normal* distribution with parameters @f$\mu\ge0@f$ and
 * 
 * @f$\sigma> 0@f$. The density is
 * @anchor REF_randvar_FoldedNormalGen_eq_fFoldedNormal @f[ f(x) =
 *         \phi\left(\frac{x-\mu}{\sigma}\right) +
 *         \phi\left(\frac{-x-\mu}{\sigma}\right) \qquad\mbox{for } x \ge0,
 *         \tag{fFoldedNormal} @f] where @f$ \phi@f$ denotes the density
...
public class FoldedNormalGen extends RandomVariateGen {
   ...
   /**
    * Creates a new *folded normal* generator with parameters @f$\mu=@f$ `mu`
