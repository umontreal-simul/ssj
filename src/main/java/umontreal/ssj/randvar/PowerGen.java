...
/**
 * This class implements random variate generators for the *power* distribution
 * with shape parameter @f$c > 0@f$, over the interval
 * 
 * @f$[a,b]@f$. Its density is
 * @anchor REF_randvar_PowerGen_eq_fpower @f[ f(x) = \frac{c(x-a)^{c - 1}}{(b -
 *         a)^c}, \tag{fpower} @f] for @f$a \le x \le b@f$, and 0 elsewhere.
 *
...
public class PowerGen extends RandomVariateGen {
   ...
   /**
    * Creates a Power random variate generator with parameters @f$a =@f$ `a`, @f$b
    * =@f$ `b` and @f$c =@f$ `c`, using stream `s`.
    */
   ...
   /**
    * Uses inversion to generate a new variate from the power distribution with
    * parameters @f$a = @f$&nbsp;`a`, @f$b = @f$&nbsp;`b`, and @f$c =
    * 
    * @f$&nbsp;`c`, using stream `s`.
    */
