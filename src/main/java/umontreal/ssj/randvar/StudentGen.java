 ...
 * function is
 * 
 * @anchor REF_randvar_StudentGen_eq_fstudent @f[ f (x) = \frac{\Gamma\left((n +
 *         1)/2 \right)}{\Gamma(n/2) \sqrt{\pi n}} \left[1 +
 *         \frac{x^2}{n}\right]^{-(n+1)/2} \qquad\qquad\mbox{for } -\infty< x <
 *         \infty, \tag{fstudent} @f] where @f$\Gamma(x)@f$ is the gamma
 *         function defined in ( {@link REF_randvar_GammaGen_eq_Gamma Gamma} ).
 *
 ...
 *         Student random variates using the different implementations available
 *         in SSJ. The second test (Q) was made with the inverse in
 * @ref umontreal.ssj.probdist.StudentDistQuick, while the first test was made
 *      with the inverse in @ref umontreal.ssj.probdist.StudentDist. These tests
 *      were made on a machine with processor AMD Athlon 4000, running Red Hat
 *      Linux, with clock speed at 2400 MHz.
 *
 *      <center>
 *
...
public class StudentGen extends RandomVariateGen {
   ...
   /**
    * Creates a Student random variate generator with @f$n@f$ degrees of freedom,
    * using stream `s`.
    */
