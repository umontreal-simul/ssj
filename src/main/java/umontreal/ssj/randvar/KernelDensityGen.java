 ...
 * 
 * @cite rHOR04a, @cite tSIL86a&thinsp;. The basic idea is to center a copy of
 *       the same symmetric density at each observation and take an equally
 *       weighted mixture of the @f$n@f$ copies as an estimator of the density
 *       from which the observations come. The resulting kernel density has the
 *       general form @f[ f_n(x) = \frac{1}{nh} \sum_{i=1}^n k((x-x_i)/h), @f]
 *       where @f$k@f$ is a fixed pre-selected density called the *kernel* and
 * @f$h@f$ is a positive constant called the *bandwidth* or *smoothing factor*.
 *         A difficult practical issue is the selection of @f$k@f$ and
 ...
 * @anchor REF_randvar_KernelDensityGen_eq_bandwidth0 @f[ h_0 = 1.36374
 *         \min(s_n, q / 1.34) n^{-1/5}, \tag{bandwidth0} @f]
 * @f$s_n@f$ and @f$q@f$ are the empirical standard deviation and the
 *           interquartile range of the @f$n@f$ observations, and @f$\alpha_k@f$
 *           is a constant that depends on the type of kernel @f$k@f$. It is
 *           defined by @f[ \alpha_k = \left(\sigma_k^{-4}
 *           \int_{-\infty}^{\infty}k(x)dx \right)^{1/5} @f]
 *           where @f$\sigma_k@f$ is the standard deviation of the
 *           density @f$k@f$. The static method #getBaseBandwidth permits one to
 *           compute @f$h_0@f$ for a given empirical distribution.
 *
 *           <center> <center>Some suggested kernels</center>
 ...
 *         </center>
 *
 *         Table&nbsp; {@link REF_randvar_KernelDensityGen_tab_kernels kernels}
 *         gives the precomputed values of @f$\sigma_k@f$ and
...
public class KernelDensityGen extends RandomVariateGen {
   ...
   /**
    * Creates a new generator for a kernel density estimated from the observations
    * given by the empirical distribution `dist`, using stream `s` to select the
    * observations, generator `kGen` to generate the added noise from the kernel
    ...
    * 
    * @f$h = \alpha_k h_0@f$ with the @f$\alpha_k@f$ suggested in Table&nbsp;
    *      {@link REF_randvar_KernelDensityGen_tab_kernels kernels} for the
    *      gaussian distribution. This kernel has an efficiency of 0.951.
    */
    ...
    * {@link REF_randvar_KernelDensityGen_eq_bandwidth0 bandwidth0} ).
    */
   ...
   /**
    * After this method is called with `true`, the generator will produce only
    * positive values, by using the *reflection method*: replace all negative
    * values by their *absolute values*. That is, #nextDouble will return @f$|x|@f$
    * if @f$x@f$ is the generated variate. The mecanism is disabled when the method
   ...
   public double nextDouble() {
      double x = (dist.inverseF(stream.nextDouble()) + bandwidth * kernelGen.nextDouble());
      if (positive)
         return Math.abs(x);
      else
         return x;
