...
/**
 * This class implements *Student* random variate generators using the *polar*
 * method of @cite rBAI94a&thinsp;. The code is adapted from UNURAN (see @cite
 * iLEY02a&thinsp;).
 *
 * The non-static `nextDouble` method generates two variates at a time and the
 * second one is saved for the next call. A pair of variates is generated every
 * second call. In the static case, two variates are generated per call but only
 * the first one is returned and the second is discarded.
 *
...
//>>>>>>>>>>>>>>>>>>>>  P R I V A T E S    M E T H O D S   <<<<<<<<<<<<<<<<<<<<
    ...
    * parameters n > 0. * * REFERENCE: - R.W. Bailey (1994): Polar generation of
    * random variates * with the t-distribution, * Mathematics of Computation 62,
    * 779-781. * * Implemented by F. Niederl, 1994 *
    *****************************************************************************
