 ...
 *       beta variate: @f[ B_{\alpha, \alpha} \stackrel{\mathcal{L}}{=}
 *       \frac{1}{2} \left( 1 + \frac{T_{2\alpha}}{\sqrt{2\alpha+
 *       T_{2\alpha}^2}}\right). @f] If @f$S@f$ is a random sign
 *       and @f$U_1@f$, @f$U_2@f$ are two independent uniform @f$[0,1]@f$ random
 *       variates, then the following gives a symmetrical beta variate @cite
 *       rDEV96a&thinsp;:
 * @anchor REF_randvar_BetaSymmetricalBestGen_eq_beta_best @f[ B_{\alpha,
 *         \alpha} \stackrel{\mathcal{L}}{=} \frac{1}{2} + \frac{S}{2 \sqrt{1 +
 *         \frac{1}{\left(U_1^{-1/\alpha} - 1\right)\cos^2(2\pi U_2)}}}
 *         \tag{eq.beta.best} @f] valid for any shape parameter @f$\alpha> 0@f$.
 *
...
public class BetaSymmetricalBestGen extends BetaSymmetricalGen {
    ...
    * 
    * @f$\alpha=@f$ `alpha`, using stream `s1` to generate @f$U_1@f$, stream `s2`
    *               to generate @f$U_2@f$ and stream `s3` to generate
    * @f$S@f$, as given in equation (
    *          {@link REF_randvar_BetaSymmetricalBestGen_eq_beta_best eq.beta.best}
    *          ).
    */
   ...
   /**
    * Creates a new generator for the distribution `dist`, using stream `s1` to
    * generate @f$U_1@f$, stream `s2` to generate @f$U_2@f$ and stream `s3` to
    * generate @f$S@f$ as given in equation (
    * {@link REF_randvar_BetaSymmetricalBestGen_eq_beta_best eq.beta.best} ).
    */
   ...
   /**
    * Creates a new generator for the distribution `dist`, using only one stream
    * `s1`.
    */
   ...
   /**
    * Generates a random number using Devroye’s one-liner method with only one
    * stream `s`. Restriction: @f$\alpha> 0@f$.
    */
