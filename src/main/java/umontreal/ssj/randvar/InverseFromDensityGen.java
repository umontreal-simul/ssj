 ...
 * an *arbitrary continuous* distribution when only the probability density is
 * known @cite rDER10a&thinsp;. The cumulative probabilities (cdf) are
 * pre-computed by numerical quadrature of the density using Gauss-Lobatto
 * integration over suitably small intervals to satisfy the required precision,
 * and these values are kept in tables. Then the algorithm uses polynomial
 * interpolation over the tabulated values to get the inverse cdf. The user can
 * select the desired precision and the degree of the interpolating polynomials.
 *
 * The algorithm may fail for some distributions for which the density becomes
 * infinite at a point (for ex. the Gamma and the Beta distributions
 * with @f$\alpha< 1@f$) if one requires too high a precision (a too small
 * `eps`, for ex. @f$\epsilon\sim10^{-15}@f$). However, it should work also for
 * continuous densities with finite discontinuities.
 *
 * While the setup time is relatively slow, the generation of random variables
 * is extremely fast and practically independent of the required precision and
 * of the specific distribution. The following table shows the time needed (in
 * seconds) to generate @f$10^8@f$ random numbers using inversion from a given
 * class, then the numerical inversion with Gauss-Lobatto integration
 * implemented here, and finally the speed ratios between the two methods. The
 * speed ratio is the speed of the latter over the former. Thus for the beta
 * distribution with parameters (5, 500), generating random variables with the
 * Gauss-Lobatto integration implemented in this class is more than 1700 times
 * faster than using inversion from the `BetaDist` class. These tests were made
 * on a machine with processor AMD Athlon 4000, running Red Hat Linux, with
 ...
 * (setup time) and to generate one random variable for this class compared to
 * the same for the inversion method specific to each class, and the ratios of
 * the times (init + one random variable) of the two methods. For inversion, we
 * initialized @f$10^8@f$ times; for this class, we initialized
 * 
 * @f$10^4@f$ times.
 *
 *            <center>
 ...
 *            is not efficient because of the slow set-up. But if one wants to
 *            generate large samples from the same distribution with fixed
 *            parameters, then this class will be very efficient. The following
 *            table gives the number of random variables generated beyond which,
 *            using this class will be worthwhile.
 *
 *            <center>
 *
 *            <table class="SSJ-table SSJ-has-hlines">
 ...
 *            </center>
 *
 *            Thus, for example, if one needs to generate less than 15266
 *            exponential random variables, then using the
 *            `InverseFromDensityGen` class is not wortwhile: it will be faster
 *            to use inversion from the `ExponentialGen` class.
 *
...
public class InverseFromDensityGen extends RandomVariateGen {
   ...
   /**
    * Creates a new generator for the *continuous* distribution `dis`, using stream
    * `stream`. `dis` must have a well-defined density method; its other methods
    * are unused. For a non-standard distribution `dis`, the user may wish to set
    * the left and the right boundaries between which the density is non-zero by
    * calling methods umontreal.ssj.probdist.ContinuousDistribution.setXinf and
    * umontreal.ssj.probdist.ContinuousDistribution.setXsup of `dis`, for better
    * efficiency. Argument `xc` can be the mean, the mode or any other @f$x@f$ for
    * which the density is relatively large. The
    * 
    * @f$u@f$-resolution `eps` is the desired absolute error in the CDF, and
    *                    `order` is the degree of the Newton interpolating
    *                    polynomial over each interval. An `order` of 3 or 5, and
    *                    an `eps` of
    * @f$10^{-6}@f$ to @f$10^{-12}@f$ are usually good choices. Restrictions: @f$3
    *               \le\mathtt{order} \le12@f$.
    */
   ...
   /**
    * Creates a new generator from the *continuous* probability density `dens`. The
    * left and the right boundaries of the density are `xleft` and `xright` (the
    * density is 0 outside the interval <tt>[xleft,
    * xright]</tt>). See the description of the other constructor.
    */
   public InverseFromDensityGen(RandomStream stream, MathFunction dens, double xc, double eps, int order, double xleft,
         double xright) {
      super(stream, null);
