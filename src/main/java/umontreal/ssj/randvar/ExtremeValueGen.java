 ...
 *
 * This class implements random variate generators for the *Gumbel* (or *extreme
 * value*) distribution. Its density is
 * 
 * @anchor REF_randvar_ExtremeValueGen_eq_fextremevalue @f[ f(x) = \lambda
 *         e^{-e^{-\lambda(x - \alpha)}-\lambda(x-\alpha)}
 *         \tag{fextremevalue} @f] where @f$\lambda>0@f$.
 *
...
public class ExtremeValueGen extends RandomVariateGen {
   ...
   /**
    * Uses inversion to generate a new variate from the extreme value distribution
    * with parameters @f$\alpha= @f$&nbsp;`alpha` and
    * 
