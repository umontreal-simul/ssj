...
/**
 * This class implements *gamma* random variate generators using a method that
 * combines acceptance-rejection with acceptance-complement, and proposed
 * in @cite rAHR72b, @cite rAHR82a&thinsp;. It uses acceptance-rejection
 * for @f$\alpha<1@f$ and acceptance-complement for
 * 
 * @f$\alpha\ge1@f$.
 *
 ...
 * @cite rLIU13a&thinsp; will be used. This algorithm generates the log value of
 *       a gamma variate and it can handle a very small @f$\alpha@f$ parameter.
 ...
 *       `double`. The method `nextDoubleLog` can be used to generate the
 *       natural log value of a gamma variate. This allows the generation of
 *       very small gamma variates.
 *
 *       For each gamma variate, the first uniform required is taken from the
 *       main stream and all additional uniforms (after the first rejection) are
...
public class GammaAcceptanceRejectionGen extends GammaGen {
    ...
    * 
    * @f$&nbsp;`alpha` and @f$\lambda= @f$&nbsp;`lambda`, using main stream `s` and
    ...
    * parameter @f$\alpha@f$ is close to 0, generating the log value of the gamma
    * variate is numerically more stable than generating directly the gamma value
    * with `nextDouble`.
    */
    ...
    *            stream `s` and auxiliary stream `aux`. When the shape
   ...
   /**
    * Generates the natural log of a gamma variable. Use only when the shape is
    * very small. When the shape parameter is small, the value of the gamma
    * variable can be numerically too small to be represented in a double.
    * 
    * The method of @cite rLIU2015 for small shape parameter.
    */
