 ...
 * *Laplace* distribution. Its density is (see @cite tJOH95b&thinsp; (page 165))
 * 
 * @anchor REF_randvar_LaplaceGen_eq_flaplace @f[ f(x) =
 *         \frac{1}{2\beta}e^{-|x-\mu|/\beta} \qquad\mbox{for } -\infty< x <
 *         \infty, \tag{flaplace} @f] where @f$\beta> 0@f$.
 *
...
public class LaplaceGen extends RandomVariateGen {
   ...
   /**
    * Creates a Laplace random variate generator with parameters @f$\mu= 0@f$
    * and @f$\beta= 1@f$, using stream `s`.
    */
    ...
    * parameters @f$\mu= @f$&nbsp;`mu` and @f$\beta= @f$&nbsp;`beta`, using stream
    * `s`.
    */
