...
/**
 * Implements *Beta* random variate generators using the rejection method with
 * log-logistic envelopes from @cite rCHE78a&thinsp;. The method draws the first
 * two uniforms from the main stream and uses the auxiliary stream for the
 * remaining uniforms, when more than two are needed (i.e., when rejection
 * occurs). The current implementation is adapted from UNURAN.
 *
...
public class BetaRejectionLoglogisticGen extends BetaGen {
    ...
    * Creates a beta random variate generator with parameters
    * 
    * @f$\alpha=@f$ `alpha` and @f$\beta=@f$ `beta` over the interval (<tt>a</tt>,
    *               <tt>b</tt>), using main stream `s` and auxiliary stream `aux`.
   ...
   /**
    * Creates a new generator for the distribution `dist`, using stream `s` and
    * auxiliary stream `aux`. The main stream is used for the first uniforms
    * (before a rejection occurs) and the auxiliary stream is used afterwards
   ...
   /**
    * Same as {@link #BetaRejectionLoglogisticGen() BetaRejectionLoglogisticGen(s,
    * s, dist)}. The auxiliary stream used will be the same as the main stream.
    */
