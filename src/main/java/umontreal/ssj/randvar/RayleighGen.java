 ...
 * distribution. Its density is @f[ f(x) = \left\{ \begin{array}{ll}
 * \frac{(x-a)}{\beta^2}  e^{-(x-a)^2/2\beta^2} \quad & \mbox{ for } x \ge a
 * \\ 0 & \mbox{ for } x < a, \end{array} \right. @f] where @f$\beta> 0@f$.
 *
...
public class RayleighGen extends RandomVariateGen {
   ...
   /**
    * Creates a Rayleigh random variate generator with parameters @f$a =@f$ `a`
    * and @f$\beta=@f$ `beta`, using stream `s`.
    */
   ...
   /**
    * Creates a Rayleigh random variate generator with parameters @f$a = 0@f$
    * and @f$\beta= @f$ `beta`, using stream `s`.
    */
   ...
   /**
    * Uses inversion to generate a new variate from the Rayleigh distribution with
    * parameters @f$a = @f$&nbsp;`a` and @f$\beta=
    * 
