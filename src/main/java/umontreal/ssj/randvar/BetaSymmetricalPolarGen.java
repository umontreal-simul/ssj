 ...
 * uniform random variables @f$x \in[0, 1]@f$ and @f$y \in[-1, 1]@f$
 * until @f$x^2 + y^2 \le1@f$. Then it returns
 * 
 * @anchor REF_randvar_BetaSymmetricalPolarGen_eq_beta_ulrich @f[ \frac{1}{2} +
 *         \frac{xy}{S}\sqrt{1 - S^{2/(2\alpha- 1)}} \tag{eq.beta.ulrich} @f]
 *         where @f$S = x^2 + y^2@f$, and @f$\alpha@f$ is the shape parameter of
 *         the beta distribution. The method is valid only when @f$\alpha>
 *         1/2@f$.
 *
...
public class BetaSymmetricalPolarGen extends BetaSymmetricalGen {
    ...
    * 
    * @f$\alpha=@f$ `alpha`, using stream `s1` to generate @f$x@f$ and stream `s2`
    *               to generate @f$y@f$, as in (
    ...
    * @f$y@f$, as in ( {@link REF_randvar_BetaSymmetricalPolarGen_eq_beta_ulrich
    *          eq.beta.ulrich} ) above. Restriction: @f$\alpha> 1/2@f$.
    */
    ...
    * generate @f$x@f$ and stream `s2` to generate @f$y@f$, as in (
    * {@link REF_randvar_BetaSymmetricalPolarGen_eq_beta_ulrich eq.beta.ulrich} )
    * above. Restriction: `dist` must have @f$\alpha> 1/2@f$.
    */
    ...
    * `s1`. Restriction: `dist` must have @f$\alpha> 1/2@f$.
    */
