...
/**
 * This class implements *noncentral chi square* random variate generators using
 * the additive property of the noncentral chi square distribution
 * 
 * @cite tKRI06a&thinsp;. It uses the following algorithm: generate a real
 * @f$X \sim N(\sqrt{\lambda}, 1)@f$ from a normal distribution with variance 1,
 *      generate a real @f$Y \sim\Gamma((\nu- 1)/2, 1/2)@f$ from a gamma
 *      distribution, then return @f$X^2 + Y@f$. Here @f$\nu@f$ is the number of
 *      degrees of freedom and @f$\lambda@f$ is the noncentrality parameter.
 *
 *      To generate the normal variates, one uses the fast
 *      *acceptance-complement ratio* method in @cite rHOR90a&thinsp; (see class
 ...
 * @ref umontreal.ssj.randvar.ChiSquareNoncentralPoisGen on the next page of
 *      this guide . For small @f$\lambda@f$, it is nearly twice as fast.
 *      As @f$\lambda@f$ increases, it is still faster but not as much.
 *
