...
/**
 * This class implements the sequence of Halton @cite rHAL60a&thinsp;, which is
 * essentially a modification of Hammersley nets for producing an infinite
 * sequence of points having low discrepancy. The @f$i@f$th point in @f$s@f$
 * dimensions is
 * 
 * @anchor REF_hups_HaltonSequence_eq_Halton_point2 @f[ \mathbf{u}_i =
 *         (\psi_{b_1}(i),\psi_{b_2}(i),…, \psi_{b_s}(i)),
 *         \tag{Halton-point2} @f] for @f$i=0,1,2,…@f$, where @f$\psi_b@f$ is
 *         the radical inverse function in base @f$b@f$, defined in class @ref
 *         RadicalInverse, and where @f$2 = b_1 < \cdots< b_s@f$ are the @f$s@f$
 *         smallest prime numbers in increasing order.
 *
 ...
 *         {@link REF_hups_overview_eq_Halton_point Halton-point} ), in the same
 *         way as for the class @ref HammersleyPointSet, for all
...
public class HaltonSequence extends PointSet {
    ...
    * <tt>x0[</tt>@f$j@f$<tt>]</tt> is the radical inverse of @f$i_j@f$. The
    * dimension of `x0` must be at least as large as the dimension of this object.
    * 
   ...
   /**
    * Initializes the Halton sequence starting at point `x0`. The dimension of `x0`
    * must be at least as large as the dimension of this object.
    * 
    * @param x0 starting point of the Halton sequence
    */
   ...
   /**
    * Permutes the digits using permutations from @cite vFAU09a&thinsp; for all
    * coordinates. After the method is called, the coordinates
    * 
    * @f$u_{i,j}@f$ are generated via @f[ u_{i,j} = \sum_{r=0}^{k-1} \pi_j[a_r]
    *               b_j^{-r-1}, @f] for @f$j=0,…,s-1@f$, where @f$\pi_j@f$ is the
    *               Faure-Lemieux (2008) permutation of @f$\{0,…,b_j-1\}@f$.
    */
   ...
   /**
    * Permutes the digits using Faure permutations for all coordinates. After the
    * method is called, the coordinates @f$u_{i,j}@f$ are generated via @f[ u_{i,j}
    * = \sum_{r=0}^{k-1} \pi_j[a_r] b_j^{-r-1}, @f] for @f$j=0,…,s-1@f$,
    * where @f$\pi_j@f$ is the Faure permutation of @f$\{0,…,b_j-1\}@f$.
    */
   ...
   public double getCoordinate(int i, int j) {
      if (radinv != null) {
         ...
         if (permuted)
            return RadicalInverse.permutedRadicalInverse(base[j], permutation[j], k);
         else
