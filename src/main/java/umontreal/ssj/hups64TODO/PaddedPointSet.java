...
/**
 * This container class realizes *padded point sets*, constructed by taking some
 * coordinates from a point set @f$P_1@f$, other coordinates from a point
 * set @f$P_2@f$, and so on. This can be used to implement *Latin Supercube
 * Sampling* as proposed in @cite vOWE98a, for example. After calling the
 * constructor to create the structure, component point sets can be padded to it
 * by calling `padPointSet` or `padPointSetPermute`.
 *
 * Only sets with the same number of points can be padded. Point sets with too
 * many points or coordinates can be trimmed down by using the class
 * 
 * @ref SubsetOfPointSet before they are padded. Infinite-dimensional point sets
 *      are allowed, but once one is padded, no additional point set can be
 *      padded.
 *
 *      The points of each padded set can be permuted randomly, independently
 *      across the padded sets. If such a random permutation is desired, the
 *      point set should be padded via `padPointSetPermute'. When calling
 *      `randomize`, random permutations are then generated for all point sets
 *      that have been padded by `padPointSetPermute`. This is all what this
 *      `randomize` method does. If other randomizations are desired, the must
...
public class PaddedPointSet extends PointSet {
   ...
   /**
    * Constructs a structure for padding at most `maxPointSets` point sets. This
    * structure is initially empty and will eventually contain the different point
    * sets that are padded, via the method `padPointSet`.
    * 
   ...
   /**
    * Pads the point set `P` to the present structure. This adds new coordinates to
    * the points.
    * 
    ...
    * will be used to access the coordinates taken from the points of `P` (i.e.,
    * these points are randomly permuted).
    * 
    * @param P point set being padded
    */
   ...
   /**
    * Erases all the random permutations of the padded point sets that are randomly
    * permuted. All the permutations are reset to the identity.
    */
   ...
   /**
    * Randomly permutes the points for the padded point sets that have been added
    * via `padPointSetPermute`.
    */
