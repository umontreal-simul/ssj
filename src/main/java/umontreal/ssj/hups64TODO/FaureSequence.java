 ...
 * of @f$n@f$ up to @f$2^{31}@f$ are allowed. One has @f$r = k@f$. The generator
 * matrices are @f[ \mathbf{C}_j = \mathbf{P}^j \mod b @f]
 ...
 * 
 * @f$l@f$ objects among @f$c@f$, @f${c\choose l}@f$, for @f$l\le c@f$ and is 0
 *         for @f$l > c@f$. The matrix @f$\mathbf{C}_0@f$ is the identity,
 * @f$\mathbf{C}_1 = \mathbf{P}@f$, and the other @f$\mathbf{C}_j@f$’s can be
 *                 defined recursively via @f$\mathbf{C}_j = \mathbf{P}
 *                 \mathbf{C}_{j-1} \mod b@f$. Our implementation uses the
 *                 recursion @f[ {c \choose l} = {{c-1} \choose l} + {{c-1}
 *                 \choose{l-1}} @f] to evaluate the binomial coefficients in
 *                 the matrices @f$\mathbf{C}_j@f$, as suggested by Fox @cite
 *                 rFOX86a&thinsp; (see also @cite fGLA04a&thinsp;, page 301).
 *                 The entries @f$x_{j,l,c}@f$ of @f$\mathbf{C}_j@f$ are
 *                 computed as follows: @f[ \begin{array}{lcll} x_{j,c,c} & = &
 *                 1 & \quad\mbox{ for } c=0,…,k-1, \\ x_{j,0,c} & = & j
 *                 x_{j,0,c-1} & \quad\mbox{ for } c=1,…,k-1, \\ x_{j,l,c} & =
 *                 & x_{j,l-1,c-1} + j x_{j,l,c-1} & \quad\mbox{ for } 2\le c <
 *                 l \le k-1, \\ x_{j,l,c} & = & 0 & \quad\mbox{ for } c>l
 *                 \mbox{ or } l \ge k. \end{array} @f] For any integer @f$m >
 *                 0@f$ and @f$\nu\ge0@f$, if we look at the
 *                 vector @f$(u_{i,j,1},…,u_{i,j,m})@f$ (the first @f$m@f$
 *                 digits of coordinate
 * @f$j@f$ of the output) when @f$i@f$ goes from @f$\nu b^m@f$ to
 * @f$(\nu+1)b^m - 1@f$, this vector takes each of its @f$b^m@f$ possible values
 *               exactly once. In particular, for @f$\nu= 0@f$, @f$u_{i,j}@f$
 *               visits each value in the set @f$\{0, 1/b^m, 2/b^m, …,
 *               (b^m-1)/b^m\}@f$ exactly once, so all one-dimensional
 *               projections of the point set are identical. However, the values
 *               are visited in a different order for the different values
 *               of @f$j@f$ (otherwise all coordinates would be identical).
 *               For @f$j=0@f$, they are visited in the same order as in the van
 *               der Corput sequence in base @f$b@f$.
 *
 *               An important property of Faure nets is that for any
 *               integers @f$m > 0@f$ and @f$\nu\ge0@f$, the point
 *               set @f$\{\mathbf{u}_i@f$ for @f$i = \nu b^m,…, (\nu+1)b^m
 *               -1\}@f$ is a @f$(0,m,s)@f$-net in base @f$b@f$. In particular,
 *               for @f$n = b^k@f$, the first @f$n@f$ points form a
 * @f$(0,k,s)@f$-net in base @f$b@f$. The Faure nets are also projection-regular
 *                   and dimension-stationary (see @cite vLEC02a&thinsp; for
 *                   definitions of these properties).
 *
 *                   To obtain digital nets from the *generalized Faure
 *                   sequence*
 * @cite rTEZ95a&thinsp;, where @f$\mathbf{P}_j@f$ is left-multiplied by some
 *       invertible matrix @f$\mathbf{A}_j@f$, it suffices to apply an
 *       appropriate matrix scramble (e.g., via #leftMatrixScramble ). This
 *       changes the order in which @f$u_{i,j}@f$ visits its different values,
 *       for each coordinate
...
public class FaureSequence extends DigitalSequence {
    ...
    *                    randomization on more than @f$k@f$ digits (e.g., a random
    *                    digital shift for @f$w > k@f$ digits, or a linear scramble
    *                    yielding @f$r > k@f$ digits), one should take @f$w = r =
    *                    k@f$ for better computational efficiency. Restrictions:
    *                    `dim` @f$\le500@f$ and @f$b^k \le2^{31}@f$.
    * @param b   base
...
//      EpsilonHalf = 0.5*normFactor;
   ...
   /**
    * Same as {@link #FaureSequence() FaureSequence(b, k, w, w, dim)} with
    * base @f$b@f$ equal to the smallest prime larger or equal to `dim`, and with
    * *at least* `n` points. The values of @f$k@f$,
    * 
    * @f$r@f$, and @f$w@f$ are taken as @f$k = \lceil\log_b n\rceil@f$ and @f$r = w
    *          = \max(k, \lfloor30 / \log_2 b\rfloor)@f$.
