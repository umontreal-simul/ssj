 ...
 * of same size and orientation. In @f$s@f$ dimensions, it divides the
 * 
 * @f$j@f$th axis in @f$k_j \ge1@f$ equal parts of length @f$d_j = 1/k_j@f$,
 *           for @f$j=0,…,s-1@f$. This partitions the unit cube @f$[0,1)^s@f$
 *           into @f$n = k_0\cdots k_{s-1}@f$ rectangular boxes of the same
 *           size. The point set is defined by generating one random point
 *           uniformly in each of those
 * @f$n@f$ boxes.
 *
 *         We number the boxes (or strata) as follows: The box determined by the
 *         interval @f$[c_j d_j, (c_j+1)d_j)@f$ for @f$j=0,…,s-1@f$ has
 *         number @f$i = c_0 + c_1 k_0 + \cdots+ c_{s-1} k_0 \cdots k_{s-2}@f$.
 *         By default, the points are enumerated by order of their box number.
 *         One can also create iterators that enumerate the points by order of
 *         any given coordinate.
 *
 *         Although this class extends `CachedPointSet`, here there is no
 *         underlying point set `p` that is cached.
 *
...
public class StratifiedUnitCube extends CachedPointSet {
    ...
    * of the vector <tt>k</tt> equal to the integer <tt>k</tt>. This gives @f$n =
    * k^s@f$ strata (and points), with
    * 
    * @f$s = {\mathtt{dim}}@f$.
    */
   public StratifiedUnitCube(int k, int dim) {
   ...
   /**
    * This randomization generates one point randomly in its corresponding box, for
    * each of the @f$n@f$ boxes. The stratified points are defined only after this
