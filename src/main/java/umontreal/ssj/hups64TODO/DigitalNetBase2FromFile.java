 ...
 * base 2* either from a file, or from a URL address. See the documentation
 * of @ref umontreal.ssj.hups.DigitalNetFromFile. The parameters used in
 * building the net are those defined in the class
 * 
 * @ref umontreal.ssj.hups.DigitalNetBase2. The format of the data files must be
 *      as follows, where @f$B@f$ represents any generating matrix @f$C_j@f$
 ...
 *  </div> </tt> </center>
 *
 *      For each dimension @f$j@f$, there must be a @f$k@f$-dimensional vector
 *      of 32-bit integers (the @f$a_i@f$) corresponding to the columns of
 * @f$\mathbf{C}_j@f$. The correspondence is such that the integer @f$a_i =
 *                     2^{30}(\mathbf{C}_j)_{1i} + 2^{29}(\mathbf{C}_j)_{2i} +
 *                     \cdots+ 2^{31 - r}(\mathbf{C}_j)_{ri}@f$.
 *
...
public class DigitalNetBase2FromFile extends DigitalNetBase2 {
    ...
    * `filename`. See the documentation in
    * 
    * @ref umontreal.ssj.hups.DigitalNetFromFile. Parameter `w` gives the number of
    *      bits of resolution, `r1` is the number of rows, and `s1` is the
    *      dimension. Restrictions: `s1` must be less than the maximal dimension,
    *      and `r1` less than the maximal number of rows in the data file. Also
    *      `w` @f$\ge@f$ `r1`.
   ...
   /**
    * Same as {@link #DigitalNetBase2FromFile() DigitalNetBase2FromFile(filename,
    * r, 31, s1)} where `s1` is the dimension and `r` is given in data file
    * `filename`.
    * 
    * @param filename Name of the file to be read
    * @param s1       Number of dimensions
