 ...
 * @anchor REF_hups_KorobovLatticeSequence_eq_Korobov_seq1 @f[ \mathbf{u}_i =
 *         (a^i (1, a, a^2, …) \bmod n) / n = (\tilde{a}^i (1, \tilde{a},
 *         \tilde{a}^2, …) \bmod n) / n \tag{Korobov-seq1} @f]
 *         for @f$i=0,…,n-1@f$. For @f$k = 0,1,…@f$, we have an increasing
 *         sequence of lattices contained in one another.
 *
 *         These embedded lattices contain an infinite sequence of points that
 *         can be enumerated as follows @cite vHIC01a&thinsp;:
 * @anchor REF_hups_KorobovLatticeSequence_eq_Korobov_seq2 @f[ \mathbf{u}_i =
 *         \psi_b(i) \left(1, a, a^2, …\right) \bmod1. \tag{Korobov-seq2} @f]
 *         where @f$\psi_b(i)@f$ is the radical inverse function in
 *         base @f$b@f$, defined in @ref RadicalInverse. The first @f$n=b^k@f$
 *         points in this sequence are exactly the same as the @f$n@f$ points in
...
public class KorobovLatticeSequence extends KorobovLattice {
   ...
   /**
    * Constructs a new lattice sequence with base `b` and `generator` @f$ = a@f$.
    * 
