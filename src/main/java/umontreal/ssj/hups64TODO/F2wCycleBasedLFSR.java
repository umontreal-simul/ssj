 ...
 * sequence. The recurrence used to produce the point set is @f[ m_n =
 * \sum_{i=1}^r b_i m_{n-i} @f] where @f$m_n\in\mathbb F_{2^w}@f$, @f$n\geq0@f$
 * and @f$b_i\in\mathbb F_{2^w}@f$. There is a polynomial in @f$\mathbb
 * F_{2^w}[z]@f$ associated with this recurrence called the *characteristic
 * polynomial*. It is @f[ P(z) = z^r + \sum_{i=1}^r b_i z^{r-i}. @f] In the
 * implementation, this polynomial is stored in an object `F2wStructure`.
 *
 ...
 *             dimensions produced by this class is @f[ \left\{
 *             (\phi(\mathbf{y}_0),\phi(\mathbf{y}_s),…,\phi(\mathbf{y}_{s(t-1)}):
 *             (\mathbf{v}_0,…,\mathbf{v}_{r-1})\in\mathbb
 *             F_2^{rw}\right\} @f] where @f$\mathbf{y}_n =
 *             \mbox{trunc}_h(\mathbf{v}_n,
 *             \mathbf{v}_{n+1},…)@f$, @f$\mathbf{v}_n@f$ is the
 *             representation of
