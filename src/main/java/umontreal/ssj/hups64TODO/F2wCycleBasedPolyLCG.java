...
/**
 * This class creates a point set based upon a linear congruential sequence in
 * the finite field @f$\mathbb F_{2^w}[z]/P(z)@f$. The recurrence is @f[ q_n(z)
 * = z^s q_{n-1}(z) \mod P(z) @f] where @f$P(z)\in\mathbb F_{2^w}[z]@f$ has
 * degree @f$r@f$ and @f$q_n(z) = q_{n,1} z^{r-1} + \cdots+ q_{n,r} \in\mathbb
 * F_{2^w}[z]/P(z)@f$. The parameter @f$s@f$ is called the stepping parameter of
 * the recurrence. The polynomial @f$P(z)@f$ is not necessarily the
 * characteristic polynomial of this recurrence, but it can still be used to
 * store the parameters of the recurrence. In the implementation, it is stored
 * in an object of the class
 * 
 * @ref umontreal.ssj.hups.F2wStructure. See the description of this class for
 *      more details on how the polynomial is stored.
 *
 *      Let @f$\mathbf{x} = (x^{(0)}, …, x^{(p-1)}) \in\mathbb F_2^p@f$ be a
 * @f$p@f$-bit vector. Let us define the function @f$\phi(\mathbf{x}) =
 *             \sum_{i=1}^p 2^{-i} x^{(i-1)}@f$. The point set in @f$t@f$
 *             dimensions produced by this class is @f[ \{
 *             (\phi(\mathbf{y}_0),\phi(\mathbf{y}_1),…,\phi(\mathbf{y}_{t-1}):
 *             (\mathbf{q}_{0,1},…,\mathbf{q}_{0,r-1})\in\mathbb
 *             F_2^{rw}\} @f] where @f$\mathbf{y}_n =
 *             (\mathbf{q}_{n,1},…,\mathbf{q}_{n,r})@f$,
 * @f$\mathbf{q}_{n,i}@f$ is the representation of @f$q_{n,i}@f$ under the
 *                        polynomial basis of @f$\mathbb F_{2^w}@f$
 *                        over @f$\mathbb F_2@f$.
 *
...
public class F2wCycleBasedPolyLCG extends CycleBasedPointSetBase2 {
   ...
   /**
    * Constructs a point set with @f$2^{rw}@f$ points. See the description of the
    * class @ref umontreal.ssj.hups.F2wStructure for the meaning of the parameters.
    */
    ...
    * <SPAN CLASS="MATH"><I>n</I></SPAN> and multiplier
    * <SPAN CLASS="MATH"><I>a</I></SPAN>. If pgcd<SPAN CLASS="MATH">(<I>a</I>,
    * <I>n</I>) = 1</SPAN>, this constructs a full-period LCG which has two cycles,
    * one containing 0 and one, the LCG period.
    *
    ...
    * parameters are located at line numbered `no` of `filename`. The available
    * files are listed in the description of class @ref
