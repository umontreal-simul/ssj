 ...
 * Let @f$2 = b_1 < b_2 < \cdots@f$ denote the sequence of all prime numbers by
 * increasing order. The Hammersley point set with @f$n@f$ points in @f$s@f$
 * dimensions contains the points
 * 
 * @anchor REF_hups_HammersleyPointSet_eq_Hammersley_point2 @f[ \mathbf{u}_i =
 *         (i/n,\psi_{b_1}(i),\psi_{b_2}(i),…, \psi_{b_{s-1}}(i)),
 *         \tag{Hammersley-point2} @f] for @f$i=0,…,n-1@f$, where @f$\psi_b@f$
 *         is the radical inverse function in base @f$b@f$, defined in @ref
 *         RadicalInverse. This class is not a subclass of @ref DigitalNet,
 *         because the basis is not the same for all coordinates. We do obtain a
 *         net in a generalized sense if @f$n = b_1^{k_1} = b_2^{k_2} = \cdots=
 *         b_{s-1}^{k_{s-1}}@f$ for some integers
 * @f$k_1,…,k_{s-1}@f$.
 *
...
public class HammersleyPointSet extends PointSet {
    ...
    * = \sum_{r=0}^{k-1} \pi_j[a_r] b_j^{-r-1}, @f] for @f$j=1,…,s-1@f$
    * and @f$u_{i,0}=i/n@f$, where @f$\pi_j@f$ is the Faure permutation
    * of @f$\{0,…,b_j-1\}@f$.
    */
   ...
   /**
    * Erases the Faure permutations: from now on, the digits will not be Faure
    * permuted.
    */
   ...
   public double getCoordinate(int i, int j) {
      ...
      if (permuted)
         return RadicalInverse.permutedRadicalInverse(base[j - 1], permutation[j - 1], i);
      else
