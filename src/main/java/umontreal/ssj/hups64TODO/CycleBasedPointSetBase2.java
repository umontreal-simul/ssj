...
/**
 * Similar to @ref CycleBasedPointSet, except that the successive values in the
 * cycles are stored as integers in the range @f$\{0,\dots,2^k-1\}@f$, where
 * 
 * @f$1\le k \le31@f$. The output values @f$u_{i,j}@f$ are obtained by dividing
 *         these integer values by @f$2^k@f$. Point sets where the successive
 *         coordinates of each point are obtained via linear recurrences modulo
 *         2 (e.g., linear feedback shift registers or Korobov-type polynomial
 *         lattice rules) are naturally expressed in this form. Storing the
 *         integers
 * @f$2^k u_{i,j}@f$ instead of the @f$u_{i,j}@f$ themselves makes it easier to
 *        apply randomizations such as digital random shifts in base 2, which
 *        are applied to the bits *before* transforming the value to a real
 *        number
 * @f$u_{i,j}@f$. When a random digital shift is performed, it applies a bitwise
...
// dim = Integer.MAX_VALUE;     // Dimension is infinite.
    ...
    * using stream `stream` to generate the random numbers, for coordinates `d1` to
    * `d2 - 1`. This applies a bitwise exclusive-or of all the points with a single
    * random point.
    */
