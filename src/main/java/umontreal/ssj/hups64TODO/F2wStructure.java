 ...
 * @ref umontreal.ssj.hups.F2wCycleBasedPolyLCG. It also stores the parameters
 *      of these point sets which will contain @f$2^{rw}@f$ points (see the
 *      meaning of @f$r@f$ and @f$w@f$ below). The parameters can be stored as a
 *      polynomial @f$P(z)@f$ over @f$\mathbb F_{2^w}[z]@f$ @f[ P(z) = z^r +
 *      \sum_{i=1}^r b_i z^{r-i} @f] where @f$b_i\in\mathbb F_{2^w}@f$
 *      for @f$i=1,…,r@f$. Let @f$\zeta@f$ be the root of an irreducible
 *      polynomial @f$Q(z)\in\mathbb F_2[z]@f$. It is well known
 *      that @f$\zeta@f$ is a generator of the finite field @f$\mathbb
 *      F_{2^w}@f$. The elements of @f$\mathbb F_{2^w}@f$ are represented using
 *      the polynomial ordered basis @f$(1,\zeta,…,\zeta^{w-1})@f$.
 *
 *      In this class, only the non-zero coefficients of @f$P(z)@f$ are stored.
 *      It is stored as @f[ P(z) = z^{\mathtt{r}} +
 *      \sum_{i=0}^{\mathtt{nbcoeff}} {\mathtt{coeff[}}i{\mathtt{]}}
 *      z^{{\mathtt{nocoeff[}}i{\mathtt{]}}} @f] where the coefficients in
 *      `coeff[]` represent the non-zero coefficients
 * @f$b_i@f$ of @f$P(z)@f$ using the polynomial basis. The finite field
 * @f$\mathbb F_{2^w}@f$ used is defined by the polynomial @f[ Q(z) = z^w +
 *            \sum_{i=1}^w a_i z^{w-i} @f] where @f$a_i\in\mathbb F_2@f$,
 *            for @f$i=1,…,w@f$. Polynomial @f$Q@f$ is stored as the bit
 *            vector `modQ` = @f$(a_w,…,a_1)@f$.
 *
 *            The class also stores the parameter `step` that is used by the
 *            classes
 * @ref umontreal.ssj.hups.F2wNetLFSR,
 * @ref umontreal.ssj.hups.F2wNetPolyLCG,
 * @ref umontreal.ssj.hups.F2wCycleBasedLFSR and
 * @ref umontreal.ssj.hups.F2wCycleBasedPolyLCG. This parameter is such that the
 *      implementation of the recurrence will output a value at every `step`
 *      iterations.
 *
...
public class F2wStructure {
    ...
    * polynomial in @f$\mathbb F_{2^w}[z]@f$, as well as a stepping parameter.
    */
   ...
   /**
    * Constructs a polynomial in @f$\mathbb F_{2^w}[z]@f$ after reading its
    * parameters from file `filename`; the parameters of this polynomial are stored
    * at line number `no` of `filename`. The files are kept in different
    * directories depending on the criteria used in the searches for the parameters
    * defining the polynomials. The different criteria for the searches and the
    * theory behind it are described in @cite rPAN04d, @cite rPAN04t&thinsp;. The
    * existing files and the number of polynomials they contain are given in the
    * following tables. The first table below contains files in subdirectory
    * <tt>LFSR_equid_max</tt>. The name of each file indicates the value of @f$r@f$
    * and @f$w@f$ for the polynomials. For example, file <tt>f2wR2_W5.dat</tt> in
    * directory <tt>LFSR_equid_max</tt> contains the parameters of 2358 polynomials
    * with @f$r=2@f$ and @f$w=5@f$. For example, to use the 5<em>-th</em>
    * polynomial of file <tt>f2wR2_W5.dat</tt>, one may call
    * <tt>F2wStructure("f2wR2_W5.dat", 5)</tt>. The files of parameters have been
    * stored at the address
    * [http://simul.iro.umontreal.ca/ssj/dataF2w/Panneton/](http://simul.iro.umontreal.ca/ssj/dataF2w/Panneton/).
    * The files should be copied in the user directory, and passed as parameter to
    * the constructor.
    */
   ...
   /**
    * This method returns a string containing the polynomial @f$P(z)@f$ and the
    * stepping parameter.
    */
