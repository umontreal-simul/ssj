...
/**
 * Linear feedback shift register (LFSR) random number generators @cite
 * rLEC96a, @cite vLEC99a, @cite rNIE92b, produce numbers by generating a
 * sequence of bits from a linear recurrence modulo 2, and forming fractional
 * numbers by taking blocks of successive bits. More precisely, let @f$\mathbb
 ...
 *         mrg}) all computations are performed in @f$\mathbb F_2@f$ (this can
 *         be identified with working in integer arithmetic modulo 2). Suppose
 *         that
 * @f$\mathbf{s}_0 = (x_0,…,x_{k-1})\in\{0,1\}^k@f$ is fixed and define
 * @anchor REF_hups_CycleBasedLFSR_taus @f[ u_n = \sum_{i=1}^L x_{ns+i-1}
 *         2^{-i}, \tag{taus} @f] where @f$s@f$ and @f$L@f$ are positive
 *         integers. If @f$P@f$ is primitive,
 * @f$\mathbf{s}_0\not0@f$, and @f$\rho= 2^k-1@f$ is coprime to @f$s@f$, then
 *                          the sequences ({@link REF_hups_CycleBasedLFSR_mrg
 *                          mrg}) and ({@link REF_hups_CycleBasedLFSR_taus
 *                          taus}) are both purely periodic with
 *                          period @f$\rho@f$. Computing @f$u_n@f$ from
 * @f$u_{n-1}@f$ involves performing @f$s@f$ steps of the recurrence
 *               ({@link REF_hups_CycleBasedLFSR_mrg mrg}).
 *
 *               Suppose now that we have @f$J@f$ LFSR recurrences,
 *               the @f$j@f$-th one having a primitive characteristic
 *               polynomial @f$P_j(z)@f$ of degree
 * @f$k_j@f$, and step size @f$s_j@f$. Let @f$\{x_{j,n},  n\ge0\}@f$ be the
 * @f$j@f$-th LFSR sequence, and define @f$x_n = (x_{1,n} + \cdots+ x_{J,n})
 *            \bmod 2@f$ and @f$u_n@f$ as in
 *            ({@link REF_hups_CycleBasedLFSR_taus taus}). Equivalently,
 *            if @f$\{u_{j,n},  n\ge0\}@f$ is the output sequence from
 *            the @f$j@f$-th LFSR, then @f$u_n = u_{1,n}\oplus\cdots\oplus
 *            u_{J,n}@f$ where
 * @f$\oplus@f$ denotes the bitwise exclusive-or in the binary expansion. The
 *              sequence @f$\{x_n\}@f$ is called the combined LFSR sequence and
 *              a generator that produces this @f$\{u_n\}@f$ is called a
...
public class CycleBasedLFSR extends CycleBasedPointSetBase2 {
    ...
    * 
    * @f$2^{k_1}@f$ points. `step1` is the number of times the recursion is
    *               executed in order to generate the next state. `nbcoeff1` is the
    *               number of non-zero coefficients of the polynomial, while
    *               `nocoeff1` is the list of the degree of the non-zero
    *               coefficients. Example: @f$P(z) = z^9 - z^4 - 1@f$, @f$k_1@f$ =
    *               9, `nbcoeff1` = 3, `nocoeff1` = \{9, 4, 0\}.
    */
   ...
   /**
    * Constructs a point set based on a combination of two polynomials in base 2
    * with @f$2^{k_1 + k_2}@f$ points. The meaning of the parameters is the same as
   ...
   /**
    * Constructs a point set after reading its parameters from file `filename`; the
    * parameters associated with number `no` of `filename` corresponds to the
    * <tt>no</tt>-th polynomial. The existing files and the number of polynomials
    * they contain are in the table below. The name of the files describe the
    * number of polynomials @f$J@f$ in the combined LFSR and the number of
    * points @f$2^k@f$ generated. For example, the parameters in file
    * <tt>j1_k11.dat</tt> are based on
    * 
    * @f$J = 1@f$ polynomial and generates @f$2^k = 2^{11}@f$ points, while those
    *      in file <tt>j2_k17.dat</tt> are based on a combination of @f$J=2@f$
    *      polynomials and generates @f$2^k = 2^{17}@f$ points. Thus to use the
    *      3<em>-th</em> combined LFSR of file <tt>j2_k17.dat</tt>, one must use
    *      <tt>CycleBasedLFSR("j2_k17",
    * 3)</tt>.
    *
    *      <center>
    *
   ...
   /**
    * This method returns a string containing the polynomials and the stepping
    * parameters.
    */
