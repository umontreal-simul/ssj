...
/**
 *
 * This container class embodies an arbitrary point set and its iterator adds a
 * random shift modulo 1 to all the points, when producing the coordinates.
 * 
 * The random shift modulo 1 is the same for all the points. This can be used
 * for example to apply a random shift modulo 1 to a @ref DigitalNet, for which
 * a @ref RandomShift would normally perform a random digital shift. Here,
 * calling `addRandomShift` generates a new random shift, represented by a
 * vector of @f$d@f$ uniforms over @f$(0,1)@f$, where @f$d@f$ is the current
 * dimension of the shift. Then, an iterator for this point set will enumerate
 * the points of the contained point set and apply this random shift modulo 1 to
 * each of them.
 *
...
public class RandShiftedMod1PointSet extends ContainerPointSet {
   ...
   /**
    * Constructs a structure to contain a randomly shifted version of `p`. The
    * random shifts will be generated in up to `dimShift` dimensions, using stream
    * `stream`.
    * 
   ...
   /**
    * Changes the stream used for the random shifts to `stream`, then refreshes the
    * shift for coordinates `d1` to `d2-1`.
    * 
   ...
   /**
    * Changes the stream used for the random shifts to `stream`, then refreshes all
    * coordinates of the random shift, up to its current dimension.
    */
    ...
    * coordinates) for coordinates `d1` to `d2-1`.
    */
   ...
   /**
    * Only the `nextCoordinate` method is reimplemented here and it returns the
    * shifted coordinate.
    */
