 ...
 *      when using a small low-dimensional point set more than once.
 *
 *      There is a #sort method available to sort the cached points via a
 * @ref umontreal.ssj.util.MultiDimSort, and a #stripCoordinates method that
 *      remove some coordinates of the cached points. Those methods affect only
 *      the array where the points are cached; they have no impact on the
...
public class CachedPointSet extends PointSet {
   ...
   } // Constructs an empty cache for a point set.

   /**
    * Creates a new `PointSet` object that contains an array storing the
    * coordinates `fromDim` (inclusive) to `toDim` (exclusive) of the points
    * numbered `fromPoint` (inclusive) to `toPoint` (exclusive) of `p`. Thus, the
    * cached points have `dim = fromDim - toDim` coordinates and there are @f$n =
    * \mathtt{toPoint}-\mathtt{fromPoint}@f$ of them. The original point set `p`
    ...
    * @param toDim     1 + index of last coordinate in the original points
    */
   ...
   /**
    * Caches the points, skipping `fromDim` coordinates and taking the next `dim`
    * ones.
    */
   protected void fillCache(int fromDim, int dim) {
      PointSetIterator itr = P.iterator();
      if (fromPoint > 0)
         itr.setCurPointIndex(fromPoint);
      for (int i = 0; i < numPoints; i++)
         itr.nextPoint(x[i], fromDim, dim);
   }

   /**
    * Same as {@link #CachedPointSet(PointSet,int,int,int,int) CachedPointSet(p, 0,
    * n, 0, dim)}.
    */
   ...
   /**
    * Creates a new `PointSet` object that contains an array storing the points of
    * `p`. The number of points and their dimension are the same as in the original
    * point set. Both must be finite.
    * 
    * @param p point set to be cached
    */
   ...
   /**
    * Constructs and returns a point set iterator that gets the values directly
    ...
    * underlying point set, this method should be redefined to randomize the cached
    * points.
    */
   ...
   /**
    * Randomizes the underlying point set using `rand` and re-caches the points. In
    * case there is no underlying point set, this method must be redefined to
    * randomize the cached points in the appropriate way. If
    * `setRandomizeParent(false)` was called, this method invokes
    ...
    * applies only to the cached points, and not to the original contained point
    * set `p`.
    */
    ...
    * 
    * @ref umontreal.ssj.util.MultiDimSort sorting algorithm `sort`. This does not
    *      affect the underlying point set `p`.
    */
   ...
   /**
    * Removes the first `d` coordinates of each cached point. This does not affect
    * the underlying point set `p`. This could be useful in particular for the
    * ArrayRQMC simulation method.
    */
