 ...
 *                     into @f$n@f$ intervals of length @f$1/n@f$. The
 *                     randomized point set has the property that for each
 *                     coordinate @f$j@f$, there is exactly one point with
 *                     coordinate @f$j@f$ in each interval on length @f$1/n@f$.
 *
 *                     To generate the points, for each coordinate @f$j@f$ we
 *                     generate a random permutation @f$\pi_j@f$ of the @f$n@f$
 *                     intervals. Then, for point
 * @f$i@f$, we generate coordinate @f$j@f$ randomly uniformly in the interval
 *          number @f$\pi_j(i)@f$. The points are enumerated by order of their
 *          first coordinate.
 *
...
public class LatinHypercube extends CachedPointSet {
   ...
   /**
    * This randomization generates a random LHS point set. The points are sorted by
    * their first coordinate. The LHS points are defined only after this method has
    * been called.
    * 
    * @param stream Random stream to generate the permutations and the random
   ...
   /**
    * Randomizes the points using LHS, regardless of what `rand` is. Equivalent to
