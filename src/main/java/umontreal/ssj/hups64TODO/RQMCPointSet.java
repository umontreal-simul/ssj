 ...
 * 
 * @cite vLEC00b, @cite vLEC02a, @cite vOWE97a, @cite vOWE97b. The idea is to
 *       randomize a point set `p` so that:
 *
 *       <ul>
 *       <li>it retains its high uniformity when taken as a set and</li>
 *       <li>each individual point is a random vector with the uniform
 *       distribution over @f$(0, 1)^s@f$.</li>
 *       </ul>
 *
 ...
 * @cite vSLO94a: Generate a single point @f$\mathbf{U}@f$ uniformly over @f$(0,
 *       1)^s@f$ and add it to each point of @f$P_n@f$, modulo 1,
 ...
 *                  coordinates in base @f$b@f$, and add the digits,
 *                  modulo @f$b@f$, to the corresponding digits of each point
 *                  of @f$P_n@f$.
 * 
...
public class RQMCPointSet {
   ...
   /**
    * Randomizes the point set. The randomization and the point set are those of
    * this object.
    */
   ...
   /**
    * Gives a label (a short string) to identify this object, e.g., when making
    * plots.
    * 
