 ...
 * from a file, or from a URL address on the World Wide Web. The parameters used
 * in building the net are those defined in class
 * 
 * @ref umontreal.ssj.hups.DigitalNet. The format of the data files must be the
 *      following: (see the format in @ref umontreal.ssj.hups)
 *
 *      <center> <tt><div class="SSJ-fbox">
 ...
 *      must be a
 * @f$k\times r@f$ matrix of integers in @f$\{0, 1, …, b-1\}@f$ (note that the
 *            matrices must appear in transposed form).
 *
 *            The predefined files of parameters are kept in different
 *            directories, depending on the criteria used in the searches for
 *            the parameters defining the digital net. These files have all been
 *            stored at the address
 *            [http://simul.iro.umontreal.ca/ssj/data](http://simul.iro.umontreal.ca/ssj/data).
 *            Each file contains the parameters for a specific digital net. The
 *            name of the files gives information about the main parameters of
 *            the digital net. For example, the file named
 *            `Edel/OOA2/B3S13R9C9St6` contains the parameters for a digital net
 *            proposed by Yves Edel (see
 *            [http://www.mathi.uni-heidelberg.de/~yves/index.html](http://www.mathi.uni-heidelberg.de/~yves/index.html))
 *            based on ordered orthogonal arrays; the digital net has base `B =
 *            3`, dimension `S = 13`, the generating matrices have `R = 9` rows
...
public class DigitalNetFromFile extends DigitalNet {
    ...
    * If a file named `filename` can be found relative to the program’s
    * directory, then the parameters will be read from this file; otherwise, they
    * will be read from the file named `filename` in the `ssj.jar` archive. If
    * `filename` is a URL string, it will be read on the World Wide Web. For
    * example, to construct a digital net from the parameters in file
    * `B3S13R9C9St6` in the current directory, one must give the string
    * `"B3S13R9C9St6"` as argument to the constructor. As an example of a file read
    * from the WWW, one may give as argument to the constructor the string `
    * "http://simul.iro.umontreal.ca/ssj/data/Edel/OOA3/B3S13R6C6St4"`. Parameter
    * `w` gives the number of digits of resolution, `r1` is the number of rows, and
    * `s1` is the dimension. Restrictions: `s1` must be less than the maximal
    * dimension, and `r1` less than the maximal number of rows in the data file.
    * Also `w` @f$\ge@f$ `r1`.
    * 
   ...
   public DigitalNetFromFile(String filename, int r1, int w, int s1) throws MalformedURLException, IOException {
      ...
      if (r1 > numRows)
         throw new IllegalArgumentException(
               "DigitalNetFromFile:   One must have   r1 <= Max num rows" + PrintfFormat.NEWLINE);
      if (s1 > dim)
         throw new IllegalArgumentException(
               "DigitalNetFromFile:   One must have   s1 <= Max dimension" + PrintfFormat.NEWLINE);
      if (w < 0) {
         r1 = w = numRows;
         s1 = dim;
      }
      if (w < numRows)
         throw new IllegalArgumentException(
               "DigitalNetFromFile:   One must have   w >= numRows" + PrintfFormat.NEWLINE);
   ...
   /**
    * Same as {@link #DigitalNetFromFile() DigitalNetFromFile(filename, r, r, s)}
    * where `s` is the dimension and `r` is given in data file `filename`.
    * 
    * @param filename Name of the file to be read
   ...
   public String toString() {
      StringBuffer sb = new StringBuffer("File:   " + filename + PrintfFormat.NEWLINE);
   ...
   public String toStringDetailed() {
      ...
      sb.append(PrintfFormat.NEWLINE + "n = " + numPoints + PrintfFormat.NEWLINE);
      ...
      try {
         ...
         for (int i = 0; i < files.length; i++) {
            if (files[i].isDirectory())
               continue;
            if (files[i].getName().endsWith("gz") || files[i].getName().endsWith("zip"))
               continue;
   ...
   public static String listDir(String dirname) throws IOException {
      try {
         ...
         sb.append(
               PrintfFormat.s(-25, "     File") + PrintfFormat.s(-15, "       Base") + PrintfFormat.s(-10, "Dimension")
                     + PrintfFormat.s(-10, " numRows") + PrintfFormat.s(-10, "numColumns" + PrintfFormat.NEWLINE));
         int base = 0;
         for (int i = 0; i < list.size(); i++) {
            ...
            sb.append(PrintfFormat.s(-25, name) + PrintfFormat.d(10, net.b) + PrintfFormat.d(10, net.dim)
                  + PrintfFormat.d(10, net.numRows) + PrintfFormat.d(10, net.numCols) + PrintfFormat.NEWLINE);
         }
         return sb.toString();
   ...
   /**
    * Creates a list of all data files in directory `dirname` and writes that list
    * in format HTML in output file `filename`. Each data file contains the
    * parameters required to build a digital net. The resulting list contains a
    * line for each data file giving the name of the file, the base, the dimension,
    * the number of rows and the number of columns of the corresponding digital
    * net.
    */
