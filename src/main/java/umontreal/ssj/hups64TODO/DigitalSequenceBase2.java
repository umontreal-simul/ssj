 ...
 * 2. Concrete classes must implement the #extendSequence method that increases
 * the number of points of the digital sequence. Calling the methods #toNet or
 * #toNetShiftCj will transform the digital sequence into a digital net, which
 * has a fixed number of points @f$n@f$.
 *
...
public abstract class DigitalSequenceBase2 extends DigitalNetBase2 {
    ...
    * coordinates of the points. Returns the digital net. Note that since this
    * class is already a subclass of DigitalNetBase2, this transformation is not
    * really needed!
    */
    ...
    * and shifting all coordinates by one position. The first coordinate of
    * point @f$i@f$ is @f$i/n@f$, where @f$n@f$ is the total number of points. Thus
    * if the coordinates of a point of the digital sequence were @f$(x_0, x_1, x_2,
    * …, x_{s-1})@f$, then the coordinates of the point of the digital net will
    * be @f$(i/n, x_0, x_1, …, x_{s-1})@f$. In other words, for the digital net,
    * 
    * @f$\mathbf{C}_0@f$ is the reflected identity and for @f$j\ge1@f$,
    *                    the @f$\mathbf{C}_j@f$ used is the @f$\mathbf{C}_{j-1}@f$
    *                    of the digital sequence. If the digital sequence uses a
    *                    digital shift, then the digital net will include the
    *                    digital shift with one more dimension also. Returns the
    *                    digital net.
    */
   ...
   /**
    * Similar to #iterator, except that the first coordinate of the points
    * is @f$i/n@f$, the second coordinate is obtained via the generating
    * matrix @f$\mathbf{C}_0@f$, the next one via
    * 
    * @f$\mathbf{C}_1@f$, and so on. Thus, this iterator shifts all coordinates of
    *                     each point one position to the right and sets the first
    *                     coordinate of point @f$i@f$ to @f$i/n@f$, so that the
    *                     points enumerated with this iterator have one more
    *                     dimension. A digital shift, if present, will have one
    ...
    * and sets the first coordinate of point @f$i@f$ to
    * 
    * @f$i/n@f$, so that the points enumerated with this iterator have one more
    *            dimension. This iterator does not use the Gray code: the points
