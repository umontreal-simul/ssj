 ...
 * points are all the vectors of @f$s@f$ successive values found in any of the
 * cycles, from any starting point. Since this is defined for any positive
 * integer @f$s@f$, the points effectively have an infinite number of
 * dimensions. The number of points, @f$n@f$, is the sum of lengths of all the
 * cycles. The cycles of the point set are simply stored as a list of arrays,
 * where each array contains the successive values for a given cycle. By
 * default, the values are stored in `double`.
 *
 ...
 * vectors of @f$s@f$ successive values of a periodic recurrence, from all its
...
//      return curCycleD[coordinate];
   ...
   /**
    * Adds the cycle `c` to the list of all cycles. This method is used by subclass
    * constructors to fill up the list of cycles.
    */
   ...
   public class CycleBasedPointSetIterator extends DefaultPointSetIterator {
      ...
      public String formatState() {
         return super.formatState() + PrintfFormat.NEWLINE + "Current cycle: " + curCycleIndex;
      }
   }
