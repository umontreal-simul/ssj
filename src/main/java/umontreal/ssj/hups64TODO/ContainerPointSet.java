...
/**
 * This acts as a generic base class for all *container classes* that contain a
 * point set and apply a specific kind of transformation to the
 * coordinates @f$u_{i,j}@f$ when the points are generated by the iterator.
 * 
 * One example of such transformation is the *antithetic* map, applied by the
 * container class @ref AntitheticPointSet, where each output
 * coordinate @f$u_{i,j}@f$ is transformed into @f$1-u_{i,j}@f$. Another example
 * is @ref RandShiftedMod1PointSet, which applies a random shift modulo 1 (the
 * same shift) to all the points. Thus, the transformation can be deterministic
 * in some cases, and in other cases it may implement a randomization.
 * 
 * The `PointSetIterator` implemented in this `ContainerPointSet` class contains
 * itself an iterator for the contained point set and uses it to access the
 * points and coordinates internally, instead of maintaining itself indices for
 * the current point and current coordinate. It gets the
 * coordinates @f$u_{i,j}@f$ from the internal iterator, applies the appropriate
 * transformation, and returns the transformed coordinates.
 *
...
public abstract class ContainerPointSet extends PointSet {
   ...
   /**
    * Initializes this container point set so that it will contain the point set
    * `p0`. This method must be called by the constructor of any subclass of the
    * present abstract class.
    * 
   ...
   /**
    * By default, returns the *untransformed* coordinate @f$u_{i,j}@f$ for the
    * contained point set.
    * 
   ...
   public String toString() {
      return "Container point set of: {" + PrintfFormat.NEWLINE + P.toString() + PrintfFormat.NEWLINE + "}";
   }
