...
public class NiedXingSequenceBase2 extends DigitalSequenceBase2 {
    ...
    * 
    * @f$n = 2^k@f$ points and @f$w@f$ output digits, in `dim` dimensions. The
    *      generator matrices @f$\mathbf{C}_j@f$ are @f$w\times k@f$ and the
    *      numbers making the bit matrices are taken from [Pirsic’s
    *      site](http://www.ricam.oeaw.ac.at/people/page/pirsic/niedxing/index.html)
    *      . The bit matrices from Pirsic’s site are transposed to be consistent
    *      with SSJ, and at most 30 bits of the matrices are used.
    *      Restrictions: @f$0\le k\le30@f$, @f$k\le w@f$, and `dim`
    * @f$\le32@f$. @param k there will be 2^k points
