...
/**
 * This container class permits one to select a subset of a point set. This is
 * done by selecting a range or providing an array of either point or coordinate
 * indices. A typical application of a range selection is to make the number of
 * points or the dimension finite. It is also possible to provide, for example,
 * a random permutation in the selection of components. It is possible also to
 * take *projections* of coordinates for selected dimensions.
 *
 ...
 * number of points and the dimension are adapted to the current selection and
 * all indices still start from 0, i.e., the subset works just like an ordinary
 * point set.
 *
 * When the points or coordinates ranges are changed, existing iterators become
 * invalid. They should be reconstructed or reset to avoid inconsistencies.
 *
...
public class SubsetOfPointSet extends PointSet {
   ...
   /**
    * Constructs a new @ref PointSet object, initially identical to `p`, and from
    * which a subset of the points and/or a subset of the coordinates is to be
    * extracted.
    * 
    ...
    * 
    * @param from index of the point, in the contained point set, corresponding to
    *             the point 0 of this point set
    ...
    * @param to   index of the last coordinate taken for each point from the
    *             contained point set
    */
   ...
   /**
    * Selects the `numCoord` coordinates whose numbers are provided in the array
    * `coordIndices`.
    * 
    * @param coordIndices array of coordinate indices to be selected
    * @param numCoord     number of coordinatess for each point in the subset of
    *                     point set
    */
