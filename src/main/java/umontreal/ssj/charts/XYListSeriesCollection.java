...
/**
 * Stores data used in a `XYLineChart` or in other related charts, and provides
 * complementary tools to draw simple curves. For example, one may add or remove
 * plots series and modify plot style. It extends extends `SSJXYSeriesCollection
 * and is linked with the JFreeChart `XYSeriesCollection` class to store data
 * plots, and linked with the JFreeChart `XYLineAndShapeRenderer` to render the
 * plot. Each series must contain enough points to plot a nice curve. It is
 * recommended to use about 30 points. However, some rapidly varying functions
 * may require many more points. It can be used to draw scatter plots.
 *
...
public class XYListSeriesCollection extends SSJXYSeriesCollection {
   ...
   /**
    * Creates a new `XYListSeriesCollection` instance with default parameters and
    * given data series. The input parameter `data` represents a set of plotting
    * data.
    *
    * For example, if one @f$n@f$-row matrix `data1` is given as argument, then the
    * first row <tt>data1</tt>@f$[0]@f$ represents the
    * 
    * @f$x@f$-coordinate vector, and every other row <tt>data1</tt>@f$[i], i=1,…,
    *                    n-1@f$, represents a @f$y@f$-coordinate set for the
    *                    points. Therefore matrix
    *                    <tt>data1</tt>@f$[i][j]@f$, @f$i=0,…, n-1@f$,
    *                    corresponds to @f$n-1@f$ curves, all with the same
    * @f$x@f$-coordinates.
    *
    *                      However, one may want to plot several curves with
    *                      different
    * @f$x@f$-coordinates. In that case, one should give the curves as matrices
    *                      with two rows. For examples, if the argument `data` is
    *                      made of three 2-row matrices `data1`, `data2` and
    *                      `data3`, then they represents three different curves,
    *                      <tt>data*</tt>@f$[0]@f$ being the
   ...
   /**
    * Creates a new `XYListSeriesCollection` instance with default parameters and
    * given points `data`. If `data` is a @f$n@f$-row matrix, then the first row
    * <tt>data</tt>@f$[0]@f$ represents the
    * 
    * @f$x@f$-coordinate vector, and every other row <tt>data</tt>@f$[i], i=1,…,
    *                    n-1@f$, represents a @f$y@f$-coordinate set of points.
    *                    Therefore, if the points represents curves to be plotted,
    *                    <tt>data</tt>@f$[i][ ]@f$, @f$i=0,…, n-1@f$, corresponds
    *                    to
    * @f$n-1@f$ curves, all with the same @f$x@f$-coordinates. Only the first
    *           `numPoints` of `data` will be considered for each of the set of
    *           points.
    * @param data      series of point sets.
   ...
   /**
    * Creates a new `XYListSeriesCollection` instance with default parameters and
    * given data. The input parameter represents a set of data plots, the
    * constructor will count the occurrence number @f$Y@f$ of each value @f$X@f$ in
    * the `DoubleArrayList`, and plot the point @f$(X, Y)@f$. Each DoubleArrayList
   ...
   /**
    * Creates a new `XYListSeriesCollection` instance with default parameters and
    * given data series. The input parameter represents a set of plotting data.
    ...
    * the @f$x@f$-coordinates and vector `y` represents the
    * 
    * @f$y@f$-coordinates of the series. Only *the first* `numPoints` of `x` and
    *                     `y` will be added to the new series.
    * @param x         @f$x_i@f$ coordinates.
    * @param y         @f$y_i@f$ coordinates.
   ...
   /**
    * Adds a data series into the series collection. The input format of `data` is
    * described in constructor `XYListSeriesCollection(double[][] data)`.
    * 
    * @param data input data.
    * @return Integer that represent the number of point sets added to the current
    *         dataset.
    */
   ...
   /**
    * Adds data series into the series collection. The input format of `data` is
    * described in constructor `XYListSeriesCollection(double[][] data)`. Only *the
    * first* `numPoints` of `data` (the first `numPoints` columns of the matrix)
   ...
   /**
    * Adds a data series into the series collection. The input format of `data` is
    * described in constructor `XYListSeriesCollection (DoubleArrayList... data)`.
    * 
   ...
   /**
    * Enables the auto completion option. When this parameter is enabled, straight
    * lines are used to approximate points on the chart bounds if the method
    * isn’t able to display all points, because the user defined bounds are
    * smaller than the most significant data point coordinate, for instance. It
    * does not extrapolate the point sets, but simply estimates point coordinates
    * on the curve at bound positions for a better visual rendering.
    */
   ...
   /**
    * Adds marks on the points of a data series. It is possible to use any of the
    * marks provided by the TikZ package, some of which are "<tt>*</tt>",
    * "<tt>+</tt>" and "<tt>x</tt>". A blank character, used by default, disables
    * marks. The PGF/TikZ documentation provides more information about placing
    * marks on plots.
    * 
    * @param series    series index.
    ...
    * "<tt>densely dotted</tt>", "<tt>loosely
    * dotted</tt>", "<tt>dashed</tt>", "<tt>densely dashed</tt>",
    * "<tt>loosely dashed</tt>" and "<tt>only marks</tt>". If "<tt>only
    * marks</tt>" is chosen, then method #setMarksType must be called to choose the
    * marks (which are blank by default).
    * 
    * @param series      series index.
   ...
   /**
    * Selects the plot style for a given series. It is possible to use all the plot
    * options provided by the TikZ package. Some of which are:
    ...
    * documentation provides more information about smooth plots, sharp plots and
    * comb plots.
    * 
    * @param series    series index.
...
//            formatter.format(" %%%n");
    ...
    * @param sens     direction of the in chart last point true : point numPoint+1
    *                 is in the chart false : point numPoint-1 is in the chart
    *
