 ...
 * customize its appearance using methods of this class, with the
 ...
 * representing the axes. All these classes depend on the `JFreeChart` API (see
 * WWW link
 * [http://www.jfree.org/jfreechart/](http://www.jfree.org/jfreechart/)) which
 * provides tools to build charts with Java, to draw them, and export them to
 * files. However, only basic features are used here.
 *
...
public abstract class XYChart {
   ...
   /**
    * Sets a title to this chart. This title will appear on the chart displayed by
    * method #view.
    * 
   ...
   /**
    * The @f$x@f$ and the @f$y@f$ ranges of the chart are set automatically.
    */
    ...
    * `right` is `true`, the vertical axis will be on the left of the points,
    * otherwise on the right. If `top` is `true`, the horizontal axis will be under
    * the points, otherwise above the points.
    * 
   ...
   /**
    * Sets the @f$x@f$ and @f$y@f$ ranges of the chart using the format: `range =
    * [xmin, xmax, ymin, ymax]`. If `right` is `true`, the vertical axis will be on
    * the left of the points, otherwise on the right. If `top` is `true`, the
    * horizontal axis will be under the points, otherwise above the points.
    * 
    * @param range new axis ranges.
   ...
   /**
    * Sets the @f$x@f$ and @f$y@f$ ranges of the chart using the format: `range =
    * [xmin, xmax, ymin, ymax]`. If `xZero` is `true`, the vertical axis will pass
    * through the point @f$(0, y)@f$. If `yZero` is `true`, the horizontal axis
    * will pass through the point @f$(x, 0)@f$.
    * 
    * @param range new axis ranges.
    * @param xZero true if vertical axis passes through point 0
    * @param yZero true if horizontal axis passes through point 0
   ...
   /**
    * Returns the chart margin, which is the fraction by which the chart is
    * enlarged on its borders. The default value is @f$0.02@f$.
    */
   ...
   /**
    * Sets the chart margin to `margin`. It is the fraction by which the chart is
    * enlarged on its borders. Restriction: @f$\mathtt{margin} \ge0@f$.
    * 
    ...
    * 
    * @f$x@f$-values. @param s series.
    */
   public abstract void setTicksSynchro(int s);

   /**
    * Draws a vertical line on the chart at @f$x@f$-coordinate `x`. `name` is
    * written near the line at @f$y@f$ position `yfrac` (a fraction of
    * the @f$y@f$-size of the chart, 0 is the bottom, 1 is the top); if `right` is
    * `true`, `name` is written on the right of the line, else on the left.
    * 
    ...
    * not always have an intersection at the corner points; this occurs only if the
    * corner points are multiples of the steps: `xstep` and `ystep` sets the step
    * in each direction.
    * 
    ...
    * constructs and returns a string that can be written to a LaTeX document to
    * render the plot. `width` and `height` represents the width and the height of
    * the produced chart. These dimensions do not take into account the axes and
    * labels extra space. The `width` and the `height` of the chart are measured in
    * centimeters.
    * 
   ...
   /**
    * Transforms the chart to LaTeX form and writes it in file `fileName`. The
    * chart’s width and height (in centimeters) are `width` and `height`.
    */
    ...
    * created LaTeX files. If `flag` is `true`, then when charts are translated
    * into LaTeX form, it will be as a self-contained file that can be directly
    ...
    * By setting `flag` to `false`, these instructions will be removed from the
    * LaTeX chart files, which can then be included in a master LaTeX file. By
