...
/**
 * Stores data used in a `EmpiricalChart`. `EmpiricalSeriesCollection` provides
 * complementary tools to draw empirical distribution charts, like add plots
 * series. This class is linked with `XYSeriesCollection` class from JFreeChart
 * to store data plots, and linked with JFreeChart `EmpiricalRenderer` to render
 * the plot. `EmpiricalRenderer` has been developed at the Université de
 * Montréal to extend the JFreeChart API, and is used to render charts with an
 * empirical chart style in a JFreeChart chart.
 *
...
public class EmpiricalSeriesCollection extends SSJXYSeriesCollection {
   ...
   /**
    * Creates a new `EmpiricalSeriesCollection` instance with default parameters
    * and given data series. Each input parameter represents an observation set.
    * The values of this observation set *must be sorted* in increasing order.
    * 
   ...
   /**
    * Creates a new `EmpiricalSeriesCollection` instance with default parameters
    * and a given series `data`. The values of `data` *must be sorted* in
    * increasing order. However, only *the first* `numPoints` of `data` will be
    ...
    * observation sets. Each <tt>DoubleArrayList</tt> input parameter represents an
    * observation set. The values of this observation set *must be sorted in
    * increasing order*. Each DoubleArrayList variable corresponds to an
    * observation set.
    * 
    * @param data series of point sets.
    */
   ...
   /**
    * Creates a new `EmpiricalSeriesCollection` instance with default parameters
    * and given data series. The input parameter represents a set of plotting data.
    ...
    * of `observationSet` will be added to the new series.
    * 
    * @param observationSet new series values.
   ...
   /**
    * Adds marks on points to a data series. It is possible to use all the marks
    * provided by the TikZ package, some of which are `*`, `+` and `x`. A blank
    * character, used by default, will disable marks. The PGF/TikZ documentation
    * provides more information about placing marks on plots.
    * 
    * @param series    series index.
    ...
    * options provided by the TikZ package: solid, dotted, densely dotted, loosely
    * dotted, dashed, densely dashed and loosely dashed.
    * 
    * @param series      series index.
