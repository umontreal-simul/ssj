...
/**
 * Provides tools to create and manage curve plots. Each `XYLineChart` object is
 * linked with a `XYListSeriesCollection` data set.
 *
...
public class XYLineChart extends XYChart {
   ...
   /**
    * Initializes a new `XYLineChart` instance with sets of points `data`. `title`
    * is a title, `XLabel` is a short description of the
    * 
    * @f$x@f$-axis, and `YLabel` a short description of the @f$y@f$-axis. The input
    *               parameter `data` represents a set of plotting data.
    *
    *               For example, if one @f$n@f$-row matrix `data1` is given as
    *               argument `data`, then the first row <tt>data1</tt>@f$[0]@f$
    *               represents the
    * @f$x@f$-coordinate vector, and every other row <tt>data1</tt>@f$[i], i=1,…,
    *                    n-1@f$, represents a @f$y@f$-coordinate set for a curve.
    *                    Therefore matrix <tt>data1</tt>@f$[i][j]@f$, @f$i=0,…,
    *                    n-1@f$, corresponds to @f$n-1@f$ curves, all with the same
    * @f$x@f$-coordinates.
    *
    ...
    *               n-1@f$, represents a
    * @f$y@f$-coordinate set of points. Therefore matrix <tt>data</tt>@f$[i][
    *                    ]@f$, @f$i=0,…, n-1@f$, corresponds to
    * @f$n-1@f$ curves, all with the same @f$x@f$-coordinates. However, only *the
    *           first* `numPoints` of `data` will be considered to plot each curve.
    * @param title     chart title.
    * @param XLabel    Label on @f$x@f$-axis.
    * @param YLabel    Label on @f$y@f$-axis.
    * @param data      series of point sets.
    ...
    * constructor @ref YListChart with `double[]... data`.
    * 
    * @param title  chart title.
    * @param XLabel Label on @f$x@f$-axis.
    * @param YLabel Label on @f$y@f$-axis.
   ...
   /**
    * Adds the new collection of data series `data` into the series collection. If
    * `data` is a @f$n@f$-row matrix, then the first row <tt>data</tt>@f$[0]@f$
    * represents the @f$x@f$-coordinate vector, and every other row
    * <tt>data</tt>@f$[i], i=1,…, n-1@f$, represents a
    * 
    * @f$y@f$-coordinate set of points. Therefore matrix <tt>data</tt>@f$[i][
    *                    ]@f$, @f$i=0,…, n-1@f$, corresponds to
    ...
    *                    ]@f$, @f$i=0,…, n-1@f$, corresponds to
    * @f$n-1@f$ curves, all with the same @f$x@f$-coordinates. However, only *the
    *           first* `numPoints` of `data` will be taken into account for the new
    *           series.
    * @param data      series of point sets.
    * @param numPoints Number of points to plot
    */
    ...
    * application containing a bar chart panel displaying the chart. The created
