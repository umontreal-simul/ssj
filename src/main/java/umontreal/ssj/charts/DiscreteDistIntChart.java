...
public class DiscreteDistIntChart {
    ...
    * probabilities of the discrete distribution `dist` over the integers.
    * 
   ...
   /**
    * Constructor for a new `DiscreteDistIntChart` instance used to plot the
    * probabilities of the discrete distribution `dist` over the
    * interval @f$[a,b]@f$.
    * 
    ...
    * application containing a chart panel displaying the chart. The created frame
    * is positioned on-screen, and displayed before it is returned. The `width` and
   ...
   /**
    * Similar to method #viewCdf above. If the interval @f$[a,b]@f$ for the graph
    * is not defined, it will be set automatically to @f$[\mu- 3\sigma, \mu+
    * 3\sigma]@f$, where @f$\mu@f$ and @f$\sigma@f$ are the mean and the variance
    * of the distribution.
    * 
    * @param width  frame width in pixels
    * @param height frame height in pixels
    ...
    * 
    * @f$[a,b]@f$ on the screen using Swing. This method creates an application
    *             containing a chart panel displaying the chart. The created frame
    *             is positioned on-screen, and displayed before it is returned. The
   ...
   /**
    * Similar to method #viewProb above. If the interval @f$[a,b]@f$ for the graph
    * is not defined, it will be set automatically to @f$[\mu- 3\sigma, \mu+
    * 3\sigma]@f$, where @f$\mu@f$ and @f$\sigma@f$ are the mean and the variance
    * of the distribution.
    * 
    ...
    * PGF/TikZ. This method constructs and returns a string that can be written to
    * a LaTeX document to render the plot. `width` and `height` represents the
    * width and the height of the produced chart. These dimensions do not take into
    * account the axes and labels extra space. The `width` and the `height` of the
    * chart are measured in centimeters.
    * 
