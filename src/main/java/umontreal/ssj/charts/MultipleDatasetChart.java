...
/**
 * Provides tools to plot many datasets on the same chart. This class is mainly
 * used to draw plots with different styles. Class `XYChart` and its subclasses
 * are to be preferred to draw simple charts with one style. Datasets are stored
 * in an `ArrayList`. The first dataset is called as the *primary dataset*.
 *
...
public class MultipleDatasetChart {
   ...
   /**
    * Initializes a new `MultipleDatasetChart` instance. `title` sets a title,
    * `XLabel` is a short description of the @f$x@f$-axis, and `YLabel` is a short
    * description of the @f$y@f$-axis.
    * 
   ...
   /**
    * Sets a title to the chart. This title will appear on the chart displayed by
    * method #view.
    * 
    * @param title chart title.
    */
   ...
   /**
    * Sets new @f$x@f$-axis and @f$y@f$-axis bounds, with format: `axisRange` =
    * [xmin, xmax, ymin, ymax].
    * 
   ...
   /**
    * Adds a new dataset to the chart at the end of the list and returns its
    * position.
    * 
   ...
   /**
    * Replaces the element at the specified position in the dataset list with the
    * specified element.
    * 
    * @param datasetNum position in the dataset list.
   ...
   /**
    * Puts grid on the background. It is important to note that the grid is always
    * placed in such a way that it contains the axes. Thus, the grid does not
    * always have an intersection at the corner points; this occurs only if the
    * corner points are multiples of the stepping. `xstep` and `ystep` sets the
   ...
   public String toLatex(double width, double height) {
      ...
      for (int i = 0; i < datasetList.size(); i++)
         formatter.format("%s",
               datasetList.get(i).toLatex(XScale, YScale, XAxis.getTwinAxisPosition(), YAxis.getTwinAxisPosition(),
