...
/**
 * Stores data used in a `HistogramChart`. `HistogramSeriesCollection` provides
 * complementary tools to draw histograms. One may add observation sets, define
 * histogram bins, set plot color and plot style, enable/disable filled shapes,
 * and set margin between shapes for each series. This class is linked with
 * class `CustomHistogramDataset` to store data plots, and linked with
 * JFreeChart `XYBarRenderer` to render the plot. `CustomHistogramDataset` has
 * been developed at the Université de Montréal to extend the JFreeChart API,
...
public class HistogramSeriesCollection extends SSJXYSeriesCollection {
    ...
    * can be changed using the method #setBins ). Each input parameter represents a
    * data series.
    * 
   ...
   /**
    * Creates a new `HistogramSeriesCollection` instance with the given data series
    * `data`. Bins the elements of data in equally spaced containers (the number of
    * bins can be changed using the method #setBins ). Only the first `numPoints`
    * of `data` will be taken into account.
    * 
   ...
   /**
    * Creates a new `HistogramSeriesCollection`. Bins the elements of data in
    * equally spaced containers (the number of bins can be changed using the method
    * #setBins ). The input parameter represents a set of data plots. Each
    * DoubleArrayList variable corresponds to a histogram on the chart.
    * 
   ...
   /**
    * Creates a new `HistogramSeriesCollection` instance with default parameters
    * and given data. The input parameter represents a collection of data
    * observation sets. Each `TallyStore` input parameter represents an observation
    * set.
    * 
    ...
    * Creates a new `HistogramSeriesCollection` instance with default parameters
    * and given data. The input parameter represents a collection of data
    * observation sets. Each `TallyHistogram` input parameter represents an
    * observation set. The 2 extra bins at the beginning and at the end of the
   ...
   /**
    * Creates a new `HistogramSeriesCollection` instance. The input parameter
    * represents a set of plotting data. Each series of the given collection
   ...
   /**
    * Adds a data series into the series collection. Only *the first* `numPoints`
    * of `data` will be added to the new series.
    * 
    * @param data      new series.
    ...
    * Adds a data series into the series collection.
    * 
    * @param tally @ref umontreal.ssj.stat.TallyStore to add values.
    * @return Integer that represent the new point set’s position in the
    *         `CustomHistogramDataset` object.
    */
   ...
   /**
    * Returns the `CustomHistogramDataset` object associated with the current
    * variable.
    * 
    * @return `CustomHistogramDataset` object associated with the current variable.
    */
    ...
    *
    * @exception IndexOutOfBoundsException if `series` is outside the specified
    *                                      range.
    */
   ...
   /**
    * Sets `bins` periodic bins from `minimum` to `maximum` for a series. Values
    * falling on the boundary of adjacent bins will be assigned to the higher
    * indexed bin.
    * 
    * @param series  the series index (in the range `0` to
    *                <tt>getSeriesCount() - 1</tt>).
   ...
   /**
    * Links bins given by table `binsTable` to a series. Values falling on the
    * boundary of adjacent bins will be assigned to the higher indexed bin.
    * 
    * @param series    the series index (in the range `0` to
    *                  <tt>getSeriesCount() - 1</tt>).
    ...
    *
    * @exception ( I ndexOutOfBoundsException if <code>series</code> is outside the
    *              specified range.
    */
   ...
   /**
    * Returns the `filled` flag associated with the <tt>series</tt>-th data series.
    * 
   ...
   /**
    * Sets the `filled` flag. This option fills histogram rectangular. The fill
    * color is the current series color, alpha’s color parameter will be used to
    * draw transparency.
    * 
    * @param series series index.
   ...
   /**
    * Sets the margin which is a percentage amount by which the bars are trimmed
    * for all series.
    * 
