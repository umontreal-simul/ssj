...
/**
 * This class extends @ref umontreal.ssj.charts.XYListSeriesCollection. The data
 * is given as lists of @f$y@f$-coordinates. The @f$x@f$-coordinates are
...
public class YListSeriesCollection extends XYListSeriesCollection {
   ...
   /**
    * Creates a new `YListSeriesCollection` instance with default parameters and
    * given data series. The input vectors represent sets of plotting data. More
    * specifically, each vector `data` represents a
    * 
    * @f$y@f$-coordinates set. Position in the vector will form the
    * @f$x@f$-coordinates. Indeed the value <tt>data</tt>@f$[j]@f$ corresponds to
    *                      the point @f$(j+1, \mbox{\texttt{data}}[j])@f$ on the
    *                      chart.
    ...
    * YListSeriesCollection(data)} above, except that if `flag` is `true`, the
    * points are @f$(j+1, \mbox{\texttt{data}}[j])@f$ for each series; but if
    * `flag` is `false`, the points are @f$((j+1)/n, \mbox{\texttt{data}}[j])@f$,
    * where @f$n@f$ is the number of points of each series in `data`.
    * 
   ...
   /**
    * Creates a new `YListSeriesCollection` instance with default parameters and
    * one data series. The vector `data` represents the
    * 
    * @f$y@f$-coordinate of the points, and position in the vector represents
    *                    the @f$x@f$-coordinate. However, only *the first*
    *                    `numPoints` of `data` will be considered in the series.
    *                    Thus the coordinates of the points are given by @f$(j,
   ...
   /**
    * Similar to the constructor {@link #YListSeriesCollection(double[],int)
   ...
   /**
    * Creates a new `YListSeriesCollection` instance with default parameters and
    * given data series. The matrix `data` represents a set of plotting data. More
    * specifically, each row of `data` represents a
    * 
    * @f$y@f$-coordinates set. Position in the vector will form the
    * @f$x@f$-coordinates. Indeed, for each serie @f$i@f$, the value
    *                      <tt>data</tt>@f$[i][j]@f$ corresponds to the
    *                      point @f$(j+1, \mbox{\texttt{data}}[j])@f$ on the chart.
    *                      However, only *the first* `numPoints` of `data` will be
    *                      considered for each series of points.
    * @param data      series of point sets.
    * @param numPoints Number of points to plot
    */
   public YListSeriesCollection(double[][] data, int numPoints) {
