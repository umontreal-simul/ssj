...
/**
 * Provides tools to create and manage scatter plots. Each `ScatterChart` object
 * is linked with a `XYListSeriesCollection` data set.
 *
...
public class ScatterChart extends XYChart {
   ...
   /**
    * Initializes a new `ScatterChart` instance with data `data`. `title` is a
    * title, `XLabel` is a short description of the @f$x@f$-axis and `YLabel` a
    * short description of the @f$y@f$-axis. The input parameter `data` represents
    * sets of plotting data. For example, if one @f$n@f$-row matrix `data1` is
    * given as argument `data`, then the first row <tt>data1</tt>@f$[0]@f$
    * represents the @f$x@f$-coordinate vector, and every other row
    * <tt>data1</tt>@f$[i], i=1,…, n-1@f$, represents the @f$y@f$-coordinates of
    * a set of points. Therefore matrix `data1` corresponds to @f$n-1@f$ sets of
    * points, all with the same @f$x@f$-coordinates. However, one may want to plot
    * sets of points with different @f$x@f$-coordinates. In that case, one should
    * give the points as matrices with two rows. For examples, if the argument
    * `data` is made of three 2-row matrices `data1`, `data2` and `data3`, then
    * they represents three different sets of points, <tt>data*</tt>@f$[0]@f$
    * giving the @f$x@f$-coordinates, and <tt>data*</tt>@f$[1]@f$
   ...
   /**
    * Initializes a new `ScatterChart` instance with sets of points `data`. `title`
    * is a title, `XLabel` is a short description of the
    * 
    * @f$x@f$-axis, and `YLabel` a short description of the @f$y@f$-axis. If `data`
    *               is a @f$n@f$-row matrix, then the first row
    *               <tt>data</tt>@f$[0]@f$ represents the @f$x@f$-coordinate
    *               vector, and every other row <tt>data</tt>@f$[i], i=1,…,
    *               n-1@f$, represents a
    * @f$y@f$-coordinate vector. Therefore matrix <tt>data</tt>@f$[i][
    *                    ]@f$, @f$i=0,…, n-1@f$, corresponds to @f$n-1@f$ sets of
    *                    points, all with the same @f$x@f$-coordinates. However,
    *                    only *the first* `numPoints` of each set
    *                    <tt>data</tt>@f$[i]@f$ (i.e. the first `numPoints` columns
    *                    of each row) will be plotted.
    * @param title     chart title.
    * @param XLabel    Label on @f$x@f$-axis.
    * @param YLabel    Label on @f$y@f$-axis.
    * @param data      series of point sets.
    ...
    * `data[x][.]` will form the @f$x@f$-coordinates and `data[y][.]` will form
    * the @f$y@f$-coordinates of the chart. `title` sets a title, `XLabel` is a
    * short description of the @f$x@f$-axis, and `YLabel` is a short description of
    * the @f$y@f$-axis. Warning: if the new
    * 
    * @f$x@f$-axis coordinates are not monotone increasing, then they will
    *              automatically be sorted in increasing order so the points will
   ...
   /**
    * Initializes a new `ScatterChart` instance with data `data`. The input
    * parameter `data` represents a set of plotting data. A DoubleArrayList from
    * the Colt library is used to store the data. The description is similar to the
    * above constructor with `double[]... data`.
    * 
    * @param title  chart title.
    * @param XLabel Label on @f$x@f$-axis.
    * @param YLabel Label on @f$y@f$-axis.
    ...
    * 
    * @ref org.jfree.data.xy.XYSeriesCollection is a `JFreeChart` container class
    *      to store @f$XY@f$ plots.
    * @param title  chart title.
    * @param XLabel Label on @f$x@f$-axis.
    * @param YLabel Label on @f$y@f$-axis.
    ...
    * 
    * @f$y@f$-coordinates of the series. `name` and `plotStyle` are the name and
    *                     the plot style associated to the series.
    ...
    * 
    * @f$x@f$-values. @param s series used to define ticks.
    */
   ...
   /**
    * Displays chart on the screen using Swing. This method creates an application
    * containing a chart panel displaying the chart. The created frame is
    * positioned on-screen, and displayed before it is returned. The `width` and
    * the `height` of the chart are measured in pixels.
    * 
   ...
   public JFrame view(int width, int height) {
       ...
       * XYLineAndShapeRenderer shape = new XYLineAndShapeRenderer(false, true); int
       * nb = getSeriesCollection().getSeriesCollection().getSeriesCount(); for (int i
       * = 0 ; i < nb ; i++) { shape.setSeriesShapesFilled(i, false);
   ...
   public String toLatex(double width, double height) {
      ...
      xunit = width / ((Math.max(XAxis.getAxis().getRange().getUpperBound(), XAxis.getTwinAxisPosition()) * XScale)
      ...
      yunit = height / ((Math.max(YAxis.getAxis().getRange().getUpperBound(), YAxis.getTwinAxisPosition()) * YScale)
      ...
      formatter.format("%s",
            dataset.toLatex(XScale, YScale, XAxis.getTwinAxisPosition(), YAxis.getTwinAxisPosition(),
                  XAxis.getAxis().getLowerBound(), XAxis.getAxis().getUpperBound(), YAxis.getAxis().getLowerBound(),
                  YAxis.getAxis().getUpperBound()));
