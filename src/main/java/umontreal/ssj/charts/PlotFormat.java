...
public class PlotFormat {
   ...
   /**
    * Parses `data` according to the standard GNUPlot format and stores the
    * extracted values in the returned table. All unrecognized characters and empty
    * lines will be cut. Deleting comments inside the input `String` is
    * recommended.
    * 
    * @param data String to parse.
    * @return Table that represent data contained in `data`.
    *
    * @exception IllegalArgumentException if each line of the input `String`
      ...
      for (int j = 0; j < string.length; j++) {
         if (string[j].charAt(0) != '#' && string[j].length() != 0) {
            ...
            for (int i = 0; i < result.length; i++) {
               try {
                  line[k] = Double.valueOf(result[i]); // les chaines de caracteres qui ne sont pas des nombres sont
                                                       // ignorees
                  k++;
   ...
   /**
    * Parses `data` according to the standard CSV format and stores the extracted
   ...
   /**
    * Parses `data` according to a user defined format and stores the extracted
    * values in the returned table. `betweenValues` sets characters between values
    * on the same line and `endLine` sets characters which separates each line of
    * the input data set. Usually, this parameter contains the classic end of line
    * character <tt>%n</tt>. This method uses regular expressions, so it is
    * possible to use regular characters as defined in the standard java API,
    * specially in the class Pattern (package java.util.regex).
    * 
    * @param betweenValues `String` which separates values on the same line.
   ...
   /**
    * Stores data tables `data` into a `String`, with format understandable by
    * GNUPlot.
    * 
   ...
   /**
    * Stores series collection `data` into a `String`, with format understandable
    * by GNUPlot.
    * 
    * @param data data tables.
    * @return String that represent data tables in GNUPlot format.
   ...
   /**
    * Stores data tables `data` into a `String` with format CSV (comma-separated
    * value tabular data). The output string could be imported from a file to a
    * matrix into Mathematica with Mathematica’s function `Import["fileName",
    * "CSV"]`, or into MATLAB with MATLAB’s function `csvread(’fileName’)`.
    * 
    * @param data data tables.
   ...
   /**
    * Stores data tables `data` into a `String` with customized format. `heading`
    * sets the head of the returned `String`, `footer` sets its end,
    * `betweenValues` sets characters between values on the same line and finally
    * `endLine` sets characters which separates each line of the input data set.
    * Normally, this parameter contains the classic end of line character ‘%n’.
    * 
    ...
    */
   public static String toCustomizedFormat(String heading, String footer, String betweenValues, String endLine,
   ...
   /**
    * Stores data tables `data` into a `String` with customized format from an
    * `XYSeriesCollection` variable.
    * 
    * @param heading       head of the returned `String`.
    * @param footer        end of the returned `String`.
    * @param betweenValues `String` which separates values on the same line.
   ...
   public static String toCustomizedFormat(String heading, String footer, String betweenValues, String endLine,
      ...
      return toCustomizedFormat(heading, footer, betweenValues, endLine, precision, toTable(data));
   }
    ...
    *
    * @exception IllegalArgumentException If the input tables doesn't have the same
    *                                     length.
    */
   ...
   /*
    * //Inutile si on exporte en CSV private static String mathematicaFormatPoint
    * (double x, double y) { // Writes the pair (x, y) in returned string, in a
    * format understood // by Mathematica StringBuffer sb = new StringBuffer();
    * String S;
    * 
    * sb.append ("   { "); if ((x != 0.0) && (x < 0.1 || x > 1.0)) { S =
    * PrintfFormat.E (16, 7, x); int exppos = S.indexOf ('E'); if (exppos != -1) S
    * = S.substring (0, exppos) + "*10^(" + S.substring (exppos+1) + ")"; } else S
    * = PrintfFormat.g (16, 8, x);
    * 
    * sb.append (S + ",     ");
    * 
    * if (y != 0.0 && (y < 0.1 || y > 1.0)) { S = PrintfFormat.E (16, 7, y); int
    * exppos = S.indexOf ('E'); if (exppos != -1) S = S.substring (0, exppos) +
    * "*10^(" + S.substring (exppos+1) + ")"; } else S = PrintfFormat.g (16, 8, y);
    * 
