...
/**
 * Provides *PP-plot* (probability-probability plot) objects that compare two
 * probability distributions. The data is given as a list
...
public class PPPlot extends XYLineChart {
   ...
   /**
    * Initializes a new `PPPlot` instance using the points `X`. `title` is a title,
    * `XLabel` is a short description of the @f$x@f$-axis, and `YLabel` a short
    * description of the @f$y@f$-axis. The plot is a PP-plot of the points @f$(i/n,
    * F(x_{(i)})@f$, @f$i= 1, 2, …, n@f$, where @f$x_i
    * = @f$<tt> X[</tt>@f$i@f$<tt>-1]</tt>, @f$x_{(i)}@f$ are the sorted points,
    * and @f$F(x) = @f$<tt>
    ...
    * PPPlot(title, XLabel, YLabel, dist, X)} above, except that only *the first*
   ...
   /**
    * Initializes a new `PPPlot` instance. `title` is a title, `XLabel` is a short
    ...
    * PP-plot, that is, one will plot only the points `data[r][i]`, for @f$i=0, 1,
    * …, (n-1)@f$ and a given
    * 
    * @f$r@f$, where @f$n@f$ is the number of points in set @f$r@f$. The points are
    *          assumed to follow the distribution `dist`.
    * @param title  chart title.
    * @param XLabel Label on @f$x@f$-axis.
    * @param YLabel Label on @f$y@f$-axis.
    * @param dist   Reference distribution
