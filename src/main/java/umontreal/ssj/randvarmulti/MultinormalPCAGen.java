 ...
 * @f$\boldsymbol{\Sigma}= \mathbf{V}\boldsymbol{\Lambda}\mathbf{V}^{\mathsf{t}}@f$
 *                         where
 ...
 *                            matrix of @f$\mathbf{V}@f$. The eigenvalues are
 *                            ordered from the largest (@f$\lambda_1@f$) to the
 *                            smallest (@f$\lambda_d@f$). The random multinormal
 *                            vector @f$\mathbf{X}@f$ is generated via @f[
 *                            \mathbf{X}= \boldsymbol{\mu}+
 *                            \mathbf{A}\mathbf{Z}, @f] where @f$\mathbf{A}=
 *                            \mathbf{V}\sqrt{\boldsymbol{\Lambda}}@f$, and
...
public class MultinormalPCAGen extends MultinormalGen {
   ...
   /**
    * Equivalent to {@link #MultinormalPCAGen(NormalGen,double[],DoubleMatrix2D)
    ...
    * of the covariance matrix, which must be symmetric and positive semi-definite.
    * If any of the above conditions is violated, an exception is thrown. The
    * vector @f$\mathbf{Z}@f$ is generated by calling @f$d@f$ times the generator
    * `gen1`, which must be a *standard normal* 1-dimensional generator.
    * 
   ...
   /**
    * Returns the matrix @f$\mathbf{A}= \mathbf{V}\sqrt{\boldsymbol{\Lambda}}@f$ of
    * this object.
    * 
   ...
   /**
    * Returns the eigenvalues of @f$\boldsymbol{\Sigma}@f$ in decreasing order.
    */
   ...
   /**
    * Sets the covariance matrix @f$\boldsymbol{\Sigma}@f$ of this multinormal
    * generator to `sigma` (and recomputes @f$\mathbf{A}@f$).
    * 
    * @param sigma the new covariance matrix.
    * @exception IllegalArgumentException if `sigma` has incorrect dimensions.
    */
   ...
   /**
    * Generates a @f$d@f$-dimensional vector from the multinormal distribution with
    * mean vector `mu` and covariance matrix `sigma`, using the one-dimensional
    * normal generator `gen1` to generate the coordinates of @f$\mathbf{Z}@f$, and
    * using the PCA decomposition of
    * 
    * @f$\boldsymbol{\Sigma}@f$. The resulting vector is put into `p`. Note that
    *                            this static method will be very slow for large
    *                            dimensions, because it recomputes the singular
    *                            value decomposition at every call. It is therefore
    *                            recommended to use a `MultinormalPCAGen` object
    *                            instead, if the method is to be called more than
    *                            once.
    ...
    *                                     covariance matrix, or if the covariance
   ...
   /**
    * Equivalent to {@link #nextPoint(NormalGen,double[],DoubleMatrix2D,double[])
   ...
   /**
    * Generates a point from this multinormal distribution. This is much faster
    * than the static method as it computes the singular value decomposition matrix
