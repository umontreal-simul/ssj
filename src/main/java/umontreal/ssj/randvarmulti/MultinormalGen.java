 ...
 *                                       denoted
 * @f$N(\boldsymbol{\mu}, \boldsymbol{\Sigma})@f$, has density @f[
 *                        f(\mathbf{X})=\frac{1}{\sqrt{(2\pi)^d\det(\boldsymbol{\Sigma})}}
 *                        \exp\left(-(\mathbf{X}-
 *                        \boldsymbol{\mu})^{\!\mathsf{t}}\boldsymbol{\Sigma}^{-1}(\mathbf{X}-
 *                        \boldsymbol{\mu})/2\right), @f] for
 *                        all @f$\mathbf{X}\in\mathbb{R}^d@f$, and
 * @f$\mathbf{X}^{\mathsf{t}}@f$ is the transpose vector of @f$\mathbf{X}@f$.
 *                               If @f$\mathbf{Z}\sim N(\boldsymbol{0},
 *                               \mathbf{I})@f$ where
 * @f$\mathbf{I}@f$ is the identity matrix, @f$\mathbf{Z}@f$ is said to have the
 *                  *standard multinormal* distribution.
 *
 *                  For the special case @f$d=2@f$, if the random
 *                  vector @f$\mathbf{X}= (X_1, X_2)^{\mathsf{t}}@f$ has a
 *                  bivariate normal distribution, then it has mean
 * @f$\boldsymbol{\mu}= (\mu_1, \mu_2)^{\mathsf{t}}@f$, and covariance
 *                      matrix @f[ \boldsymbol{\Sigma}= \left[\begin{array}{cc}
 *                      \sigma_1^2 & \rho\sigma_1\sigma_2 \\
 *                      \rho\sigma_1\sigma_2 & \sigma_2^2 \end{array}\right] @f]
 *                      if and only if @f$\mathrm{Var}[X_1] =
 *                      \sigma_1^2@f$, @f$\mathrm{Var}[X_2] = \sigma_2^2@f$, and
 *                      the linear correlation between @f$X_1@f$ and
...
public class MultinormalGen extends RandomMultivariateGen {
   ...
   private void initMN(NormalGen gen1, double[] mu, int d) {
      ...
      /*
       * NormalDist dist = (NormalDist) gen1.getDistribution(); if (dist != null) { if
       * (dist.getMu() != 0.0) throw new IllegalArgumentException ("mu != 0"); if
       * (dist.getSigma() != 1.0) throw new IllegalArgumentException ("sigma != 1"); }
       * dist = null;
       */
      this.gen1 = gen1;
    ...
    *                         vector @f$\mathbf{Z}@f$ will be generated via @f$d@f$
    *                         successive calls to `gen1`, which must be a *standard
    *                         normal* generator.
    * @param gen1 the one-dimensional generator
    * @param d    the dimension of the generated vectors
    * @exception IllegalArgumentException if the one-dimensional normal generator
    *                                     uses a normal distribution with @f$\mu@f$
    *                                     not equal to 0, or @f$\sigma@f$ not equal
    *                                     to 1.
   ...
   /**
    * Constructs a multinormal generator with mean vector `mu` and covariance
    * matrix `sigma`. The mean vector must have the same length as the dimensions
    * of the covariance matrix, which must be symmetric and positive-definite. If
    * any of the above conditions is violated, an exception is thrown. The
    * vector @f$\mathbf{Z}@f$ is generated by calling @f$d@f$ times the generator
    * `gen1`, which must be *standard normal*.
    * 
    * @param gen1  the one-dimensional generator
    * @param mu    the mean vector.
    * @param sigma the covariance matrix.
    * @exception NullPointerException     if any argument is `null`.
    * @exception IllegalArgumentException if the length of the mean vector is
    *                                     incompatible with the dimensions of the
    *                                     covariance matrix.
    */
   ...
   /**
    * Equivalent to {@link #MultinormalGen(NormalGen,double[],DoubleMatrix2D)
   ...
   /**
    * Returns the @f$i@f$-th component of the mean vector for this generator.
    * 
    ...
    * @exception IllegalArgumentException if the length of `mu` does not correspond
    *                                     to #getDimension.
    */
