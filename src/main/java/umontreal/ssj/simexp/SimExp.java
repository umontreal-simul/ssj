 ...
 * simulation logic. It also provides utility methods to estimate the required
 * number of additional observations that would be necessary for an estimator to
 * reach a given precision, for sequential sampling.
 *
...
public abstract class SimExp {
   ...
   /**
    * Constructs a new object performing experiments using the given simulator
    * `sim`.
    * 
   ...
   /**
    * Sets the simulator associated with this experiment to `sim`. This method
    * should not be called while this object is simulating.
    * 
   ...
   /**
    * Performs an experiment whose logic depends on the used subclass. Before
    * starting the simulation, this method should set #simulating to `true`, and
    * reset it to `false` after the simulation is done. It is recommended to reset
    * #simulating to `false` inside a `finally` clause to prevent the indicator
    * from remaining `true` in the case of error during simulation.
    * 
    * @exception IllegalStateException if #simulating is `true` when calling this
    *                                  method.
    */
   public abstract void simulate();

   /**
    * Returns the approximate number of additional observations required to reach a
    * relative error smaller than or equal to `targetError` for each tally in the
    * array `a` when confidence intervals are computed with confidence level
    * `level`. For each statistical collector in the given array, a confidence
    * interval is computed independently of the other collectors, and an error
    * check is performed by #getRequiredNewObservations(StatProbe,double,double) to
    * determine the required number of additional observations. The method returns
    * the maximal number of required observations.
    * 
    * @param a           the array of probes.
    * @param targetError the target relative error.
    * @param level       the desired probability that, for a given statistical
    *                    collector, the (random) confidence interval covers the
    *                    true mean (a constant).
    * @return an estimate of the required number of additional observations to
    *         reach the precision.
    */
   ...
   /**
    * Returns the approximate number of additional observations required to reach a
    * relative error smaller than or equal to `targetError` for each tally
    * enumerated by `it` when confidence intervals are computed with confidence
    * level `level`. For each statistical collector returned by the iterator
    * obtained from `it`, a confidence interval is computed independently of the
    * other collectors, and an error check is performed by
    * #getRequiredNewObservations(StatProbe,double,double) to determine the
    * required number of additional observations. The method returns the maximal
   ...
   /**
    * Calls #getRequiredNewObservations(double,double,int,double) with the average,
    * confidence interval radius, and number of observations given by the
    * statistical probe `probe`. This method always returns 0 if the probe is not a
    * tally. For a @ref umontreal.ssj.stat.Tally, the confidence interval is
    * computed using
    * umontreal.ssj.stat.Tally.confidenceIntervalStudent(double,double[]). For
    * a @ref umontreal.ssj.stat.FunctionOfMultipleMeansTally, it is computed using
    ...
    * @param targetError the target relative error.
    * @param level       the desired probability that the (random) confidence
   ...
   /**
    * Calls #getRequiredNewObservations(double,double,int,double) with the average,
    * confidence interval radius, and number of observations given by the tally
    * `ta`. The confidence interval is computed using
   ...
   /**
    * Calls #getRequiredNewObservations(double,double,int,double) with the average,
    * confidence interval radius, and number of observations given by the function
    * of multiple means `fmmt`. The confidence interval is computed using
    * umontreal.ssj.stat.FunctionOfMultipleMeansTally.confidenceIntervalDelta(double,double[]).
    * 
    ...
    * @return the number of required additional observations.
    */
   public static int getRequiredNewObservationsTally(FunctionOfMultipleMeansTally fmmt, double targetError,
         double level) {
      double[] cr = new double[2];
   ...
   /**
    * Returns the approximate number of additional observations needed for the
    * point estimator @f$\bar{X}_n=@f$&nbsp;`center`, computed using
    * 
    * @f$n=@f$&nbsp;`numberObs` observations and with a confidence interval having
    *                           radius @f$\delta_n/\sqrt{n}=@f$&nbsp;`radius`, to
    *                           have a relative error less than or equal to
    * @f$\epsilon=@f$&nbsp;`targetError`. It is assumed that
    * @f$\bar{X}_n@f$ is an estimator of a mean @f$\mu@f$, @f$n@f$ is the number of
    *                 observations `numberObs`, and that
    ...
    *                             The method returns
    * @f$\mathrm{round}((\delta_n\sqrt{n}/(\epsilon|\bar{X}_n|))^2)-n@f$ where @f$\mathrm{round}(\cdot)@f$
    *                                                                    rounds its
    *                                                                    argument
    *                                                                    to the
    *                                                                    nearest
    ...
    *         reach the precision.
    *
    * @exception IllegalArgumentException if `radius` or `targetError` are
    *                                     negative.
    */
