...
/**
 * Represents a *geometric Brownian motion* (GBM) process @f$\{S(t), t\ge0\}@f$,
 * which evolves according to the stochastic differential equation
 * 
 * @anchor REF_stochprocess_GeometricBrownianMotion_eq_GBM @f[ dS(t) = \mu S(t)
 *         dt + \sigma S(t) dB(t), \tag{GBM} @f] where @f$\mu@f$
 *         and @f$\sigma@f$ are the drift and volatility parameters,
 *         and @f$\{B(t),  t\ge0\}@f$ is a standard Brownian motion (for
 *         which @f$B(t)\sim N(0,t)@f$). This process can also be written as the
 *         exponential of a Brownian motion:
 * @anchor REF_stochprocess_GeometricBrownianMotion_eq_GBM2 @f[ S(t) = S(0)
 *         \exp\left[ (\mu- \sigma^2/2) t + \sigma B(t) \right] = S(0)
 *         \exp\left[ X(t) \right], \tag{GBM2} @f] where @f$X(t) = (\mu-
 *         \sigma^2/2) t + \sigma B(t)@f$. The GBM process is simulated by
 *         simulating the BM process @f$X@f$ and taking the exponential. This BM
...
public class GeometricBrownianMotion extends StochasticProcess {
    ...
    * \mathtt{mu}@f$, @f$\sigma= \mathtt{sigma}@f$, and @f$S(t_0) = \mathtt{s0}@f$,
    * using `bm` as the underlying @ref BrownianMotion. The parameters of `bm` are
    * automatically reset to
    * 
    * @f$\mu-\sigma^2/2@f$ and @f$\sigma@f$, regardless of the original parameters
    *                      of `bm`. The observation times are the same as those of
    *                      `bm`. The generation method depends on that of `bm`
   ...
   /**
    * Same as in `StochasticProcess`, but also invokes `resetStartProcess` for the
    * underlying `BrownianMotion` object.
    */
   ...
   /**
    * Sets the parameters @f$S(t_0) = \mathtt{s0}@f$, @f$\mu= \mathtt{mu}@f$
    * and @f$\sigma= \mathtt{sigma}@f$ of the process. *Warning*: This method will
   ...
   /**
    * Resets the @ref umontreal.ssj.rng.RandomStream for the underlying Brownian
    * motion to `stream`.
    */
   public void setStream(RandomStream stream) {
      (bm.gen).setStream(stream);
   }

   /**
    * Returns the @ref umontreal.ssj.rng.RandomStream for the underlying Brownian
    * motion.
    */
   public RandomStream getStream() {
      return (bm.gen).getStream();
   }
   ...
   /**
    * Returns a reference to the @ref BrownianMotion object used to generate the
    * process.
    */
