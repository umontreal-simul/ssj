...
/**
 * Same as @ref GammaProcessPCABridge, but uses the fast inversion method for
 * the symmetrical beta distribution, proposed by L’Ecuyer and Simard
 * 
 * @cite rLEC06a&thinsp;, to accelerate the generation of the beta random
 *       variables. This class works only in the case where the number of
 *       intervals is a power of 2 and all these intervals are of equal size.
 *
...
public class GammaProcessPCASymmetricalBridge extends GammaProcessPCABridge {
    ...
    *         \mathtt{s0}@f$. The @ref umontreal.ssj.rng.RandomStream `stream` is
    *         used in the @ref umontreal.ssj.randvar.GammaGen and in the @ref
    *         umontreal.ssj.randvar.BetaSymmetricalGen. These two generators use
    *         inversion to generate random numbers. The first uniform random number
    *         generated by `stream` is used for the gamma, and the other @f$d-1@f$
    *         for the beta’s.
    */
