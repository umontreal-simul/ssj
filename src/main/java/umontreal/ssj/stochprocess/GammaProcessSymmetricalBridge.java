 ...
 * then possible to generate the bridge process using a special implementation
 * of the beta random variate generator (using the *symmetrical* beta
 * distribution) that is much faster (HOW MUCH? QUANTIFY!) than the general
 * case. Note that when the method `setObservationTimes` is called, the equality
 * of the size of the time steps is verified. To allow for differences due to
 * floating point errors, time steps are considered to be equal if their
 * relative difference is less than @f$10^{-15}@f$.
...
public class GammaProcessSymmetricalBridge extends GammaProcessBridge {
    ...
    *         \mathtt{s0}@f$. The random variables are created using the @ref
    *         umontreal.ssj.rng.RandomStream `stream`. Note that the same @ref
    *         umontreal.ssj.rng.RandomStream `stream` is used for the
    ...
    * 
    * @f$\mu= \mathtt{mu}@f$, @f$\nu= \mathtt{nu}@f$ and initial value @f$S(t_0) =
    *         \mathtt{s0}@f$. Note that the
    * @ref umontreal.ssj.rng.RandomStream included in the
    * @ref umontreal.ssj.randvar.BetaSymmetricalGen is sets to the one included in
    *      the @ref umontreal.ssj.randvar.GammaGen to avoid confusion. This @ref
    *      umontreal.ssj.rng.RandomStream is then used to generate all the random
    *      variables.
    */
