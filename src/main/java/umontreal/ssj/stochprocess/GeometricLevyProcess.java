 ...
 * process @f$X(t)@f$: @f[ S(t) = S(0) \exp\left(X(t) + (r - \omega_{RN})
 * t\right). @f] The interest rate is denoted @f$r@f$ and is referred to as
 * `muGeom` in the class below. The risk neutral correction is given
 * by @f$\omega_{RN}@f$ and takes into account risk aversion in the pricing of
 * assets; its value depends on the specific Lévy process that is used. *From
 * Pierre:* Maybe we should remove these parameters and make it more general by
 * just putting a linear trend in @f$X@f$.
 *
 * @ref GeometricNormalInverseGaussianProcess is implemented as a child of this
 *      class and so could @ref GeometricVarianceGammaProcess and
...
public abstract class GeometricLevyProcess extends StochasticProcess {
   ...
   /**
    * Returns the next observation. It will also work on a Lévy process which is
    * sampled using the bridge order, but it will return the observations in the
    * bridge order. If the underlying Lévy process is of the PCA type, this method
    * is not usable.
    */
   ...
   /**
    * Resets the step counter of the geometric process and the underlying Lévy
    * process to the start value.
    */
   ...
   /**
    * Sets the observation times on the geometric process and the underlying Lévy
    * process.
    */
   ...
   /**
    * Changes the value of @f$\omega_{RN}@f$. There should usually be no need to
    * redefine the risk neutral correction from the value set by the constructor.
    * However it is sometimes not unique, e.g. in
    * 
   ...
   /**
    * Returns the stream from the underlying Lévy process. If the underlying Lévy
    * process has multiple streams, it returns what the `getStream()` method of
    * that process was made to return.
    */
   ...
   /**
    * Resets the stream in the underlying Lévy process. If the underlying Lévy
    * process has multiple streams, it sets the streams on this process in the same
    * way as `setStream()` for that process.
    */
