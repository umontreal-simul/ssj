 ...
 * 
 * @ref OrnsteinUhlenbeckProcess, but the process is generated using the simple
 *      Euler scheme
 * @anchor REF_stochprocess_OrnsteinUhlenbeckProcessEuler_eq_ornstein_seqEuler @f[
 *         X(t_j) - X(t_{j-1}) = \alpha(b - X(t_{j-1}))(t_j - t_{j-1}) +
 *         \sigma\sqrt{t_j - t_{j-1}}  Z_j \tag{ornstein-seqEuler} @f]
 *         where @f$Z_j \sim N(0,1)@f$. This is a good approximation only for
 *         small time intervals @f$t_j - t_{j-1}@f$.
 *
...
public class OrnsteinUhlenbeckProcessEuler extends OrnsteinUhlenbeckProcess {
    ...
    *               variates @f$Z_j@f$ will be generated by inversion using the
    *               stream `stream`.
    */
   ...
   public double nextObservation() {
      ...
      double x = xOld + (beta - xOld) * alphadt[observationIndex] + sigmasqrdt[observationIndex] * gen.nextDouble();
      observationIndex++;
   ...
   /**
    * Generates and returns an observation of the process in `dt` time units,
    * assuming that the process has value @f$x@f$ at the current time. Uses the
    ...
    * `x0`, the initial value of the process, and `t[]` should have at
    * least @f$d+1@f$ elements (see the `setObservationTimes` method).
    */
