...
/**
 * Uses a faster generating method (MSH) @cite rMIC76a&thinsp; than the simple
 * inversion of the distribution function used by
 * 
 * @ref InverseGaussianProcess. It is about 60 times faster. However it requires
 *      two @ref umontreal.ssj.rng.RandomStream ’s instead of only one
 *      for @ref InverseGaussianProcess. The second stream is called
 *      `otherStream` below and it is used to randomly choose between two roots
 *      at each time step.
 *
...
public class InverseGaussianProcessMSH extends InverseGaussianProcess {
   ...
   /**
    * Constructs a new `InverseGaussianProcessMSH`. The initial value `s0` will be
    * overridden by @f$t[0]@f$ when the observation times are set.
    */
   public InverseGaussianProcessMSH(double s0, double delta, double gamma, RandomStream stream,
   ...
   /**
    * Generates the path. It is done by successively calling `nextObservation()`,
    * therefore the two
    * 
   ...
   public double[] generatePath() {
      double s = x0;
      for (int i = 0; i < d; i++) {
         s += InverseGaussianMSHGen.nextDouble(otherStream, normalGen, imu[i], ilam[i]);
         path[i + 1] = s;
      }
      observationIndex = d;
   ...
   /**
    * Instead of using the internal streams to generate the path, uses two arrays
    * of uniforms @f$U[0,1)@f$. The length of the arrays should be equal to the
    * number of periods in the observation times. This method is useful for @ref
   ...
   public double[] generatePath(double[] unifNorm, double[] unifOther) {
      ...
      for (int i = 0; i < d; i++) {
         s += InverseGaussianMSHGen.nextDouble(nonRandOther, normalGen, imu[i], ilam[i]);
         path[i + 1] = s;
      }
      observationIndex = d;
      observationCounter = d;
   ...
   /**
    * Not implemented, requires two @ref umontreal.ssj.rng.RandomStream ’s.
    */
   ...
   public double nextObservation() {
      ...
      s += InverseGaussianMSHGen.nextDouble(otherStream, normalGen, imu[observationIndex], ilam[observationIndex]);
      observationIndex++;
   ...
   /**
    * Only returns a stream if both inner @ref umontreal.ssj.rng.RandomStream ’s
    * are the same.
    */
   ...
   /**
    * Sets the `otherStream`, which is the stream used to choose between the two
    * roots in the MSH method.
    */
   ...
   /**
    * Returns the `otherStream`, which is the stream used to choose between the two
    * quadratic roots from the MSH method.
    */
   ...
   /**
    * Sets the normal generator. It also sets one of the two inner streams to the
    * stream of the normal generator.
    */
   ...
   protected class NonRandomStream implements RandomStream {
      ...
      public String toString() {
         return new String("NonRandomStream of length " + array.length);
      }
   }
