...
/**
 * Represents a Brownian motion process @f$\{X(t) : t \geq0 \}@f$ sampled using
 * the *bridge sampling* technique (see for example
 * 
 ...
 *               on. If the process has already been sampled at times @f$t_i <
 *               t_k@f$ but not in between, the next sampling point in that
 *               interval will be @f$t_j@f$ where @f$j = \lfloor(i + k)/2
 *               \rfloor@f$. For example, if the sampling times used
 *               are @f$\{t_0, t_1, t_2, t_3, t_4, t_5\}@f$, then the
 *               observations are generated in the following
 *               order: @f$X(t_5)@f$,
 * @f$X(t_2)@f$, @f$X(t_1)@f$, @f$X(t_3)@f$, @f$X(t_4)@f$.
 *
 *               *Warning*: Both the `generatePath` and the `nextObservation`
 *               methods from
 * @ref umontreal.ssj.stochprocess.BrownianMotion are modified to use the bridge
 *      method.
 ...
 *         observations returned are *not* ordered chronologically. However they
 *         will be once an entire path is generated and the observations are
 *         read from the internal array (referenced by the `getPath` method)
 *         that contains them.
 *
 *         The method `nextObservation(double nextTime)` differs from that of
 *         the class @ref umontreal.ssj.stochprocess.BrownianMotion in that
 *         `nextTime` represents the next observation time *of the Brownian
 *         bridge*. However, the @f$t_i@f$ supplied must still be non-decreasing
 *         with @f$i@f$.
 *
 *         Note also that, if the path is not entirely generated before being
 *         read from this array, there will be "pollution" from the previous
 *         path generated, and the observations will not represent a sample path
 *         of this process.
 *
...
public class BrownianMotionBridge extends BrownianMotion {
    ...
    * \mathtt{mu}@f$, @f$\sigma= \mathtt{sigma}@f$ and initial value @f$X(t_0) =
    * \mathtt{x0}@f$. The normal variates will be generated by inversion using
    * the @ref umontreal.ssj.rng.RandomStream `stream`.
    */
    ...
    * Constructs a new `BrownianMotionBridge` with parameters @f$\mu=
    * \mathtt{mu}@f$, @f$\sigma= \mathtt{sigma}@f$ and initial value @f$X(t_0) =
    * \mathtt{x0}@f$. The normal variates will be generated by the @ref
    * umontreal.ssj.randvar.NormalGen `gen`.
    */
