...
/**
 * The inverse Gaussian process is a non-decreasing process where the increments
 * are additive and are given by the inverse gaussian distribution, @ref
 * umontreal.ssj.probdist.InverseGaussianDist. With parameters @f$\delta@f$
 * and @f$\gamma@f$, the time increments are given by @ref
 * umontreal.ssj.probdist.InverseGaussianDist @f$(\delta dt/\gamma, \delta^2
 * dt^2)@f$.
 *
 ...
 * @f$IGDist^{\star}(\delta, \gamma)@f$, then the increment distribution of our
 *                           process would have been written more simply as
 ...
 * @ref umontreal.ssj.rng.RandomStream. Subclasses of this class use different
 *      generating methods and some need two
...
public class InverseGaussianProcess extends StochasticProcess {
   ...
   /**
    * Constructs a new `InverseGaussianProcess`. The initial value @f$s0@f$ will be
    ...
    * uniforms @f$U[0,1)@f$. The array should be of the length of the number of
    * periods in the observation times. This method is useful for
    * 
