 ...
 * @ref GammaProcessPCA with the higher eigenvalue gets the next uniform random
 *      number. If one should decide to create a
 ...
 *      numbers would not be given this way to the @ref GammaProcessPCA ’s;
 *      this might give less variance reduction when used with QMC.
 *
...
public class VarianceGammaProcessDiffPCA extends VarianceGammaProcessDiff {
    ...
    * @f$\nu= \mathtt{nu}@f$ and initial value @f$S(t_0) = \mathtt{s0}@f$. There is
    *         only one
    * @ref umontreal.ssj.rng.RandomStream here which is used for the two inner @ref
    *      GammaProcessPCA ’s. The other parameters are set as in
   ...
   public VarianceGammaProcessDiffPCA(double s0, double theta, double sigma, double nu, RandomStream stream) {
      super(s0, theta, sigma, nu, new GammaProcessPCA(0.0, 1.0, 1.0, stream),
    ...
    * 
    * @f$\theta= \mathtt{theta}@f$, @f$\sigma= \mathtt{sigma}@f$,
    * @f$\nu= \mathtt{nu}@f$ and initial value @f$S(t_0) = \mathtt{s0}@f$. As
    *         in @ref VarianceGammaProcessDiff, the
    * @ref umontreal.ssj.rng.RandomStream of `gneg` is replaced by the one of
    *      `gpos` to avoid any confusion.
    */
   public VarianceGammaProcessDiffPCA(double s0, double theta, double sigma, double nu, GammaProcessPCA gpos,
   ...
   /**
    * This method is not implemented is this class since the path cannot be
   ...
   protected void init() {
      ...
      if (observationTimesSet) {
         // Two lines below (casts) should be reinstated after fix inheritance
         // PCA/PCABridge.
