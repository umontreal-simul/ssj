 ...
 * 
 * @anchor REF_stochprocess_MultivariateGeometricBrownianMotion_eq_GBM2 @f[
 *         S_i(t) = S_i(0) \exp\left[ X_i(t) \right], \tag{GBM2} @f]
 *         where @f$\mathbf{X}(t) = (X_1(t),…,X_c(t))@f$ is a multivariate
 *         Brownian motion. The GBM process is simulated by simulating the BM
 *         process
...
public class MultivariateGeometricBrownianMotion extends MultivariateStochasticProcess {
    ...
    * 
    * @ref MultivariateBrownianMotion. The parameters of `mbm` are automatically
    *      reset to @f$\mu-\sigma^2/2@f$ and @f$\sigma@f$, regardless of the
    *      original parameters of `mbm`. The correlation structure is determined by
    *      the underlying
    * @ref MultivariateBrownianMotion. The observation times are the same as those
    *      of `mbm`. The generation method depends on that of `mbm` (sequential,
    *      bridge sampling, PCA, etc.).
    */
   public MultivariateGeometricBrownianMotion(int c, double[] x0, double[] mu, double[] sigma,
   ...
   /**
    * Sets the observation times of the `MultivariateGeometricBrownianMotion`, but
    * also those of the inner
    * 
...
//         return s;
   ...
   /**
    * Sets the parameters @f$S(t_0) = \mathtt{x0}@f$, @f$\mu= \mathtt{mu}@f$
   ...
   /**
    * Returns a reference to the @ref MultivariateBrownianMotion object used to
