 ...
 * 
 * @anchor REF_stochprocess_VarianceGammaProcessDiff_dblGammaEqn @f[ X(t;
 *         \theta, \sigma, \nu) := X(0) + \Gamma^+(t; \mu_p, \nu_p) -
 *         \Gamma^-(t; \mu_n, \nu_n) \tag{dblGammaEqn} @f] where @f$X(0)@f$ is a
 *         constant corresponding to the initial value of the process and
 * @anchor REF_stochprocess_VarianceGammaProcessDiff_dblGammaParams @f[
 *         \begin{array}{rcl} \mu_p & = & (\sqrt{ \theta^2 + 2\sigma^2/\nu} +
 *         \theta)/2 \\ \mu_n & = & (\sqrt{ \theta^2 + 2\sigma^2/\nu} -
 *         \theta)/2 \\ \nu_p & = & \nu\mu_p^2 \\ \nu_n & = & \nu\mu_n^2
 *         \end{array} \tag{dblGammaParams} @f]
...
public class VarianceGammaProcessDiff extends VarianceGammaProcess {
    ...
    * @f$\nu= \mathtt{nu}@f$ and initial value @f$S(t_0) = \mathtt{s0}@f$. `stream`
    *         is used by two instances of
    * @ref GammaProcess, @f$\Gamma^+@f$ and @f$\Gamma^-@f$, respectively. The other
    *      parameters are as in the class
    * @ref VarianceGammaProcess. The @ref GammaProcess objects for
    * @f$\Gamma^+@f$ and @f$\Gamma^-@f$ are constructed using the parameters from (
    *                {@link REF_stochprocess_VarianceGammaProcessDiff_dblGammaParams
    *                dblGammaParams} ) and their initial values @f$\Gamma^+(t_0)@f$
    *                and
...
//     }
    ...
    * 
    * @f$\Gamma^+@f$ is first generated and then the path of @f$\Gamma^-@f$. This
    *                is not the optimal way of proceeding in order to reduce the
    *                variance in QMC simulations; for that, use
    *                `generatePath(double[] uniform01)` instead.
    */
   public double[] generatePath() {
    ...
    * tables, the first one containing the odd indices of `uniform01` are used to
    * generate the path of
    * 
   ...
   /**
    * Sets the observation times on the `VarianceGammaProcessDiff` as usual, but
    * also applies the `resetStartProcess` method to the two
    * 
   ...
   /**
    * Returns a reference to the @ref GammaProcess object `gpos` used to generate
    * the @f$\Gamma^+@f$ component of the process.
    */
   ...
   /**
    * Returns a reference to the @ref GammaProcess object `gneg` used to generate
    * the @f$\Gamma^-@f$ component of the process.
    */
