...
/**
 * Represents a *gamma* process sampled using the principal component analysis
 * (PCA). To simulate the gamma process at times @f$t_0 < t_1 < \cdots< t_d@f$
 * by PCA sampling, a Brownian motion @f$\{ W(t), t \geq0 \}@f$ with
 * mean @f$0@f$ and variance parameter @f$\nu@f$ is first generated at
 * times @f$t_0 < t_1 < \cdots< t_d@f$ by PCA sampling (see class @ref
 * BrownianMotionPCA ). The independent increments @f$W(t_j) - W(t_{j-1})@f$ of
 * this process are then transformed into independent @f$U(0, 1)@f$ random
 * variates @f$V_j@f$ via @f[ V_j = \Phi\left(\sqrt{\tau_j-\tau_{j-1}}
 * [W(\tau_j)-W(\tau_{j-1})]\right), \quad j=1,…,s @f] Finally, the increments
 * of the Gamma process are computed as @f$ Y(t_j) - Y(t_{j-1}) =
 * G^{-1}(V_j)@f$, where @f$G@f$ is the gamma distribution function.
 *
...
public class GammaProcessPCA extends GammaProcess {
    ...
    * \mathtt{s0}@f$. The random variables are created using `stream`. Note that
    * the same @ref umontreal.ssj.rng.RandomStream is used for the
    * `GammaProcessPCA` and for the @ref BrownianMotionPCA included in this class.
    * Both the @ref GammaProcessPCA and the
    * 
    ...
    * Constructs a new `GammaProcessPCA` with parameters @f$\mu=
    * \mathtt{mu}@f$, @f$\nu= \mathtt{nu}@f$ and initial value @f$S(t_0) =
    * \mathtt{s0}@f$. All the random variables, i.e. the gamma ones and the normal
    * ones, are created using the
    * 
    * @ref umontreal.ssj.rng.RandomStream included in the
    * @ref umontreal.ssj.randvar.GammaGen `Ggen`. Note that the parameters of
    *      the @ref umontreal.ssj.randvar.GammaGen object are not important since
    *      the implementation forces the generator to use the correct parameters
    *      (as defined above).
    */
   ...
   public double[] generatePath() {
      ...
      path[0] = x0;
      for (i = 0; i < d; i++) {
         path[i + 1] = path[i] + GammaDist.inverseF(mu2dtOverNu[i], muOverNu, 10, uniformsV[i]);
      }
      observationIndex = d;
   ...
   /**
    * Sets the parameters `s0`, @f$\mu@f$ and @f$\nu@f$ to new values, and sets the
    * variance parameters of the @ref BrownianMotionPCA to @f$\nu@f$.
    */
   ...
   /**
    * Resets the @ref umontreal.ssj.rng.RandomStream of the gamma generator and
    * the @ref umontreal.ssj.rng.RandomStream of the inner
    * 
