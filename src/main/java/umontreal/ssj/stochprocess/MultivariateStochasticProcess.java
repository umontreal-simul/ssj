...
/**
 * This class is a multivariate version of @ref StochasticProcess where the
 * process evolves in the @f$c@f$-dimensional real space. It is an abstract
 * (base) class for a multivariate stochastic process @f$\{\mathbf{X}(t) =
 * (X_1(t),…,X_c(t)), t \geq0 \}@f$, sampled (or observed) at a finite number
 * of time points, @f$0 = t_0 < t_1 < \cdots< t_d@f$. The observation times can
 * be specified by #setObservationTimes. The method #generatePath
 * generates @f$\mathbf{X}(t_1),…,\mathbf{X}(t_d)@f$ and memorizes them in a
 * (one-dimensional) vector, which can be recovered by #getPath. The
 * element @f$cj+i-1@f$ of this vector contains @f$X_i(t_j)@f$,
 * for @f$j=0,…,d@f$ and
 * 
 * @f$i=1,…,c@f$. Alternatively, in some cases, the observations
 * @f$\mathbf{X}(t_j)@f$ can be generated sequentially, one at a time, by
 *                       invoking #resetStartProcess first, and then
 *                       #nextObservationVector repeatedly.
 *
...
public abstract class MultivariateStochasticProcess extends StochasticProcess {
    ...
    * 
    * @f$\{\mathbf{X}(t_0), \mathbf{X}(t_1), …, \mathbf{X}(t_d)\}@f$, which can
    *                       then be accessed via `getPath`, `getSubpath`, or
    *                       `getObservation`. The generation method depends on the
    *                       process type. If `path[]` denotes the returned array,
    *                       then `path[cj + i-1]` contains @f$X_i(t_j)@f$
    *                       for @f$j=0,…,d@f$ and @f$i=1,…,c@f$.
    */
    ...
    * 
    * @f$t_0 = \mathtt{t[0]}@f$ and @f$t_d = \mathtt{t[d]}@f$. The size of `t` must
    *        be @f$d+1@f$.
    */
   public void setObservationTimes(double[] t, int d) {
      ...
      /***
       * Process specific initialization; usually precomputes quantities that depend
