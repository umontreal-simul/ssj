...
/**
 * This class represents a *CIR* process as in @ref CIRProcess, but the process
 * is generated using the simple Euler scheme
 * 
 * @anchor REF_stochprocess_CIRProcessEuler_eq_cir_seq_euler @f[ X(t_j) -
 *         X(t_{j-1}) = \alpha(b - X(t_{j-1}))(t_j - t_{j-1}) + \sigma\sqrt{(t_j
 *         - t_{j-1})X(t_{j-1})}  Z_j \tag{cir-seq-euler} @f] where @f$Z_j
 *         \sim N(0,1)@f$. This is a good approximation only for small time
 *         intervals @f$t_j - t_{j-1}@f$.
 *
...
public class CIRProcessEuler extends StochasticProcess {
    ...
    * `alpha`, @f$b@f$, @f$\sigma=@f$ `sigma` and initial value @f$X(t_0) =@f$
    * `x0`. The normal variates @f$Z_j@f$ will be generated by inversion using the
    * stream `stream`.
    */
