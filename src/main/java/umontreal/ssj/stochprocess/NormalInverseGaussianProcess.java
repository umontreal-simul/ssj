 ...
 * stochastic differential equation @cite pBAR98a&thinsp;
 * 
 * @anchor REF_stochprocess_NormalInverseGaussianProcess_eq_nig @f[ dX(t) = \mu
 *         dt + dB(h(t)), \tag{nig} @f] where @f$\{B(t),  t\ge0\}@f$ is a @ref
 *         BrownianMotion with drift
 * @f$\beta@f$ and variance 1, and @f$h(t)@f$ is an
 * @ref InverseGaussianProcess @f$IG(\nu/\gamma,\nu^2)@f$, with @f$\nu= \delta
 *      dt@f$ and @f$\gamma= \sqrt{\alpha^2 - \beta^2}@f$.
 *
 *      In this class, the process is generated using the sequential technique:
 * @f$X(0)=x_0@f$ and @f[ X(t_j) - X(t_{j-1}) =\mu dt + \beta Y_j + \sqrt{Y_j}
 *                Z_j, @f] where @f$Z_j \sim N(0,1)@f$, and @f$Y_j \sim
 *                IG(\nu/\gamma,\nu^2)@f$ with @f$\nu= \delta(t_j - t_{j-1})@f$.
 *
 *                There is one @ref umontreal.ssj.rng.RandomStream used to
 *                generate the
 * @f$Z_j@f$’s and there are one or two streams used to generate the
 *               underlying @ref InverseGaussianProcess, depending on which IG
 *               subclass is used.
 *
 *               In finance, a NIG process usually means that the log-return is
 *               given by a NIG process; @ref
 *               GeometricNormalInverseGaussianProcess should be used in that
 *               case.
 *
...
public class NormalInverseGaussianProcess extends StochasticProcess {
    ...
    * `NormalInverseGaussianProcess`. The parameters and observation times of the
    * IG process will be overriden by the parameters of the NIG process. If there
    * are two @ref umontreal.ssj.rng.RandomStream ’s in the @ref
    * InverseGaussianProcess, this constructor assumes that both streams have been
    * set to the same stream.
    */
   ...
   /**
    * Constructs a new <tt>NormalInverseGaussianProcess</tt>. The string argument
    * corresponds to the type of underlying
    * 
    * @ref InverseGaussianProcess. The choices are SEQUENTIAL_SLOW, SEQUENTIAL_MSH,
    *      BRIDGE and PCA, which correspond respectively to
    ...
    * @ref umontreal.ssj.rng.RandomStream, `streamIG2`, will not be used at all if
    *      the SEQUENTIAL_SLOW or PCA methods are chosen.
    */
   public NormalInverseGaussianProcess(double x0, double alpha, double beta, double mu, double delta,
   ...
   /**
    * Same as above, but all @ref umontreal.ssj.rng.RandomStream ’s are set to
    * the same stream, `streamAll`.
    */
   public NormalInverseGaussianProcess(double x0, double alpha, double beta, double mu, double delta,
   ...
   /**
    * Generates the path. This method samples each stream alternatively, which is
    * useful for quasi-Monte Carlo, where all streams are in fact the same iterator
    * on a @ref umontreal.ssj.hups.PointSet.
    */
    ...
    * 
    * @ref InverseGaussianProcess is of type @ref InverseGaussianProcessPCA, this
    *      method cannot be used. It will work with
    * @ref InverseGaussianProcessBridge, but the return order of the observations
    *      is the bridge order.
    */
    ...
    * furthermore sets the starting *value* of the @ref InverseGaussianProcess to
    * `t[0]`.
    */
   ...
   /**
    * Sets the parameters. Also, computes @f$\gamma= \sqrt{\alpha^2-\beta^2}@f$.
    */
   ...
   /**
    * Returns the analytic average, which is @f$\mu t + \delta t \beta/ \gamma@f$.
    */
   ...
   /**
    * Returns the analytic variance, which is @f$\delta t \alpha^2 / \gamma^3@f$.
    */
   ...
   /**
    * Only returns the stream if all streams are equal, including the stream(s) in
   ...
   /**
    * Sets all internal streams to `stream`, including the stream(s) of the
    * underlying @ref InverseGaussianProcess.
    */
