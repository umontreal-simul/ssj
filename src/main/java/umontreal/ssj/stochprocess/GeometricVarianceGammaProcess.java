 ...
 * 
 * @cite fMAD98a&thinsp; (page 86)). This stochastic process is defined by the
 *       equation
 * @anchor REF_stochprocess_GeometricVarianceGammaProcess_GeoVGeqn @f[ S(t) =
 *         S(0) \mbox{ exp}(\mu t + X(t; \sigma, \nu, \theta) + \omega t),
 *         \tag{GeoVGeqn} @f] where @f$X@f$ is a variance gamma process and
 * @anchor REF_stochprocess_GeometricVarianceGammaProcess_omegaEqn @f[ \omega=
 *         (1/\nu) \mbox{ ln}( 1 - \theta\nu- \sigma^2 \nu/2).
 *         \tag{omegaEqn} @f]
...
public class GeometricVarianceGammaProcess extends StochasticProcess {
    ...
    * @f$\nu= \mathtt{nu}@f$, @f$\mu= \mathtt{mu}@f$ and initial value @f$S(t_0) =
    *         \mathtt{s0}@f$. The `stream` is used to generate the
    * @ref VarianceGammaProcess object used to implement @f$X@f$ in (
    *      {@link REF_stochprocess_GeometricVarianceGammaProcess_GeoVGeqn GeoVGeqn}
    *      ).
    */
    ...
    * @ref VarianceGammaProcess `vargamma`. The parameter @f$\mu@f$ is set to `mu`
    *      and the initial values @f$S(t_0) = \mathtt{s0}@f$.
    */
    ...
    * `resetStartProcess` method to the @ref VarianceGammaProcess object used to
    ...
    * and @f$\mu= \mathtt{mu}@f$ of the process. *Warning*: This method will
    ...
    * {@link REF_stochprocess_GeometricVarianceGammaProcess_omegaEqn omegaEqn} ).
    */
   ...
   /**
    * Returns a reference to the variance gamma process @f$X@f$ defined in the
    * constructor.
    */
