 ...
 * @anchor REF_stochprocess_MultivariateBrownianMotion_eq_Brownian_motion_sequential_multi @f[
 *         X_i(t_j) - X_i(t_{j-1}) = (t_j - t_{j-1})\mu_i + \sqrt{t_j - t_{j-1}}
 *         \sigma_i Z_{j,i} \tag{Brownian-motion-sequential-multi} @f]
 ...
 *                     (\sigma_1,…,\sigma_c)^{\mathsf{t}}@f$,
 *                     and @f$\boldsymbol{\Sigma}@f$ for the covariance matrix
 *                     of @f$\mathbf{X}(1)-\mathbf{X}(0)@f$, which equals
 * @f$\boldsymbol{\Sigma}= \boldsymbol{\sigma}\mathbf{R}_z\boldsymbol{\sigma}^{\mathsf{t}}@f$
 *                         (so the element @f$(k,l)@f$
 *                         or @f$\boldsymbol{\Sigma}@f$ is the element
 * @f$(k,l)@f$ of @f$\mathbf{R}_z@f$ multiplied by @f$\sigma_k\sigma_l@f$). The
 *             trajectories are sampled by the sequential (or random walk)
 *             method.
 *
...
public class MultivariateBrownianMotion extends MultivariateStochasticProcess {
    ...
    *                      = \mathtt{x0}@f$. The normal variates @f$Z_j@f$ in are
    *                      generated by inversion using the @ref
    *                      umontreal.ssj.rng.RandomStream `stream`.
    */
    ...
    * 
    * @f$\mathbf{X}(t_j)@f$ of the multivariate stochastic process. The processe is
    *                       sampled *sequentially*, i.e. if the last observation
    *                       generated was for time @f$t_{j-1}@f$, the next
   ...
   /**
    * Generates and returns the next observation @f$\mathbf{X}(t_j)@f$ of the
    * multivariate stochastic process in a vector created automatically. The
    * processe is sampled *sequentially*, i.e. if the last observation generated
    * was for time @f$t_{j-1}@f$, the next observation returned will be for
    * time @f$t_j@f$.
    */
   ...
   /**
    * Generates and returns the vector of next observations, at time @f$t_{j+1} =
    * \mathtt{nextTime}@f$, using the previous observation time @f$t_j@f$ defined
    * earlier (either by this method or by <tt>setObservationTimes</tt>), as well
    * as the value of the previous observation @f$X(t_j)@f$. *Warning* : This
    * method will reset the observations time @f$t_{j+1}@f$ for this process to
    * `nextTime`. The user must make sure that the @f$t_{j+1}@f$ supplied
    * is @f$\geq t_j@f$.
    */
   ...
   /**
    * Generates an observation (vector) of the process in `dt` time units, assuming
    * that the process has (vector) value @f$x@f$ at the current time. Uses the
    * process parameters specified in the constructor. Note that this method does
    * not affect the sample path of the process stored internally (if any).
    */
    ...
    * 
    * @f$\mathbf{X}(t_0) = \mathtt{x0}@f$, the average @f$\mu= \mathtt{mu}@f$, the
    *                    volatility @f$\sigma= \mathtt{sigma}@f$ and the
    *                    correlation matrix to `corrZ`. The vectors `x0`, `mu` ans
    *                    `sigma` must be of size `c` as well as the matrix corrZ
    *                    must be of size `c x c`. *Warning*: This method will
    *                    recompute some quantities stored internally, which may be
    *                    slow if called too frequently.
    */
    ...
    * 
    * @f$\mathbf{X}(t_0) = \mathtt{x0}@f$, the average @f$\mu= \mathtt{mu}@f$, the
    *                    volatility @f$\sigma= \mathtt{sigma}@f$. *Warning*: This
    *                    method will recompute some quantities stored internally,
   ...
   /**
    * Returns the normal random variate generator used. The `RandomStream` used for
    * that generator can be changed via `getGen().setStream(stream)`, for example.
    */
