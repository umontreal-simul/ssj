 ...
 * 
 * @f$\boldsymbol{\Sigma}@f$ as in @ref MultivariateBrownianMotion and decompose
 *                           it as @f$\boldsymbol{\Sigma}= B B^{\mathsf{t}}@f$
 *                           via PCA. We also compute the
 *                           matrix @f$\mathbf{C}@f$ whose element @f$(i,j)@f$
 *                           is
 * @f$\mathrm{Cov}[B(t_{i+1}),B(t_{j+1})] = \min(t_{i+1},t_{j+1})@f$ and its PCA
 *                                        decomposition @f$\mathbf{C}=
 *                                        \mathbf{A}\mathbf{A}^{\mathsf{t}}@f$,
 *                                        as in class @ref BrownianMotionPCA.
 *
