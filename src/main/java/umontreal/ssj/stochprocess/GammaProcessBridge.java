 ...
 * \}@f$ with mean parameter @f$\mu@f$ and variance parameter
 * 
 ...
 *
 *       Note that gamma bridge sampling requires not only gamma variates, but
 *       also *beta* variates. The latter generally take a longer time to
 *       generate than the former. The class `GammaSymmetricalBridgeProcess`
 *       provides a faster implementation when the number of observation times
 *       is a power of two.
 *
 *       The warning from class @ref BrownianMotionBridge applies verbatim to
 *       this class.
 *
...
public class GammaProcessBridge extends GammaProcess {
    ...
    * \mathtt{s0}@f$. Uses `stream` to generate the gamma and beta variates by
    * inversion.
    */
   ...
   /**
    * Constructs a new `GammaProcessBridge`. Uses the random variate generators
    * `Ggen` and `Bgen` to generate the gamma and beta variates, respectively. Note
    * that both generator uses the same
    * 
    * @ref umontreal.ssj.rng.RandomStream. Furthermore, the parameters of the @ref
    *      umontreal.ssj.randvar.GammaGen and
    * @ref umontreal.ssj.randvar.BetaGen objects are not important since the
    *      implementation forces the generators to use the correct parameters. (as
    *      defined in @cite fRIB02a&thinsp; (page 7)).
    */
