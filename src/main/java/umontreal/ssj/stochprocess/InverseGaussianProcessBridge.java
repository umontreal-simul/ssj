...
/**
 * Samples the path by bridge sampling: first finding the process value at the
 * final time and then the middle time, etc. The method `nextObservation()`
 * returns the path value in that non-sequential order. This class uses two @ref
 * umontreal.ssj.rng.RandomStream ’s to generate a path @cite fWEB03a&thinsp;.
 *
...
public class InverseGaussianProcessBridge extends InverseGaussianProcessMSH {
   ...
   /**
    * Constructs a new `InverseGaussianProcessBridge`. The initial value `s0` will
   ...
   /**
    * Instead of using the internal streams to generate the path, it uses two
    * arrays of uniforms @f$U[0,1)@f$. The length of the arrays `unifNorm` and
    * `unifOther` should be equal to the number of time steps, excluding @f$t_0@f$.
    */
