 ...
 * 
 * @cite fCOX85a&thinsp; @f$\{X(t) : t \geq0 \}@f$, sampled at times @f$0 = t_0
 *       < t_1 < \cdots< t_d@f$. This process obeys the stochastic differential
 *       equation
 * @anchor REF_stochprocess_CIRProcess_eq_cir @f[ dX(t) = \alpha(b - X(t)) dt +
 *         \sigma\sqrt{X(t)}  dB(t) \tag{cir} @f] with initial
 ...
 *              process is *mean-reverting* in the sense that it always tends to
 *              drift toward its general mean @f$b@f$. The process is generated
 *              using the sequential technique @cite fGLA04a&thinsp; (p. 122)
 * @anchor REF_stochprocess_CIRProcess_eq_cir_seq @f[ X(t_j) =
 *         \frac{\sigma^2\left(1 - e^{-\alpha(t_j - t_{j-1})}\right)}{4\alpha}
 *         \chi^{\prime 2}_{\nu}\left(\frac{4\alpha e^{-\alpha(t_j - t_{j-1})
 *         } X(t_{j-1})}{\sigma^2\left(1 - e^{-\alpha(t_j -
 *         t_{j-1})}\right)}\right), \tag{cir-seq} @f] where @f$\nu=
 *         4b\alpha/\sigma^2@f$, and @f$\chi^{\prime 2}_{\nu}(\lambda)@f$ is a
 *         noncentral chi-square random variable with
...
public class CIRProcess extends StochasticProcess {
    ...
    * \mathtt{alpha}@f$, @f$b@f$, @f$\sigma= \mathtt{sigma}@f$ and initial
    * value @f$X(t_0) = \mathtt{x0}@f$. The noncentral chi-square
    * variates @f$\chi^{\prime2}_{\nu}(\lambda)@f$ will be generated by inversion
    * using the stream `stream`.
    */
   ...
   /**
    * The noncentral chi-square variate generator `gen` is specified directly
    * instead of specifying the stream. `gen` can use a method other than
    * inversion.
    */
    ...
    * as the value of the previous observation @f$X(t_j)@f$. *Warning*: This method
    ...
    * \mathtt{alpha}@f$, @f$b = \mathtt{b}@f$ and @f$\sigma= \mathtt{sigma}@f$ of
    * the process. *Warning*: This method will recompute some quantities stored
