...
/**
 * This class represents a *gamma* process @cite fMAD98a&thinsp; (page 82) @f$\{
 * S(t) = G(t; \mu, \nu) : t \geq0 \}@f$ with mean parameter
 * 
 * @f$\mu@f$ and variance parameter @f$\nu@f$. It is a continuous-time process
 *           with stationary, independent gamma increments such that for any
 * @f$\Delta t > 0@f$,
 * @anchor REF_stochprocess_GammaProcess_GammaEqn @f[ S(t + \Delta t) = S(t) +
 *         X,\tag{GammaEqn} @f] where @f$X@f$ is a random variate from the gamma
 *         distribution <tt>Gamma</tt>@f$(\mu^2\Delta t / \nu, \mu/ \nu)@f$.
 *
 *         In this class, the gamma process is sampled sequentially using
 *         equation ( {@link REF_stochprocess_GammaProcess_GammaEqn GammaEqn} ).
 *
...
public class GammaProcess extends StochasticProcess {
    ...
    * {@link REF_stochprocess_GammaProcess_GammaEqn GammaEqn} ) are generated by
...
// The 'real' parameters are supplied at time of generation
    ...
    * \mathtt{mu}@f$, @f$\nu= \mathtt{nu}@f$ and initial value @f$S(t_0) =
    * \mathtt{s0}@f$. The gamma variates @f$X@f$ in (
    * {@link REF_stochprocess_GammaProcess_GammaEqn GammaEqn} ) are supplied by the
    * gamma random variate generator `Ggen`. Note that the parameters of the
    * 
    * @ref umontreal.ssj.randvar.GammaGen object `Ggen` are not important since the
    *      implementation forces the generator to use the correct parameters (as
    *      defined above).
    */
    ...
    * will reset the observations time @f$t_{j+1}@f$ for this process to `nextT`.
    * The user must make sure that the @f$t_{j+1}@f$ supplied is @f$\geq t_j@f$.
    */
   ...
   /**
    * Generates, returns and saves the path @f$\{X(t_0), X(t_1), …, X(t_d)\}@f$.
    * The gamma variates @f$X@f$ in ( {@link REF_stochprocess_GammaProcess_GammaEqn
    ...
    * @ref umontreal.ssj.randvar.GammaGen `Ggen`. It uses the vector of uniform
    *      random numbers @f$U(0, 1)@f$ provided by the user and generates the path
    *      by inversion. The vector `uniform01` must be of dimension @f$d@f$.
    */
    ...
    * and @f$\nu= \mathtt{nu}@f$ of the process. *Warning*: This method will
