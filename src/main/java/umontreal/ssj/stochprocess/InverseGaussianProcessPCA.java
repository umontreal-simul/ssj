 ...
 * 
 * @ref BrownianMotionPCA with a covariance matrix identical to the one of our
 *      `InverseGaussianProcess` is used to generate the path of our
 *      `InverseGaussianProcess` @cite fLEC08a&thinsp;. Such a path is a
 *      perfectly random path and it is hoped that it will provide reduction in
 *      the simulation variance when using quasi-Monte Carlo.
 *
...
public class InverseGaussianProcessPCA extends InverseGaussianProcess {
   ...
   /**
    * Constructs a new `InverseGaussianProcessPCA`. The initial value `s0` will be
   ...
   public double[] generatePath() {
      ...
      for (int i = 0; i < d; i++) {
         ...
         uniformIncrement[i] = NormalDistQuick.cdf01((BMpath[i + 1] - BMpath[i] - bmPCA.mu * dt) / sigma);
      }
      path[0] = x0;
      for (int i = 0; i < d; i++)
         path[i + 1] = path[i] + InverseGaussianDist.inverseF(imu[i], ilam[i], uniformIncrement[i]);
   ...
   /**
    * Instead of using the internal stream to generate the path, uses an array of
    * uniforms @f$U[0,1)@f$. The length of the array should be equal to the length
    * of the number of periods in the observation times. This method is useful
    * for @ref NormalInverseGaussianProcess.
    */
