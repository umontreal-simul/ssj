 ...
 * subordinate of the Brownian motion process @f$B(t;\theta,\sigma)@f$ using the
 * operational time @f$G(t;1,\nu)@f$ (see @cite pFEL66a,
 * 
 * @cite fAVR06a&thinsp;):
 * @anchor REF_stochprocess_VarianceGammaProcess_VGeqn @f[ X(t; \theta, \sigma,
 *         \nu) := B(G(t;1,\nu),\theta, \sigma). \tag{VGeqn} @f] See also @cite
 *         fMAD98a, @cite fMAD91a, @cite fMAD90a&thinsp; for applications to
 *         modelling asset returns and option pricing.
 *
 *         The process is sampled as follows: when `generatePath()` is called,
 *         the method <tt>generatePath()</tt> of the inner @ref GammaProcess is
 *         called; its path is then used to set the observation times of the
 ...
 *      generated first, followed by the corresponding brownian motion
 *      increment, which is then returned.
 *
 *      Note that if one wishes to use *bridge* sampling with the
 *      `nextObservation` method, both the gamma process @f$G@f$ and the
 *      Brownian motion process @f$B@f$ should use bridge sampling so that their
...
public class VarianceGammaProcess extends StochasticProcess {
   ...
   protected double theta, sigma, nu;
    ...
    * initial value @f$S(t_0) = \mathtt{s0}@f$. `stream` is used to generate both
    * the @ref BrownianMotion @f$B@f$ and the @ref GammaProcess @f$G@f$ in (
    * {@link REF_stochprocess_VarianceGammaProcess_VGeqn VGeqn} ).
    */
   ...
   /**
    * Generates the observation for the next time. It also works with bridge
    * sampling; however *both* @ref BrownianMotionBridge and
    * 
    * @ref GammaProcessBridge must be used in the constructor in that case.
    *      Furthermore, for bridge sampling, the order of the observations is that
    *      of the bridge, not sequential order.
    */
   ...
   /**
    * Generates and returns the path. To do so, it first generates the complete
    * path of the inner @ref GammaProcess and sets the observation times of the
    * inner @ref BrownianMotion to this path. This method is not optimal to reduce
    * the variance in QMC simulations; use `generatePath(double[] uniform01)` for
    * that.
    */
   ...
   /**
    * Similar to the usual `generatePath()`, but here the uniform random numbers
    * used for the simulation must be provided to the method. This allows to
    * properly use the uniform random variates in QMC simulations. This method
    * divides the table of uniform random numbers `uniform01` in two smaller
    * tables, the first one, containing the odd indices of `uniform01` which are
    * used to generate the path of the inner @ref GammaProcess, and the even
    * indices (in the second table) are used to generate the path of the inner @ref
    * BrownianMotion. This way of proceeding reduces the variance as much as
    * possible for QMC simulations.
    */
    ...
    * 
    * @f$\sigma=@f$ `sigma` and @f$\nu=@f$ `nu` of the process. *Warning*: This
    *               method will recompute some quantities stored internally, which
    *               may be slow if called repeatedly.
    */
   ...
   /**
    * Sets the observation times on the `VarianceGammaProcess` as usual, but also
    * sets the observation times of the underlying @ref GammaProcess. It
   ...
   /**
    * Resets the @ref umontreal.ssj.rng.RandomStream ’s. Warning: this method
    * sets both the @ref umontreal.ssj.rng.RandomStream of the
    * 
   ...
   /**
    * Returns the random stream of the @ref BrownianMotion process, which should be
    * the same as for the @ref GammaProcess.
    */
