...
public class TallyHistogram extends Tally {
    ...
    * 
    * @f$[a,b]@f$ into @f$s@f$ bins of equal width and initializes a counter to 0
    *             for each bin. Whenever an observation falls into a bin, the bin
    *             counter is increased by 1.
    * @param a       left boundary of interval
    * @param b       right boundary of interval
    * @param numBins number of bins (of equal width)
    */
   ...
   public void init(double a, double b, int numBins) {
      /*
       * The counters count[0] to count[s-1] contains the number of observations
   ...
   /**
    * Initializes all the counters and accumulators, including those of the `Tally`
    * object.
    *
    */
   public void init() {
    ...
    * added to the extra bin counters.
    * 
   ...
   /**
    * Remove empty bins in the tails (left and right), without changing the bin
    * size. This gives a new @ref TallyHistogram which may have fewer bins.
    */
   ...
   /**
    * Merges bins by groups of size @f$g@f$. If there are @f$m@f$ bins initially,
    * the new number of bins will be @f$\lceil m/g\rceil@f$. The last bin may
    * regroup less than @f$g@f$ original bins if @f$m@f$ is not a multiple
    * of @f$g@f$. In this case the upper bound @f$b@f$ is increased accordingly.
    **/
   ...
   /**
    * Returns the array of bin counters. Each counter contains the number of
   ...
   /**
    * Returns the proportion of the collected observations that lie within the
    * boundaries
    * 
    * @f$[a,b]@f$ of the histogram; that is, the number that fell
    *             within @f$[a,b]@f$ divided by the total number that were
    *             collected.
    * 
   ...
   public String toString() {
      ...
      sb.append("   (" + PrintfFormat.f(6, 3, m_b) + ", inf)    " + rightCount + PrintfFormat.NEWLINE);
      sb.append("}" + PrintfFormat.NEWLINE);
      return sb.toString();
   }
