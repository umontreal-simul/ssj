 ...
 * evaluate it only once per bin. To this end, this class provides methods to
...
public class DEHistogram extends DensityEstimator {
	 ...
	 *            the array of the \ref umontreal.ssj.stat.ScaledHistogram which
	 *            define the histograms.
	 * @return the histogram for each element of \a scaledHistArray evaluated one in
	 *         each bin.
	 */
	public static double[][] evalDensity(ScaledHistogram[] scaledHistArray) {
		...
		double[][] density = new double[m][];
		for (int r = 0; r < m; r++)
			density[m] = evalDensity(scaledHistArray[r]);
