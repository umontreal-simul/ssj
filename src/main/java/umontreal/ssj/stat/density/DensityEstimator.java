 ...
 * 
 * Both static and non-static methods are offered. In a majority of cases, on
 * simply wishes to estimate the density at a finite set of evaluation points,
 * from a given set of data, and perhaps plot the estimated density. To do that,
 * there is no need to create an object. One can simply use a static
 * `evalDensity` method followed by `plotDensity`. Note that calling the
 * `evalDensity` method only once for a vector of evaluation points is typically
 * much faster than calling it separately for each evaluation point.
 * 
 * In case one plans to evaluate the same density several times with the same
 * data, then it may be worthwhile to construct a `DensityEstimator` object and
 * build the density estimate from the given data. After that, one can evaluate
 * the density at any given point, often much faster than by calling the static
 ...
 * convergence behavior of the DEs in terms of their IV, ISB, and MISE. As these
 * only require evaluations of the density estimator, they are implemented as
 * static methods.
 * 
 * One such method estimates the empirical IV by evaluating the empirical
 * variance at set (or grid) of evaluation points and averaging. See the methods
 * `computeIV`, which can do that for one DE or for a list of several DEs. The
 * MISE can also be estimated in situations where either the ISB is known to be
...
public abstract class DensityEstimator {
    ...
    *
    * @param evalPoints the evaluation points.
    * @param data       the two-dimensional array carrying the observations of
    ...
    * 
    * @param density the estimated density of \f$m\f$ independent realizations of
    *                the estimator, each evaluated at \f$k\f$ evaluation points.
    ...
    * array \a density, which is also described in `computeVariance(double[][])`
    * and can be obtained by `evalDensity(double[], double[][])` . The boundaries
    * of the interval are given by \a a and \a b. Note that the array \a variance
    * needs to be of length \f$k\f$.
    * 
    * 
    * @param density  the \f$m\times k\f$ array that contains the data of
    *                 evaluating \f$m\f$ replicates of the density estimator at
    *                 \f$k\f$ evaluation points
    * @param a        the left boundary of the interval.
    * @param b        the right boundary of the interval.
    * @param variance the array of length \f$k\f$ in which the variance at each
    ...
    * 
    * 
    * @param listDensity list containing \f$m\times k\f$ arrays that contain the
    *                    data of evaluating \f$m\f$ replicates of each density
    *                    estimator at \f$k\f$ evaluation points \a evalPoints.
    * @param a           the left boundary of the interval.
    * @param b           the right boundary of the interval.
    * @param listIV      the list to which the estimated empirical IV of each
    *                    density estimator will be added.
    * 
    * @remark **Florian:** I kept the return type as "void" instead of
    *         "ArrayList<double[][]>" and pass the corresponding list \a
    *         listDensity to allow for more flexibility when working with it.
    ...
    * 
    * @param dist       the true density.
    * @param evalPoints the \f$k\f$ evaluation points.
    * @param density    the \f$m\times k\f$ array that contains the data of
    *                   evaluating \f$m\f$ replicates of the density estimator at
    *                   \f$k\f$ evaluation points \a evalPoints.
    * @param a          the left boundary of the interval.
    * @param b          the right boundary of the interval.
    * @param variance   the array of length \f$k\f$ in which the variance at each
    *                   evaluation point is stored.
    * @param sqBias     the array of length \f$k\f$ in which the square-bias at
    *                   each evaluation point is stored.
    * @param mse        the array of length \f$k\f$ in which the MSE at each
    *                   evaluation point is stored.
    ...
    * obtained by calling `evalDensity(ArrayList, double[], double[][],
    * ArrayList)`, for instance.
    * 
    * The method then calls `computeMISE(ContinuousDistribution, double[],
    * double[][], double, double, double[], double[], double[])` for each element
    * of \a listDensity. This results in an array containing the estimated
    * empirical IV, ISB, and MISE in exactly this order, which is then added to the
    * list \a listMISE.
    * 
    * @param dist        the true density.
    * @param evalPoints  the \f$k\f$ evaluation points.
    * @param listDensity list of \f$m\times k\f$ arrays that contain the data of
    *                    evaluating \f$m\f$ replicates of each density estimator at
    *                    \f$k\f$ evaluation points \a evalPoints.
    * @param a           the left boundary of the interval.
    * @param b           the right boundary of the interval.
    * @param listMISE    a list to which the arrays containing the estimated
    *                    empirical IV, ISB, and MISE of each density estimator are
    *                    added.
    * @remark **Florian:** I kept the return type as "void" instead of
    *         "ArrayList<double[][]>" and pass the corresponding list \a
    *         listDensity to allow for more flexibility when working with it.
    ...
    * 
    * @param evalPoints the \f$x\f$-values.
    * @param density    the \f$y\f$-values.
