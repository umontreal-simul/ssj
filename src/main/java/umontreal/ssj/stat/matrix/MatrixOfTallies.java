 ...
 * matrix is an instance of @ref umontreal.ssj.stat.Tally, and observations can
 * be added with the #add(DoubleMatrix2D) method. This class defines factory
 * methods to fill a newly-constructed matrix with `Tally` or `TallyStore`
 * instances.
 *
...
public class MatrixOfTallies<E extends Tally> extends MatrixOfStatProbes<E> {
   ...
   /**
    * Constructs a new matrix of tallies with name `name`, `numRows` rows, and
    * `numColumns` columns, and filled with `null` references.
    * 
   ...
   /**
    * This factory method constructs and returns a matrix of tallies with `numRows`
    * rows, `numColumns` columns, and filled with new instances of @ref
   ...
   /**
    * Adds the observation `x.get(r, c)` in the tally whose row is `r` and column
    * is `c`, for <tt>r = 0, …, </tt>
    * umontreal.ssj.stat.matrix.MatrixOfStatProbes.rows ` - 1`, and <tt>c
    * = 0, …, </tt> umontreal.ssj.stat.matrix.MatrixOfStatProbes.columns ` - 1`.
    * No observation is added if the value is `Double.NaN`, or if observations
    * collecting is turned OFF. If broadcasting is ON, the given matrix is notified
    * to all registered observers. The given matrix `x` not being stored by this
    * object, it can be freely used and modified after the call to this method.
    * 
    ...
    *                                     correspond to the dimensions of this
    *                                     matrix of tallies.
    */
   public void add(DoubleMatrix2D x) {
      ...
      if (x.rows() != rows || x.columns() != columns)
         throw new IllegalArgumentException("Incompatible matrix dimensions: given " + x.rows() + "x" + x.columns()
               + ", required " + rows() + "x" + columns());
      if (collect)
      ...
      if (collect)
         for (int r = 0; r < rows; r++) {
            if (x[r].length != columns)
               throw new IllegalArgumentException("Incompatible number of columns in row " + r + ": given "
    ...
    * observations, returns the number of observations in tally (0, 0), or 0 if
    * this matrix has 0 row or column.
    * 
    ...
    * observations. This returns `true` if and only if all tallies have the same
   ...
   /**
    * Computes the average for each tally in the matrix. If the tally @f$(r, c)@f$
    * has no observation, the `Double.NaN` value is stored in the given matrix, at
    * row&nbsp;`r`, and column&nbsp;`c`.
    */
    ...
    * @param m the matrix to be filled with sample variances.
    * @exception NullPointerException     if `m` is `null`.
    * @exception IllegalArgumentException if `m.rows()` does not correspond to \ref
    *                                     #rows , or `m.columns()` does not
    ...
    * into the given matrix. This is equivalent to calling
    * #variance(DoubleMatrix2D) and performing a square root on every element of
    * the filled matrix.
    * 
