...
public class MatrixOfFunctionOfMultipleMeansTallies<E extends FunctionOfMultipleMeansTally>
   ...
   /**
    * Constructs a new unnamed matrix of function of multiple means tallies with
    * `numRows` rows, and `numColumns` columns, and filled with `null` references.
    * 
   ...
   /**
    * Constructs a new empty matrix of function of multiple means tallies with name
    * `name`, `numRows` rows, and `numColumns` columns, and filled with `null`
    * references.
    * 
   ...
   /**
    * This factory method constructs and returns a matrix of function of multiple
    * means tallies with `numRows` rows, `numColumns` columns, and filled with
    * instances of<br>
    * 
   ...
   /**
    * For each function of multiple means tally with row index&nbsp;`r` and column
    * index&nbsp;`c`, adds the vector of observations `x[r][c]` if collecting is
    * turned ON.
    * 
    * @param x the vectors of observations to be added.
    * @exception IllegalArgumentException if the dimensions of `x`, `x[r]`, or
    *                                     `x[r][c]` are incorrect.
    */
   ...
   /**
    * Equivalent to {@link #add(double[][][]) add(x.toArray())}, without copying
    * the elements of `x` into a temporary 3D array. Each slice of the 3D matrix
    * `x` corresponds to a row of the matrix of probes. Each row of the 3D matrix
    * corresponds to a column of the matrix of probes. Each column of the 3D matrix
    * corresponds to a component of a vector of observations added to a function of
    * multiple means tally. This can be used only when all the function of multiple
    * means tallies in the array share the same dimension.
    * 
    * @param x the matrix of observations.
    * @exception IllegalArgumentException if the dimensions of the matrix are
    *                                     incorrect.
    */
   ...
   /**
    * For each element (<tt>r</tt>, <tt>c</tt>) of this matrix of tallies, adds the
    * vector of observations `x[0].get (r, c)`, …, `x[d-1].get (r, c)`. This
    * method can be used only when all tallies in this matrix share the same
    * dimension. It creates a transposed 3D array compatible with
    * #add(double[][][]), and calls the latter method with this array. For example,
    * let `mt` be a matrix of tallies with dimensions
    * 
    * @f$r\times c@f$ whose <tt>l</tt>&nbsp;@f$=rc@f$ elements correspond to
    *            ratios. If a program can generate two matrices `num` and `den` of
    *            dimensions @f$r\times c@f$ representing observations for the
    *            numerator and denominator of the ratios, respectively, the
    *            observations can be added using `mt.add (num, den)` instead of
    *            creating an intermediate 3D array of observations.
    ...
    *                                     dimensions of the matrices `x[j]` are not
    *                                     equal for all <tt>j=0,…,d-1</tt>.
    */
   ...
   /**
    * Assuming that each tally in this matrix has the same dimension, returns the
    * dimension of tally (0, 0), or 0 if the matrix has no row or column.
    * 
    ...
    * observations, returns the number of observations in tally (0, 0), or 0 if the
    * matrix has no row or column.
    * 
    ...
    * observations. This returns `true` if and only if the matrix has no row and no
    * column, or if all tallies have the same number of observations.
    * 
    ...
    * If the tally @f$(r, c)@f$ has no observation, the `Double.NaN` value is
    * stored in the given matrix, at row&nbsp;`r`, column&nbsp;`c`.
    */
   ...
   /**
    * For each tally in the matrix, computes the sample variance, and stores it
    * into the given matrix. If, for some tally @f$(r, c)@f$, there are not enough
    * observations for estimating the variance, `Double.NaN` is stored in the
    * corresponding element of the given matrix `m`.
    * 
    ...
    *                                     umontreal.ssj.stat.matrix.MatrixOfStatProbes.rows,
   ...
   /**
    * For each tally in the matrix, computes the standard deviation, and stores it
    * into the matrix `m`. This is equivalent to calling #variance(DoubleMatrix2D)
    * and performing a square root on every element of the filled matrix.
    * 
    * @param m the matrix to be filled with standard deviations.
    * @exception NullPointerException     if `m` is `null`.
    * @exception IllegalArgumentException if `m.rows()` does not correspond to
    *                                     umontreal.ssj.stat.matrix.MatrixOfStatProbes.rows,
    *                                     or `m.columns()` does not correspond to
   ...
   /**
    * Clones this object. Makes a shallow copy of this matrix, i.e., does not clone
    * all the probes in the matrix.
    */
