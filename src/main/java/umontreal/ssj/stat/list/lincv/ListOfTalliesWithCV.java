 ...
 * @f$\mathbf{C}@f$. Methods are provided for adding observations with control
 *                   variables, and estimating the @f$\boldsymbol{\beta}^*@f$
 *                   matrix from the sample covariances. Convenience methods are
 *                   also provided for the common cases where @f$q=1@f$
 *                   and @f$p=1@f$.
 *
 *                   During simulation or pilot runs, one uses an `add` method
 *                   to add observations with the associated values of the
 *                   control variables. After the vectors of observations (and
 *                   controls) are collected, one can obtain an estimate
 *                   of @f$\boldsymbol{\beta}^*@f$, by using #estimateBeta. One
 *                   can also set @f$\boldsymbol{\beta}@f$ to an arbitrary
 *                   matrix.
 *
 *                   Before obtaining results with control variables, it is
 *                   important to set the expected values of the controls, by
 *                   using #setExpectedValues(double[]).
 *                   After @f$E[\mathbf{C}]@f$ is set, methods are then
 *                   available to get the controlled average, and the controlled
 *                   covariance matrix. One can also compute a confidence
 *                   interval on a component @f$X_i@f$ of @f$\mathbf{X}@f$
 *                   taking the control variables into account.
 *
 *                   The @f$\boldsymbol{\beta}^*@f$ matrix can be estimated
 *                   using pilot runs. In this context, if the list of tallies
 *                   is reset, the values of
 * @f$\boldsymbol{\beta}@f$ and @f$E[\mathbf{C}]@f$ are retained. Observations
 *                          generated from production runs can then be added to
 *                          the list of tallies, and the controlled average can
 *                          be computed.
 *
 *                          The following pseudocode illustrates how @f$q@f$
 *                          control variables can be used with
 *                          a @f$p@f$-dimensional vector, the value
 *                          of @f$E[\mathbf{C}]@f$ being stored in
 *                          `expControls`.
 *
 * @code
 *
 *       ListOfTalliesWithCV<Tally> list = ListOfTalliesWithCV.createWithTally
 *       (p, q); list.setExpectedValues (expControls); // Choose between NO
 *       PILOT RUNS or PILOT RUNS
 *
 *       NO PILOT RUNS { // Make production runs and use list.add to add (X, C)
 *       vectors. list.estimateBeta(); // Biased consistent estimator of the
 *       beta matrix }
 *
 *       PILOT RUNS { // Make pilot runs and use list.add to add (X, C) vectors.
 *       list.estimateBeta(); // Unbiased estimator of the beta matrix
 *       list.init(); // Make production runs and use list.add to add (X, C)
 *       vectors. }
 *
...
public class ListOfTalliesWithCV<E extends Tally> extends ListOfTalliesWithCovariance<E> {
   ...
   /**
    * Constructs a new empty list of tallies with no control variable. After
    * calling this constructor, one must fill the list with tallies, call
    * `setNumControlVariables`, and `init`.
    */
   ...
   /**
    * Constructs a new empty list of tallies with no control variable and name
    * `name`. After calling this constructor, one must fill the list with tallies,
   ...
   /**
    * This factory method constructs and returns a list of tallies with `p+q` new
    * instances of @ref umontreal.ssj.stat.Tally, `q` being the number of control
    * variables.
    * 
   ...
   /**
    * This factory method constructs and returns a list of tallies with `p+q` new
    * instances of @ref umontreal.ssj.stat.TallyStore, `q` being the number of
    * control variables.
    * 
    * @param p the value of @f$p@f$.
    * @param q the value of @f$q@f$.
   ...
   /**
    * Returns the current matrix @f$\boldsymbol{\beta}@f$. By default, this matrix
   ...
   /**
    * Sets the @f$\boldsymbol{\beta}@f$ matrix to `beta`. The given matrix must
    * be @f$q\times p@f$, or an exception is thrown.
    * 
   ...
   /**
    * Gets the expected value of the @f$i@f$th control variable. By default, this
    * is set to 0.
    * 
   ...
   /**
    * Returns @f$E[\mathbf{C}]@f$, the expected value of the vector of control
    * variables.
    * 
   ...
   /**
    * Sets @f$E[\mathbf{C}]@f$ to `exp`. The length of the given array must
    * be @f$q@f$, or an exception is thrown.
    * 
   ...
   /**
    * Sets the number of control variables to `q`. The new value of `q` must not be
    * negative or exceed `size()`. This method throws an exception if it is called
    * after #init.
    * 
    ...
    * 
    * @param c the @f$p\times p@f$ matrix filled with correlations.
    */
    ...
    * 
    * @param c the @f$p\times p@f$ matrix filled with covariances.
    */
    ...
    * 
    * @param c the @f$q\times q@f$ matrix filled with correlations.
    */
    ...
    * 
    * @param c the @f$q\times q@f$ matrix filled with covariances.
    */
    ...
    * 
    * @f$\mathbf{X}@f$.
    * @param c the @f$q\times p@f$ matrix filled with correlations.
    */
    ...
    * 
    * @f$\mathbf{X}@f$.
    * @param c the @f$q\times p@f$ matrix filled with covariances.
    */
   ...
   /**
    * Adds a new observation @f$(\mathbf{X}, \mathbf{C})@f$ to this list of
    * tallies. The array `x` contains the value for @f$\mathbf{X}@f$, while `c`
    * contains the value of @f$\mathbf{C}@f$.
    * 
   ...
   /**
    * Variant of the #add(double[],double[]) method that can be used when there is
    ...
    * 
    * @f$p=q=1@f$. @param x the output variable.
    ...
    * 
    * @f$\mathbf{X}_{\mathrm{C}}@f$, denoted @f${X}_{\mathrm{C},i}@f$. This
    *                                corresponds to @f[ {X}_{\mathrm{C},i}=X_i -
    *                                (\boldsymbol{\beta}_{\cdot,i})^{\mathsf{t}}(\mathbf{C}-
    *                                E[\mathbf{C}]) @f]
    *                                where @f$\boldsymbol{\beta}_{\cdot,i}@f$ is
    *                                the @f$i@f$th column of
   ...
   public double averageWithCV(int i) {
      ...
      if (tally == null || tally.numberObs() == 0)
         ...
         for (int j = 0; j < q; j++)
            avg -= beta.getQuick(j, i) * (get(p + j).average() - exp[j]);
         return avg;
      }
   }
    ...
    * @f$\boldsymbol{\Sigma}_{\mathrm{CX}}@f$ with the corresponding matrices of
    *                                         empirical covariances. These are
    *                                         obtained by calling #covarianceX,
    *                                         #covarianceC, and #covarianceCX,
    *                                         respectively. This gives an estimate
    *                                         of @f[
    *                                         \mathrm{Cov}(\mathbf{X}_{\mathrm{C}})
    *                                         = \boldsymbol{\Sigma}_{\mathrm{X}}+
    *                                         \boldsymbol{\beta}^{\mathsf{t}}\boldsymbol{\Sigma}_{\mathrm{C}}\boldsymbol{\beta}-
    *                                         2\boldsymbol{\beta}^{\mathsf{t}}\boldsymbol{\Sigma}_{\mathrm{CX}}. @f]
    * @param covCV the @f$p\times p@f$ matrix that will be filled with covariances.
    */
   public void covarianceWithCV(DoubleMatrix2D covCV) {
      final int p = sizeWithoutCV();
      final int q = getNumControlVariables();
      if (covCV.rows() != p || covCV.columns() != p)  // ???????????
         throw new IllegalArgumentException("Invalid dimensions of covCV, (p,q) = " + p + " " + q);
    ...
    * 
    * @f$\mathbf{X}_{\mathrm{C}}@f$. This is given by @f[
    *                                \mathrm{Cov}({X}_{\mathrm{C},i},
    *                                {X}_{\mathrm{C},j}) =\mathrm{Cov}(X_i, X_j) +
    *                                (\boldsymbol{\beta}_{\cdot,
    *                                i})^{\mathsf{t}}\boldsymbol{\Sigma}_{\mathrm{C}}\boldsymbol{\beta}_{\cdot,
    *                                j} - (\boldsymbol{\Sigma}_{\mathrm{CX}, \cdot,
    *                                i})^{\mathsf{t}}\boldsymbol{\beta}_{\cdot, j}
    *                                - (\boldsymbol{\Sigma}_{\mathrm{CX}, \cdot,
    *                                j})^{\mathsf{t}}\boldsymbol{\beta}_{\cdot,
    *                                i}. @f]
   ...
   public void averageWithCV(double[] a) {
      ...
      if (a.length != p)
         throw new IllegalArgumentException(
               "Invalid length of the given array: given length is " + a.length + ", required length is " + p);
      for (int i = 0; i < p; i++)
         a[i] = averageWithCV(i);
   }
   ...
   public void averageX(double[] a) {
      ...
      if (a.length != l)
         throw new IllegalArgumentException(
               "Invalid length of the given array: given length is " + a.length + ", required length is " + l);
   ...
   /**
    * Fills the given array with the square root of the variance of each component
    * of @f$\mathbf{X}_{\mathrm{C}}@f$.
    */
    ...
    *                                except that the variance with control
    *                                variables, obtained by `covarianceWithCV (i,
    *                                i)`, is used instead of the ordinary variance.
    * @param i               the index of the component.
    * @param level           the level of confidence of the interval.
    * @param centerAndRadius the array that will be filled with the center and
   ...
   /**
    * Estimates the @f$\boldsymbol{\beta}^*@f$ matrix from the observations
    * currently in this list of tallies. This uses #covarianceC(DoubleMatrix2D) and
    * #covarianceCX(DoubleMatrix2D) to get estimates
    * of @f$\boldsymbol{\Sigma}_{\mathrm{C}}@f$, and
    * 
    * @f$\boldsymbol{\Sigma}_{\mathrm{CX}}@f$. The result of the estimation of @f[
    *                                          \boldsymbol{\beta}^* =
    *                                          \boldsymbol{\Sigma}_{\mathrm{C}}^{-1}\boldsymbol{\Sigma}_{\mathrm{CX}} @f]
    *                                          is stored in the matrix returned by
    *                                          #getBeta.
    */
   public void estimateBeta() {
      ...
      assert beta.rows() == getNumControlVariables() && beta.columns() == sizeWithoutCV();
   }

   /**
    * Fills the given list of tallies with controlled observations. The list must
    * have size @f$p@f$, and each element is @f[
    * \mathbf{X}_{\mathrm{C},i}=\mathbf{X}_i -
    * \boldsymbol{\beta}^{\mathsf{t}}(\mathbf{C}_i - E[\mathbf{C}]), @f]
    * where @f$\mathbf{X}_{\mathrm{C},i}@f$ is the @f$i@f$th controlled
    * observation, and @f$\mathbf{X}_i@f$ is the @f$i@f$th observation. This method
    * uses the matrix returned by #getBeta, and the expectations returned by
    * #getExpectedValues to obtain @f$\boldsymbol{\beta}@f$, and
    * 
    ...
    * int p = sizeWithoutCV(); if (a.size() != p) throw new
    * IllegalArgumentException ("Invalid length of the given array of tallies"); if
    * (p == 0) return; int numObs = numberObs(); double[] tmp = new double[p]; for
    * (int obs = 0; obs < numObs; obs++) { for (int i = 0; i < p; i++) { TallyStore
    * tallyStore = (TallyStore)get (i); double v = tallyStore.getArray().getQuick
    * (obs); for (int j = 0; j < q; j++) { tallyStore = (TallyStore)get (j + p);
    * double c = tallyStore.getArray().getQuick (obs); v -= beta.getQuick (j, i)*(c
    * - exp[j]); } tmp[i] = v; } a.add (tmp); } }
    */

   /**
    * This is a variant of #addControlledObservations(ListOfTallies) for the case
    * where @f$p=1@f$. Instead of adding the controlled observations in a list of
    * tallies, the method adds them into a regular tally.
    * 
    * @param ta the tally filled with controlled observations.
    */
   /*
    * public void addControlledObservations (Tally ta) { int p = sizeWithoutCV();
    * int q = getNumControlVariables(); if (p != 1) throw new
    * IllegalArgumentException ("This method can only be called with p=1"); int
    * numObs = get (0).numberObs(); ta.init(); TallyStore tallyStore =
    * (TallyStore)get (0); DoubleArrayList array = tallyStore.getArray(); for (int
    * obs = 0; obs < numObs; obs++) { double v = array.getQuick (obs); for (int j =
    * 0; j < q; j++) { tallyStore = (TallyStore)get (j + p); double c =
    * tallyStore.getArray().getQuick (obs); v -= beta.getQuick (j, 0)*(c - exp[j]);
    * } ta.add (v); } }
    */

   /**
    * Clones this object. This clones the list of tallies as well as the data
    * structures holding the sums of products, the
    * 
    * @f$\boldsymbol{\beta}@f$ matrix, and the @f$\mathbf{C}@f$ vector, but this
    *                          does not clone the tallies comprising the list. The
    *                          created clone is modifiable, even though the
