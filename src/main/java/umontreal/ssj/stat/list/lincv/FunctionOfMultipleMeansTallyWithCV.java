 ...
 * control variables. This extends the function of multiple means tally to use a
 * function @f$h(\boldsymbol{\mu},
 * \boldsymbol{\nu})=g(\boldsymbol{\mu})-\boldsymbol{\beta}_{\mathrm{f}}^{\mathsf{t}}(\mathbf{C}-
 * \boldsymbol{\nu})@f$, where
 * 
 * @f$\boldsymbol{\beta}_{\mathrm{f}}=(\beta_{\mathrm{f},0},…,\beta_{\mathrm{f},q-1})@f$ is
 *                                                                                         a @f$q@f$-dimensional
 *                                                                                         vector.
 *                                                                                         One
 *                                                                                         must
...
public class FunctionOfMultipleMeansTallyWithCV extends FunctionOfMultipleMeansTally {
   ...
   /**
    * Creates a new function of multiple means tally for a function `funcNoCV` of
    * `p` variables, and with `q` control variables. The constructed tally requires
    * vectors of `p+q` values as observations.
    * 
   ...
   /**
    * Constructs a new function of multiple means tally with control variables from
    * the list of tallies `l`, and the function `funcNoCV`.
    * 
   ...
   /**
    * Returns the dimension of this tally excluding the control variables. This
    * corresponds to the result of
    * umontreal.ssj.stat.FunctionOfMultipleMeansTally.getDimension minus the number
   ...
   /**
    * Returns the value of @f$\beta_{\mathrm{f},i}@f$. This is set to 0 by default.
    * 
   ...
   /**
    * Returns the @f$\boldsymbol{\beta}_{\mathrm{f}}@f$ vector. By default, this is
    * set to an array of 0’s.
    * 
   ...
   /**
    * Sets the value of @f$\boldsymbol{\beta}_{\mathrm{f}}@f$ vector to `beta`. The
    * given array must have length @f$q@f$.
    * 
    * @param beta the new @f$\boldsymbol{\beta}_{\mathrm{f}}@f$ vector.
    */
    ...
    * 
    * @f$\boldsymbol{\nu}@f$. This is set to 0 by default, and is equivalent to
    *                         calling `getExpectedValue` on the internal list of
    *                         tallies.
    ...
    * array of 0’s by default, and is equivalent to calling `getExpectedValues`
    * on the internal list of tallies.
    * 
   ...
   /**
    * Sets the vector @f$\boldsymbol{\nu}@f$ to to given array `exp`. The length of
    * `exp` must be @f$q@f$. This is equivalent to calling `setExpectedValues` on
   ...
   /**
    * Uses the sample averages and covariances obtained from the internal list of
    * tallies to estimate the
    * 
    * @f$\boldsymbol{\beta}_{\mathrm{f}}@f$ vector minimizing the variance
    *                                       of @f$h(\bar{\mathbf{X}}_n,
    *                                       \mathbf{C})@f$. The asymptotically
    *                                       optimal vector is @f[
    *                                       \boldsymbol{\beta}_{\mathrm{f}}^* =
    *                                       \boldsymbol{\Sigma}_{\mathrm{C}}^{-1}\boldsymbol{\Sigma}_{\mathrm{CX}}\nabla
    *                                       g(\boldsymbol{\mu}). @f] The method
    *                                       estimates this vector by replacing
    *                                       every quantity by its estimator. It
    *                                       first uses
    *                                       umontreal.ssj.stat.list.lincv.ListOfTalliesWithCV.estimateBeta
    *                                       from the internal list of tallies to
    *                                       get a @f$\boldsymbol{\beta}@f$ matrix,
    *                                       and calls
    *                                       #estimateBetaFromMatrix(DoubleMatrix2D)
    *                                       to get
    *                                       the @f$\boldsymbol{\beta}_{\mathrm{f}}@f$
    *                                       vector.
    */
    ...
    * @f$\boldsymbol{\beta}_{\mathrm{f}}^*@f$ vector minimizing the variance. The
    *                                         matrix is usually obtained by
    *                                         umontreal.ssj.stat.list.lincv.ListOfTalliesWithCV.estimateBeta.
   ...
   /**
    * Clones this object and the function which is stored inside. This clones the
    * internal list of tallies as well as each tally in this list.
    */
      ...
      public double evaluate(double... x) {
         ...
         if (x.length != getDimension())
            throw new IllegalArgumentException(
                  "x has length " + x.length + ", which differs from the dimension " + getDimension());
