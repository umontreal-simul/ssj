 ...
 *      concrete subclass of the @ref umontreal.ssj.stat.StatProbe objects in
 *      it. One then creates an empty list of probes, and fills it with
 *      statistical probes. If the list is not intended to be modified, one can
 *      then use the #setUnmodifiable to prevent any change in the contents of
 *      the list.
 *
 *      Each list of statistical probes can have a global name describing the
 *      contents of its elements, and local names associated with each
 *      individual probe. For example, a list of statistical probes for the
 *      waiting times can have the global name `Waiting times` while the
 *      individual probes have local names `type 1`, `type 2`, etc. These names
 *      are used for formatting reports.
 *
 *      Facilities are provided to fill arrays with sums, averages, etc.
 *      obtained from the individual statistical probes. Methods are also
 *      provided to manipulate the contents of the list. However, one should
 *      always call `init` immediately after adding or removing statistical
 *      probes in the list.
 *
...
public class ListOfStatProbes<E extends StatProbe> implements Cloneable, List<E>, RandomAccess {
   ...
   /**
    * Determines if this list of statistical probes is modifiable, i.e., if probes
    * can be added or removed. Any list of statistical probes is modifiable by
    * default, until one calls the #setUnmodifiable method.
    */
   ...
   /**
    * Forbids any future modification to this list of statistical probes. After
    * this method is called, any attempt to modify the list results in an
    * exception. Setting a list unmodifiable can be useful if some data structures
    ...
    * umontreal.ssj.stat.StatProbe.sum, and stores the results into the array `s`.
    * This method throws an exception if the size of `s` mismatches with the size
    * of the list.
    * 
    * @param s the array to be filled with sums.
    * @exception NullPointerException     if `s` is `null`.
    * @exception IllegalArgumentException if `s.length` does not correspond to
    *                                     #size.
    */
    ...
    * umontreal.ssj.stat.StatProbe.average, and stores the results into the array
    * `a`. This method throws an exception if the size of `s` mismatches with the
    * size of the list.
    * 
    * @param a the array to be filled with averages.
    * @exception NullPointerException     if `a` is `null`.
    * @exception IllegalArgumentException if `a.length` does not correspond to
    *                                     #size.
    */
   ...
   /**
    * Determines if this list of statistical probes is collecting values. Each
    * probe of the list could or could not be collecting values. The default is
    * `true`.
    * 
   ...
   /**
    * Determines if this list of statistical probes is broadcasting observations to
   ...
   /**
    * Adds the observation listener `l` to the list of observers of this list of
    * statistical probes.
    * 
   ...
   /**
    * Removes the observation listener `l` from the list of observers of this list
   ...
   /**
    * Removes all observation listeners from the list of observers of this list of
    * statistical probes.
    */
   ...
   /**
    * Formats a report for each probe in the list of statistical probes. The
    * returned string is constructed by using `StatProbe.report (getName(), this)`.
    * 
    ...
    * not clone all the probes in the list. The created clone is modifiable, even
