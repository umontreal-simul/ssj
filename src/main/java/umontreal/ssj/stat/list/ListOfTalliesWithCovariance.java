...
/**
 * Extends @ref ListOfTallies to add support for the computation of the sample
 * covariance between each pair of elements in a list, without storing all
 * observations. This list of tallies contains internal structures to keep track
 * of @f$\bar{X}_{n, i}@f$ for @f$i=0, …, d-1@f$, and @f$\sum_{k=0}^{n-1}
 * (X_{i, k} - \bar{X}_{k, i})(X_{j, k} - \bar{X}_{k, j})/n@f$, for @f$i=0,…,
 * d-2@f$ and @f$j=1,…,d-1@f$, with @f$j>i@f$. Here,
 * 
 * @f$\bar{X}_{n, i}@f$ is the @f$i@f$th component of
 * @f$\bar{\mathbf{X}}_n@f$, the average vector, and @f$\bar{X}_{0, i}=0@f$
 *                           for @f$i=0,…,d-1@f$. The value @f$X_{i,k}@f$
 *                           corresponds to the @f$i@f$th component of
 *                           the @f$k@f$th observation @f$\mathbf{X}_k@f$. These
 *                           sums are updated every time a vector is added to
 *                           this list, and are used to estimate the
 *                           covariances.
 *
 *                           Note: the size of the list of tallies must remain
 *                           fixed because of the data structures used for
 *                           computing sample covariances. As a result, the
 *                           first call to `init` makes this list unmodifiable.
 *
 *                           Note: for the sample covariance to be computed
 *                           between a pair of tallies, the number of
 *                           observations in each tally should be the same. It
 *                           is therefore recommended to always add complete
 *                           vectors of observations to this list. Moreover, one
 *                           must use the #add method in this class to add
 *                           vectors of observations for the sums used for
 *                           covariance estimation to be updated correctly.
 *                           Failure to use this method, e.g., adding
 *                           observations to each individual tally in the list,
 *                           will result in an incorrect estimate of the
 *                           covariances, unless the tallies in the list can
 *                           store observations. For example, the following
 *                           code, which adds the vector `v` in the list of
 *                           tallies `list`, works correctly only if the list
 *                           contains instances of
 * @ref umontreal.ssj.stat.TallyStore:
 *
 * @code
 *
 *       for (int i = 0; i < v.length; i++) list.get (i).add (v[i]);
 *
 *       @endcode
 *
...
public class ListOfTalliesWithCovariance<E extends Tally> extends ListOfTallies<E> {
   ...
   /**
    * Creates an empty list of tallies with covariance support. One must fill the
    * list with tallies, and call #init before adding any observation.
    */
   ...
   /**
    * Creates an empty list of tallies with covariance support and name `name`. One
    * must fill the list with tallies, and call #init before adding any
    * observation.
    * 
    ...
    * must use this method instead of adding observations to individual tallies to
    * get a covariance estimate.
    * 
      ...
      if (isStable) {
         ...
         for (int i1 = 0; i1 < l - 1; i1++)
            for (int i2 = i1 + 1; i2 < l; i2++)
               curSum2[i1][i2 - i1 - 1] += (numObs - 1) * (x[i1] - curAverages[i1]) * (x[i2] - curAverages[i2])
                     / numObs;
         for (int i = 0; i < l; i++)
   ...
   public double covariance(int i, int j) {
      ...
      if (n != tallyj.numberObs()) {
         log.logp(Level.WARNING, "ListOfTalliesWithCovariance", "covariance",
               "Tally " + i + ", with name " + tallyi.getName() + ", contains " + n + " observations while " + "tally "
                     + j + ", with name " + tallyj.getName() + ", contains " + tallyj.numberObs() + "observations");
         return Double.NaN;
      }
   ...
   /**
    * Clones this object. This clones the list of tallies and the data structures
    * holding the sums of products but not the tallies comprising the list. The
    * created clone is modifiable, even though the original list is unmodifiable.
    */
