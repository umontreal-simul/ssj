 ...
 *       let @f$\{\bar{\mathbf{X}}_n, n \ge0\}@f$ be a sequence of vectors
 ...
 *                              are defined using an implementation of @ref
 *                              umontreal.ssj.util.MultivariateFunction.
 *
 *                              This class defines the methods {@link #add()
 *                              add(double[])} for adding vectors of
 *                              observations to the tally, {@link #average()
 *                              average()} for estimating @f$\nu@f$, and
 *                              {@link #confidenceIntervalDelta()
 *                              confidenceIntervalDelta(double, double[])} for
 *                              applying the delta theorem to compute a
 *                              confidence interval on @f$\nu@f$. It uses an
 *                              internal
 * @ref umontreal.ssj.stat.list.ListOfTalliesWithCovariance instance to manage
 *      the tallies and covariance estimation.
 *
...
public class FunctionOfMultipleMeansTally extends StatProbe implements Cloneable {
   ...
   /**
    * Constructs a function of multiple means tally with dimension `d`, and
    * function `func`.
    * 
   ...
   /**
    * Constructs a function of multiple means tally with name `name`, dimension
    * `d`, and function `func`. The given name is also used as a global name for
   ...
   /**
    * Constructs a function of multiple means tally using the function `func` and
    * the list of tallies `ta` for observation management and covariance
    * estimation.
    * 
    * @param func the function being computed.
    ...
    * returned list must always have the same number of observations. Therefore, it
    * is not recommended to manually add observations to the individual tallies, by
    * using umontreal.ssj.stat.Tally.add(double).
    * 
   ...
   /**
    * Adds a new vector <tt>x = </tt>@f$(X_0, …, X_{d-1})@f$ of observations to
    * this tally. If statistical collecting is turned ON, this adds the vector to
    * the internal list of tallies. This method takes a variable number of
    * arguments to allow one to pass the components of @f$\mathbf{X}@f$ without
    * creating an array when the dimension is fixed. However, one can also give an
    * array to this method. For example, if the computed function is a ratio, for
    * estimating a ratio of two expectations, @f$d=2@f$ and the call `add(x, y)` is
    * equivalent to <tt>add(new double[] \{x, y\})</tt>. On the other hand, if the
    * function is a sum or a product of @f$d@f$ terms, the dimension is not
    * constant in general, e.g., it can depend on an input parameter. In this case,
    * one must pass an array to this method, and could also call the `add` method
    * of the internal list of tallies directly.
    * 
    * @param x the vector of observations being added.
    * @exception NullPointerException     if `x` is `null`.
    * @exception IllegalArgumentException if the length of `x` does not correspond
    *                                     to the dimension of this tally.
    */
   public void add(double... x) {
      if (x.length != ta.size())
         throw new IllegalArgumentException("Incompatible length of vectors of observations: given length is "
               + x.length + ", but required length is " + ta.size());
      if (collect)
         ta.add(x);
   }

   /**
    * Returns the number of vectors of observations given to this probe since its
    * last initialization.
    * 
   ...
   /**
    * Computes @f$\bar{\nu}_n=g(\bar{\mathbf{X}}_n)@f$, an estimate of the function
    * of means @f$\nu@f$. Note that if @f$g(\mathbf{X})@f$ is non-linear,
    * the @f$\bar{\nu}_n@f$ estimator is biased but
    * consistent: @f$g(\bar{\mathbf{X}}_n)\to g(\boldsymbol{\mu})@f$ if
    * 
    * @f$n\to\infty@f$. @return the estimation of @f$\nu@f$.
    */
   ...
   /**
    * Estimates @f$n\mathrm{Var}(g(\bar{\mathbf{X}}_n))@f$ where @f$n@f$ is the
    * number of vectors of observations given to this collector since the last
    * initialization. Assuming that, as @f$n\to\infty@f$,
    * 
    * @f$\sqrt{n}(\bar{\mathbf{X}}_n - \boldsymbol{\mu})@f$ converges to a random
    *                                vector @f$\mathbf{Y}@f$ with
    *                                mean @f$\mathbf{0}@f$ and covariance
    *                                matrix @f$\boldsymbol{\Sigma}@f$
    *                                (@f$\boldsymbol{\Sigma}@f$ is also the
    *                                covariance matrix of
    * @f$\mathbf{X}@f$), the delta theorem @cite tSER80a&thinsp; shows
    *                    that @f$\sqrt{n}(g(\bar{\mathbf{X}}_n) -
    *                    g(\boldsymbol{\mu}))\Rightarrow(\nabla
    *                    g(\boldsymbol{\mu}))^{\mathsf{t}}\mathbf{Y}@f$. As a
    *                    result,
    ...
    *                                                                             g(\boldsymbol{\mu}))^{\mathsf{t}}\mathbf{Y}]=(\nabla
    *                                                                             g(\boldsymbol{\mu}))^{\mathsf{t}}\boldsymbol{\Sigma}\nabla
    *                                                                             g(\boldsymbol{\mu})=\sigma^2@f$.
    *                                                                             Here, @f$\nabla
    *                                                                             g(\boldsymbol{\mu})@f$
    *                                                                             is
    *                                                                             the
    *                                                                             gradient
    *                                                                             of
    * @f$g(\boldsymbol{\mu})@f$. This method computes @f[ S_n^2 = (\nabla
    *                            g(\bar{\mathbf{X}}_n))^{\mathsf{t}}\mathbf{S}_n\nabla
    *                            g(\bar{\mathbf{X}}_n), @f]
    *                            where @f$\mathbf{S}_n@f$ is the matrix of
    *                            empirical covariances of
   ...
   public double variance() {
      if (ta.numberObs() < 2) {
         ...
         log.logp(Level.WARNING, "FunctionOfMultipleMeansTally", "variance", "FunctionOfMultipleMeansTally " + name
               + ":   calling variance() with " + ta.numberObs() + " observation");
         return Double.NaN;
      }
      if (temp.length != ta.size()) {
         // The dimension of the internal tally could (rarely) change.
    ...
    * @cite tSER80a&thinsp;. Returns, in elements 0 and 1 of the array object
    *       `centerAndRadius[]`, the center and half-length (radius) of a
    ...
    * @f$\bar{\mathbf{X}}_n@f$ has the @f$d@f$-dimensional normal distribution,
    *                          which is approximately true if @f$n@f$ is large
    *                          enough. With this assumption,
    *                          as @f$n\to\infty@f$, @f[ \sqrt{n}(\bar{\mathbf{X}}_n
    *                          - \boldsymbol{\mu}) @f] converges to a random
    *                          vector @f$\mathbf{Y}@f$ following the
    * @f$d@f$-dimensional normal distribution with mean @f$\mathbf{0}@f$ and
    *                     covariance matrix @f$\boldsymbol{\Sigma}@f$. According to
    *                     the delta theorem, @f[ \sqrt{n}(g(\bar{\mathbf{X}}_n) -
    *                     g(\boldsymbol{\mu}))\Rightarrow(\nabla
    *                     g(\boldsymbol{\mu}))^{\mathsf{t}}\mathbf{Y}, @f] which
    *                     follows the normal distribution with mean 0 and
    *                     variance @f[ \sigma^2=(\nabla
    *                     g(\boldsymbol{\mu}))^{\mathsf{t}}\boldsymbol{\Sigma}\nabla
    *                     g(\boldsymbol{\mu}). @f] This variance is estimated
    *                     by @f$S_n^2@f$, which replaces
    * @f$\boldsymbol{\mu}@f$ by @f$\bar{\mathbf{X}}_n@f$, and
    * @f$\boldsymbol{\Sigma}@f$ by a matrix of sample covariances computed by @ref
    *                           umontreal.ssj.stat.list.ListOfTallies.
    *
    *                           The confidence interval is computed based on the
    *                           statistic @f[ Z = {\sqrt{n}(g(\bar{\mathbf{X}}_n) -
    *                           g(\boldsymbol{\mu}))\over{S_n}} @f] where @f$n@f$
    *                           is the number of vectors of observations given to
    *                           this collector since its last
    *                           initialization, @f$g(\bar{\mathbf{X}}_n) =@f$&nbsp;
    *                           #average is the average of these observations,
    *                           and @f$S_n =@f$&nbsp; #standardDeviation is the
    *                           standard deviation. Under the previous
    *                           assumptions, @f$Z@f$ has the normal distribution
    *                           with mean&nbsp;0 and variance&nbsp;1. The
    *                           confidence interval given by this method is valid
    *                           *only if* these assumptions are approximately
    *                           verified.
    * @param level           desired probability that the (random) confidence
    *                        interval covers the true function of means (a
    *                        constant).
    * @param centerAndRadius array of size 2 in which are returned the center and
    *                        radius of the confidence interval, respectively.
    */
   ...
   /**
    * Similar to #confidenceIntervalDelta(double,double[]), but returns the
    * confidence interval in a formatted string of the form "<tt>95%
    * confidence interval for function of means: (32.431,  32.487)</tt>",
    * using @f$d@f$ decimal digits of accuracy.
    * 
    * @param level desired probability that the confidence interval covers the true
    *              function of means.
   ...
   /**
    * Returns a string containing a formatted report on this probe. The report
    * contains the function of averages, the standard deviation of
    * 
    * @f$g(\mathbf{X})@f$, as well as the number of vectors of observations.
   ...
   /**
    * Returns a string containing a formatted report on this probe (as in #report
    * ), followed by a confidence interval (as in #formatCIDelta(double,int) ).
    * 
    * @param level desired probability that the confidence interval covers the true
    *              mean.
    * @param d     the number of decimal digits of accuracy.
    * @return statistical report with a confidence interval, formatted as a string.
    */
   ...
   /**
    * Same as {@link #reportAndCIDelta(double,int) reportAndCIDelta(level, 3)}.
    * 
    * @param level desired probability that the confidence interval covers the true
    *              mean.
    * @return statistical report with a confidence interval, formatted as a string.
   ...
   /**
    * Formats and returns a short statistical report for this function of multiple
    * means tally. The returned single-line report contains the function of
    * averages @f$g(\bar{\mathbf{X}}_n)@f$, and the standard deviation @f$S_n@f$,
    * in that order, separated by three spaces. If the number of observations is
    * included in short reports, a column containing the number of observations is
    * added.
    * 
    ...
    * 
    * @return desired probability that the (random) confidence interval covers the
    *         true mean (a constant)
    */
    ...
    * reports.
    * 
    * @param level desired probability that the (random) confidence interval covers
    ...
    * formatted by #report, and #shortReport. This restores the default behavior of
   ...
   /**
    * Indicates that a confidence interval on the true mean, based on the delta and
    * central limit theorems, needs to be included in reports formatted by #report,
    * and #shortReport. The confidence interval is formatted using #formatCIDelta.
    */
   ...
   /**
    * Clones this object. This clones the internal list of tallies as well as each
    * tally in this list.
    */
