...
/**
 * This class is similar to @ref TallyHistogram, except that it does not
 * maintain the min, max, average, and variance of the observations. Only the
 * counters for the histogram are maintained. Also it does not maintain counters
 * for virtual bins on the left and on the right to count the observations that
 * fall outside @f$[a,b]@f$. The methods that are supposed to return these
 * values return an error message instead. The only advantage of not maintaining
 * these counters and values is to increase speed (slightly). We define this
 * class as an extension of @ref TallyHistogram to avoid duplicating code.
 *
...
public class HistogramOnly extends TallyHistogram implements Cloneable {
    ...
    *             counter is increased by 1. There are two extra bins (and
    *             counters) that count the number of observations @f$x@f$ that fall
    *             outside the interval @f$[a,b]@f$: one for those @f$x< a@f$, and
    *             the other for those @f$x > b@f$.
   ...
   /**
    * Gives a new observation @f$x@f$ to the statistical collectors. Increases by 1
    * the bin counter in which value @f$x@f$ falls.
    * 
