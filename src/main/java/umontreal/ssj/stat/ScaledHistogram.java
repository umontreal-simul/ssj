...
/**
 * This class provides histograms for which the bin counts (heights of
 * rectangles) are replaced by real-valued frequencies (in `double`) and can be
 * rescaled. The histogram is over a bounded interval @f$[a,b]@f$ and has a
 * fixed number of bins of equal width @f$h@f$. The frequencies can be chosen
 * (rescaled) so that the integral of the histogram is equal to a specific
 * value. If this value is taken as 1, i.e., if @f$h@f$ times the sum of
 * frequencies is equal to 1, then the histogram can be seen as a density
 * estimator for a density whose support is contained in @f$[a,b]@f$. If part of
 * the density is outside @f$[a,b]@f$, then the integral of the estimated
...
public class ScaledHistogram {
    ...
    * which is divided into `numBins` bins of equal width. Is initializes the
    * frequency of each bin to 0.
    * 
    * @param a       left boundary of interval
    * @param b       right boundary of interval
   ...
   /**
    * Initializes this `ScaledHistogram` using the @ref TallyHistogram `hist`. It
    * uses the same interval @f$[a,b]@f$, same bins, and rescaled the counters so
    * the integral of the histogram equal the value specified by `integral`.
    */
   ...
   /**
    * Rescales the histogram by renormalizing the frequencies so its integral has
    ...
    * is over a close interval @f$[a,b]@f$ and is rescaled differently for the
    ...
    * to sum to 1; they are rescaled so the sum of weights that go to any given bin
    * is 1.
    */
   ...
   /**
    * Similar to `averageShiftedHistogramTrunc`, except that uses a weighted
    * average. For the new average in a given bin, any neighbor bin at
    ...
    * the @f$U(0,1)@f$ distribution. Assumes the histogram integrates to 1.
    */
