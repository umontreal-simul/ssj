 ...
 * individual observations, but only their number, sum, sum of squares, maximum,
 * and minimum. The subclass @ref TallyStore offers a collector that memorizes
 * the observations.
 *
...
public class Tally extends StatProbe implements Cloneable {
    ...
    * observers is activated for this object, this method also transmits the new
    * information to the registered observers by invoking the method
    * #notifyListeners.
    * 
   ...
   /**
    * Computes a confidence interval on the mean. Returns, in elements 0 and 1 of
    * the array object `centerAndRadius[]`, the center and half-length (radius) of
    * a confidence interval on the true mean of the random variable @f$X@f$, with
    * confidence level `level`, assuming that the @f$n@f$ observations given to
    * this collector are independent and identically distributed (i.i.d.) copies
    * of @f$X@f$, and that @f$n@f$ is large enough for the central limit theorem to
    * hold. This confidence interval is computed based on the statistic @f[ Z =
    * {\bar{X}_n - \mu\over{S_{n,x}/\sqrt{n}}} @f] where @f$n@f$ is the number of
    * observations given to this collector since its last
    * initialization, @f$\bar{X}_n =@f$ `average()` is the average of these
    * observations, @f$S_{n,x} =@f$ `standardDeviation()` is the empirical standard
    * deviation. Under the assumption that the observations of @f$X@f$ are i.i.d.
    * and @f$n@f$ is large, @f$Z@f$ has the standard normal distribution. The
    * confidence interval given by this method is valid *only if* this assumption
    ...
    * the array object `centerAndRadius[]`, the center and half-length (radius) of
    * a confidence interval on the true mean of the random variable @f$X@f$, with
    * confidence level `level`, assuming that the observations given to this
    * collector are independent and identically distributed (i.i.d.) copies
    * of @f$X@f$, and that @f$X@f$ has the normal distribution. This confidence
    * interval is computed based on the statistic @f[ T = {\bar{X}_n -
    * \mu\over{S_{n,x}/\sqrt{n}}} @f] where @f$n@f$ is the number of observations
    * given to this collector since its last initialization, @f$\bar{X}_n =@f$
    * `average()` is the average of these observations, @f$S_{n,x} =@f$
    * `standardDeviation()` is the empirical standard deviation. Under the
    * assumption that the observations of @f$X@f$ are i.i.d. and normally
    * distributed, @f$T@f$ has the Student distribution with @f$n-1@f$ degrees of
    * freedom. The confidence interval given by this method is valid *only if* this
    * assumption is approximately verified, or if @f$n@f$ is large enough so
    * that @f$\bar{X}_n@f$ is approximately normally distributed.
    * 
    * @param level           desired probability that the (random) confidence
    *                        interval covers the true mean (a constant)
    * @param centerAndRadius array of size 2 in which are returned the center and
   ...
   /**
    * Similar to #confidenceIntervalNormal. Returns the confidence interval in a
   ...
   /**
    * Similar to #confidenceIntervalStudent. Returns the confidence interval in a
    * formatted string of the form<br>
    * <center>"<tt>95%
   ...
   /**
    * Computes a confidence interval on the variance. Returns, in elements 0 and 1
    * of array `interval`, the left and right boundaries
    * 
    * @f$[I_1,I_2]@f$ of a confidence interval on the true variance
    * @f$\sigma^2@f$ of the random variable @f$X@f$, with confidence level `level`,
    *                assuming that the observations given to this collector are
    *                independent and identically distributed (i.i.d.) copies
    *                of @f$X@f$, and that @f$X@f$ has the normal distribution. This
    *                confidence interval is computed based on the statistic @f$
    *                \chi^2_{n-1} = (n-1)S^2_n/\sigma^2 @f$ where @f$n@f$ is the
    *                number of observations given to this collector since its last
    *                initialization, and @f$S^2_n =@f$ `variance()` is the
    *                empirical variance of these observations. Under the assumption
    *                that the observations of @f$X@f$ are i.i.d. and normally
    *                distributed, @f$\chi^2_{n-1}@f$ has the chi-square
    *                distribution with @f$n-1@f$ degrees of freedom. Given the
    *                `level` @f$ = 1 - \alpha@f$, one has @f$P[\chi^2_{n-1} < x_1]
    *                = P[\chi^2_{n-1} > x_2] = \alpha/2@f$ and @f$[I_1,I_2] =
    *                [(n-1)S^2_n/x_2,\; (n-1)S^2_n/x_1]@f$.
    * @param level    desired probability that the (random) confidence interval
    *                 covers the true mean (a constant)
    * @param interval array of size 2 in which are returned the left and right
   ...
   /**
    * Similar to #confidenceIntervalVarianceChi2. Returns the confidence interval
    * in a formatted string of the form <br>
    * <center>"<tt>95.0%
    ...
    * confidence interval level `level` using @f$d@f$ fractional decimal digits.
    * 
   ...
   /**
    * Formats and returns a short statistical report for this tally. The returned
    * single-line report contains the minimum value, the maximum value, the
    * average, the variance, and the standard deviation, in that order, separated
    * by three spaces. If the number of observations is shown in the short report,
    * a column containing the number of observations in this tally is added.
    * 
    ...
    * #report ), followed by a confidence interval (as in #formatCIStudent ),
    * using @f$d@f$ fractional decimal digits.
    * 
    * @param level desired probability that the (random) confidence interval covers
    *              the true mean (a constant)
    * @param d     number of fractional decimal digits
   ...
   /**
    * Indicates that a confidence interval on the true mean, based on the central
    * limit theorem, needs to be included in reports formatted by #report and
    * #shortReport. The confidence interval is formatted using #formatCINormal.
    */
   ...
   /**
    * Indicates that a confidence interval on the true mean, based on the normality
    * assumption, needs to be included in reports formatted by #report and
    * #shortReport. The confidence interval is formatted using #formatCIStudent.
    */
