 ...
 * 
 * @f$p@f$, where @f$n@f$ is a positive integer and @f$0\le p\le1@f$. Its mass
 *          function is given by
 * @anchor REF_probdist_BinomialDist_eq_fmass_binomial @f[ p(x) = \binom{n}{x}
 *         p^x (1-p)^{n-x} = \frac{n!}{x!(n-x)!}\; p^x (1-p)^{n-x}
 *         \qquad\mbox{for } x = 0, 1, 2,…n, \tag{fmass-binomial} @f] and its
 *         distribution function is
 * @anchor REF_probdist_BinomialDist_eq_Fbinomial @f[ F (x) = \sum_{j=0}^x
 *         \binom{n}{j}\; p^j (1-p)^{n-j} \qquad\mbox{for } x = 0, 1, 2, …n,
 *         \tag{Fbinomial} @f] where nCr@f$(n,x)@f$ is the number of possible
 *         combinations of @f$x@f$ elements chosen among a set of @f$n@f$
 *         elements.
 *
...
public class BinomialDist extends DiscreteDistributionInt {
    ...
    * {@link REF_probdist_BinomialDist_eq_fmass_binomial fmass-binomial} ),
    * for @f$0\le x\le n@f$, and the corresponding cumulative function. These
    * values are computed and stored in dynamic arrays, unless @f$n@f$ exceeds
    * `MAXN`.
    */
   ...
   /**
    * A generalization of the previous method. Computes and returns the binomial
    * term
    * 
    * @anchor REF_probdist_BinomialDist_eq_fmass_binom @f[ f(x) = \binom{n}{x} p^x
    *         q^{n-x} = \frac{n!}{x!(n-x)!}\; p^x q^{n-x}, \tag{fmass-binom} @f]
    *         where @f$p@f$ and @f$q@f$ are arbitrary real numbers (@f$q@f$ is not
    *         necessarily equal to @f$1-p@f$). In the case where @f$0 \le p \le1@f$
    *         and @f$q = 1-p@f$, the returned value is a probability term for the
   ...
   /**
    * Computes @f$F(x)@f$, the distribution function of a binomial random variable
    * with parameters @f$n@f$ and @f$p@f$, evaluated at @f$x@f$. If @f$n
    * \le10000@f$, the non-negligible terms of the sum are added explicitly.
    * If @f$n > 10000@f$ and @f$np (1-p) > 100@f$, the Camp-Paulson normal
    * approximation @cite tCAM51a,
    * 
    * @cite tMOL70a&thinsp; is used, otherwise, a Poisson approximation due to
    *       Bol’shev @cite tBOL64a, @cite tMOL70a&thinsp; is used.
    */
...
//      final double EPS = DiscreteDistributionInt.EPSILON * EPS_EXTRA;
    ...
    * function. The default implementation returns `1 - cdf(x-1)`, which is not
    * accurate when @f$F(x)@f$ is close to 1.
    */
   ...
   /**
    * Computes @f$x = F^{-1}(u)@f$, the inverse of the binomial distribution.
    * If @f$n@f$ is larger than 10000, the linear search starts from 0 and the
    * #cdf(int,double,int) static method is used to compute @f$F(x)@f$ at different
    * values of @f$x@f$, which is much less efficient.
    */
   ...
   /**
    * Estimates the parameters @f$(n,p)@f$ of the binomial distribution using the
    * maximum likelihood method, from the @f$m@f$ observations
    * 
    ...
    * @f{align*}{ \hat{n}\hat{p} & = \bar{x}_m \\ \sum_{i=0}^{R-1}
    *             \frac{f_i}{\hat{n} - i} & = - m \ln\left(1 -
    *             \frac{\bar{x}_m}{\hat{n}}\right)
    * @f} where @f$\bar{x}_m@f$ is the average of @f$x[0],…,x[m-1]@f$,
   ...
   /**
    * Estimates the parameter @f$p@f$ of the binomial distribution with given
    * (fixed) parameter @f$n@f$, by the maximum likelihood method, from the @f$m@f$
    * observations @f$x[i]@f$, @f$i = 0, 1,…, m-1@f$. Returns the estimator in an
    ...
    * parameter @f$n@f$, and with parameter @f$p@f$ estimated by the maximum
    * likelihood method based on the @f$m@f$ observations
    * 
   ...
   /**
    * Computes the variance @f$\mbox{Var}[X] = np(1 - p)@f$ of the binomial
    * distribution with parameters @f$n@f$ and @f$p@f$.
    * 
    * @return the variance of the binomial distribution @f$\mbox{Var}[X] = np(1 -
    *         p)@f$
    */
   ...
   /**
    * Returns a table that contains the parameters @f$(n,p)@f$ of the current
    * distribution, in regular order: [@f$n@f$, @f$p@f$].
    */
   ...
   /**
    * Resets the parameters to these new values and recomputes everything as in the
    * constructor. From the performance viewpoint, it is essentially the same as
    * constructing a new @ref BinomialDist object.
    */
