...
/**
 * This class extends the class @ref ContinuousDistribution for the *Rayleigh*
 * distribution @cite tEVA00a&thinsp; with location parameter
 * 
 * @f$a@f$, and scale parameter @f$\beta> 0@f$. The density function is
 * @anchor REF_probdist_RayleighDist_eq_frayleigh @f[ f(x) =
 *         \frac{(x-a)}{\beta^2}  e^{-(x-a)^2/(2\beta^2)} \qquad\mbox{for } x
 *         \ge a, \tag{frayleigh} @f] and @f$f(x) = 0@f$ for @f$x < a@f$. The
 *         distribution function is
 * @anchor REF_probdist_RayleighDist_eq_Frayleigh @f[ F(x) = 1 - e^{-(x -
 *         a)^2/(2\beta^2)} \qquad\mbox{for } x \ge a, \tag{Frayleigh} @f] and
 *         the inverse distribution function is
 * @anchor REF_probdist_RayleighDist_eq_Invrayleigh @f[ F^{-1}(u) = x = a +
 *         \beta\sqrt{-2\ln(1-u)} \qquad\mbox{for } 0 \le u \le1.
 *         \tag{Invrayleigh} @f] <div class="SSJ-bigskip"></div>
 *
...
public class RayleighDist extends ContinuousDistribution {
   ...
   /**
    * Computes the density function ( {@link REF_probdist_RayleighDist_eq_frayleigh
    * frayleigh} ).
    * 
    ...
    * {@link REF_probdist_RayleighDist_eq_Invrayleigh Invrayleigh} ).
    * 
    * @param a    the location parameter
    * @param beta the scale parameter
   ...
   /**
    * Estimates the parameter @f$\beta@f$ of the Rayleigh distribution using the
    * maximum likelihood method, assuming that @f$a@f$ is known, from the @f$n@f$
    * observations @f$x[i]@f$, @f$i = 0, 1, …, n-1@f$. The estimate is returned
    * in a one-element array: [@f$\hat{\beta}@f$]. The maximum likelihood estimator
    * is the value
    * 
    * @f$\hat{\beta}@f$ that satisfies the equation @f[ \hat{\beta} =
    *                   \sqrt{\frac{1}{2n}\sum_{i=1}^n x_i^2} @f]
    ...
    * 
    * @f$a@f$ and @f$\hat{\beta}@f$. This last is estimated using the maximum
   ...
   /**
    * Returns the mean @f$a + \beta\sqrt{\pi/2}@f$ of the Rayleigh distribution
    * with parameters @f$a@f$ and @f$\beta@f$.
    * 
    * @param a    the location parameter
    * @param beta the scale parameter
    ...
    * 
    * @f$\beta@f$. @param beta the scale parameter
   ...
   /**
    * Returns the standard deviation @f$\beta\sqrt{2 - \pi/2}@f$ of the Rayleigh
    * distribution with parameter @f$\beta@f$.
    * 
    * @param beta the scale parameter
   ...
   /**
    * Return an array containing the parameters of the current distribution in the
    * order: [@f$a@f$, @f$\beta@f$].
    * 
    ...
    * 
    * @return a `String` containing information about the current distribution
    */
