 ...
 * @anchor REF_probdist_InverseGaussianDist_eq_fInverseGaussian @f[ f(x) =
 *         \sqrt{\frac{\lambda}{2\pi x^3}}\; e^{{-\lambda(x -
 *         \mu)^2}/{(2\mu^2x)}}, \qquad\mbox{for } x > 0.
 *         \tag{fInverseGaussian} @f] The distribution function is given by
 * @anchor REF_probdist_InverseGaussianDist_eq_FInverseGaussian @f[ F(x) =
 *         \Phi\left(\sqrt{\frac{\lambda}{x}}\left(\frac{x}{\mu} -
 *         1\right)\right) +
 *         e^{({2\lambda}/{\mu})}\Phi\left(-\sqrt{\frac{\lambda}{x}}\left(\frac{x}{\mu}
 *         + 1\right)\right), \tag{FInverseGaussian} @f] where @f$\Phi@f$ is the
...
public class InverseGaussianDist extends ContinuousDistribution {
    ...
    * {@link REF_probdist_InverseGaussianDist_eq_fInverseGaussian fInverseGaussian}
    ...
    * {@link REF_probdist_InverseGaussianDist_eq_FInverseGaussian FInverseGaussian}
    * ) of the inverse gaussian distribution with parameters @f$\mu@f$
    * and @f$\lambda@f$, evaluated at @f$x@f$.
    */
   ...
   /**
    * Computes the complementary distribution function of the inverse gaussian
    * distribution with parameters @f$\mu@f$ and @f$\lambda@f$, evaluated
    * at @f$x@f$.
    */
   ...
   /**
    * Estimates the parameters @f$(\mu, \lambda)@f$ of the inverse gaussian
    ...
    *         [@f$\mu@f$, @f$\lambda@f$]. The maximum likelihood estimators are the
    *         values @f$(\hat{\mu}, \hat{\lambda})@f$ that satisfy the equations:
    * @f{align*}{ \hat{\mu} & = \bar{x}_n \\ \frac{1}{\hat{\lambda}} & =
    *             \frac{1}{n} \sum_{i=1}^n \left(\frac{1}{x_i} -
    *             \frac{1}{\hat{\mu}}\right),
    ...
    * parameters @f$\mu@f$ and @f$\lambda@f$ estimated using the maximum likelihood
   ...
   /**
    * Returns the mean @f$E[X] = \mu@f$ of the inverse gaussian distribution with
    * parameters @f$\mu@f$ and @f$\lambda@f$.
    * 
    * @return the mean of the inverse gaussian distribution @f$E[X] = \mu@f$
    */
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = \mu^3/\lambda@f$ of the
    * inverse gaussian distribution with parameters @f$\mu@f$ and @f$\lambda@f$.
    * 
    ...
    * table is put in regular order: [@f$\mu@f$, @f$\lambda@f$].
    */
