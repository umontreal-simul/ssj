...
/**
 * Extends the class @ref ContinuousDistribution for the *Kolmogorov–Smirnov*+
 * distribution (see @cite tDAR60a, @cite tDUR73a,
 * 
 ...
 * @f{align}{ D_n^+ & = \max_{1\le j\le n} \left(j/n - U_{(j)}\right), \tag{DNp}
 *            \\ D_n^- & = \max_{1\le j\le n} \left(U_{(j)} - (j-1)/n\right),
 *            \tag{DNm}
 * @f} where the @f$U_{(j)}@f$ are the @f$U_i@f$ sorted in increasing order.
 *     Both statistics follows the same distribution function, i.e. @f$F_n(x) =
...
public class KolmogorovSmirnovPlusDist extends ContinuousDistribution {
    ...
    * 
    * @f$F_n(x)@f$ with parameter @f$n@f$. The distribution function can be
    *              approximated via the following expressions:
    ...
    * @f{align}{ P[D_n^+ \le x] & = 1 - x \sum_{i=0}^{\lfloor n(1-x)\rfloor}
    *            \binom{n}{i} \left(\frac{i}{n} + x \right)^{i-1} \left(1 -
    *            \frac{i}{n} - x \right)^{n-i} \tag{DistKS1} \\ & = x
    *            \sum_{j=0}^{\lfloor nx \rfloor} \binom{n}{j} \left(\frac{j}{n} - x
    *            \right)^j \left(1 - \frac{j}{n} + x \right)^{n-j-1} \tag{DistKS2}
    *            \\ & \approx 1 - e^{-2 n x^2} \left[1 - \frac{2x}{3} \left( 1 -
    *            x\left(1 - \frac{2 n x^2}{3}\right) \right.\right. \nonumber \\ &
    *            \left.\left. - \frac{2}{3n} \left(\frac{1}{5} - \frac{19 n
    *            x^2}{15} + \frac{2n^2 x^4}{3}\right) \right) + O(n^{-2}) \right].
    *            \tag{DistKS3}
    * @f} Formula ( {@link REF_probdist_KolmogorovSmirnovPlusDist_DistKS1 DistKS1}
    *     ) and ( {@link REF_probdist_KolmogorovSmirnovPlusDist_DistKS2 DistKS2} )
    *     can be found in @cite tDUR73a&thinsp;, equations (2.1.12) and (2.1.16),
    *     while ( {@link REF_probdist_KolmogorovSmirnovPlusDist_DistKS3 DistKS3} )
    *     can be found in @cite tDAR60a&thinsp;. Formula (
    *     {@link REF_probdist_KolmogorovSmirnovPlusDist_DistKS2 DistKS2} ) becomes
    *     numerically unstable as @f$nx@f$ increases. The approximation (
    *     {@link REF_probdist_KolmogorovSmirnovPlusDist_DistKS3 DistKS3} ) is
    *     simpler to compute and excellent when @f$nx@f$ is large. The relative
    *     error on @f$F_n(x) = P[D_n^+ \le x]@f$ is always less than @f$10^{-5}@f$.
    */
