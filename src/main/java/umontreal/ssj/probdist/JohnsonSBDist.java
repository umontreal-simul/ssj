 ...
 * @cite tFLY06a&thinsp; with shape parameters @f$\gamma@f$ and @f$\delta> 0@f$,
 *       location parameter @f$\xi@f$, and scale parameter @f$\lambda>0@f$.
 *       Denoting @f$t=(x-\xi)/\lambda@f$ and @f$z = \gamma+
 *       \delta\ln(t/(1-t))@f$, the density is
 * @anchor REF_probdist_JohnsonSBDist_eq_JohnsonSB_density @f[ f(x) =
 *         \frac{\delta e^{-z^2/2}}{\lambda t(1 - t)\sqrt{2\pi}}, \qquad\mbox{
 *         for } \xi< x < \xi+\lambda, \tag{JohnsonSB-density} @f] and 0
 *         elsewhere. The distribution function is
 * @anchor REF_probdist_JohnsonSBDist_eq_JohnsonSB_dist @f[ F(x) = \Phi(z),
 *         \mbox{ for } \xi< x < \xi+\lambda, \tag{JohnsonSB-dist} @f]
 *         where @f$\Phi@f$ is the standard normal distribution function. The
 *         inverse distribution function is
 * @anchor REF_probdist_JohnsonSBDist_eq_JohnsonSB_inverse @f[ F^{-1}(u) = \xi+
 *         \lambda\left(1/\left(1+e^{-v(u)}\right)\right) \qquad\mbox{for } 0
 *         \le u \le1, \tag{JohnsonSB-inverse} @f] where @f[ v(u) =
 *         [\Phi^{-1}(u) - \gamma]/\delta. @f] This class relies on the methods
 *         NormalDist.cdf01 and NormalDist.inverseF01 of @ref NormalDist to
 *         approximate @f$\Phi@f$ and
 * @f$\Phi^{-1}@f$.
 *
...
public class JohnsonSBDist extends JohnsonSystem {
    ...
    * 
    * @f$\gamma@f$ and @f$\delta@f$, location parameter @f$\xi@f$ and scale
    ...
    * {@link REF_probdist_JohnsonSBDist_eq_JohnsonSB_density JohnsonSB-density} ).
    */
   public static double density(double gamma, double delta, double xi, double lambda, double x) {
      ...
      return delta / (lambda * y * (1.0 - y) * Math.sqrt(2.0 * Math.PI)) * Math.exp(-z * z / 2.0);
   }

   /**
    * Returns the distribution function (
    * {@link REF_probdist_JohnsonSBDist_eq_JohnsonSB_dist JohnsonSB-dist} ).
    */
    ...
    * {@link REF_probdist_JohnsonSBDist_eq_JohnsonSB_inverse JohnsonSB-inverse} ).
    */
    ...
    * 
    * @f$S_B@f$ distribution, using the maximum likelihood method, from the @f$n@f$
    ...
    * @f$\hat{\gamma}= -\bar{f} / s_f@f$ and @f$\hat{\delta}= 1/s_f@f$, where @f$f
    *                  = \ln(t/(1-t))@f$, @f$\bar{f}@f$ is the sample mean of
    *                  the @f$f_i@f$, and @f[ s_f = \sqrt{\frac{1}{n}
    *                  \sum_{i=0}^{n-1} (f_i - \bar{f})^2}, @f] with @f$f_i =
    *                  \ln(t_i/(1-t_i))@f$.
    ...
    * likelihood method based on the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1,
    * …, n-1@f$. Given the parameters @f$\xi= \mathtt{xi}@f$ and
    * 
   ...
   /**
    * Returns the mean @cite tJOH49a&thinsp; of the Johnson @f$S_B@f$ distribution
    * with parameters @f$\gamma@f$, @f$\delta@f$,
    * 
   ...
   public static double getVariance(double gamma, double delta, double xi, double lambda) {
      ...
      double tvar = mu * (1 - delta * gamma) - mu * mu + delta / tpsi[0] * (A - B - mu * C * D);
      return lambda * lambda * tvar;

   }

   /**
    * Returns the standard deviation of the Johnson @f$S_B@f$ distribution with
    * parameters @f$\gamma@f$, @f$\delta@f$, @f$\xi@f$,
    * 
    * @f$\lambda@f$. @return the standard deviation of the Johnson @f$S_B@f$
    *                distribution
    */
