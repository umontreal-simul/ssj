 ...
 *          function is
 * @anchor REF_probdist_PascalDist_eq_fmass_pascal @f[ p(x) = \binom{n + x -
 *         1}{x} p^n (1 - p)^x, \qquad\mbox{for } x = 0, 1, 2,
 *         …\tag{fmass-pascal} @f] This @f$p(x)@f$ can be interpreted as the
 ...
 *         success @f$p@f$. For @f$n=1@f$, this gives the *geometric*
 *         distribution.
 *
...
public class PascalDist extends NegativeBinomialDist {
    ...
    * {@link REF_probdist_PascalDist_eq_fmass_pascal fmass-pascal} ) and the
    * distribution function for the Pascal distribution with parameter @f$n@f$
    * and @f$p@f$.
    */
   ...
   /**
    * Estimates the parameter @f$(n, p)@f$ of the Pascal distribution using the
    ...
    * @f{align*}{ \frac{\hat{n}(1 - \hat{p})}{\hat{p}} & = \bar{x}_m \\ \ln(1 +
    *             \hat{p}) & = \sum_{j=1}^{\infty} \frac{F_j}{(\hat{n} + j - 1)}
    * @f} where @f$\bar{x}_m@f$ is the average of @f$x[0],…,x[m-1]@f$, and
    * @f$F_j = \sum_{i=j}^{\infty} f_i@f$ = proportion of @f$x@f$’s which are
    *        greater than or equal to @f$j@f$ @cite tJOH69a&thinsp; (page 132).
    ...
    * 
    * @f$n@f$ and @f$p@f$ estimated using the maximum likelihood method based on
