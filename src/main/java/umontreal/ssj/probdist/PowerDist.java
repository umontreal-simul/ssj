 ...
 * 0@f$, over the interval @f$[a,b]@f$, where @f$a < b@f$. This distribution has
 * density
 * 
 * @anchor REF_probdist_PowerDist_eq_fpower @f[ f(x) = \frac{c(x-a)^{c - 1}}{(b
 *         - a)^c}, \qquad\mbox{for } a \le x \le b, \tag{fpower} @f]
 *         and @f$f(x) = 0@f$ elsewhere. Its distribution function is
 * @anchor REF_probdist_PowerDist_eq_Fpower @f[ F(x) = \frac{(x - a)^c}{(b -
 *         a)^c}, \qquad\mbox{for } a \le x \le b, \tag{Fpower} @f] with @f$F(x)
 *         = 0@f$ for @f$x \le a@f$ and @f$F(x) = 1@f$ for @f$x \ge b@f$.
 *
...
public class PowerDist extends ContinuousDistribution {
   ...
   /**
    * Constructs a `PowerDist` object with parameters @f$a =@f$ `a`, @f$b =@f$ `b`
    * and @f$c =@f$ `c`.
    */
   ...
   /**
    * Constructs a `PowerDist` object with parameters @f$a = 0@f$, @f$b =@f$ `b`
    * and @f$c =@f$ `c`.
    */
   ...
   /**
    * Constructs a `PowerDist` object with parameters @f$a = 0@f$, @f$b =1@f$
    * and @f$c =@f$ `c`.
    */
   ...
   /**
    * Computes the density function ( {@link REF_probdist_PowerDist_eq_fpower
    * fpower} ).
    * 
   ...
   /**
    * Computes the distribution function ( {@link REF_probdist_PowerDist_eq_Fpower
    * Fpower} ).
    * 
    * @param a left limit of interval
    * @param b right limit of interval
    ...
    * 
    * @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1, …, n-1@f$, using the maximum
    *         likelihood method and assuming that @f$a@f$ and @f$b@f$ are known.
    *         The estimate is returned in a one-element array: [@f$c@f$]. The
    *         maximum likelihood estimator is the value @f$\hat{c}@f$ that
    *         satisfies the equation
    * @f{align*}{ \frac{1}{\hat{c}} = -\frac{1}{n} \sum_{i=1}^n \ln\left(\frac{x_i
    *             - a}{b - a} \right)
    * @f}
    ...
    * 
    * @f$a@f$ and @f$b@f$, with @f$c@f$ estimated using the maximum likelihood
    *         method based on the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, …,
    *         n-1@f$.
    * @param x the list of observations to use to evaluate parameters
    * @param n the number of observations to use to evaluate parameters
    * @param a left limit of interval
   ...
   /**
    * Returns the mean @f$a + (b-a)c/(c+1)@f$ of the power distribution with
    * parameters @f$a@f$, @f$b@f$ and @f$c@f$.
    * 
    * @param a left limit of interval
    * @param b right limit of interval
    * @param c shape parameter
   ...
   /**
    * Computes and returns the variance @f$(b-a)^2 c / [(c+1)^2(c+2)]@f$ of the
    * power distribution with parameters @f$a@f$, @f$b@f$ and
    * 
    * @f$c@f$. @param a left limit of interval
    * @param b right limit of interval
    * @param c shape parameter
   ...
   /**
    * Computes and returns the standard deviation of the power distribution with
    * parameters @f$a@f$, @f$b@f$ and @f$c@f$.
    * 
    ...
    * table is put in regular order: [@f$a@f$, @f$b@f$, @f$c@f$].
    * 
