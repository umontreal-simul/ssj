 ...
 * distribution with shape parameters @f$\alpha_1 > 0@f$ and @f$\alpha_2 > 0@f$,
 * and scale parameter @f$\beta> 0@f$. The density function is given by
 * 
 * @anchor REF_probdist_Pearson6Dist_eq_fpearson6 @f[ f(x)
 *         =\left\{\begin{array}{ll}
 *         \displaystyle\frac{\left(x/{\beta}\right)^{\alpha_1 -
 *         1}}{\beta\mathcal{B}(\alpha_1, \alpha_2)(1 + x/{\beta})^{\alpha_1 +
 *         \alpha_2}} & \quad\mbox{for } x > 0, \\ 0 & \quad\mbox{otherwise,}
 *         \end{array} \right. \tag{fpearson6} @f] where @f$\mathcal{B}@f$ is
 *         the beta function. The distribution function is given by
 * @anchor REF_probdist_Pearson6Dist_eq_Fpearson6 @f[ F(x) = F_B\left(\frac{x}{x
 *         + \beta}\right) \qquad\mbox{for } x > 0, \tag{Fpearson6} @f]
 *         and @f$F(x) = 0@f$ otherwise, where @f$F_B(x)@f$ is the distribution
 *         function of a beta distribution with shape parameters @f$\alpha_1@f$
 *         and
 * @f$\alpha_2@f$.
 *
...
public class Pearson6Dist extends ContinuousDistribution {
   ...
   private static class Optim implements Uncmin_methods {
      ...
      public double f_to_minimize(double[] param) {
         ...
         return (n * (Math.log(param[3]) + Num.lnBeta(param[1], param[2])) - (param[1] - 1.0) * sumLogY
               + (param[1] + param[2]) * sumLog1_Y);
      }
   ...
   public double density(double x) {
      if (x <= 0.0)
         return 0.0;
      return Math.exp((alpha1 - 1.0) * Math.log(x / beta) - (logBeta + (alpha1 + alpha2) * Math.log1p(x / beta)))
            / beta;
   }
    ...
    * parameters @f$\alpha_1@f$ and @f$\alpha_2@f$, and scale
    * parameter @f$\beta@f$.
    */
   ...
   /**
    * Computes the distribution function of a Pearson VI distribution with shape
    * parameters @f$\alpha_1@f$ and @f$\alpha_2@f$, and scale
    * parameter @f$\beta@f$.
    */
    ...
    * with shape parameters @f$\alpha_1@f$ and
    * 
   ...
   /**
    * Computes the inverse distribution function of a Pearson VI distribution with
    * shape parameters @f$\alpha_1@f$ and
    * 
    * @f$\alpha_2@f$, and scale parameter @f$\beta@f$.
    */
   ...
   /**
    * Estimates the parameters @f$(\alpha_1,\alpha_2,\beta)@f$ of the Pearson VI
    ...
    * in a three-element array, in regular order: [@f$\alpha_1,
    * \alpha_2@f$, @f$\beta@f$]. The estimate of the parameters is given by
    ...
    * @return returns the parameters [@f$\hat{\alpha_1}, \hat{\alpha_2},
    *         \hat{\beta}@f$]
    */
    ...
    * 
    * @f$\alpha_1@f$, @f$\alpha_2@f$ and @f$\beta@f$, estimated using the maximum
    *                 likelihood method based on the @f$n@f$ observations
   ...
   /**
    * Computes and returns the mean @f$E[X] = (\beta\alpha_1) / (\alpha_2 - 1)@f$
    * of a Pearson VI distribution with shape parameters @f$\alpha_1@f$
    * and @f$\alpha_2@f$, and scale parameter @f$\beta@f$.
    */
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = [\beta^2 \alpha_1
    * (\alpha_1 + \alpha_2 - 1)] / [(\alpha_2 - 1)^2(\alpha_2 - 2)]@f$ of a Pearson
    * VI distribution with shape parameters @f$\alpha_1@f$ and @f$\alpha_2@f$, and
   ...
   /**
    * Computes and returns the standard deviation of a Pearson VI distribution with
    * shape parameters @f$\alpha_1@f$ and
    * 
    * @f$\alpha_2@f$, and scale parameter @f$\beta@f$.
    */
    ...
    * table is put in regular order: [@f$\alpha_1@f$, @f$\alpha_2@f$, @f$\beta@f$].
    */
