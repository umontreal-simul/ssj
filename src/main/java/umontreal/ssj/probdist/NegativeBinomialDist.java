...
/**
 * Extends the class @ref DiscreteDistributionInt for the *negative binomial*
 * distribution @cite sLAW00a&thinsp; (page 324) with real parameters @f$n@f$
 * and @f$p@f$, where @f$n > 0@f$ and @f$0\le p\le1@f$. Its mass function is
 * 
 * @anchor REF_probdist_NegativeBinomialDist_eq_fmass_negbin @f[ p(x) =
 *         \frac{\Gamma(n + x)}{\Gamma(n)\; x!} p^n (1 - p)^x, \qquad\mbox{for }
 *         x = 0, 1, 2, …\tag{fmass-negbin} @f] where @f$\Gamma(x)@f$ is the
 *         gamma function.
 *
 *         If @f$n@f$ is an integer, @f$p(x)@f$ can be interpreted as the
 *         probability of having @f$x@f$ failures before the @f$n@f$-th success
 *         in a sequence of independent Bernoulli trials with probability of
 *         success @f$p@f$. This special case is implemented as the Pascal
 *         distribution (see
 * @ref PascalDist ).
 *
...
public class NegativeBinomialDist extends DiscreteDistributionInt {
   ...
   /**
    * If the maximum term is greater than this constant, then the tables will *not*
    * be precomputed.
    */
    ...
    * {@link REF_probdist_NegativeBinomialDist_eq_fmass_negbin fmass-negbin} ) and
    * the distribution function for the negative binomial distribution with
    ...
    * {@link REF_probdist_NegativeBinomialDist_eq_fmass_negbin fmass-negbin} ).
    */
   ...
   /**
    * Returns @f$\bar{F}(x) = P[X \ge x]@f$, the complementary distribution
    * function.
    */
   ...
   /**
    * Estimates the parameter @f$p@f$ of the negative binomial distribution using
    * the maximum likelihood method, from the @f$m@f$ observations @f$x[i]@f$, @f$i
    * = 0, 1, …, m-1@f$. The parameter
    * 
    * @f$n@f$ is assumed known. The estimate @f$\hat{p}@f$ is returned in element 0
    *         of the returned array. The maximum likelihood estimator
    * @f$\hat{p}@f$ satisfies the equation @f$\hat{p} = n /(n + \bar{x}_m)@f$,
    *               where @f$\bar{x}_m@f$ is the average of @f$x[0], …,
    *               x[m-1]@f$.
    ...
    * parameters @f$n@f$ given and @f$\hat{p}@f$ estimated using the maximum
    * likelihood method, from the @f$m@f$ observations @f$x[i]@f$,
    * 
    * @f$i = 0, 1, …, m-1@f$.
   ...
   /**
    * Estimates the parameter @f$n@f$ of the negative binomial distribution using
    * the maximum likelihood method, from the @f$m@f$ observations @f$x[i]@f$, @f$i
    * = 0, 1, …, m-1@f$. The parameter
    * 
    * @f$p@f$ is assumed known. The estimate @f$\hat{n}@f$ is returned in element 0
    *         of the returned array. The maximum likelihood estimator
    * @f$\hat{p}@f$ satisfies the equation @f[ \frac{1}{m}\sum_{j=0}^{m-1} \psi(n
    *               +x_j) = \psi(n) - \ln(p) @f] where @f$\psi(x)@f$ is the digamma
    *               function, i.e. the logarithmic derivative of the Gamma
    *               function @f$\psi(x) = \Gamma^{\prime}(x)/\Gamma(x)@f$.
    * @param x the list of observations used to evaluate parameters
    * @param m the number of observations used to evaluate parameters
   ...
   /**
    * Creates a new instance of a negative binomial distribution with
    * parameters @f$p@f$ given and @f$\hat{n}@f$ estimated using the maximum
    * likelihood method, from the @f$m@f$ observations @f$x[i]@f$,
    * 
    * @f$i = 0, 1, …, m-1@f$.
    * @param x the list of observations to use to evaluate parameters
   ...
   /**
    * Estimates the parameter @f$(n, p)@f$ of the negative binomial distribution
    * using the maximum likelihood method, from the @f$m@f$
    * observations @f$x[i]@f$, @f$i = 0, 1, …, m-1@f$. The estimates are returned
    * in a two-element array, in regular order: [@f$n@f$, @f$p@f$]. The maximum
    * likelihood estimators are the values
    * 
    * @f$(\hat{n}@f$, @f$\hat{p})@f$ satisfying the equations
    * @f{align*}{ \frac{\hat{n}(1 - \hat{p})}{\hat{p}} & = \bar{x}_m \\
    *             \sum_{j=1}^{\infty} \frac{F_j}{(\hat{n} + j - 1)} & =
    *             -m\ln(\hat{p})
    * @f} where @f$\bar{x}_m@f$ is the average of @f$x[0],…,x[m-1]@f$, and
    * @f$F_j = \sum_{i=j}^{\infty} f_i@f$ = number of @f$x_i \ge j@f$ (see @cite
    *        tJOH69a&thinsp; (page 132)).
    * @param x the list of observations used to evaluate parameters
    * @param m the number of observations used to evaluate parameters
   ...
   /**
    * Creates a new instance of a negative binomial distribution with
    * parameters @f$n@f$ and @f$p@f$ estimated using the maximum likelihood method
    * based on the @f$m@f$ observations @f$x[i]@f$, @f$i = 0, 1, …, m-1@f$.
    * 
    * @param x the list of observations to use to evaluate parameters
    * @param m the number of observations used to evaluate parameters
    */
   ...
   /**
    * Estimates and returns the parameter @f$\nu= 1/\hat{n}@f$ of the negative
    * binomial distribution using the maximum likelihood method, from the @f$m@f$
    * observations @f$x[i]@f$, @f$i = 0, 1, …, m-1@f$. The maximum likelihood
    * estimator is the value @f$\nu@f$ satisfying the equation @f[
    * \sum_{j=1}^{\infty} \frac{\nu F_j}{1 + \nu(j - 1)} = m\ln(1 +
    * \nu\bar{x}_m) @f] where @f$\bar{x}_m@f$ is the average
    * of @f$x[0],…,x[m-1]@f$, and
    * 
    * @f$F_j = \sum_{i=j}^{\infty} f_i@f$ = number of @f$x_i \ge j@f$ (see @cite
    *        tJOH69a&thinsp; (page 132)).
   ...
   /**
    * Computes and returns the mean @f$E[X] = n(1 - p)/p@f$ of the negative
    * binomial distribution with parameters @f$n@f$ and @f$p@f$.
    * 
    * @return the mean of the negative binomial distribution @f$E[X] = n(1 - p) /
    *         p@f$
    */
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = n(1 - p)/p^2@f$ of the
    * negative binomial distribution with parameters @f$n@f$ and
    * 
    * @f$p@f$. @return the variance of the negative binomial distribution
   ...
   public void setParams(double n, double p) {
      /**
       * Compute all probability terms of the negative binomial distribution; start at
       * the mode, and calculate probabilities on each side until they become smaller
       * than EPSILON. Set all others to 0.
       */
      supportA = 0;
      int i, mode, Nmax;
      int imin, imax;
      ...
      /**
       * For mode > MAXN, we shall not use pre-computed arrays. mode < 0 should be
       * impossible, unless overflow of long occur, in which case mode will be =
       * LONG_MIN.
       */
      ...
      /**
       * In theory, the negative binomial distribution has an infinite range. But for
       * i > Nmax, probabilities should be extremely small. Nmax = Mean + 16 *
       * Standard deviation.
       */
    ...
    * table is put in regular order: [@f$n@f$, @f$p@f$].
    */
