 ...
 * class. Such distributions are characterized by a *density* function
 * 
 * @f$f(x)@f$, thus the signature of a `density` method is supplied here. This
 *             class also provides default implementations for @f$\bar{F}(x)@f$
 *             and for @f$F^{-1}(u)@f$, the latter using the Brent-Dekker method
 *             to find the inverse of a generic distribution function @f$F@f$.
 *
...
public abstract class ContinuousDistribution implements Distribution {
   ...
   /**
    * Returns the complementary distribution function. The default implementation
    * computes @f$\bar{F}(x) = 1 - F(x)@f$.
    * 
    ...
    *         calculations are done with an approximate precision of `tol`.
    *         Returns @f$x = F^{-1}(u)@f$. Restrictions: @f$u \in[0,1]@f$.
   ...
   /**
    * Computes and returns the inverse distribution function @f$x = F^{-1}(u)@f$,
    * using bisection. Restrictions: @f$u \in[0,1]@f$.
    * 
    * @param u value at which the inverse distribution function is evaluated
   ...
   /**
    * Returns @f$x_a@f$ such that the probability density is 0 everywhere outside
    * the interval @f$[x_a, x_b]@f$.
    * 
   ...
   /**
    * Returns @f$x_b@f$ such that the probability density is 0 everywhere outside
    * the interval @f$[x_a, x_b]@f$.
    * 
   ...
   /**
    * Sets the value @f$x_a=@f$ `xa`, such that the probability density is 0
    * everywhere outside the interval @f$[x_a, x_b]@f$.
    * 
   ...
   /**
    * Sets the value @f$x_b=@f$ `xb`, such that the probability density is 0
    * everywhere outside the interval @f$[x_a, x_b]@f$.
    * 
