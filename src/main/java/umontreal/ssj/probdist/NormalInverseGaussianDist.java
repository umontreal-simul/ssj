 ...
 * @anchor REF_probdist_NormalInverseGaussianDist_eq_fNormalInverseGaussian @f[
 *         f(x) = \frac{\alpha\delta e^{\delta\gamma+ \beta(x - \mu)}
 *         K_1\left(\alpha\sqrt{\delta^2 + (x -
 *         \mu)^2}\right)}{\pi\sqrt{\delta^2 + (x - \mu)^2}}, \qquad\mbox{for }
 *         -\infty< x < \infty, \tag{fNormalInverseGaussian} @f] where @f$K_1@f$
 *         is the modified Bessel function of the second kind of order 1,
 *         and @f$\gamma= \sqrt{\alpha^2 - \beta^2}@f$.
 *
 *         The distribution function is given by
 * @anchor REF_probdist_NormalInverseGaussianDist_eq_FNormalInverseGaussian @f[
 *         F(x) = \int_{-\infty}^x dt f(t), \tag{FNormalInverseGaussian} @f]
...
public class NormalInverseGaussianDist extends ContinuousDistribution {
    ...
    * fNormalInverseGaussian} ) for the *normal inverse gaussian* distribution with
    * parameters @f$\alpha@f$, @f$\beta@f$, @f$\mu@f$ and @f$\delta@f$, evaluated
    * at @f$x@f$.
    */
    ...
    * {@link REF_probdist_NormalInverseGaussianDist_eq_FNormalInverseGaussian
    * FNormalInverseGaussian} ) of the *normal inverse gaussian* distribution with
    * parameters @f$\alpha@f$, @f$\beta@f$, @f$\mu@f$ and @f$\delta@f$, evaluated
    * at @f$x@f$.
    */
   ...
   /**
    * NOT IMPLEMENTED. Computes the complementary distribution function of the
    * *normal inverse gaussian* distribution with parameters
    * 
    * @f$\alpha@f$, @f$\beta@f$, @f$\mu@f$ and @f$\delta@f$, evaluated at @f$x@f$.
    */
   ...
   /**
    * NOT IMPLEMENTED. Computes the inverse of the *normal inverse gaussian*
    * distribution with parameters @f$\alpha@f$, @f$\beta@f$,
    * 
   ...
   public static double[] getMLE(double[] x, int n) {
      ...
      /*
       * double[] parameters = new double[4]; double sum = 0; for (int i = 0; i < n;
       * i++) { sum += x[i]; }
       */
   ...
   /**
    * Returns the mean @f$E[X] = \mu+ \delta\beta/\gamma@f$ of the *normal inverse
    * gaussian* distribution with parameters
    * 
    * @f$\alpha@f$, @f$\beta@f$, @f$\mu@f$ and @f$\delta@f$.
    * @return the mean of the normal inverse gaussian distribution @f$E[X] = \mu+
    *         \delta\beta/\gamma@f$
    */
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = \delta\alpha^2 /
    * \gamma^3@f$ of the *normal inverse gaussian* distribution with
    * parameters @f$\alpha@f$, @f$\beta@f$, @f$\mu@f$ and @f$\delta@f$.
    * 
   ...
   /**
    * Returns a table containing the parameters of the current distribution. This
    * table is put in regular order:
    * [@f$\alpha@f$, @f$\beta@f$, @f$\mu@f$, @f$\delta@f$].
    */
   ...
   public String toString() {
      return getClass().getSimpleName() + ": alpha = " + alpha + ", beta = " + beta + ", mu = " + mu + ", delta = "
            + delta;
   }
