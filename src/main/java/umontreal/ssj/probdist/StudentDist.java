 ...
 * 
 * @f$t@f$-distribution @cite tJOH95b&thinsp; (page 362) with @f$n@f$ degrees of
 *                      freedom, where @f$n@f$ is a positive integer. Its
 *                      density is
 * @anchor REF_probdist_StudentDist_eq_fstudent @f[ f (x) = \frac{\Gamma\left((n
 *         + 1)/2 \right)}{\Gamma(n/2) \sqrt{\pi n}} \left(1 +
 *         \frac{x^2}{n}\right)^{-(n+1)/2} \qquad\mbox{for } -\infty< x <
 *         \infty, \tag{fstudent} @f] where @f$\Gamma(x)@f$ is the gamma
 *         function defined in ( {@link REF_probdist_GammaDist_eq_Gamma Gamma}
 *         ).
 *
...
public class StudentDist extends ContinuousDistribution {
   ...
   /**
    * Computes the density function ( {@link REF_probdist_StudentDist_eq_fstudent
    * fstudent} ) of a Student @f$t@f$-distribution with @f$n@f$ degrees of
    * freedom.
    */
    ...
    * @f$n \le10^5@f$. For @f$n > 10^5@f$, gives at least 6 decimal digits of
    *      precision everywhere, and at least 9 decimal digits of precision for
    *      all @f$u > 10^{-15}@f$.
    */
   ...
   /**
    * Computes the complementary distribution function @f$v = \bar{F}(x)@f$
    * with @f$n@f$ degrees of freedom. Gives 13 decimal digits of precision
    * for @f$n \le10^5@f$. For @f$n > 10^5@f$, gives at least 6 decimal digits of
    * precision everywhere, and at least 9 decimal digits of precision for all @f$v
    * > 10^{-15}@f$.
    */
    ...
    * 
    * @f$t@f$-distribution function with @f$n@f$ degrees of freedom. Gives 13
    *                      decimal digits of precision for @f$n \le10^5@f$, and at
    *                      least 9 decimal digits of precision for @f$n > 10^5@f$.
    */
   ...
   /**
    * Estimates the parameter @f$n@f$ of the Student @f$t@f$-distribution using the
    ...
    * parameter @f$n@f$ estimated using the maximum likelihood method based on
    * the @f$m@f$ observations @f$x[i]@f$, @f$i = 0, 1, …, m-1@f$.
    * 
    * @param x the list of observations to use to evaluate parameters
    * @param m the number of observations to use to evaluate parameters
    */
   ...
   /**
    * Returns the mean @f$E[X] = 0@f$ of the Student @f$t@f$-distribution with
    * parameter @f$n@f$.
    * 
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = n/(n - 2)@f$ of the
    * Student @f$t@f$-distribution with parameter @f$n@f$.
    * 
