 ...
 * @anchor REF_probdist_GumbelDist_eq_densgumbel @f[ f (x) = \frac{e^{-z}
 *         e^{-e^{-z}}}{|\beta|}, \qquad\mbox{for } -\infty< x < \infty
 *         \tag{densgumbel} @f] and distribution function @f[ F(x) = \left\{
 *         \begin{array}{ll} e^{-e^{-z}}, \qquad & \mbox{for } \beta> 0 \\ 1 -
 *         e^{-e^{-z}}, \qquad & \mbox{for } \beta< 0. \end{array} \right. @f]
...
public class GumbelDist extends ContinuousDistribution {
   ...
   /**
    * Constructs a `GumbelDist` object with parameters @f$\beta@f$ = `beta`
    * and @f$\delta@f$ = `delta`.
    */
   ...
   /**
    * Computes and returns the complementary distribution function @f$1 - F(x)@f$.
    */
   ...
   /**
    * Estimates the parameters @f$(\beta,\delta)@f$ of the Gumbel distribution,
    * <em>assuming that @f$\beta> 0@f$</em>, and using the maximum likelihood
    * method with the @f$n@f$ observations @f$x[i]@f$,
    * 
    * @f$i = 0, 1,…, n-1@f$. The estimates are returned in a two-element array,
    *      in regular order: [@f$\beta@f$, @f$\delta@f$]. The maximum likelihood
    *      estimators are the values @f$(\hat{\beta}, \hat{\delta})@f$ that satisfy
    *      the equations:
    * @f{align*}{ \hat{\beta} & = \bar{x}_n - \frac{\sum_{i=1}^n x_i  e^{-
    *             x_i/\hat{\beta}}}{\sum_{i=1}^n e^{- x_i / \hat{\beta}}} \\
    ...
    * 
    * @f$\beta@f$ and @f$\delta@f$ estimated using the maximum likelihood method
    *             based on the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1, …,
    *             n-1@f$, <em>assuming that @f$\beta> 0@f$</em>.
   ...
   /**
    * Similar to #getInstanceFromMLE, but <em>for the case @f$\beta< 0@f$</em>.
    * 
    * @param x the list of observations to use to evaluate parameters
    * @param n the number of observations to use to evaluate parameters
    */
    ...
    * @return the mean of the Extreme Value distribution @f$E[X] = \delta+ \gamma*
    *         \beta@f$
    */
   ...
   /**
    * Returns the variance @f$\mbox{Var}[X] = \pi^2 \beta^2\!/6@f$ of the Gumbel
    * distribution with parameters @f$\beta@f$ and
    * 
    * @f$\delta@f$. @return the variance of the Gumbel
    *               distribution @f$\mbox{Var}[X] = ()\pi\beta)^2/6@f$
    */
    ...
    * table is put in regular order: [@f$\beta@f$, @f$\delta@f$].
    */
