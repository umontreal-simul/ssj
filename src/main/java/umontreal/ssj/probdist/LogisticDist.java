...
/**
 * Extends the class @ref ContinuousDistribution for the *logistic* distribution
 * (e.g., @cite tJOH95b&thinsp; (page 115)). It has location
 * parameter @f$\alpha@f$ and scale parameter @f$\lambda> 0@f$. The density is
 * 
 * @anchor REF_probdist_LogisticDist_eq_flogistic @f[ f (x) = \frac{\lambda
 *         e^{-\lambda(x - \alpha)}}{(1 + e^{-\lambda(x - \alpha)})^2}
 *         \qquad\qquad\mbox{for } -\infty< x < \infty, \tag{flogistic} @f] and
 *         the distribution function is
 * @anchor REF_probdist_LogisticDist_eq_Flogistic @f[ F(x) = \frac{1}{1 +
 *         e^{-\lambda(x - \alpha)}} \qquad\qquad\mbox{for } -\infty< x <
 *         \infty. \tag{Flogistic} @f] For @f$\lambda=1@f$ and @f$\alpha=0@f$,
 *         one can write @f[ F(x) = \frac{1 + \tanh({x/2})}{2}. @f] The inverse
 *         distribution function is given by @f[ F^{-1}(u) =
 *         \ln(u/(1-u))/\lambda+ \alpha\qquad\mbox{for } 0 \le u < 1. @f]
...
public class LogisticDist extends ContinuousDistribution {
   ...
   /**
    * Constructs a `LogisticDist` object with parameters @f$\alpha@f$ = `alpha`
    ...
    * @f{align*}{ \sum_{i=1}^n \frac{1}{1 + e^{\hat{\lambda} (x_i - \hat{\alpha})}}
    *             & = \frac{n}{2} \\ \sum_{i=1}^n \hat{\lambda} (x_i -
    *             \hat{\alpha}) \frac{1 - e^{\hat{\lambda} (x_i - \hat{\alpha})}}{1
    *             + e^{\hat{\lambda} (x_i - \hat{\alpha})}} & = n.
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = \pi^2 /(3\lambda^2)@f$
    * of the logistic distribution with parameters
    * 
    * @f$\alpha@f$ and @f$\lambda@f$.
    * @return the variance of the logistic distribution @f$\mbox{Var}[X] = 1 / 3
    *         \pi^2 * (1 / \lambda^2)@f$
    */
