 ...
 *                has shape parameters @f$\gamma@f$ and @f$\delta> 0@f$,
 *                location parameter @f$\xi@f$, and scale parameter @f$\lambda>
 *                0@f$. Denoting
 * @f$t=(x-\xi)/\lambda@f$ and @f$z = \gamma+ \delta\ln(t)@f$, the distribution
 *                         has density @f[ f(x) = \frac{\delta
 *                         e^{-z^2/2}}{\lambda t \sqrt{2\pi}}, \qquad\mbox{for }
 *                         \xi< x < \infty, @f] and distribution function @f[
 *                         F(x) = \Phi(z), \qquad\mbox{for } \xi< x <
 *                         \infty, @f] where @f$\Phi@f$ is the standard normal
 *                         distribution function. The inverse distribution
 *                         function is @f[ F^{-1} (u) = \xi+ \lambda e^{v(u)},
 *                         \qquad\mbox{for } 0 \le u \le1, @f] where @f[ v(u) =
 *                         [\Phi^{-1}(u) - \gamma]/\delta. @f] Without loss of
 *                         generality, one may choose @f$\gamma= 0@f$ or
 * @f$\lambda=1@f$.
 *
...
public class JohnsonSLDist extends JohnsonSystem {
    ...
    * 
    * @f$\lambda)@f$ of the <em>Johnson @f$S_L@f$</em> distribution using the
    ...
    * @return returns the parameters [0, @f$\delta@f$, @f$\xi@f$, @f$\lambda@f$]
    */
   ...
   /**
    * Creates a new instance of a <em>Johnson @f$S_L@f$</em> distribution with
    * parameters 0, @f$\delta@f$, @f$\xi@f$ and @f$\lambda@f$ over the
    * interval @f$[\xi,\infty]@f$ estimated using the maximum likelihood method
    * based on the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1, …, n-1@f$.
    * 
    * @param x the list of observations to use to evaluate parameters
    * @param n the number of observations to use to evaluate parameters
    */
   ...
   /**
    * Returns the mean @f[ E[X] = \xi+ \lambda e^{1/2\delta^2 - \gamma/\delta} @f]
    * of the Johnson @f$S_L@f$ distribution with parameters @f$\gamma@f$,
    * 
    * @f$\delta@f$, @f$\xi@f$ and @f$\lambda@f$.
    * @return the mean of the Johnson @f$S_L@f$ distribution @f$E[X] = \xi+ \lambda
    *         e^{1/2\delta^2 - \gamma/\delta}@f$
    */
   ...
   /**
    * Returns the variance @f[ \mbox{Var}[X] = \lambda^2 \left(e^{1/\delta^2} -
    * 1\right) e^{1/\delta^2 - 2\gamma/\delta} @f] of the Johnson @f$S_L@f$
    * distribution with parameters @f$\gamma@f$,
    * 
    * @f$\delta@f$, @f$\xi@f$ and @f$\lambda@f$.
    * @return the variance of the Johnson @f$S_L@f$ distribution
    * @f$\mbox{Var}[X] = \lambda^2 \left(e^{1/\delta^2} - 1\right) e^{1/\delta^2 -
    *                  2\gamma/\delta}@f$
    */
   ...
   /**
    * Returns the standard deviation of the Johnson @f$S_L@f$ distribution with
    * parameters @f$\gamma@f$, @f$\delta@f$, @f$\xi@f$,
    * 
    * @f$\lambda@f$. @return the standard deviation of the Johnson @f$S_L@f$
    *                distribution
    */
