...
/**
 * Extends the class @ref ContinuousDistribution for the *Watson U* distribution
 * (see @cite tDUR73a, @cite tSTE70a, @cite tSTE86b&thinsp;). Given a sample
 * of @f$n@f$ independent uniforms @f$u_i@f$ over @f$[0,1]@f$, the *Watson*
 * statistic @f$U_n^2@f$ is defined by
 * 
 * @anchor REF_probdist_WatsonUDist_eq_WatsonU
 * @f{align*}{ W_n^2 & = \frac{1}{12n} + \sum_{j=1}^n \left\{u_{(j)} - \frac{(j-
 *             1/2)}{n}\right\}^2, \\ U_n^2 & = W_n^2 - n\left(\bar{u}_n -
 *             1/2\right)^2. \tag{WatsonU}
 * @f} where the @f$u_{(j)}@f$ are the @f$u_i@f$ sorted in increasing order, and
 * @f$\bar{u}_n@f$ is the average of the observations @f$u_i@f$. The
 *                 distribution function (the cumulative probabilities) is
 *                 defined as @f$F_n(x) = P[U_n^2 \le x]@f$.
 *
...
public class WatsonUDist extends ContinuousDistribution {
   ...
   /**
    * Computes the density of the *Watson U* distribution with parameter @f$n@f$.
    */
   ...
   /**
    * Computes the Watson @f$U@f$ distribution function, i.e. returns @f$P[U_n^2
    * \le x]@f$, where @f$U_n^2@f$ is the Watson statistic defined in (
    * {@link REF_probdist_WatsonUDist_eq_WatsonU WatsonU} ). We use the asymptotic
    * distribution for @f$n \to\infty@f$, plus a correction in @f$O(1/n)@f$, as
    * given in @cite tCSO96a&thinsp;.
    */
    ...
    * where @f$F_n@f$ is the *Watson* @f$U@f$ distribution with parameter @f$n@f$.
    */
   ...
   /**
    * Returns the standard deviation of the *Watson* @f$U@f$ distribution with
    * parameter @f$n@f$.
    * 
