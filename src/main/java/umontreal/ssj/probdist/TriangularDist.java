 ...
 *       shape parameter) @f$m@f$, where @f$a\le m\le b@f$. The density function
 *       is
 * @anchor REF_probdist_TriangularDist_eq_ftrian @f[ f(x) =
 *         \left\{\begin{array}{ll} \frac{2(x-a)}{(b-a)(m-a)} & \mbox{ if } a\le
 *         x\le m, \\ \frac{2(b-x)}{(b-a)(b-m)} & \mbox{ if } m\le x\le b, \\ 0
 *         & \mbox{ elsewhere, } \end{array}\right. \tag{ftrian} @f] the
 *         distribution function is @f[ F (x) = \left\{\begin{array}{ll} 0 &
 *         \mbox{ for } x < a, \\ \frac{(x - a)^2}{(b - a)(m - a)} & \mbox{ if }
 *         a\le x\le m, \\ 1 - \frac{(b - x)^2}{(b - a)(b - m)} & \mbox{ if }
 *         m\le x\le b, \\ 1 & \mbox{ for } x > b, \end{array}\right. @f] and
 *         the inverse distribution function is given by @f[ F^{-1}(u) =
 *         \left\{\begin{array}{ll} a + \sqrt{(b - a)(m - a)u} & \mbox{ if }
 *         0\le u\le\frac{m-a}{b-a}, \\ b - \sqrt{(b - a)(b - m)(1 - u)} &
 *         \mbox{ if } \frac{m-a}{b-a}\le u \le1. \end{array}\right. @f]
...
public class TriangularDist extends ContinuousDistribution {
   ...
   /**
    * Estimates the parameter @f$m@f$ of the triangular distribution using the
    * maximum likelihood method, from the @f$n@f$ observations
    * 
    * @f$x[i]@f$, @f$i = 0, 1,…, n-1@f$. The estimated parameter is returned in a
    *             one-element array: [@f$\hat{m}@f$]. See @cite tOLI72a,
   ...
   /**
    * Creates a new instance of a triangular distribution with parameters `a` and
    * `b`. @f$m@f$ is estimated using the maximum likelihood method based on
   ...
   /**
    * Computes and returns the mean @f$E[X] = (a + b + m)/3@f$ of the triangular
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = (a^2 + b^2 + m^2 - ab -
    * am - bm)/18@f$ of the triangular distribution with
   ...
   /**
    * Computes and returns the standard deviation of the triangular distribution
    * with parameters @f$a@f$, @f$b@f$, @f$m@f$.
    * 
    ...
    * table is put in regular order: [@f$a@f$, @f$b@f$, @f$m@f$].
    */
