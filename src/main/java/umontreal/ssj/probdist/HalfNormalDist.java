 ...
 * distribution with parameters @f$\mu@f$ and @f$\sigma> 0@f$. Its density is
 * 
 * @anchor REF_probdist_HalfNormalDist_eq_fHalfNormal
 * @f{align}{ f(x) & = \frac{1}{\sigma}\sqrt{\frac{2}{\pi}}\;
 *            e^{-(x-\mu)^2/2\sigma^2}, \qquad\mbox{for } x \ge\mu. \\
 *            \tag{fHalfNormal} f(x) & = 0, \qquad\mbox{for } x < \mu. \nonumber
...
public class HalfNormalDist extends ContinuousDistribution {
   ...
   /**
    * Constructs a `HalfNormalDist` object with parameters @f$\mu=@f$ `mu`
    * and @f$\sigma=@f$ `sigma`.
    */
   ...
   /**
    * Estimates the parameters @f$\mu@f$ and @f$\sigma@f$ of the half-normal
    ...
    * in a two-element array: [@f$\mu@f$, @f$\sigma@f$]. The maximum likelihood
    * estimators are the values
    * 
    * @f$\hat{\mu}@f$ and @f$\hat{\sigma}@f$ that satisfy the equation
    * @f{align*}{ \hat{\mu}= \min_j \{x_j\}, \\ \hat{\sigma}=
    *             \sqrt{\frac{1}{n}\Sigma_j(x_j-\hat{\mu})^2}.
    * @f}
   ...
   /**
    * Estimates the parameter @f$\sigma@f$ of the half-normal distribution using
    * the maximum likelihood method from the @f$n@f$ observations @f$x[i]@f$, @f$i
    * = 0, 1, …, n-1@f$ and the parameter
    * 
    ...
    * @f{align*}{ \hat{\sigma}= \sqrt{\frac{1}{n}\Sigma_j(x_j-\mu)^2}.
    * @f}
    * @param x  the list of observations to use to evaluate parameters
   ...
   /**
    * Computes and returns the mean @f$ E[X] = \mu+ \sigma\sqrt{2 / \pi}. @f$
    * 
    * @param mu    the parameter mu
    * @param sigma the parameter sigma
