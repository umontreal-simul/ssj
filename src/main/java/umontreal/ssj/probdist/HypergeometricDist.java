 ...
 * @anchor REF_probdist_HypergeometricDist_eq_fheperg @f[ p(x) = \frac{
 *         \binom{m}{x} \binom{l - m}{k-x}}{\binom{l}{k}} \qquad\mbox{for }
 *         \max(0,k-l+m)\le x\le\min(k, m). \tag{fheperg} @f]
...
public class HypergeometricDist extends DiscreteDistributionInt {
   ...
   /**
    * If the number of integers in the interval @f$[\max(0,k-l+m), \min(k,m)]@f$ is
    * larger than this constant, the tables will *not* be precomputed by the
    * constructor.
    */
   ...
   /**
    * Computes @f$F^{-1}(u)@f$ for the hypergeometric distribution without using
    * precomputed tables. The inversion is computed using the chop-down
    * algorithm @cite sKAC85a&thinsp;.
    */
   ...
   /**
    * Computes and returns the mean @f$E[X] = km/l@f$ of the Hypergeometric
    * distribution with parameters @f$m@f$, @f$l@f$ and
    * 
    * @f$k@f$. @return the mean of the hypergeometric distribution @f$E[X] = km /
    *          l@f$
    */
    ...
    * 
    * @f$\mbox{Var}[X] = \frac{(km/l)(1 - m/l)(l - k)}{l - 1}@f$ of the
    *                  hypergeometric distribution with parameters @f$m@f$, @f$l@f$
    *                  and @f$k@f$.
   ...
   /**
    * Resets the parameters of this object to @f$m@f$, @f$l@f$ and @f$k@f$.
    */
