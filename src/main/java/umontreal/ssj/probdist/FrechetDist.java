 ...
 * @f$\alpha> 0@f$, where we use the notation @f$z = (x-\delta)/\beta@f$. It has
 *            density @f[ f (x) = \frac{\alpha e^{-z^{-\alpha}}}{\beta
 *            z^{\alpha+1}}, \qquad\mbox{for } x > \delta @f] and distribution
 *            function @f[ F(x) = e^{-z^{-\alpha}}, \qquad\mbox{for } x >
 *            \delta. @f] Both the density and the distribution are 0 for @f$x
 *            \le\delta@f$.
 *
 *            The mean is given by @f[ E[X] = \delta+ \beta\Gamma\!\left(1 -
 *            \frac{1}{\alpha}\right), @f] where @f$\Gamma(x)@f$ is the gamma
 *            function. The variance is @f[ \mbox{Var}[X] = \beta^2
 *            \left[\Gamma\!\left(1 - \frac{2}{\alpha}\right) -
 *            \Gamma^2\!\left(1 - \frac{1}{\alpha}\right)\right]. @f]
...
public class FrechetDist extends ContinuousDistribution {
   ...
   /**
    * Given @f$\delta=@f$ `delta`, estimates the parameters @f$(\alpha, \beta)@f$
    * of the <em>Fréchet</em> distribution using the maximum likelihood method
    * with the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1,…, n-1@f$. The
    ...
    * 
    * @f{align*}{ \hat{\beta} & = \left(\frac{1}{n} \sum_{i=0}^{n-1} (x_i -
    *             \delta)^{-\hat{\alpha}}\right)^{\!\!-1/\hat{\alpha}} \\
    *             \frac{1}{n} \sum_{i=0}^{n-1} \ln(x_i - \delta) & =
    *             \frac{1}{\hat{\alpha}} + \frac{\sum_{i=0}^{n-1} (x_i -
    *             \delta)^{-\hat{\alpha}}\ln(x_i - \delta)}{\sum_{i=0}^{n-1} (x_i -
    *             \delta)^{-\hat{\alpha}}}.
    * @f}
   ...
   /**
    * Given @f$\delta=@f$ `delta`, creates a new instance of a <em>Fréchet</em>
   ...
   /**
    * Returns the standard deviation of the <em>Fréchet</em> distribution with
    * parameters @f$\alpha@f$, @f$\beta@f$ and @f$\delta@f$.
    * 
   ...
   /**
    * Return an array containing the parameters of the current object in regular
    * order: [@f$\alpha@f$, @f$\beta@f$, @f$\delta@f$].
    */
