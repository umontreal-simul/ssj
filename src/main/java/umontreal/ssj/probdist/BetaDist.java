 ...
 *       This distribution has density @f[ f(x) = \frac{ (x-a)^{\alpha- 1}(b -
 *       x)^{\beta- 1}}{\mathcal{B} (\alpha, \beta)(b - a)^{\alpha+ \beta- 1}},
 *       \qquad\mbox{for } a\le x\le b, \mbox{ and }0\mbox{ elsewhere}, @f] and
 *       distribution function
 * @anchor REF_probdist_BetaDist_eq_Fbeta @f[ F(x) = I_{\alpha,\beta}(x) =
 *         \int_a^x \frac{(\xi- a)^{\alpha-1} (b - \xi)^{\beta-1}}{\mathcal{B}
 *         (\alpha, \beta)(b - a)^{\alpha+ \beta- 1}} d\xi, \qquad\mbox{for }
 *         a\le x\le b, \tag{Fbeta} @f] where @f$\mathcal{B}(\alpha,\beta)@f$ is
 *         the *beta* function defined by
 * @anchor REF_probdist_BetaDist_eq_betadef @f[ \mathcal{B} (\alpha,\beta) =
 *         \frac{\Gamma(\alpha) \Gamma(\beta)}{
 *         \Gamma(\alpha+\beta)},\tag{betadef} @f] and @f$\Gamma(x)@f$ is the
 *         gamma function defined in ( {@link REF_probdist_GammaDist_eq_Gamma
 *         Gamma} ).
 *
...
//      return factor*Math.pow (x - a, alpha - 1)*Math.pow (b - x, beta - 1);
   ...
   /**
    * Same as {@link #density(double,double,double,double,double) density(alpha,
    * beta, 0, 1, x)}.
    */
   ...
   /**
    * Same as {@link #cdf(double,double,double,double,double) cdf(alpha, beta, 0,
    * 1, x)}.
    */
   ...
   /**
    * Computes the distribution function. If @f$\max(\alpha, \beta) \le10^4@f$,
    * uses a continuous fraction in @f$\alpha@f$ and
    * 
    * @f$\beta@f$ given in @cite tDID92a, @cite tCUY08a&thinsp;. Otherwise,
    *             if @f$\min(\alpha, \beta) \le30@f$, uses an approximation due to
    *             Bol’shev @cite tMAR78a&thinsp;, else uses a normal
    *             approximation @cite tPEI68a&thinsp;.
    */
   ...
   /**
    * Same as {@link #barF(double,double,double,double,double) barF(alpha, beta, 0,
    * 1, x)}.
    */
   ...
   /**
    * Same as {@link #inverseF(double,double,double,double,double) inverseF(alpha,
    * beta, 0, 1, u)}.
    */
    ...
    * implemented in @cite iMOS00a&thinsp;. The method performs interval halving or
    * Newton iterations to compute the inverse.
    */
   ...
   /**
    * Estimates the parameters @f$(\alpha,\beta)@f$ of the beta distribution over
    * the interval @f$[0,1]@f$ using the maximum likelihood method, from
    * the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1,…, n-1@f$. The estimates
    * are returned in a two-element array, in regular order:
    * [@f$\alpha@f$, @f$\beta@f$]. The maximum likelihood estimators are the
    * values @f$(\hat{\alpha}, \hat{\beta})@f$ that satisfy the equations:
    * 
    * @f{align*}{ \psi(\alpha) - \psi(\alpha+ \beta) & = \frac{1}{n} \sum_{i=1}^n
    *             \ln(x_i) \\ \psi(\beta) - \psi(\alpha+ \beta) & = \frac{1}{n}
    *             \sum_{i=1}^n \ln(1 - x_i)
    * @f} where @f$\bar{x}_n@f$ is the average of @f$x[0],…,x[n-1]@f$, and
    * @f$\psi@f$ is the logarithmic derivative of the Gamma function @f$\psi(x) =
    *            \Gamma’(x) / \Gamma(x)@f$.
    ...
    * 
    * @f$\alpha@f$ and @f$\beta@f$ over the interval @f$[0,1]@f$ estimated using
    *              the maximum likelihood method based on the @f$n@f$
   ...
   /**
    * Computes and returns the mean @f$E[X] = \alpha/ (\alpha+ \beta)@f$ of the
    * beta distribution with parameters @f$\alpha@f$ and @f$\beta@f$, over the
    * interval @f$[0, 1]@f$.
    * 
   ...
   /**
    * Computes and returns the mean @f$E[X] = (b\alpha+ a\beta)/ (\alpha+ \beta)@f$
    ...
    * \frac{\alpha\beta}{(\alpha+ \beta)^2 (\alpha+ \beta+ 1)}@f$ of the beta
    * distribution with parameters @f$\alpha@f$ and
    * 
    * @f$\beta@f$, over the interval @f$[0, 1]@f$.
    * @return the variance of the beta distribution @f$\mbox{Var}[X] = \alpha\beta/
    *         [(\alpha+ \beta)^2 (\alpha+ \beta+ 1)]@f$.
    */
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] =
    * \frac{\alpha\beta(b-a)^2}{(\alpha+ \beta)^2 (\alpha+ \beta+ 1)}@f$ of the
    * beta distribution with parameters @f$\alpha@f$ and
    * 
    ...
    * parameters @f$\alpha@f$ and @f$\beta@f$, over the interval @f$[0, 1]@f$.
    * 
   ...
   /**
    * Computes the standard deviation of the beta distribution with
    * parameters @f$\alpha@f$ and @f$\beta@f$, over the interval @f$[a, b]@f$.
    * 
    * @return the standard deviation of the Beta distribution
    */
...
//      this.factor = 1.0 / (Beta * Math.pow (bminusa, alpha + beta - 1));
    ...
    * [@f$\alpha@f$, @f$\beta@f$, @f$a@f$, @f$b@f$].
    */
