 ...
 *                shape parameters @f$\gamma@f$ and @f$\delta> 0@f$, location
 *                parameter
 * @f$\xi@f$, and scale parameter @f$\lambda> 0@f$. Denoting
 * @f$t=(x-\xi)/\lambda@f$ and @f$z = \gamma+ \delta\ln\left(t + \sqrt{t^2 +
 *                         1}\right)@f$, the distribution has density @f[ f(x) =
 *                         \frac{\delta e^{-z^2/2}}{\lambda\sqrt{2\pi(t^2 +
 *                         1)}}, \qquad\mbox{for } -\infty< x < \infty, @f] and
 *                         distribution function @f[ F(x) = \Phi(z),
 *                         \qquad\mbox{for } -\infty< x < \infty, @f]
 *                         where @f$\Phi@f$ is the standard normal distribution
 *                         function. The inverse distribution function is @f[
 *                         F^{-1} (u) = \xi+ \lambda\sinh(v(u)), \qquad\mbox{for
 *                         } 0 \le u \le1, @f] where @f[ v(u) = [\Phi^{-1}(u) -
 *                         \gamma]/\delta. @f] This class relies on the methods
 *                         NormalDist.cdf01 and NormalDist.inverseF01 of @ref
 *                         NormalDist to approximate @f$\Phi@f$ and
 * @f$\Phi^{-1}@f$.
 *
...
public class JohnsonSUDist extends JohnsonSystem {
   ...
   /**
    * Returns the mean @f[ E[X] = \xi- \lambda e^{1/(2\delta^2)}
    * \sinh({\gamma}/{\delta}) @f] of the Johnson @f$S_U@f$ distribution with
    * parameters @f$\gamma@f$,
    * 
    * @f$\delta@f$, @f$\xi@f$ and @f$\lambda@f$.
    * @return the mean of the Johnson @f$S_U@f$ distribution @f$E[X] = \xi-
    *         \lambda\exp^{1 / (2\delta^2)} sinh(\gamma/ \delta)@f$
    */
   ...
   /**
    * Returns the variance @f[ \mbox{Var}[X] = \frac{\lambda^2}{2}
    * \left(e^{1/\delta^2} - 1\right)\left(e^{1/\delta^2} \cosh(2
    * {\gamma}/{\delta}) + 1\right) @f] of the Johnson @f$S_U@f$ distribution with
    * parameters @f$\gamma@f$,
    * 
    * @f$\delta@f$, @f$\xi@f$ and @f$\lambda@f$.
    * @return the variance of the Johnson @f$S_U@f$ distribution
    * @f$\mbox{Var}[X] = (\lambda^2/2)(\exp^{1/\delta^2} - 1)(\exp^{1/\delta^2}
    *                  cosh(2 \gamma/ \delta) + 1)@f$
    */
   ...
   /**
    * Returns the standard deviation of the Johnson @f$S_U@f$ distribution with
    * parameters @f$\gamma@f$, @f$\delta@f$, @f$\xi@f$,
    * 
    * @f$\lambda@f$. @return the standard deviation of the Johnson @f$S_U@f$
    *                distribution
    */
