 ...
 * @f$U_i@f$ over @f$(0,1)@f$, the *Anderson–Darling* statistic @f$A_n^2@f$ is
 *           defined by
 * @anchor REF_probdist_AndersonDarlingDist_eq_Andar
 * @f{align*}{ A_n^2 & = -n -\frac{1}{n} \sum_{j=1}^n \left\{ (2j-1)\ln(U_{(j)})
 *             + (2n+1-2j) \ln(1-U_{(j)}) \right\}, \tag{Andar}
 * @f} where the @f$U_{(j)}@f$ are the @f$U_i@f$ sorted in increasing order. The
 *     distribution function (the cumulative probabilities) is defined
 *     as @f$F_n(x) = P[A_n^2 \le x]@f$.
 *
...
public class AndersonDarlingDist extends ContinuousDistribution {
   ...
   /**
    * Return an array containing the parameter @f$n@f$ of the current distribution.
    */
