 ...
 * parameter @f$\beta@f$ and shape parameter @f$\gamma@f$. Its density is
 * 
 * @anchor REF_probdist_FatigueLifeDist_eq_fFatigueLife @f[ f(x) =
 *         \left[\frac{\sqrt{\frac{x - \mu}{\beta}} + \sqrt{\frac{\beta}{x -
 *         \mu}}}{2\gamma(x - \mu)}\right] \phi\left(\frac{\sqrt{\frac{x -
 *         \mu}{\beta}} - \sqrt{\frac{\beta}{x - \mu}}}{\gamma}\right),
 *         \qquad\mbox{for } x>\mu, \tag{fFatigueLife} @f] where @f$\phi@f$ is
 *         the probability density of the standard normal distribution. The
 *         distribution function is given by
 * @anchor REF_probdist_FatigueLifeDist_eq_FFatigueLife @f[ F(x) =
 *         \Phi\left(\frac{\sqrt{\frac{x - \mu}{\beta}} - \sqrt{\frac{\beta}{x -
 *         \mu}}}{\gamma}\right), \qquad\mbox{for } x>\mu,
 *         \tag{FFatigueLife} @f] where @f$\Phi@f$ is the standard normal
 *         distribution function. Restrictions: @f$\beta> 0@f$, @f$\gamma> 0@f$.
 *
...
public class FatigueLifeDist extends ContinuousDistribution {
   ...
   /**
    * Computes the density ( {@link REF_probdist_FatigueLifeDist_eq_fFatigueLife
   ...
   public static double density(double mu, double beta, double gamma, double x) {
      ...
      return (((Math.sqrt((x - mu) / beta) + Math.sqrt(beta / (x - mu))) / (2 * gamma * (x - mu)))
            * NormalDist.density(0.0, 1.0, y));
   }
    ...
    * distribution with parameters @f$\mu@f$, @f$\beta@f$ and @f$\gamma@f$.
    */
   ...
   /**
    * Estimates the parameters (@f$\mu@f$, @f$\beta@f$, @f$\gamma@f$) of the
    * fatigue life distribution using the maximum likelihood method, from
    * the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1,…, n-1@f$. The estimates
    * are returned in a three-element array, in regular order:
    * [@f$\mu@f$, @f$\beta@f$, @f$\gamma@f$]. The estimate of the parameters is
    * given by maximizing numerically the log-likelihood function, using the Uncmin
    * package @cite iSCHa,
    * 
    * @cite iVERa&thinsp;.
   ...
   /**
    * Computes and returns the mean @f$E[X] = \mu+ \beta(1 + \gamma^2/2)@f$ of the
    * fatigue life distribution with parameters
    * 
    * @f$\mu@f$, @f$\beta@f$ and @f$\gamma@f$.
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = \beta^2 \gamma^2 (1 + 5
    * \gamma^2/4)@f$ of the fatigue life distribution with
    * parameters @f$\mu@f$, @f$\beta@f$ and @f$\gamma@f$.
    * 
   ...
   /**
    * Computes and returns the standard deviation of the fatigue life distribution
    * with parameters @f$\mu@f$, @f$\beta@f$ and
    * 
    * @f$\gamma@f$. @return the standard deviation of the fatigue life distribution
    */
    ...
    * table is put in regular order: [@f$\mu@f$, @f$\beta@f$, @f$\gamma@f$].
    */
