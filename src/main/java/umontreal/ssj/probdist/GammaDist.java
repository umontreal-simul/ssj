 ...
 * @anchor REF_probdist_GammaDist_eq_fgamma @f[ f(x) =
 *         \frac{\lambda^{\alpha}x^{\alpha- 1}e^{-\lambda x}}{\Gamma(\alpha)},
 *         \qquad\mbox{for } x > 0,\tag{fgamma} @f] where @f$\Gamma@f$ is the
 *         gamma function, defined by
 * @anchor REF_probdist_GammaDist_eq_Gamma @f[ \Gamma(\alpha) =
 *         \int_0^{\infty}x^{\alpha-1} e^{-x} dx. \tag{Gamma} @f] In
 *         particular, @f$\Gamma(n) = (n-1)!@f$ when @f$n@f$ is a positive
 *         integer.
 *
...
public class GammaDist extends ContinuousDistribution {
   ...
   /**
    * Constructs a `GammaDist` object with parameters @f$\alpha@f$ = `alpha`
    * and @f$\lambda=1@f$.
    */
   ...
   /**
    * Constructs a `GammaDist` object with parameters @f$\alpha@f$ = `alpha`
    * and @f$\lambda@f$ = `lambda`, and approximations of roughly `d` decimal
    * digits of precision when computing functions.
    */
   ...
   /**
    * Computes the density function ( {@link REF_probdist_GammaDist_eq_fgamma
    * fgamma} ) at @f$x@f$.
    */
    ...
    * parameters @f$\alpha@f$ = `alpha` and @f$\lambda@f$ = `lambda`, whose density
    * is given by ( {@link REF_probdist_GammaDist_eq_fgamma fgamma} ). The
    * approximation is an improved version of the algorithm in
    * 
    * @cite tBAT70a&thinsp;. The function tries to return @f$d@f$ decimals digits
    *       of precision. For @f$\alpha@f$ not too large
    *       (e.g., @f$\alpha\le1000@f$), @f$d@f$ gives a good idea of the precision
    *       attained.
    */
   ...
   /**
    * Same as {@link #barF(double,double,int,double) barF(alpha, 1.0, d, x)}.
    */
   ...
   /**
    * Same as {@link #inverseF(double,double,int,double) inverseF(alpha, 1, d, u)}.
    */
   ...
   /**
    * Estimates the parameters @f$(\alpha,\lambda)@f$ of the gamma distribution
    * using the maximum likelihood method, from the @f$n@f$
    * observations @f$x[i]@f$, @f$i = 0, 1,…, n-1@f$. The estimates are returned
    * in a two-element array, in regular order: [@f$\alpha@f$, @f$\lambda@f$]. The
    * maximum likelihood estimators are the values
    * 
    * @f$(\hat{\alpha}, \hat{\lambda})@f$ that satisfy the equations:
    * @f{align*}{ \frac{1}{n} \sum_{i=1}^n \ln(x_i) - \ln(\bar{x}_n) & =
    *             \psi(\hat{\alpha}) - \ln(\hat{\alpha}) \\ \hat{\lambda} \bar{x}_n
    *             & = \hat{\alpha}
    ...
    *            \Gamma’(x) / \Gamma(x)@f$ (@cite tJOH95a&thinsp; (page 361)).
    ...
    * 
    * @f$\alpha@f$ and @f$\lambda@f$ estimated using the maximum likelihood method
   ...
   /**
    * Computes and returns the mean @f$E[X] = \alpha/\lambda@f$ of the gamma
    * distribution with parameters @f$\alpha@f$ and @f$\lambda@f$.
    * 
    * @return the mean of the gamma distribution @f$E[X] = \alpha/ \lambda@f$
    */
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = \alpha/\lambda^2@f$ of
    * the gamma distribution with parameters
    * 
    * @f$\alpha@f$ and @f$\lambda@f$.
    * @return the variance of the gamma distribution @f$\mbox{Var}[X] = \alpha/
    *         \lambda^2@f$
    */
   ...
   /**
    * Computes and returns the standard deviation of the gamma distribution with
