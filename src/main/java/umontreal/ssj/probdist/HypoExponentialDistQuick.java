...
/**
 * This class is a subclass of @ref HypoExponentialDist and also implements the
 * *hypoexponential* distribution. It uses different algorithms to compute the
 * probabilities. The formula (
 * {@link REF_probdist_HypoExponentialDist_eq_tail_hypoexp tail-hypoexp} ) for
 * the complementary distribution is mathematically equivalent to (see @cite
 * pROS07b&thinsp; (page 299) and
 * 
 * @cite pGER10a&thinsp; (Appendix B))
 * @anchor REF_probdist_HypoExponentialDistQuick_eq_convolution_hypo @f[
 *         \bar{F}(x) = \mathbb P\left[X_1 + \cdots+ X_k > x \right] =
 *         \sum_{i=1}^k e^{-\lambda_i x} \prod_{\substack {j=1\\j\not i}}^k
 *         \frac{\lambda_j}{\lambda_j - \lambda_i}. \tag{convolution-hypo} @f]
 *         The expression (
 *         {@link REF_probdist_HypoExponentialDistQuick_eq_convolution_hypo
 *         convolution-hypo} ) is much faster to compute than the matrix
 *         exponential formula (
 *         {@link REF_probdist_HypoExponentialDist_eq_tail_hypoexp tail-hypoexp}
 *         ), but it becomes numerically unstable when @f$k@f$ gets large and/or
 *         the differences between the @f$\lambda_i@f$ are too small, because it
 *         is an alternating sum with relatively large terms of similar size.
 *         When the @f$\lambda_i@f$ are close, many of the factors
 * @f$\lambda_j - \lambda_i@f$ in (
 *              {@link REF_probdist_HypoExponentialDistQuick_eq_convolution_hypo
 *              convolution-hypo} ) are small, and the effect of this is
 *              amplified when
 * @f$k@f$ is large. This gives rise to large terms of opposite sign in the sum
 *         and the formula becomes unstable due to subtractive cancellation. For
 *         example, with the computations done in standard 64-bit floating-point
 *         arithmetic, if the @f$\lambda_i@f$ are regularly spaced with
 *         differences of @f$\lambda_{i+1} - \lambda_i = 0.1@f$ for all @f$i@f$,
 *         the formula (
 *         {@link REF_probdist_HypoExponentialDistQuick_eq_convolution_hypo
 *         convolution-hypo} ) breaks down already for @f$k \approx15@f$, while
 *         if the differences @f$\lambda_{i+1} - \lambda_i = 3@f$, it gives a
 *         few decimal digits of precision for @f$k@f$ up to @f$\approx300@f$.
 *
 *         The formula ( {@link REF_probdist_HypoExponentialDist_eq_fhypoexp
 *         fhypoexp} ) for the density is mathematically equivalent to the much
 *         faster formula
 * @anchor REF_probdist_HypoExponentialDistQuick_eq_fhypoexp2 @f[ f(x) =
 *         \sum_{i=1}^k\lambda_i e^{-\lambda_i x} \prod_{\substack {j=1\\j\not
 *         i}}^k \frac{\lambda_j}{\lambda_j - \lambda_i}, \tag{fhypoexp2} @f]
