 ...
 * @anchor REF_probdist_LoglogisticDist_eq_floglogistic @f[ f(x) =
 *         \frac{\alpha(x / \beta)^{\alpha- 1}}{\beta[1 + (x /
 *         \beta)^{\alpha}]^2} \qquad\qquad\mbox{for } x > 0
 *         \tag{floglogistic} @f] and its distribution function is
 * @anchor REF_probdist_LoglogisticDist_eq_Floglogistic @f[ F(x) = \frac{1}{1 +
 *         (\frac{x}{\beta})^{-\alpha}} \qquad\qquad\mbox{for } x > 0.
 *         \tag{Floglogistic} @f] The complementary distribution is
 * @anchor REF_probdist_LoglogisticDist_eq_Fbarloglogistic @f[ \bar{F}(x) =
 *         \frac{1}{1 + (\frac{x}{\beta})^{\alpha}} \qquad\qquad\mbox{for } x >
 *         0. \tag{Fbarloglogistic} @f] <div class="SSJ-bigskip"></div>
 *
...
public class LoglogisticDist extends ContinuousDistribution {
    ...
    * {@link REF_probdist_LoglogisticDist_eq_floglogistic floglogistic} ) for a
    * log-logisitic distribution with parameters
    * 
    ...
    * {@link REF_probdist_LoglogisticDist_eq_Floglogistic Floglogistic} ) of the
    * log-logistic distribution with parameters
    * 
    * @f$\alpha@f$ and @f$\beta@f$.
    */
    ...
    * {@link REF_probdist_LoglogisticDist_eq_Fbarloglogistic Fbarloglogistic} ) of
    ...
    * parameters @f$\alpha@f$ and @f$\beta@f$ estimated using the maximum
   ...
   /**
    * Computes and returns the mean @f$E[X] = \beta\theta \mbox{cosec}(\theta),
    * \mbox{ where } \theta= \pi/\alpha,@f$ of the log-logistic distribution with
    * parameters @f$\alpha@f$ and
    * 
    * @f$\beta@f$. @return the mean of the log-logistic distribution @f$E[X] =
    ...
    *                  {\pi/\alpha},@f$ of the log-logistic distribution with
    *                  parameters @f$\alpha@f$ and
    * @f$\beta@f$. @return the variance of the log-logistic distribution
    * @f$\mbox{Var}[X] = \beta^2 \theta(2 \mbox{cosec}(2 \theta) -
   ...
   /**
    * Computes and returns the standard deviation of the log-logistic distribution
    ...
    * table is put in regular order: [@f$\alpha@f$, @f$\beta@f$].
    */
