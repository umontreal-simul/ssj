 ...
 * 
 * @anchor REF_probdist_UniformIntDist_eq_fmassuniformint @f[ p(x) = \frac{1}{j
 *         - i + 1} \qquad\mbox{ for } x = i, i + 1, …, j
 *         \tag{fmassuniformint} @f] and 0 elsewhere. The distribution function
 *         is
 * @anchor REF_probdist_UniformIntDist_eq_cdfuniformint @f[ F(x) =
 *         \left\{\begin{array}{ll} 0, & \mbox{ for } x < i \\
 *         \displaystyle\frac{\lfloor x\rfloor-i+1}{j-i+1}, & \mbox{ for } i\le
 *         x < j \\ 1, & \mbox{ for } x \ge j. \end{array}\right.
 *         \tag{cdfuniformint} @f] and its inverse is
 * @anchor REF_probdist_UniformIntDist_eq_invuniformint @f[ F^{-1}(u) = i +
 *         \lfloor(j - i + 1)u\rfloor\qquad\mbox{for }0 \le u \le1.
 *         \tag{invuniformint} @f] <div class="SSJ-bigskip"></div>
 *
...
public class UniformIntDist extends DiscreteDistributionInt {
   ...
   /**
    * Constructs a discrete uniform distribution over the interval @f$[i,j]@f$.
    */
    ...
    * {@link REF_probdist_UniformIntDist_eq_fmassuniformint fmassuniformint} ).
    */
    ...
    * {@link REF_probdist_UniformIntDist_eq_cdfuniformint cdfuniformint} ).
    */
    ...
    * 
    * @f$\bar{F}(x)@f$. *WARNING:* The complementary distribution function is
    *                   defined as @f$\bar{F}(x) = P[X \ge x]@f$.
    */
    ...
    * {@link REF_probdist_UniformIntDist_eq_invuniformint invuniformint} ).
    */
   ...
   /**
    * Estimates the parameters @f$(i, j)@f$ of the uniform distribution over
    * integers using the maximum likelihood method, from the @f$n@f$
    * observations @f$x[k]@f$, @f$k = 0, 1, …, n-1@f$. The estimates are returned
    * in a two-element array, in regular order: [@f$i@f$, @f$j@f$]. The maximum
    * likelihood estimators are the values
    * 
    * @f$(\hat{\imath}@f$, @f$\hat{\jmath})@f$ that satisfy the equations
    * @f{align*}{ \hat{\imath} & = \mbox{min} \{x_k\} \\ \hat{\jmath} & =
    *             \mbox{max} \{x_k\}
   ...
   /**
    * Creates a new instance of a discrete uniform distribution over integers with
    * parameters @f$i@f$ and @f$j@f$ estimated using the maximum likelihood method
    * based on the @f$n@f$ observations
    * 
   ...
   /**
    * Computes and returns the mean @f$E[X] = (i + j)/2@f$ of the discrete uniform
    * distribution.
    * 
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = [(j - i + 1)^2 -
    * 1]/{12}@f$ of the discrete uniform distribution.
    * 
    ...
    * table is put in regular order: [@f$i@f$, @f$j@f$].
    */
