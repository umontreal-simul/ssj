 ...
 * equidistant @f$\lambda_i = (n+1-i)h@f$. We have @f$\lambda_{i+1} - \lambda_i
 * = h@f$, with @f$h@f$ a constant, and @f$n \ge k@f$ are integers.
 *
 * The formula (
 * {@link REF_probdist_HypoExponentialDistQuick_eq_convolution_hypo
 * convolution-hypo} ) becomes
 * 
 * @anchor REF_probdist_HypoExponentialDistEqual_eq_conv_hypo_equal @f[
 *         \bar{F}(x) = \mathbb P\left[X_1 + \cdots+ X_k > x \right] =
 *         \sum_{i=1}^k e^{-(n+1-i)h x} \prod_{\substack {j=1\\j\not i}}^k
 *         \frac{n+1-j}{i - j}. \tag{conv-hypo-equal} @f] The formula (
 *         {@link REF_probdist_HypoExponentialDistQuick_eq_fhypoexp2 fhypoexp2}
 *         ) for the density becomes
 * @anchor REF_probdist_HypoExponentialDistEqual_eq_fhypoexp3 @f[ f(x) =
 *         \sum_{i=1}^k (n+1-i)h e^{-(n+1-i)h x} \prod_{\substack {j=1\\j\not
 *         i}}^k \frac{n+1-j}{i - j}. \tag{fhypoexp3} @f]
...
public class HypoExponentialDistEqual extends HypoExponentialDist {
   ...
   /**
    * Constructor for equidistant rates. The rates are @f$\lambda_i = (n+1-i)h@f$,
    * for @f$i = 1,…,k@f$.
    * 
   ...
   /**
    * Computes the density function @f$f(x)@f$, with the same arguments as in the
    * constructor.
    * 
   ...
   /**
    * Computes the distribution function @f$F(x)@f$, with arguments as in the
    * constructor.
    * 
    * @param n max possible number of @f$\lambda_i@f$
    * @param k effective number of @f$\lambda_i@f$
   ...
   /**
    * Computes the complementary distribution @f$\bar{F}(x)@f$, as in formula (
   ...
   /**
    * Computes the inverse distribution @f$x=F^{-1}(u)@f$, with arguments as in the
    * constructor.
    * 
    * @param n max possible number of @f$\lambda_i@f$
    * @param k effective number of @f$\lambda_i@f$
