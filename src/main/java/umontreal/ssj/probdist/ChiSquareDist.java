 ...
 * 
 * @anchor REF_probdist_ChiSquareDist_eq_Fchi2 @f[ f(x) =
 *         \frac{x^{(n/2)-1}e^{-x/2}}{2^{n/2}\Gamma(n/2)},\qquad\mbox{for } x >
 *         0 \tag{Fchi2} @f] where @f$\Gamma(x)@f$ is the gamma function defined
 *         in ( {@link REF_probdist_GammaDist_eq_Gamma Gamma} ). The
 *         *chi-square* distribution is a special case of the *gamma*
 *         distribution with shape parameter @f$n/2@f$ and scale
 *         parameter @f$1/2@f$. Therefore, one can use the methods of @ref
 *         GammaDist for this distribution.
 *
 *         The non-static versions of the methods `cdf`, `barF`, and `inverseF`
...
public class ChiSquareDist extends ContinuousDistribution {
   ...
   /**
    * Computes the density function ( {@link REF_probdist_ChiSquareDist_eq_Fchi2
    * Fchi2} ) for a *chi-square* distribution with @f$n@f$ degrees of freedom.
    */
   ...
   /**
    * Computes the chi-square distribution function with @f$n@f$ degrees of
    * freedom, evaluated at @f$x@f$. The method tries to return @f$d@f$ decimals
    ...
    * 
    * @f$n@f$ degrees of freedom, evaluated at @f$x@f$. The method tries to
    *         return @f$d@f$ decimals digits of precision, but there is no
    *         guarantee.
    */
   ...
   /**
    * Computes an approximation of @f$F^{-1}(u)@f$, where @f$F@f$ is the chi-square
    * distribution with @f$n@f$ degrees of freedom. Uses the approximation given
    * in @cite tBES75a&thinsp; and in Figure L.23 of
    * 
    * @cite sBRA87a&thinsp;. It gives at least 6 decimal digits of precision,
    *       except far in the tails (that is, for @f$u< 10^{-5}@f$ or
    * @f$u > 1 - 10^{-5}@f$) where the function calls the method
    *      `GammaDist.inverseF (n/2, 7, u)` and multiplies the result by 2.0. To
    *      get better precision, one may call `GammaDist.inverseF`, but this method
    *      is slower than the current method, especially for large
   ...
   /**
    * Estimates the parameter @f$n@f$ of the chi-square distribution using the
    * maximum likelihood method, from the @f$m@f$ observations
    * 
    * @f$x[i]@f$, @f$i = 0, 1, …, m-1@f$. The estimate is returned in element 0
   ...
   /**
    * Computes and returns the mean @f$E[X] = n@f$ of the chi-square distribution
   ...
   /**
    * Estimates and returns the parameter [@f$\hat{n}@f$] of the chi-square
    * distribution using the moments method based on the
    * 
   ...
   /**
    * Returns the variance @f$\mbox{Var}[X] = 2n@f$ of the chi-square distribution
    * with parameter @f$n@f$.
    * 
    * @return the variance of the chi-square distribution @f$\mbox{Var}X] = 2n@f$
    */
