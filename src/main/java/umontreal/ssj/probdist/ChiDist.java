 ...
 *       the number of degrees of freedom @f$\nu@f$ is a positive integer. The
 *       density function is given by
 * @anchor REF_probdist_ChiDist_eq_Fchi @f[ f (x) = \frac{e^{-x^2 /2}
 *         x^{\nu-1}}{2^{(\nu/2)-1}\Gamma(\nu/2)}, \qquad\mbox{ for } x > 0,
 *         \tag{Fchi} @f] where @f$\Gamma(x)@f$ is the gamma function defined in
 *         ( {@link REF_probdist_GammaDist_eq_Gamma Gamma} ). The distribution
 *         function is @f[ F (x) = \frac{1}{\Gamma(\nu/2)} \int_0^{x^2/2}
 *         t^{\nu/2-1}e^{-t} dt. @f] It is equivalent to the gamma distribution
...
public class ChiDist extends ContinuousDistribution {
   ...
   public static double density(int nu, double x) {
      ...
      if (x <= 0.0)
         return 0.0;
      return Math.exp((nu - 1) * Math.log(x) - x * x / 2.0 - (nu / 2.0 - 1.0) * Num.LN2 - Num.lnGamma(nu / 2.0));
   }
   ...
   /**
    * Estimates the parameter @f$\nu@f$ of the chi distribution using the maximum
    * likelihood method, from the @f$n@f$ observations @f$x[i]@f$,
    * 
    ...
    * 
    * @f$\nu@f$ estimated using the maximum likelihood method based on the @f$n@f$
   ...
   /**
    * Computes and returns the mean @f[ E[X] = \frac{\sqrt{2} \Gamma( \frac{\nu+
    * 1}{2} )}{\Gamma(\frac{\nu}{2})} @f] of the chi distribution with
    * parameter @f$\nu@f$.
    * 
    * @return the mean of the chi distribution @f$E[X] = \sqrt{2}\Gamma((\nu+ 1) /
    *         2) / \Gamma(\nu/ 2)@f$
    */
   ...
   /**
    * Computes and returns the standard deviation of the chi distribution with
    * parameter @f$\nu@f$.
    * 
