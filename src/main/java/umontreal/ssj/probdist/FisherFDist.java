...
/**
 * Extends the class @ref ContinuousDistribution for the *Fisher F* distribution
 * with @f$n_1@f$ and @f$n_2@f$ degrees of freedom, where
 * 
 * @f$n_1@f$ and @f$n_2@f$ are positive integers. Its density is
 * @anchor REF_probdist_FisherFDist_eq_FisherF @f[ f(x) = \frac{\Gamma(\frac{n_1
 *         +
 *         n_2}{2})n_1^{\frac{n_1}{2}}n_2^{\frac{n_2}{2}}}{\Gamma(\frac{n_1}{2})\Gamma(\frac{n_2}{2})}
 *         \frac{x^{\frac{n_1 - 2}{2}}}{(n_2 + n_1x)^{\frac{n_1 + n_2}{2}}},
 *         \qquad\mbox{for } x > 0 \tag{FisherF} @f] where @f$\Gamma(x)@f$ is
 *         the gamma function defined in (
...
public class FisherFDist extends ContinuousDistribution {
   ...
   /**
    * Constructs a Fisher @f$F@f$ distribution with `n1` and `n2` degrees of
    * freedom.
    */
   ...
   /**
    * Computes the density function ( {@link REF_probdist_FisherFDist_eq_FisherF
    * FisherF} ) for a Fisher @f$F@f$ distribution with `n1` and `n2` degrees of
    * freedom, evaluated at @f$x@f$.
    */
   ...
   /**
    * Computes the distribution function of the Fisher @f$F@f$ distribution with
    * parameters `n1` and `n2`, evaluated at @f$x@f$, with roughly @f$d@f$ decimal
    * digits of precision.
    */
   @Deprecated
   ...
   /**
    * Computes the distribution function of the Fisher @f$F@f$ distribution with
    * parameters `n1` and `n2`, evaluated at @f$x@f$.
    */
    ...
    * Computes the complementary distribution function of the Fisher
    * 
    * @f$F@f$ distribution with parameters `n1` and `n2`, evaluated at @f$x@f$.
    */
   ...
   /**
    * Computes the inverse of the Fisher @f$F@f$ distribution with parameters `n1`
    * and `n2`, evaluated at @f$u@f$, with roughly @f$d@f$ decimal digits of
    * precision.
    */
   @Deprecated
    ...
    * 
    * @f$=n_2@f$. @return the mean of the Fisher @f$F@f$ distribution
    */
   ...
   /**
    * Computes and returns the variance @f[ \mbox{Var}[X] = \frac{2n_2^2 (n_2 + n_1
    * - 2)}{n_1 (n_2 - 2)^2 (n_2 - 4)} @f] of the Fisher @f$F@f$ distribution with
    * parameters `n1` @f$=n_1@f$ and `n2` @f$=n_2@f$.
    * 
    * @return the variance of the Fisher @f$F@f$ distribution
    * @f$\mbox{Var}[X] = (2n2^2 (n2 + n1 - 2)) / (n1 (n2 - 2)^2 (n2 - 4))@f$
    */
    ...
    * table is put in regular order: [<tt>n1</tt>, <tt>n2</tt>].
    */
