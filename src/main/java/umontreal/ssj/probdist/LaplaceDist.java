...
/**
 * Extends the class @ref ContinuousDistribution for the *Laplace* distribution
 * (see, e.g., @cite tJOH95b&thinsp; (page 165)). It has location
 * parameter @f$\mu@f$ and scale parameter @f$\beta> 0@f$. The density function
 * is given by
 * 
 * @anchor REF_probdist_LaplaceDist_eq_flaplace @f[ f(x) = \frac{e^{-|x -
 *         \mu|/\beta}}{2\beta} \qquad\mbox{ for }-\infty< x < \infty.
 *         \tag{flaplace} @f] The distribution function is @f[ F (x) =
 *         \left\{\begin{array}{ll} \frac{1}{2} e^{(x - \mu)/\beta} & \mbox{ if
 *         } x\le\mu, \\ 1 - \frac{1}{2} e^{(\mu- x)/\beta} & \mbox{ otherwise,
 *         } \end{array}\right. @f] and its inverse is @f[ F^{-1} (u) =
 *         \left\{\begin{array}{ll} \beta\log(2u) + \mu & \mbox{ if } 0\le
 *         u\le\frac{1}{2}, \\ \mu- \beta\log(2(1-u)) & \mbox{ otherwise. }
 *         \end{array}\right. @f] <div class="SSJ-bigskip"></div>
 *
...
public class LaplaceDist extends ContinuousDistribution {
   ...
   /**
    * Constructs a `LaplaceDist` object with default parameters @f$\mu= 0@f$
    * and @f$\beta= 1@f$.
    */
   public LaplaceDist() {
      mu = 0;
   ...
   /**
    * Estimates the parameters @f$(\mu, \beta)@f$ of the Laplace distribution using
    ...
    * regular order: [@f$\mu@f$, @f$\beta@f$]. The maximum likelihood estimators
    * are the values
    * 
    * @f$(\hat{\mu}, \hat{\beta})@f$ that satisfy the equations:
    * @f{align*}{ \hat{\mu} & = \mbox{the median of } \{x_1,…,x_n\} \\
    *             \hat{\beta} & = \frac{1}{n} \sum_{i=1}^n |x_i - \hat{\mu}|.
    * @f} See @cite tJOH95b&thinsp; (page 172).
    ...
    * 
    * @f$\mu@f$ and @f$\beta@f$ estimated using the maximum likelihood method based
   ...
   /**
    * Computes and returns the mean @f$E[X] = \mu@f$ of the Laplace distribution
    * with parameters @f$\mu@f$ and @f$\beta@f$.
    * 
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = 2 \beta^2@f$ of the
    * Laplace distribution with parameters @f$\mu@f$ and
    * 
    * @f$\beta@f$. @return the variance of the Laplace
    *              distribution @f$\mbox{Var}[X] = 2 \beta^2@f$
    */
   ...
   /**
    * Computes and returns the standard deviation of the Laplace distribution with
    * parameters @f$\mu@f$ and @f$\beta@f$.
    * 
    ...
    * table is put in regular order: [@f$\mu@f$, @f$\beta@f$].
    */
