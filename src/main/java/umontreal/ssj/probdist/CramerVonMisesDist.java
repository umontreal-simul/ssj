 ...
 * distribution (see @cite tDUR73a, @cite tSTE70a, @cite tSTE86b&thinsp;). Given
 * a sample of @f$n@f$ independent uniforms @f$U_i@f$ over @f$[0,1]@f$, the
 * Cramér-von Mises statistic @f$W_n^2@f$ is defined by
 * 
 * @anchor REF_probdist_CramerVonMisesDist_eq_CraMis @f[ W_n^2 = \frac{1}{12n} +
 *         \sum_{j=1}^n \left(U_{(j)} - \frac{(j-0.5)}{n}\right)^2,
 *         \tag{CraMis} @f] where the @f$U_{(j)}@f$ are the @f$U_i@f$ sorted in
 *         increasing order. The distribution function (the cumulative
 *         probabilities) is defined as @f$F_n(x) = P[W_n^2 \le x]@f$.
 *
...
public class CramerVonMisesDist extends ContinuousDistribution {
    ...
    * @cite tSTE86b, @cite tAND52a, @cite tKNO74a&thinsp;). The approximation is
    *       based on the distribution function of @f$W^2 = \lim_{n\to\infty}
    *       W_n^2@f$, which has the following series expansion derived by Anderson
    *       and Darling @cite tAND52a&thinsp;: @f[ \qquad P(W^2 \le x) =
    *       \frac{1}{\pi\sqrt{x}} \sum_{j=0}^{\infty}(-1)^j \binom{-1/2}{j}
    *       \sqrt{4j+1}\;\; {exp}\left\{-\frac{(4j+1)^2}{16 x}\right\}
    *       K_{1/4}\left(\frac{(4j+1)^2}{16 x}\right), @f] where @f$K_{\nu}@f$ is
    *       the modified Bessel function of the second kind. To correct for the
    *       deviation between @f$P(W_n^2\le x)@f$ and
    * @f$P(W^2\le x)@f$, we add a correction in @f$1/n@f$, obtained empirically by
    *             simulation. For @f$n = 10@f$, 20, 40, the error is less than
    *             0.002, 0.001, and 0.0005, respectively, while for @f$n \ge100@f$
    *             it is less than 0.0005. For @f$n \to\infty@f$, we estimate that
    *             the method returns at least 6 decimal digits of precision.
    *             For @f$n = 1@f$, the method uses the exact
    *             distribution: @f$P(W_1^2 \le x) = 2 \sqrt{x - 1/12}@f$
    *             for @f$1/12 \le x \le1/3@f$.
    */
   ...
   /**
    * Computes @f$x = F_n^{-1}(u)@f$, where @f$F_n@f$ is the <em>Cramér-von
    * Mises</em> distribution with parameter @f$n@f$.
    */
    ...
    * 
    * @f$n@f$. @return the standard deviation
    */
