 ...
 * @anchor REF_probdist_HypoExponentialDist_eq_tail_hypomatrix @f[
 *         \tag{tail-hypomatrix} \mathbf{A}= \begin{pmatrix} -\lambda_1 &
 *         \lambda_1 & 0 & … & 0 \\ 0 & -\lambda_2 & \lambda_2 & … & 0 \\
 *         \vdots & \vdots & \ddots & &nbsp;&nbsp;\ddots & \vdots \\ 0 & … & 0
 *         & -\lambda_{k-1} & \lambda_{k-1} \\ 0 & … & 0 & 0 & -\lambda_k
 *         \end{pmatrix} @f] with @f$\lambda_j@f$ the rates of the @f$k@f$
 *         exponential random variables; then the cumulative complementary
 *         probability of the hypoexponential distribution is given by @cite
 *         pNEU81a,
 * @cite pLAT99a&thinsp;
 * @anchor REF_probdist_HypoExponentialDist_eq_tail_hypoexp @f[
 *         \tag{tail-hypoexp} \bar{F}(x) = \mathbb P \left[X_1 + \cdots+ X_k > x
 *         \right] = \sum_{j=1}^k \left(e^{\mathbf{A}x}\right)_{1j}, @f] i.e.,
 *         it is the sum of the elements of the first row of matrix
 * @f$e^{\mathbf{A}x}@f$. The density of the hypoexponential distribution is
 * @anchor REF_probdist_HypoExponentialDist_eq_fhypoexp @f[ f(x) =
 *         \left(-e^{\mathbf{A}x}\mathbf{A}\right)_{1k} = \lambda_k
 *         \left(e^{\mathbf{A}x}\right)_{1k}, \tag{fhypoexp} @f] i.e., it is
 *         element @f$(1,k)@f$ of matrix
 * @f$-e^{\mathbf{A}x}\mathbf{A}@f$. The distribution function is as
 *                                   usual @f$F(x) = 1 - \bar{F}(x)@f$.
 *
 *                                   See the class @ref HypoExponentialDistQuick
 *                                   for alternative formulae for the
 *                                   probabilities.
 *
...
public class HypoExponentialDist extends ContinuousDistribution {
   ...
   /**
    * Constructs a `HypoExponentialDist` object, with rates @f$\lambda_i = @f$
    ...
    * @f$1 - @f$<tt>barF(lambda, x)</tt>, which is much faster than `cdf` but loses
    *      precision in the lower tail.
    * @param lambda rates of the hypoexponential distribution
    * @param x      value at which the distribution is evaluated
    * @return distribution at @f$x@f$
    */
   ...
   /**
    * Returns the variance, @f$\mbox{Var}[X] = \sum_{i=1}^k 1/\lambda_i^2@f$, of
    * the hypoexponential distribution with rates
    * 
    * @f$\lambda_i = @f$ <tt>lambda[</tt>@f$i-1@f$<tt>]</tt>, @f$i = 1,…,k@f$.
    * @param lambda rates of the hypoexponential distribution
   ...
   /**
    * Returns the standard deviation of the hypoexponential distribution with
    * rates @f$\lambda_i = @f$ <tt>lambda[</tt>@f$i-1@f$<tt>]</tt>,
    * 
    * @f$i = 1,…,k@f$.
    * @param lambda rates of the hypoexponential distribution
    ...
    * 
    * @f$<tt>lambda[</tt>@f$i-1@f$<tt>]</tt>, @f$i = 1,…,k@f$ for this object.
    */
