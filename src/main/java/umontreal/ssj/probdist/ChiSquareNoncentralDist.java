...
/**
 * Extends the class @ref ContinuousDistribution for the *noncentral chi-square*
 * distribution with @f$\nu@f$ degrees of freedom and noncentrality
 * parameter @f$\lambda@f$, where @f$\nu> 0@f$ and
 * 
 * @f$\lambda> 0@f$ @cite tJOH95b&thinsp; (page 436). Its density is
 * @anchor REF_probdist_ChiSquareNoncentralDist_eq_nc_fchi2 @f[ f(x)
 *         =\frac{e^{-(x + \lambda)/2}}{2}
 *         \left(\frac{x}{\lambda}\right)^{(\nu-2)/4} I_{\nu/2 -
 *         1}\left(\sqrt{\lambda x}\right) \qquad\mbox{for } x > 0,
 *         \tag{nc-fchi2} @f] where @f$I_{\nu}(x)@f$ is the modified Bessel
 *         function of the first kind of order @f$\nu@f$ given by @f[ I_{\nu}(z)
 *         = \sum_{j=0}^{\infty}\frac{(z/2)^{\nu+ 2j}}{j!\; \Gamma(\nu+ j
 *         +1)}, @f] where @f$\Gamma(x)@f$ is the gamma function. Notice that
 *         this distribution is more general than the *chi-square* distribution
 *         since its number of degrees of freedom can be any positive real
 *         number. For
 * @f$\lambda= 0@f$ and @f$\nu@f$ a positive integer, we have the ordinary
 *             *chi-square* distribution.
 *
 *             The cumulative probability function can be written as
 * @anchor REF_probdist_ChiSquareNoncentralDist_eq_nc_cdfchi2 @f[ P[X \le x] =
 *         \sum_{j=0}^{\infty}\frac{e^{-\lambda/2}(\lambda/2)^j}{j!}
 *         P[\chi^2_{\nu+ 2j} \le x], \tag{nc-cdfchi2} @f] where @f$\chi^2_{\nu+
 *         2j}@f$ is the *central chi-square* distribution with @f$\nu+ 2j@f$
 *         degrees of freedom.
 *
...
public class ChiSquareNoncentralDist extends ContinuousDistribution {
    ...
    * of freedom and noncentrality parameter @f$\lambda= @f$ `lambda`.
    */
    ...
    * {@link REF_probdist_ChiSquareNoncentralDist_eq_nc_fchi2 nc-fchi2} ) for a
    * *noncentral chi-square* distribution with @f$\nu= @f$ `nu` degrees of freedom
    * and parameter @f$\lambda= @f$ `lambda`.
    */
    ...
    * {@link REF_probdist_ChiSquareNoncentralDist_eq_nc_cdfchi2 nc-cdfchi2} )
    * with @f$\nu= @f$ `nu` degrees of freedom and parameter @f$\lambda= @f$
    * `lambda`. For @f$\lambda< 1000@f$, we use a reasonnably efficient method of
    * summing the cumulative function (
    * {@link REF_probdist_ChiSquareNoncentralDist_eq_nc_cdfchi2 nc-cdfchi2} ) by
    * using recurrence relations to compute both the Poisson and the chi-square
    * probabilities in terms of neighboring probability terms.
    * For @f$\lambda\ge1000@f$, we use the normal approximation given in @cite
    * tPEN00a&thinsp;, except very near the mean where we use the central chi
    * square approximation from
    * 
    * @cite tPEA59a&thinsp;. This function returns at least 6 decimal degits of
    *       precision nearly everywhere.
    */
   ...
   /**
    * Computes the complementary noncentral chi-square distribution function
    ...
    * 
    * @f$\nu= @f$ `nu` degrees of freedom and parameter @f$\lambda= @f$ `lambda`.
    */
    ...
    * the noncentral chi-square distribution with parameters @f$\nu=@f$ `nu`
   ...
   /**
    * Computes and returns the standard deviation of the noncentral chi-square
