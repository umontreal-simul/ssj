 ...
 * @f$X@f$ can take the value @f$x_k@f$ with probability @f$p_k = P[X = x_k]@f$.
...
public class DiscreteDistribution implements Distribution {
   ...
   /**
    * Constructs a discrete distribution over the @f$n@f$ values contained in array
    * `values`, with probabilities given in array `prob`. Both arrays must have at
    * least @f$n@f$ elements, the probabilities must sum to 1, and the values are
    * assumed to be sorted by increasing order.
    */
   ...
   /**
    * Computes the variance @f$\mbox{Var}[X] = \sum_i^{} p_i (x_i - E[X])^2@f$ of
    * the distribution.
    */
    ...
    * table is built in regular order, according to constructor
