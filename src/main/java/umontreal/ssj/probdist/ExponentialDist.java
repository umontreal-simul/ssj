 ...
 * 
 * @anchor REF_probdist_ExponentialDist_eq_fexpon @f[ f(x) = \lambda e^{-\lambda
 *         x} \qquad\mbox{for }x\ge0, \tag{fexpon} @f] its distribution function
 *         is
 * @anchor REF_probdist_ExponentialDist_eq_Fexpon @f[ F(x) = 1 - e^{-\lambda
 *         x},\qquad\mbox{for }x \ge0, \tag{Fexpon} @f] and its inverse
 *         distribution function is @f[ F^{-1}(u) = -\ln(1-u)/\lambda,
 *         \qquad\mbox{for } 0 < u < 1. @f] <div class="SSJ-bigskip"></div>
 *
...
public class ExponentialDist extends ContinuousDistribution {
   ...
   /**
    * Constructs an `ExponentialDist` object with rate parameter @f$\lambda@f$ = 1.
    */
   ...
   /**
    * Estimates the rate parameter @f$\lambda@f$ of the exponential distribution
    * using the maximum likelihood method, from the @f$n@f$
    * observations @f$x[i]@f$, @f$i = 0, 1,…, n-1@f$. The estimate is returned in
    * a one-element array, as element 0. The equation of the maximum likelihood is
    * defined as @f$\hat{\lambda} = 1/\bar{x}_n@f$, where @f$\bar{x}_n@f$ is the
    * average of @f$x[0],…,x[n-1]@f$ (see
    * 
   ...
   /**
    * Computes and returns the mean, @f$E[X] = 1/\lambda@f$, of the exponential
    * distribution with rate parameter @f$\lambda@f$.
    * 
    * @return the mean of the exponential distribution @f$E[X] = 1 / \lambda@f$
    */
   ...
   /**
    * Computes and returns the variance, @f$\mbox{Var}[X] = 1/\lambda^2@f$, of the
    * exponential distribution with rate parameter
    * 
    * @f$\lambda@f$. @return the variance of the Exponential distribution
   ...
   /**
    * Computes and returns the standard deviation of the exponential distribution
