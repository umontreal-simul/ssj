 ...
 * defined as follows (e.g., @cite sLAW00a&thinsp; (page 318)). The distribution
 * function starts at @f$X_{(1)}@f$ and climbs linearly by
 * 
 * @f$1/(n-1)@f$ between any two successive observations. The density is @f[
 *               f(x) = \frac{1}{(n-1)(X_{(i+1)} - X_{(i)})} \mbox{ for
 *               }X_{(i)}\le x < X_{(i+1)}\mbox{ and } i=1,2,…,n-1. @f] The
 *               distribution function is @f[ F(x) = \left\{\begin{array}{ll} 0
 *               & \mbox{ for } x < X_{(1)}, \\ \displaystyle\frac{i-1}{n-1} +
 *               \frac{x - X_{(i)}}{(n-1)(X_{(i+1)} - X_{(i)})} & \mbox{ for }
 *               X_{(i)} \le x < X_{(i+1)} \mbox{ and } i<n, \\ 1 & \mbox{ for }
 *               x \ge X_{(n)}, \end{array}\right. @f] whose inverse is @f[
 *               F^{-1}(u) = X_{(i)} + ((n-1)u - i + 1)(X_{(i+1)} - X_{(i)}) @f]
...
public class PiecewiseLinearEmpiricalDist extends ContinuousDistribution {
    ...
    * is ignored. The file is read until its end. One must be careful about lines
