 ...
 * inherit from this class. It specifies the signatures of methods for
 * computing the mass function (or probability) @f$p(x) = P[X=x]@f$,
 * distribution function @f$F(x)@f$, complementary distribution function
 * @f$\bar{F}(x)@f$, and inverse distribution function @f$F^{-1}(u)@f$, for a
 * random variable @f$X@f$ with a discrete distribution over the integers.
 *
 ...
 * distributions in SSJ, @f$F(j) + \bar{F}(j) \neq1@f$ since both include
 * the term @f$P[X = j]@f$.
 *
 * The implementing classes provide both static and non-static methods to
 * compute the above functions. The non-static methods require the creation
 * of an object of class
 * @ref umontreal.ssj.probdist.DiscreteDistributionInt; all the
 * non-negligible terms of the mass and distribution functions will be
 * precomputed by the constructor and kept in arrays. Subsequent accesses
 * will be very fast. The static methods do not require the construction of
 * an object. These static methods are not specified in this abstract class
 * because the number and types of their parameters depend on the
 * distribution. When methods have to be called several times with the same
 * parameters for the distributions, it is usually more efficient to create
 * an object and use its non-static methods instead of the static ones. This
...
/**
 * Environment variable that determines what probability terms can be
 * considered as negligible when building precomputed tables for distribution
 * and mass functions. Probabilities smaller than `EPSILON` are not stored in
 * the  @ref umontreal.ssj.probdist.DiscreteDistribution objects (such as
 * those of class  @ref PoissonDist, etc.), but are computed directly each
 * time they are needed (which should be very seldom). The default value is
 * set to @f$10^{-16}@f$.
 */
...
public abstract double prob (int x);
    ...
    *  @param x            value at which the distribution function must
    *                      be evaluated
   ...
   /**
    * Returns @f$\bar{F}(x)@f$, the complementary distribution function.
   ...
   /**
    * Returns @f$\bar{F}(x)@f$, the complementary distribution function.
    * *See the WARNING above*. The default implementation returns `1.0 -
    * cdf(x - 1)`, which is not accurate when @f$F(x)@f$ is near 1.
    *  @param x            value at which the complementary distribution
    *                      function must be evaluated
    *  @return the complementary distribution function evaluated at `x`
    */
   ...
   /**
    * Returns the lower limit @f$x_a@f$ of the support of the probability
    * mass function. The probability is 0 for all @f$x < x_a@f$.
   ...
   /**
    * Returns the upper limit @f$x_b@f$ of the support of the probability
    * mass function. The probability is 0 for all @f$x > x_b@f$.
