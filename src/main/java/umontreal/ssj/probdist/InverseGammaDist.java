 ...
 * 
 * @f$\beta> 0@f$, also known as the *Pearson type V* distribution. The density
 *           function is given by
 * @anchor REF_probdist_InverseGammaDist_eq_dinvgam @f[ f(x) =
 *         \left\{\begin{array}{ll} \displaystyle\frac{\beta^{\alpha}e^{-\beta/
 *         x}}{x^{\alpha+ 1} \Gamma(\alpha)} & \quad\mbox{for } x > 0 \\ 0 &
 *         \quad\mbox{otherwise,} \end{array} \right. \tag{dinvgam} @f]
 *         where @f$\Gamma@f$ is the gamma function. The distribution function
 *         is given by
 * @anchor REF_probdist_InverseGammaDist_eq_Finvgam @f[ F(x) = 1 -
 *         F_G\left(\frac{1}{x}\right) \qquad\mbox{for } x > 0,
 *         \tag{Finvgam} @f] and @f$F(x) = 0@f$ otherwise, where @f$F_G(x)@f$ is
 *         the distribution function of a gamma distribution with shape
...
public class InverseGammaDist extends ContinuousDistribution {
    ...
    * distribution with shape parameter @f$\alpha@f$ and scale
    * parameter @f$\beta@f$.
    */
   ...
   /**
    * Computes the complementary distribution function of the inverse gamma
    * distribution with shape parameter @f$\alpha@f$ and scale
    * parameter @f$\beta@f$.
    */
    ...
    * equations of the maximum likelihood are the same as the equations of the
    * gamma distribution, with the sample @f$y_i = 1/x_i@f$.
    * 
   ...
   /**
    * Returns the mean @f$E[X] = \beta/ (\alpha- 1)@f$ of the inverse gamma
    * distribution with shape parameter @f$\alpha@f$ and scale
    * parameter @f$\beta@f$.
    */
   ...
   /**
    * Returns the variance @f$\mbox{Var}[X] = \beta^2 / ((\alpha- 1)^2(\alpha-
    * 2))@f$ of the inverse gamma distribution with shape parameter @f$\alpha@f$
    * and scale parameter @f$\beta@f$.
    */
   ...
   /**
    * Returns the standard deviation of the inverse gamma distribution with shape
    * parameter @f$\alpha@f$ and scale parameter @f$\beta@f$.
    */
