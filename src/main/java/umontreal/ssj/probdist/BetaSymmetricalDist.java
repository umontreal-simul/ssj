 ...
 * 
 * @f$\alpha= \beta@f$. Faster methods are implemented here for this special
 *            case @cite rLEC06a&thinsp;. Because of the symmetry around 1/2,
 *            four series are used to compute the `cdf`, two around @f$x = 0@f$
 *            and two around @f$x = 1/2@f$.
 *
...
public class BetaSymmetricalDist extends BetaDist {
   ...
   /**
    * Same as `BetaSymmetricalDist (alpha)`, but using approximations of roughly
    * `d` decimal digits of precision when computing the distribution,
    * complementary distribution, and inverse functions.
    */
   ...
   /**
    * Same as {@link #cdf(double,double,int,double) cdf(alpha, alpha, d, x)}.
    */
   ...
   /**
    * Returns the inverse distribution function evaluated at @f$u@f$, for the
    * symmetrical beta distribution over the interval @f$[0,1]@f$, with shape
    * parameters @f$0 < \alpha= \beta@f$ = `alpha`. Uses four different
    * hypergeometric series to compute the distribution @f$u = F(x)@f$ (for the
    * four cases @f$x@f$ close to 0 and @f$\alpha< 1@f$,
    * 
    * @f$x@f$ close to 0 and @f$\alpha> 1@f$, @f$x@f$ close to 1/2 and
    * @f$\alpha< 1@f$, and @f$x@f$ close to 1/2 and @f$\alpha> 1@f$), which are
    *            then solved by Newton’s method for the solution of equations.
    *            For @f$\alpha> 100000@f$, uses a normal approximation given
    *            in @cite tPEI68a&thinsp;.
    */
   ...
   /**
    * Estimates the parameter @f$\alpha@f$ of the symmetrical beta distribution
    * over the interval [0, 1] using the maximum likelihood method, from
    * the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1, …, n-1@f$. The estimate
    * is returned in element 0 of the returned array. The maximum likelihood
    * estimator @f$\hat{\alpha}@f$ satisfies the equation
    * 
    * @f{align*}{ \psi(\hat{\alpha}) - \psi(2\hat{\alpha}) = \frac{1}{2n}
    *             \sum_{i=1}^n \ln(x_i(1 - x_i))
    ...
    * parameter @f$\alpha@f$ estimated using the maximum likelihood method based on
   ...
   /**
    * Computes and returns the mean @f$E[X] = 1/2@f$ of the symmetrical beta
    * distribution with parameter @f$\alpha@f$.
    * 
    * @return the mean of the symmetrical beta distribution @f$E[X] = 1/2@f$
    */
   ...
   /**
    * Computes and returns the variance, @f$\mbox{Var}[X] = 1/(8\alpha+ 4)@f$, of
    * the symmetrical beta distribution with parameter
    * 
    * @f$\alpha@f$. @return the variance of the symmetrical beta distribution
