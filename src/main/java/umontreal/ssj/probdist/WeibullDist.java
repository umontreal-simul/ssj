...
/**
 * This class extends the class @ref ContinuousDistribution for the *Weibull*
 * distribution @cite tJOH95a&thinsp; (page 628) with shape parameter @f$\alpha>
 * 0@f$, location parameter @f$\delta@f$, and scale parameter @f$\lambda> 0@f$.
 * The density function is
 * 
 * @anchor REF_probdist_WeibullDist_eq_fweibull @f[ f(x) =
 *         \alpha\lambda^{\alpha}(x-\delta)^{\alpha-1}
 *         e^{-(\lambda(x-\delta))^{\alpha}} \qquad\mbox{for }x>\delta,
 *         \tag{fweibull} @f] the distribution function is
 * @anchor REF_probdist_WeibullDist_eq_Fweibull @f[ F(x) = 1 - e^{-(\lambda(x -
 *         \delta))^{\alpha}} \qquad\mbox{for }x>\delta, \tag{Fweibull} @f] and
 *         the inverse distribution function is @f[ F^{-1}(u) =
 *         (-\ln(1-u))^{1/\alpha}/\lambda+ \delta\qquad\mbox{for } 0 \le u <
 *         1. @f] <div class="SSJ-bigskip"></div>
 *
...
// Verifier cette fonction si delta != 0.
   ...
   /**
    * Estimates the parameters @f$(\alpha, \lambda)@f$ of the Weibull distribution,
    * assuming that @f$\delta= 0@f$, using the maximum likelihood method, from
    * the @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1, …, n-1@f$. The estimates
    * are returned in a two-element array, in regular order:
    * [@f$\alpha@f$, @f$\lambda@f$]. The maximum likelihood estimators are the
    * values @f$(\hat{\alpha}@f$, @f$\hat{\lambda})@f$ that satisfy the equations
    * 
    * @f{align*}{ \frac{\sum_{i=1}^n x_i^{\hat{\alpha}} \ln(x_i)}{\sum_{i=1}^n
    *             x_i^{\hat{\alpha}}} - \frac{1}{\hat{\alpha}} & =
    *             \frac{\sum_{i=1}^n \ln(x_i)}{n} \\ \hat{\lambda} & = \left(
    *             \frac{n}{\sum_{i=1}^n x_i^{\hat{\alpha}}}
    *             \right)^{1/\hat{\alpha}}
    * @f} See @cite sLAW00a&thinsp; (page 303).
    ...
    * 
    * @f$\alpha@f$, @f$\lambda@f$ and @f$\delta= 0@f$ estimated using the maximum
   ...
   /**
    * Computes and returns the mean @f$E[X] = \delta+ \Gamma(1 +
    * 1/\alpha)/\lambda@f$ of the Weibull distribution with
    * parameters @f$\alpha@f$,
    * 
    * @f$\lambda@f$ and @f$\delta@f$.
    * @return the mean of the Weibull distribution @f$E[X] = \delta+ \Gamma(1 +
    *         1/\alpha) / \lambda@f$
    */
    ...
    * 
    * @f$\mbox{Var}[X] = | \Gamma(2/\alpha+ 1) - \Gamma^2(1/\alpha+ 1) |
    *                  /\lambda^2@f$ of the Weibull distribution with
    *                  parameters @f$\alpha@f$,
    * @f$\lambda@f$ and @f$\delta@f$.
    * @return the variance of the Weibull distribution @f$\mbox{Var}[X] = 1 /
    *         \lambda^2 | \Gamma(2/\alpha+ 1) - \Gamma^2(1/\alpha+ 1) |@f$
    */
   ...
   /**
    * Computes and returns the standard deviation of the Weibull distribution with
    * parameters @f$\alpha@f$, @f$\lambda@f$ and
    * 
    * @f$\delta@f$. @return the standard deviation of the Weibull distribution
    */
    ...
    * table is put in regular order: [@f$\alpha@f$, @f$\lambda@f$, @f$\delta@f$].
    */
