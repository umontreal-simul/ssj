 ...
 * 
 * @cite rDER09a&thinsp;. The cumulative probabilities (cdf) are pre-computed by
 *       numerical quadrature of the density using Gauss-Lobatto integration
 *       over suitably small intervals to satisfy the required precision, and
 *       these values are kept in tables. Then the algorithm uses polynomial
 *       interpolation over the tabulated values to get the inverse cdf. The
 *       user can select the desired precision and the degree of the
 *       interpolating polynomials.
 *
 *       The algorithm may fail for some distributions for which the density
 *       becomes infinite at a point (for ex. the Gamma and the Beta
 *       distributions with @f$\alpha< 1@f$) if one chooses too high a precision
 *       (a too small `eps`, for ex. @f$\epsilon\sim10^{-15}@f$). However, it
 *       should work also for continuous densities with finite discontinuities.
 *
...
public class InverseDistFromDensity extends ContinuousDistribution {
   ...
   /*
    * For NormalDist(0,1), local concavity c = 1/z^2 For CauchyDist(0,1), local
    * concavity c = -1/2 + 1/(2z^2) For GammaDist(a,lam), local concavity c =
    * (a-1)/(1 - a +lam*x)^2
    */
   ...
   private void init(double xc, double epsu, int n) {
      ...
      double eps = 0;
      if (DEBUG)
         System.out.println("  k                 a_k                      F_k                h");
      ...
      while (A[k] < br) {
         ...
         copy(k, cs, us, xs, n);
         if (DEBUG)
            System.out.printf(" %d       %16.12f       %20.16g      %g%n", k, A[k], F[k], h);
         if (F[k] > 1.01)
      ...
      if (DEBUG) {
         System.out.printf(" %d       %16.12f       %20.16g      %g%n", k, A[k], F[k], A[k] - A[k - 1]);
         System.out.println("\nFin du tableau");
      }
      Kmax = k;
      while (k > 0 && F[k] >= 1.) {
   ...
   /**
    * Given a continuous distribution `dist` with a well-defined density method,
    * this class will compute tables for the numerical inverse of the distribution.
    * The user may wish to set the left and the right boundaries between which the
    * density is non-zero by calling methods
    * umontreal.ssj.probdist.ContinuousDistribution.setXinf and
    * umontreal.ssj.probdist.ContinuousDistribution.setXsup of `dist`, for better
    * efficiency. Argument `xc` can be the mean, the mode or any other @f$x@f$ for
    * which the density is relatively large. The
    * 
    * @f$u@f$-resolution `eps` is the required absolute error in the cdf, and
   ...
   /**
    * Given a continuous probability density `dens`, this class will compute tables
    * for the numerical inverse of the distribution. The left and the right
    * boundaries of the density are `xleft` and `xright` (the density is 0 outside
    * the interval <tt>[xleft,
    ...
    * table is returned as: [<tt>xc</tt>, `eps`, <tt>order</tt>].
    */
...
//       System.out.println("NEval");
   ...
   private double binSearch(double xa, double xb, double eps, boolean right) {
      /*
       * Binary search: find x such that fa*epslow < f < fb*eps in the left tail find
       * x such that fa*eps > f > fb*epslow in the right tail where fa = density(xa),
       * fb = density(xb), f = density(x). We find an x such that density(x) is a
       * little smaller than eps
       */
      ...
      if (DEBUG)
         System.out.printf("binSearch   x =  %g    f =  %g     r =  %g%n", x, y, y / eps);
   ...
   private void findSupport(double xc) {
      /*
       * Find interval where density is non-negligible (above some epsilon): find
       * points bleft < xc < bright such that density(bleft) ~ density(bright) ~
       * 10^(-13)*density(xc)
       */
      boolean flagL = false;
      boolean flagR = false;
   ...
   private void findCutoff(double x0, double eps, boolean right) {
      /*
       * Find cut-off points for the computational domain. Find cut-off x in the tails
       * such that cdf(x) = eps in the left tail, and eps = 1 - cdf(x) in the right
       * tail. Uses successive approximations starting at x = x0. If right is true,
       * case of the right tail; otherwise the left tail. The program uses
       * T_c-concavity of densities as described in Leydold et al.
       */
      ...
      if (right) {
         ...
         if ((supportB < Double.POSITIVE_INFINITY)
               && (supportB - bright <= epsx || supportB - bright <= Math.abs(supportB) * epsx)) {
         ...
         if ((supportA > Double.NEGATIVE_INFINITY)
               && (bleft - supportA <= epsx || bleft - supportA <= Math.abs(supportA) * epsx)) {
      ...
      while (!fini && iter < ITERMAX) {
         ...
         if (DEBUG)
            System.out.printf("Cutoff   x =  %g    y =  %g     c =  %g%n", xnew, y, c);

         if ((Math.abs(xnew - x) <= Math.abs(x) * epsx) || (Math.abs(xnew - x) <= epsx))
