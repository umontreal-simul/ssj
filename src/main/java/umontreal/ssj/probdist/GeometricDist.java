 ...
 * 
 * @anchor REF_probdist_GeometricDist_eq_fgeom @f[ p (x) = p  (1-p)^x,
 *         \qquad\mbox{for } x = 0, 1, 2, …\tag{fgeom} @f] The distribution
 *         function is given by
 * @anchor REF_probdist_GeometricDist_eq_Fgeom @f[ F (x) = 1 - (1-p)^{x+1},
 *         \qquad\mbox{for } x = 0, 1, 2, …\tag{Fgeom} @f] and its inverse is
 * @anchor REF_probdist_GeometricDist_eq_FInvgeom @f[ F^{-1}(u) =
 *         \left\lfloor\frac{\ln(1 - u)}{\ln(1 - p)} \right\rfloor,
 *         \qquad\mbox{for } 0 \le u < 1. \tag{FInvgeom} @f]
...
public class GeometricDist extends DiscreteDistributionInt {
    ...
    * complementary distribution function is defined as @f$\bar{F}(x) = P[X \ge
    * x]@f$.
    */
    ...
    * {@link REF_probdist_GeometricDist_eq_FInvgeom FInvgeom} ).
    */
   ...
   /**
    * Estimates the parameter @f$p@f$ of the geometric distribution using the
    ...
    * @f$\hat{p}@f$ satisfies the equation (see @cite sLAW00a&thinsp; (page 323))
    * @f{align*}{ \hat{p} = \frac{1}{\bar{x}_n + 1\Rule{0.0pt}{11.0pt}{0.0pt}}
   ...
   /**
    * Computes and returns the mean @f$E[X] = (1 - p)/p@f$ of the geometric
    * distribution with parameter @f$p@f$.
    * 
    * @return the mean of the geometric distribution @f$E[X] = (1 - p) / p@f$
    */
   ...
   /**
    * Computes and returns the variance @f$\mbox{Var}[X] = (1 - p)/p^2@f$ of the
    * geometric distribution with parameter @f$p@f$.
    * 
    * @return the variance of the Geometric distribution @f$\mbox{Var}[X] = (1 - p)
    *         / p^2@f$
    */
   ...
   /**
    * Computes and returns the standard deviation of the geometric distribution
    * with parameter @f$p@f$.
    * 
