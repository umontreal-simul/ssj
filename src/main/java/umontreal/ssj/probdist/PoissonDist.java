 ...
 * @f{align}{ p(x) & = \frac{e^{-\lambda} \lambda^x}{x!}, \qquad\mbox{for }
 *            x=0,1,…\tag{fmass-Poisson}
 * @f} and the distribution function is
 * @anchor REF_probdist_PoissonDist_eq_FPoisson
 * @f{align}{ F(x) & = e^{-\lambda} \sum_{j=0}^x\; \frac{\lambda^j}{j!},
 *            \qquad\mbox{for } x=0,1,…. \tag{FPoisson}
 * @f} If one has to compute @f$p(x)@f$ and/or @f$F(x)@f$ for several values of
 * @f$x@f$ with the same @f$\lambda@f$, where @f$\lambda@f$ is not too large,
 *         then it is more efficient to instantiate an object and use the
 *         non-static methods, since the functions will then be computed once
 *         and kept in arrays.
 *
 *         For the static methods that compute @f$F(x)@f$ and @f$\bar{F}(x)@f$,
 *         we exploit the relationship @f$F(x) = 1 - G_{x+1}(\lambda)@f$, where
 * @f$G_{x+1}@f$ is the *gamma* distribution function with
 *               parameters @f$(\alpha,\lambda) = (x+1, 1)@f$.
 *
...
public class PoissonDist extends DiscreteDistributionInt {
   ...
   /**
    * The value of the parameter @f$\lambda@f$ above which the tables are *not*
   ...
   /**
    * Creates an object that contains the probability and distribution functions,
    * for the Poisson distribution with parameter `lambda`, which are computed and
    * stored in dynamic arrays inside that object.
    */
...
//         return GammaDist.cdf ((double)x, 15, lambda);
    ...
    *     {@link REF_probdist_PoissonDist_eq_fmass_Poisson fmass-Poisson} ).
    *     If @f$\lambda\ge20@f$, this (static) method uses the logarithm of the
    *     gamma function, defined in ( {@link REF_probdist_GammaDist_eq_Gamma
    *     Gamma} ), to estimate the density.
    */
    ...
    *            {@link REF_probdist_PoissonDist_eq_FPoisson FPoisson} ). To
    *            compute @f$F(x)@f$, all non-negligible terms of the sum are added
    *            if @f$\lambda\le200@f$; otherwise, the relationship
   ...
   /**
    * Computes and returns the value of the complementary Poisson distribution
    * function, for @f$\lambda= @f$ `lambda`. *WARNING:* The complementary
    * distribution function is defined as @f$\bar{F}(x) = P[X \ge x]@f$.
    */
   ...
   /**
    * Performs a linear search to get the inverse function without precomputed
    * tables.
    */
   ...
   /**
    * Estimates the parameter @f$\lambda@f$ of the Poisson distribution using the
    * maximum likelihood method, from the @f$n@f$ observations
    * 
    * @f$x[i]@f$, @f$i = 0, 1, …, n-1@f$. The maximum likelihood estimator
    * @f$\hat{\lambda}@f$ satisfy the equation @f$\hat{\lambda} = \bar{x}_n@f$,
    *                     where @f$\bar{x}_n@f$ is the average of @f$x[0], …,
    ...
    * 
    * @f$\lambda@f$ estimated using the maximum likelihood method based on
   ...
   /**
    * Computes and returns the variance @f$= \lambda@f$ of the Poisson distribution
    * with parameter @f$\lambda@f$.
    * 
   ...
   /**
    * Computes and returns the standard deviation of the Poisson distribution with
