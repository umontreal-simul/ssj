 ...
 * methods. The non-static version of `inverseF` calls the static version. This
 * method is not very accurate for small @f$n@f$ but becomes better as
 * 
...
public class ChiSquareDistQuick extends ChiSquareDist {
   ...
   /**
    * Computes a quick-and-dirty approximation of @f$F^{-1}(u)@f$, where @f$F@f$ is
    * the *chi-square* distribution with @f$n@f$ degrees of freedom. Uses the
    * approximation given in Figure L.24 of @cite sBRA87a&thinsp; over most of the
    * range. For @f$u < 0.02@f$ or @f$u > 0.98@f$, it uses the approximation given
    * in @cite tGOL73a&thinsp; for @f$n \ge10@f$, and returns `2.0 *`
    * {@link GammaDist.inverseF(double,int,double) inverseF(n/2, 6, u)} for @f$n <
    * 10@f$ in order to avoid the loss of precision of the above approximations.
    * When @f$n \ge10@f$ or @f$0.02 < u < 0.98@f$, it is between 20 to 30 times
    * faster than the same method in @ref ChiSquareDist for @f$n@f$
    * between @f$10@f$ and @f$1000@f$ and even faster for larger
    * 
    * @f$n@f$.
    *
