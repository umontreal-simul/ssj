...
/**
 * This class implements a string API for the package `probdist`. It uses Java
 * Reflection to allow the creation of probability distribution objects from a
 * string. This permits one to obtain distribution specifications from a file or
 * dynamically from user input during program execution. This string API is
 * similar to that of [UNURAN](http://statistik.wu-wien.ac.at/unuran/)
 * 
 * @cite iLEY02a&thinsp;.
 *
 *       The (static) methods of this class invoke the constructor specified in
 ...
 *       `str` of the methods) must be a valid call of the constructor of a
 *       class that extends @ref ContinuousDistribution or @ref
 *       DiscreteDistribution, and all parameter values must be numerical values
 *       (variable names are not allowed). If no parentheses follow, the default
 *       parameters are used, i.e. a no-parameter constructor is searched and
 *       invoked. Distribution parameters are surrounded with parentheses and
 *       are separated from each other using commas. The order and types of the
 *       parameters should be the same as the corresponding class constructor.
 *       When specifying decimal values, the dot should be used for decimal
 *       separation, not the comma. When specifying a `float` or a `double`, one
 *       can use `-infinity` or `infinity` to denote
 * @f$-\infty@f$ and @f$+\infty@f$, respectively. This is parsed to
 *               <tt>Double.NEGATIVE_INFINITY</tt> and
 *               <tt>Double.POSITIVE_INFINITY</tt>, respectively. However, this
 *               is not accepted by all probability distributions.
 *
 *               For example, if one uses the string `Normal`, a @ref NormalDist
 *               object with @f$\mu= 0@f$ and @f$\sigma= 1@f$ will be created.
 *               If one uses `Exponential(2.5)`, an @ref ExponentialDist object
 *               with
 * @f$\lambda=2.5@f$ will be constructed.
 *
 *                   The distribution parameters can also be estimated from a
 *                   set of observations instead of being passed to the
 *                   constructor. In that case, one passes the vector of
 *                   observations, and the constructor estimates the parameters
...
public class DistributionFactory {
    ...
    *      specified by the string `str`. This method throws exceptions if it
    *      cannot parse the given string and returns `null` if the distribution
    *      object simply could not be created due to a Java-specific instantiation
    *      problem.
    * @param str distribution specification string
    * @return a distribution object or `null` if it could not be instantiated
    *
    * @exception IllegalArgumentException if parsing problems occured when reading
    *                                     `str`
    */
   /**
    * Uses the Java Reflection API to construct a @ref ContinuousDistribution
    ...
    * 
    * @ref ContinuousDistribution object by executing the code contained in the
    *      string `str`. This code should be a valid invocation of the constructor
    *      of a @ref ContinuousDistribution object. This method throws exceptions
    *      if it cannot parse the given string and returns `null` if the
    *      distribution object could not be created due to a Java-specific
    *      instantiation problem.
    * @param str string that contains a call to the constructor of a continuous
    *            distribution
    * @return a continuous distribution object or `null` if it could not be
    *         instantiated
    *
    * @exception IllegalArgumentException if parsing problems occured when reading
    *                                     `str`
   ...
   /**
    * Same as #getContinuousDistribution, but for discrete distributions over the
    * real numbers.
    * 
    * @param str string that contains a call to the constructor of a discrete
    *            distribution
    * @return a discrete distribution object, or `null` if it could not be
    *         instantiated
    *
    * @exception IllegalArgumentException if parsing problems occured when reading
    *                                     `str`
