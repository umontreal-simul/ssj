 ...
 * <em>Gumbel</em>) distribution @cite tJOH95b&thinsp; (page 2), with location
 * parameter @f$\alpha@f$ and scale parameter @f$\lambda> 0@f$. It has density
 * 
 * @anchor REF_probdist_ExtremeValueDist_eq_fextremevalue @f[ f (x) = \lambda
 *         e^{-\lambda(x-\alpha)} e^{-e^{-\lambda(x-\alpha)}},
 *         \qquad\qquad\mbox{for } -\infty< x < \infty, \tag{fextremevalue} @f]
 *         distribution function
 * @anchor REF_probdist_ExtremeValueDist_eq_Fextreme @f[ F(x) =
 *         e^{-e^{-\lambda(x - \alpha)}} \qquad\qquad\mbox{for } -\infty< x <
 *         \infty, \tag{Fextreme} @f] and inverse distribution function @f[
 *         F^{-1}(u) = -\ln(-\ln(u))/\lambda+ \alpha, \qquad\mbox{for } 0 \le u
 *         \le1. @f] <div class="SSJ-bigskip"></div>
 *
...
public class ExtremeValueDist extends ContinuousDistribution {
   ...
   /**
    * <strong>THIS CLASS HAS BEEN REPLACED BY @ref GumbelDist </strong>. Constructs
    * a `ExtremeValueDist` object with parameters @f$\alpha@f$ = 0
    * and @f$\lambda@f$ = 1.
    */
   ...
   /**
    * <strong>THIS CLASS HAS BEEN REPLACED BY @ref GumbelDist </strong>. Constructs
    * a `ExtremeValueDist` object with parameters @f$\alpha@f$ = `alpha`
   ...
   /**
    * <strong>THIS CLASS HAS BEEN REPLACED BY @ref GumbelDist </strong>. Computes
   ...
   /**
    * Estimates the parameters @f$(\alpha,\lambda)@f$ of the extreme value
    * distribution using the maximum likelihood method, from the
    * 
    * @f$n@f$ observations @f$x[i]@f$, @f$i = 0, 1,…, n-1@f$. The estimates are
    *         returned in a two-element array, in regular order:
    *         [@f$\alpha@f$, @f$\lambda@f$]. The maximum likelihood estimators are
    *         the values @f$(\hat{\alpha}, \hat{\lambda})@f$ that satisfy the
    *         equations:
    * @f{align*}{ \hat{\lambda} & = \bar{x}_n - \frac{\sum_{i=1}^n x_i  e^{-
    *             \hat{\lambda} x_i}}{\sum_{i=1}^n e^{-\hat{\lambda} x_i}} \\
    *             \hat{\alpha} & = - \frac{1}{\hat{\lambda}} \ln\left( \frac{1}{n}
    *             \sum_{i=1}^n e^{-\hat{\lambda} x_i} \right),
    * @f} where @f$\bar{x}_n@f$ is the average of @f$x[0],…,x[n-1]@f$
    ...
    * parameters @f$\alpha@f$ and @f$\lambda@f$ estimated using the maximum
   ...
   /**
    * Computes and returns the mean, @f$E[X] = \alpha+ \gamma/\lambda@f$, of the
    * extreme value distribution with parameters @f$\alpha@f$ and @f$\lambda@f$,
    * where @f$\gamma= 0.5772156649@f$ is the Euler-Mascheroni constant.
    * 
    * @return the mean of the Extreme Value distribution @f$E[X] = \alpha+ \gamma/
    *         \lambda@f$
    */
   ...
   /**
    * Computes and returns the variance, @f$\mbox{Var}[X] = \pi^2/(6\lambda^2)@f$,
    * of the extreme value distribution with parameters @f$\alpha@f$
    * and @f$\lambda@f$.
    * 
   ...
   /**
    * Computes and returns the standard deviation of the extreme value distribution
    * with parameters @f$\alpha@f$ and @f$\lambda@f$.
    * 
    ...
    * table is put in regular order: [@f$\alpha@f$, @f$\lambda@f$].
    */
