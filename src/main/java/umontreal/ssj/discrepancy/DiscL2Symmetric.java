 ...
 * @anchor REF_discrepancy_DiscL2Symmetric_disc_sym @f[
 *         [\mathcal{D}_s(\mathcal{P})]^2 = \left(\frac{4}{3}\right)^s -
 *         \frac{2}{n} \sum_{i=1}^n \prod_{k=1}^s \left(1 + 2z_{ik} -
 *         2z_{ik}^2\right) + \frac{2^s}{n^2} \sum_{i=1}^n\sum_{j=1}^n
 *         \prod_{k=1}^s \left(1 - |z_{ik} - z_{jk}|\right), \tag{sym} @f]
 *         where @f$n@f$ is the number of points of @f$\mathcal{P}@f$, @f$s@f$
 *         is the dimension, and @f$z_{ik}@f$ is the @f$k@f$-th coordinate of
 *         point @f$i@f$.
 *
 *         In one dimension, formula (
 *         {@link REF_discrepancy_DiscL2Symmetric_disc_sym sym} ) is equivalent
 *         to
 * @anchor REF_discrepancy_DiscL2Symmetric_disc_symD1 @f[
 *         [\mathcal{D}_s(\mathcal{P})]^2 = \frac{4}{3} - \frac{4}{n}
 *         \sum_{i=1}^n z_i(1 - z_i) - \frac{2}{n^2}
 *         \sum_{i=1}^n\sum_{j=1}^n|z_i - z_j|, \tag{symD1} @f] where @f$z_i@f$
 *         is the coordinate of point @f$i@f$.
 *
...
public class DiscL2Symmetric extends Discrepancy {
   ...
   /**
    * Computes the @f$\mathcal{L}_2@f$-<strong>symmetric discrepancy</strong> for
    * the set of @f$n@f$ @f$s@f$-dimensional points `points`, using formula (
   ...
   /**
    * Computes the @f$\mathcal{L}_2@f$-<strong>symmetric discrepancy</strong> for
    * the set of @f$n@f$ 1-dimensional points
    * 
    * @f$T@f$, using formula ( {@link REF_discrepancy_DiscL2Symmetric_disc_symD1
    *          symD1} ).
    */
