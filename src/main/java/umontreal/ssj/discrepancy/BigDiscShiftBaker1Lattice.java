...
/**
 * This class computes the same discrepancy as in `DiscShiftBaker1Lattice` [see
 * eq. ( {@link REF_discrepancy_DiscShiftBaker1Lattice_shiftBaker1lat
...
public class BigDiscShiftBaker1Lattice extends BigDiscShiftBaker1 {
   ...
   /**
    * Constructor for a lattice of @f$n@f$ points in at most @f$s@f$ dimensions,
   ...
   protected void setFactorBig(int n, int s) {
      ...
      for (int i = 0; i < n; i++) {
         for (int j = 0; j < s; j++) {
            FactorBig[i][j] = computeFactor(UBig[i], C1Big[j], C2Big[j], C3Big[j]);
         }
      }
   }
    ...
    * ) for a rank-1 lattice in dimension @f$s@f$. This lattice is generated
    * by @f$a_r =@f$ `a[r-1]`, @f$r=1,2, …, s@f$.
    */
