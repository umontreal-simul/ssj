 ...
 * \le a_j < n@f$ for each @f$j@f$, the points are defined by @f[ \mathbf{u}_i =
 * (i/n)(a_0, a_1, …, a_{s-1}) \bmod1 @f] for @f$i=0,…,n-1@f$. Here we
 * always choose @f$a_0=1@f$.
 *
 * The discrepancy object in the `Searcher` constructor *must* fix the number of
 ...
 * find the best amongst those examined. Searches may be exhaustive or random.
 *
...
public class Searcher {
    ...
    * each lattice will be generated in the search. The flag `primeN` indicates
    ...
    * discrepancy in dimension @f$s@f$. The search runs over all values of the
    * generator @f$a_j = 1, 2, …, (n-1)@f$ and over all dimensions up to @f$s@f$
    * inclusively. The first component @f$a_0@f$ of the generator is always set to
    * 1. The method returns the best value of the discrepancy found in
    * dimension @f$s@f$.
    */
   ...
   /**
    * Random search to find the lattice with the best (the smallest) discrepancy in
    * dimension @f$s@f$. At most @f$k@f$ random (@f$s@f$-dimensional)
    * generators @f$\mathbf{a}@f$ are examined. Each random component @f$a_j@f$
    * takes values over the integers @f$1, 2, …, (n-1)@f$. The first
    * component @f$a_0@f$ is always set to 1. The method returns the best value of
   ...
   /**
    * Returns the generator of the lattice which gave the best value of the
    * discrepancy in the last search. The components of this generator are returned
    * as @f$a_j@f$, for @f$j = 0, 1, …, (s-1)@f$.
    */
   ...
   /**
    * Initializes the random number generator used in random searches with the
    * starting seed `seed`. If this method is not called, a default seed will be
    * used.
    */
