 ...
 * eq. ( {@link REF_discrepancy_DiscShiftBaker1_baker1 baker1} ) for class @ref
 * DiscShiftBaker1, but for the special case when the points are the nodes of an
 * integration lattice. It is given by
 * 
 * @anchor REF_discrepancy_DiscShiftBaker1Lattice_shiftBaker1lat
 * @f{align}{ [\mathcal{D}(\mathcal{L})]^2 & = -1 + \frac{1}{n} \sum_{i=1}^n
 *            \prod_{r=1}^s \left[1 - \frac{4\gamma_r^2}{3} \left[B_4(x_{ir}) -
 *            B_4(\{x_{ir}-1/2\})\right]\right. - \nonumber \\ &
 *            \frac{\gamma_r^4}{9} \left[7B_4(x_{ir}) -
 *            2B_4(\{x_{ir}-1/2\})\right] \left. {} - \frac{16\gamma_r^4}{45}
 *            \left[B_6(x_{ir}) -B_6(\{x_{ir}-1/2\})\right] \right],
 *            \tag{shiftBaker1lat}
 * @f} where @f$n@f$ is the number of points of @f$\mathcal{L}@f$, @f$s@f$ is
 ...
 *                     (chap. 23) of degree @f$\alpha@f$. The discrepancy is
 *                     much faster to calculate for a lattice than for a general
 *                     point set. For a *1-dimensional* lattice, the discrepancy
 *                     becomes
 * @anchor REF_discrepancy_DiscShiftBaker1Lattice_shiftBaker1latdim1
 * @f{align}{ [\mathcal{D}(\mathcal{L})]^2 & = - \frac{1}{n} \sum_{i=1}^n
 *            \left[\frac{4\gamma^2}{3} \left[B_4(x_i) -
 *            B_4(\{x_i-1/2\})\right]\right. + \nonumber \\ & \frac{\gamma^4}{9}
 *            \left[7B_4(x_i) - 2B_4(\{x_i-1/2\})\right] \left. {} +
 *            \frac{16\gamma^4}{45} \left[B_6(x_i) -B_6(\{x_i-1/2\})\right]
 *            \right], \tag{shiftBaker1latdim1}
 * @f} <div class="SSJ-bigskip"></div><div class="SSJ-bigskip"></div>
 */
public class DiscShiftBaker1Lattice extends DiscShiftBaker1 {
    ...
    * with all weights @f$\gamma_r =1@f$. `points[i][r]` is the <tt>r</tt>-th
    ...
    * {@link REF_discrepancy_DiscShiftBaker1Lattice_shiftBaker1lat shiftBaker1lat}
    * ) for the @f$s@f$-dimensional points of lattice `points`, containing @f$n@f$
    ...
    * {@link REF_discrepancy_DiscShiftBaker1Lattice_shiftBaker1lat shiftBaker1lat}
    * ) for the @f$s@f$-dimensional points of lattice `points`, containing @f$n@f$
    * points, with weights @f$\gamma_r = @f$ `gamma[r-1]`.
    */
    ...
    * shiftBaker1latdim1} ) with weight @f$\gamma=1@f$ for the 1-dimensional
