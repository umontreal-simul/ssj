 ...
 * {@link #compute(double[][],int,int) compute(points, n, s)}.
...
//   protected boolean power2F;                 // true if numPoints is a power of 2
    ...
    * 
    * @f$i@f$. Both @f$i@f$ and @f$j@f$ start at 0. One may also choose `points =
    *          null` in which case, the points must be set later.
    */
    ...
    * the @f$s@f$ weight factors <tt>gamma[</tt>@f$j@f$<tt>]</tt>, @f$j = 0, 1,
    * …, (s-1)@f$. `points[i][j]` is the @f$j@f$-th coordinate of point @f$i@f$.
    * Both
    * 
    * @f$i@f$ and @f$j@f$ start at 0. One may also choose `points = null` in which
    *         case, the points must be set later.
    */
   ...
   /**
    * Computes the discrepancy of all the points in maximal dimension (dimension of
    * the points).
    */
   ...
   /**
    * Computes the discrepancy of the first `n` points of `points` in dimension `s`
    * with weights `gamma`.
    */
    ...
    * Calls method {@link #compute(double[][],int,int,double[]) compute(points,
    ...
    * Copies the points in an array of arrays and calls method {@link #compute()
    * compute(double[][], n, 1)}. It should be reimplemented in subclasses for
    * better efficiency.
    */
   ...
   /**
    * Computes the discrepancy of all the points of `T` in 1 dimension. Calls
    * method {@link #compute(double[],int,double) compute(T, T.length, gamma[0])}.
    */
   ...
   /**
    * Computes the discrepancy of the first `n` points of `T` in 1 dimension with
    * weight `gamma`.
    */
   ...
   /**
    * Computes the discrepancy of all the points in `set` in the same dimension as
    * the point set and with weights `gamma`.
    */
   ...
   /**
    * Computes the discrepancy of all the points in `set` in the same dimension as
    * the point set. All the weights @f$=1@f$.
    */
   ...
   /**
    * Sets the points to `points` and the dimension to @f$s@f$. The number of
    * points is @f$n@f$.
    */
   ...
   /**
    * Sets the points to `points`. The number of points and the dimension are the
    * same as in `points`.
    */
   ...
   /**
    * Sorts the first @f$n@f$ points of @f$T@f$. Returns the sorted points.
    * *Warning:* @f$T@f$ is sorted also.
    */
