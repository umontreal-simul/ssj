 ...
 * parameter value to this index (using #setParam ) so that a graph or a data
 * file is created showing the discrepancies as functions of the parameter. One
 * can also scale the discrepancies of an index with a given scale factor or
 * take the logarithm of the discrepancies.
 *
 * The discrepancies are computed, assuming that the theoretical distribution of
 * the points is over the unit hypercube @f$[0,1]^s@f$; thus all the coordinates
 * of the points must be in @f$[0,1]@f$.
 *
...
public class DiscrepancyContainer {
   ...
   /**
    * Initialize the container with enough space for `n` values of the parameter
    * and sets the values to 0. Also sets the labels for the parameter and the
    * discrepancy that will be used for graphs and data file.
    * 
   ...
   /**
    * Computes the discrepancies of the first `n` values contained in `points` and
    * sets the values at index `i`.
    * 
   ...
   /**
    * Computes the discrepancies of the first `n` values contained in `points`
    * using the first `s` coordinates and sets the values at index `i`.
    * 
    * @param i      index where to add the discrepancies
    * @param points values for which to compute the discrepancies
    * @param n      number of points to use
   ...
   /**
    * Computes the discrepancies of the first `n` values contained in `points`, and
    * adds the values at index `i`. **NOTE: This method does not replace the
    * values, it adds to them. Can be used with #scale to calculate an average
   ...
   /**
    * Computes the square of the discrepancies of the first `n` values contained in
    * `points`, and adds the values at index `i`. **NOTE: This method does not
    * replace the values, it adds to them. Can be used with #scale to calculate an
   ...
   /**
    * Computes the discrepancies of the first `n` values contained in `points`
    * using the first `s` coordinates, and adds the values at index `i`. **NOTE:
    * This method does not replace the values, it adds to them. Can be used with
    * #scale to calculate an average discrepancy over more than one point set.
    * 
    * @param i      index where to add the discrepancies
    * @param points values for which to compute the discrepancies
    * @param n      number of points to use
   ...
   /**
    * Computes the square discrepancies of the first `n` values contained in
    * `points` using the first `s` coordinates, and adds the values at index `i`.
    * **NOTE: This method does not replace the values, it adds to them. Can be used
    * with #scale to calculate an average square discrepancy over more than one
    * point set.
    * 
    * @param i      index where to add the discrepancies
    * @param points values for which to compute the discrepancies
   ...
   /**
    * Calls {@link #scale(int,double) scale(i,scale)} for all indices `i`.
    * 
    * @param scale scale factor
    */
   ...
   /**
    * Computes the linear regression slope for the discrepancies as function of the
    * parameter. Used by #regressionToString.
    */
   ...
   /**
    * Formats and returns a `String` containing the linear regression slopes for
    * the discrepancies as function of the parameter.
    */
   public String regressionToString() {
      ...
      for (int j = 0; j < nbDisc; ++j)
         sb.append(PrintfFormat.s(15, discrepancies[j].toString()) + PrintfFormat.g(15, 6, regression[j])
               + PrintfFormat.NEWLINE);
      return sb.toString();
   }

   /**
    * Creates a file named `filename.tex` containing LaTeX code that can be
    * compiled by `pdfLaTeX` to a graph of the discrepancies as function of the
    * parameter.
    * 
    * @param filename name of the LaTeX file to be created (without the `.tex`
    *                 extension)
    */
   ...
   /**
    * Creates a file named `filename.dat` and writes in it the given `header`, a
    * table showing the discrepancies for the different values of the parameter and
    * the linear regression slopes.
    * 
   ...
   /**
    * Returns a `String` containing a table showing the discrepancies for the
