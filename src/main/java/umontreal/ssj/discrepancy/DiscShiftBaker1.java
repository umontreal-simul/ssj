 ...
 * @f{align}{ [\mathcal{D}(\mathcal{P})]^2 & = -1 + \frac{1}{n^2} \sum_{i=1}^n
 *            \sum_{j=1}^n \prod_{r=1}^s \left(1 - \frac{4\gamma_r^2}{3}
 *            \left[B_4(\{x_{ir} - x_{jr}\}) - B_4(\{\{x_{ir} -
 *            x_{jr}\}-1/2\})\right]\right. \nonumber \\ & \qquad\qquad{} -
 *            \frac{\gamma_r^4}{9} \left[7B_4(\{x_{ir} - x_{jr}\}) -
 *            2B_4(\{\{x_{ir} - x_{jr}\}-1/2\})\right] \tag{baker1} \\ &
 *            \qquad\qquad{} \left. {} - \frac{16\gamma_r^4}{45}
 *            \left[B_6(\{x_{ir} - x_{jr}\}) -B_6(\{\{x_{ir} -
 *            x_{jr}\}-1/2\})\right] \right)\nonumber,
 * @f} where @f$n@f$ is the number of points of @f$\mathcal{P}@f$, @f$s@f$ is
 *     the dimension of the points, @f$x_{ir}@f$ is the @f$r@f$-th coordinate of
 ...
 *                     <tt>util/Num</tt>), and the notation @f$\{x\}@f$ means
 *                     the fractional part of @f$x@f$, defined here as @f$\{x\}
 *                     = x \bmod1@f$. In one dimension, the formula simplifies
 *                     to
 * @anchor REF_discrepancy_DiscShiftBaker1_baker1dim1
 * @f{align}{ [\mathcal{D}(\mathcal{P})]^2 & = \frac{1}{n^2} \sum_{i=1}^n
 *            \sum_{j=1}^n \left[ - \frac{4\gamma^2}{3} \left[B_4(\{x_i - x_j\})
 *            - B_4(\{\{x_i - x_j\}-1/2\})\right]\right. \nonumber \\ &
 *            \qquad\qquad{} - \frac{\gamma^4}{9} \left[7B_4(\{x_i - x_j\}) -
 *            2B_4(\{\{x_i - x_j\}-1/2\})\right] \tag{baker1dim1} \\ &
 *            \qquad\qquad{} \left. {} - \frac{16\gamma^4}{45} \left[B_6(\{x_i -
 *            x_j\}) -B_6(\{\{x_i - x_j\}-1/2\})\right] \right]\nonumber.
 * @f} The discrepancy represents a worst-case error criterion for the
 *     approximation of integrals, when the integrands have a certain degree of
 *     smoothness and lie in a Hilbert space @f$\mathcal{H}@f$ with a
 ...
 *     \int_{[0,1]^v}d\mathbf{x}_v\left[\int_{[0,1]^{S-v}} \frac{\partial^{|u| +
...
public class DiscShiftBaker1 extends Discrepancy {
    ...
    * all the weights @f$\gamma_r = 1@f$. `points[i][r]` is the <tt>r</tt>-th
    * coordinate of point `i`. Indices `i` and `r` start at 0.
    */
   ...
   /**
    * Constructor with the @f$n@f$ points `points[i]` in @f$s@f$ dimensions, with
    * weights @f$\gamma_r = @f$ `gamma[r-1]`. `points[i][r]` is the <tt>r</tt>-th
    * coordinate of point `i`. Indices `i` and `r` start at 0.
    */
    ...
    * 
    * @f$s@f$ weight factors are <tt>gamma[</tt>@f$r@f$<tt>]</tt>, @f$r = 0, 1,
   ...
   /**
    * Computes the discrepancy ( {@link REF_discrepancy_DiscShiftBaker1_baker1
    * baker1} ) for the @f$s@f$-dimensional points of set `points`, containing
    * 
   ...
   /**
    * Computes the discrepancy ( {@link REF_discrepancy_DiscShiftBaker1_baker1
    * baker1} ) for the first @f$n@f$ points of `points` in dimension @f$s@f$ and
   ...
   /**
    * Computes the discrepancy ( {@link REF_discrepancy_DiscShiftBaker1_baker1dim1
    * baker1dim1} ) for the first @f$n@f$ points of @f$T@f$ in 1 dimension, with
    * weight @f$\gamma= 1@f$.
    */
