 ...
 *              hypercube @f$[0, 1)^s@f$ in @f$s@f$ dimensions. @f$\Psi_s@f$
 *              contains
 * @f$n@f$ points. For an arbitrary integer @f$\alpha> 1@f$, it is defined
 *         as @cite mSLO94a, @cite vHIC98c, @cite vHIC01a&thinsp; @f[
 *         P_{\alpha}(s) = \sum_{\mathbf{0\neq h}\in L_s^*}
 *         \|\mathbf{h}\|^{-\alpha}, @f] where @f$L_s^*@f$ is the lattice dual
 *         to @f$L_s@f$, and the norm is defined as @f$\|\mathbf{h}\| =
 *         \prod^s_{j=1} \max\{1, |h_j|\}@f$. When
 * @f$\alpha@f$ is even, @f$P_{\alpha}@f$ can be evaluated explicitly as
 * @anchor REF_discrepancy_Palpha_palpha_1 @f[ \qquad P_{\alpha}(s) = -1 +
 *         \frac{1}{n}\sum_{i=1}^n \prod_{j=1}^s \left[1 -
 *         \frac{(-1)^{\alpha/2}(2\pi)^{\alpha}}{\alpha!}
 *         B_{\alpha}(u_{ij})\right], \tag{palpha.1} @f] where @f$u_{ij}@f$ is
 *         the @f$j@f$-th coordinate of point @f$i@f$, and
 * @f$B_{\alpha}(x)@f$ is the Bernoulli polynomial of degree @f$\alpha@f$ (see
 *                     umontreal.ssj.util.Num.bernoulliPoly in class
 *                     <tt>util/Num</tt>).
 *
 *                     One may generalize the @f$P_{\alpha}@f$ by introducing a
 *                     weight for each dimension to give the
 *                     *weighted* @f$P_{\alpha}@f$ defined by
 * @cite vHIC98c&thinsp; @f[ P_{\alpha}(s) = \sum_{\mathbf{0\neq h}\in
 *       L_s^*}\beta_I^2 \|\mathbf{h}\|^{-\alpha}, @f] where the weights are
 *       such that @f$\beta_I =\beta_0\prod_{j=1}^s\beta_j^{\alpha}@f$, and for
 *       even @f$\alpha@f$
 * @anchor REF_discrepancy_Palpha_palpha_2 @f[ \qquad P_{\alpha}(s) = \beta_0
 *         \left\{-1 + \frac{1}{n}\sum_{i=1}^n \prod_{j=1}^s \left[1 -
 *         \frac{(-1)^{\alpha/2}(2\pi\beta_j)^{\alpha}}{\alpha!}
 *         B_{\alpha}(u_{ij})\right] \right\}. \tag{palpha.2} @f] One recovers
 *         the original criterion ( {@link REF_discrepancy_Palpha_palpha_1
 *         palpha.1} ) for
...
public class Palpha extends Discrepancy {
   ...
   /**
    * Constructor with @f$n@f$ points in @f$s@f$ dimensions and with `alpha` @f$=
    * \alpha@f$. `points[i][j]` is the @f$j@f$-th coordinate of point @f$i@f$. Both
    * `i` and `j` start at 0. The weights `beta[j]`, @f$j=0, 1,…, s@f$ are as in
    * eq. ( {@link REF_discrepancy_Palpha_palpha_2 palpha.2} ). The points and
    * dimensions in ( {@link REF_discrepancy_Palpha_palpha_2 palpha.2} ) are
    * 
   ...
   /**
    * Constructor with @f$n@f$ points in @f$s@f$ dimensions and with `alpha` @f$=
    * \alpha@f$. The @f$n@f$ points will be chosen later. The weights
    * `beta[j]`, @f$j=0, 1,…, s@f$ are as in eq. (
    * {@link REF_discrepancy_Palpha_palpha_2 palpha.2} ), with @f$\beta_j = @f$
    * `beta[j]`. Restriction: `alpha` @f$ \in\{2, 4, 6, 8\}@f$.
    */
   ...
   /**
    * Constructor with parameter `alpha` @f$= \alpha@f$. The points and parameters
    * *must* be defined before calling methods of this class. Restriction:
    * `alpha` @f$ \in\{2, 4, 6, 8\}@f$.
    */
    ...
    * <tt>beta[j]</tt>@f$=\beta_j@f$ and parameter `alpha` @f$= \alpha@f$. All the
    * points are copied in an internal array. Restriction: `alpha` @f$ \in\{2, 4,
    * 6, 8\}@f$.
    */
   ...
   /**
    * Computes the discrepancy ( {@link REF_discrepancy_DiscShift1Lattice_shift1lat
    ...
    * 
    * @f$\alpha=@f$ `alpha`. Restriction: `alpha` @f$ \in\{2, 4, 6, 8\}@f$.
    */
    ...
    * Computes the discrepancy ( {@link REF_discrepancy_DiscShift1Lattice_shift1lat
    * shift1lat} ) for the @f$s@f$-dimensional points of lattice `points`,
    * containing @f$n@f$ points, with weights @f$\beta_j=@f$ `beta[j]` and
    * with @f$\alpha=@f$ `alpha`. Restriction: `alpha` @f$ \in\{2, 4, 6, 8\}@f$.
    */
   ...
   /**
    * This method computes the value of @f$P_{\alpha}@f$ with @f$\alpha= 2@f$ for
    * the set of points in dimension @f$s@f$.
    */
   ...
   /**
    * This method computes the value of @f$P_{\alpha}@f$ with @f$\alpha= 4@f$ for
    * the set of points in dimension @f$s@f$.
    */
   ...
   /**
    * This method computes the value of @f$P_{\alpha}@f$ with @f$\alpha= 6@f$ for
    * the set of points in dimension @f$s@f$.
    */
   ...
   /**
    * This method computes the value of @f$P_{\alpha}@f$ with @f$\alpha= 8@f$ for
    * the set of points in dimension @f$s@f$.
    */
