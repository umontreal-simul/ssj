...
public class PolInterp extends Polynomial implements Serializable {
    ...
    * y[0]), ..., (x[n], y[n])`. This constructs a polynomial of degree `n` from
    * `n+1` points.
    * 
    ...
    * @exception IllegalArgumentException if the lengths of `x` and `y` are
    *                                     different, or if less than two points are
    *                                     specified.
    */
   ...
   /**
    * Computes and returns the coefficients the polynomial interpolating through
    * the given points `(x[0], y[0]), ..., (x[n], y[n])`. This polynomial has
    * degree `n` and there are `n+1` coefficients.
    * 
    * @param x the @f$x@f$ coordinates of the points.
    * @param y the @f$y@f$ coordinates of the points.
   ...
   /**
    * Calls {@link #toString(double[],double[]) toString(double[], double[])} with
