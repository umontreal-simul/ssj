 ...
 *                       and let @f$t_j@f$, for @f$j=0,…,m-1@f$ be a *knot*. A
 *                       B-spline
 * @cite mDEB78a&thinsp; of degree @f$p=m-n-1@f$ is a parametric curve defined
 *       as @f[ \mathbf{P(t)} = \sum_{i=0}^{n-1} N_{i, p}(t) \mathbf{P_i},\mbox{
 *       for }t_p\le t\le t_{m-p-1}. @f] Here,
 * @f{align*}{ N_{i, p}(t) & = \frac{t-t_i}{t_{i+p} - t_i}N_{i, p-1}(t) +
 *             \frac{t_{i+p+1} - t}{t_{i+p+1} - t_{i+1}}N_{i+1, p-1}(t) \\ N_{i,
 *             0}(t) & = \left\{ \begin{array}{ll} 1 & \mbox{ for }t_i\le t\le
 *             t_{i+1}, \\ 0 \mbox{ elsewhere}. \end{array} \right.
 * @f} This class provides methods to evaluate @f$\mathbf{P(t)}=(X(t), Y(t))@f$
 ...
 *     uses a root finder to determine the value of @f$t^*@f$ for
 *     which @f$X(t^*)=x@f$ before it computes @f$Y(t^*)@f$.
 *
...
public class BSpline implements MathFunction
   ...
   /**
    * Constructs a new uniform B-spline of degree `degree` with control points at
    * (<tt>x[i]</tt>, <tt>y[i]</tt>). The knots of the resulting B-spline are set
    * uniformly from `x[0]` to `x[n-1]`.
    * 
   ...
   /**
    * Constructs a new uniform B-spline with control points at (<tt>x[i]</tt>,
    * <tt>y[i]</tt>), and knot vector given by the array `knots`.
    * 
    * @param x     the values of @f$X@f$.
    * @param y     the values of @f$Y@f$.
    ...
    * 
    * @f$(x_i, y_i)@f$ points @cite mDEB78a&thinsp;. This method uses the uniformly
    *          spaced method for interpolating points with a B-spline curve, and a
    *          uniformed clamped knot vector, as described in
   ...
   /**
    * Returns a B-spline curve of degree `degree` smoothing @f$(x_i, y_i)@f$,
    * for @f$i=0,…,n@f$ points. The precision depends on the
    * parameter @f$hp1@f$: @f$1 \le\mathtt{degree} \le hp1<n@f$, which represents
    * the number of control points used by the new B-spline curve, minimizing the
    * quadratic error @f[ L = \sum_{i=0}^n\left( \frac{Y_i -
    * S_i(X_i)}{W_i}\right)^2. @f] This method uses the uniformly spaced method for
    * interpolating points with a B-spline curve and a uniformed clamped knot
   ...
   /**
    * Returns the derivative B-spline object of the current variable. Using this
    * function and the returned object, instead of the `derivative` method, is
    * strongly recommended if one wants to compute many derivative values.
    * 
    ...
    * variable; @f$i@f$ must be less than the degree of the original B-spline.
    * Using this function and the returned object, instead of the `derivative`
    * method, is strongly recommended if one wants to compute many derivative
    * values.
    * 
