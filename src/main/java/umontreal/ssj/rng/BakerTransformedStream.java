...
/**
 * This container class permits one to apply the baker’s transformation to the
 * output of any @ref RandomStream. It transforms each @f$u \in[0,1]@f$
 ...
 * other `next...` methods are affected accordingly. Any instance of this class
 * contains a @ref RandomStream called its *base stream*, used to generate its
 * numbers and to which the transformation is applied. Any call to one of the
 * `next...` methods of this class will modify the state of the base stream.
 *
 * The baker transformation is often applied when the @ref RandomStream is
 * actually an iterator over a point set used for quasi-Monte Carlo integration
 * (see the `hups` package).
 *
...
public class BakerTransformedStream implements RandomStream {
   ...
   /**
    * Constructs a new baker transformed stream, using the random numbers from the
    * base stream `stream`.
    */
   ...
   /**
    * Returns the baker transformation of `s.nextDouble()` where `s` is the base
    * stream.
    */
