 ...
 * `nextArrayOfDouble`, `nextInt` and <tt>nextArrayOfInt</tt>) call `nextValue`.
 * Thus, to define a subclass that implements a RNG, it suffices to implement
 * `nextValue`, in addition to the `reset...` and `toString` methods. Of course,
 * the other methods may also be overridden for improved efficiency.
 *
 * If the `nextValue` already generates numbers with a precision of 53-bits or
 * higher, then #nextDouble can be overridden to improve the performance. The
 * mechanism for increasing the precision assumes that `nextValue` returns at
 * least 29 bits of precision, in which case the higher precision numbers will
 * have roughly 52 bits of precision. This mechanism was designed primarily for
 * RNGs that return numbers with around 30 to 32 bits of precision.
 *
 * @ref RandomStreamBase and its subclasses are implementing the Serializable
 *      interface. Each class has a serial number wich represent the class
 *      version. For instance `70510` means that the last change was the `10th
 *      May 2007`.
 *
...
public abstract class RandomStreamBase implements CloneableRandomStream, Serializable {
   ...
   /**
    * After calling this method with `incp = true`, each call to the RNG (direct or
    * indirect) for this stream will return a uniform random number with more bits
    * of precision than what is returned by `nextValue`, and will advance the state
    * of the stream by 2 steps instead of 1 (i.e., `nextValue` will be called twice
    * for each random number).
    *
    * More precisely, if `s` is a stream of a subclass of `RandomStreamBase`, when
    * the precision has been increased, the instruction
    * "<tt>u = s.nextDouble()</tt>", is equivalent to "<tt>u =
    * (s.nextValue() + s.nextValue()*fact) % 1.0</tt>" where the constant `fact` is
    * equal to @f$2^{-24}@f$. This also applies when calling `nextDouble`
    * indirectly (e.g., via `nextInt`, etc.). By default, or if this method is
    * called again with `incp = false`, each call to `nextDouble` for this stream
    * advances the state by 1 step and returns the same number as `nextValue`.
    * 
   ...
   /**
    * This method should return the next random number (between 0 and 1) from the
    * current stream. If the stream is set to the high precision mode
    * (<tt>increasedPrecision(true)</tt> was called), then each call to
    * `nextDouble` will call `nextValue` twice, otherwise it will call it only
    * once.
    * 
   ...
   /**
    * Returns a uniform random number between 0 and 1 from the stream. Its behavior
    * depends on the last call to #increasedPrecision. The generators programmed in
    * SSJ never return the values 0 or 1.
    * 
    * @return a number in the interval (0,1)
    */
