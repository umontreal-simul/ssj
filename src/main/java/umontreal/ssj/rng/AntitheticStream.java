...
/**
 * This container class allows the user to force any @ref RandomStream to return
 * antithetic variates. That is, #nextDouble returns @f$1-u@f$ instead
 * of @f$u@f$ and the corresponding change is made in #nextInt. Any instance of
 * this class behaves exactly like a @ref RandomStream, except that it depends
 * on another random number generator stream, called the *base stream*, to
 * generate its numbers. Any call to one of the `next...` methods of this class
 * will modify the state of the base stream.
 *
...
public class AntitheticStream implements RandomStream {
   ...
   /**
    * Constructs a new antithetic stream, using the random numbers from the base
    * stream `stream`.
    */
