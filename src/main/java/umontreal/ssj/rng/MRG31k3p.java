 ...
 *      L’Ecuyer and Touzin @cite rLEC00b&thinsp;, implemented in 32-bit
 *      integer arithmetic. This RNG has a period length
 *      of @f$\rho\approx@f$ @f$2^{185}@f$. The values of @f$V@f$, @f$W@f$
 *      and @f$Z@f$ are @f$2^{62}@f$, @f$2^{72}@f$ and @f$2^{134}@f$
 *      respectively. (See
 * @ref RandomStream for their definition.) The seed and the state of a stream
 *      at any given step are six-dimensional vectors of 32-bit integers. The
 *      default initial seed is @f$(12345, 12345, 12345, 12345, 12345,
...
public class MRG31k3p extends RandomStreamBase {
   ...
   /**
    * Constructs a new stream with the identifier `name` (used when formatting the
    * stream state).
    * 
   ...
   /**
    * Sets the initial seed for the class `MRG31k3p` to the six integers of the
    * vector `seed[0..5]`. This will be the initial state (or seed) of the next
    * created stream. By default, if this method is not called, the first stream is
    * created with the seed @f$(12345, 12345, 12345, 12345, 12345, 12345)@f$. If it
    * is called, the first 3 values of the seed must all be less than @f$m_1 =
    * 2147483647@f$, and not all 0; and the last 3 values must all be less
    * than @f$m_2 = 2147462579@f$, and not all 0.
    * 
   ...
   /**
    * *Use of this method is strongly discouraged*. Initializes the stream at the
    * beginning of a stream with the initial seed `seed[0..5]`. This vector must
    * satisfy the same conditions as in `setPackageSeed`. This method only affects
    * the specified stream, all the others are not modified, so the beginning of
    * the streams are no longer spaced
    * 
    * @f$Z@f$ values apart. For this reason, this method should be used only in
    *         very exceptional situations; proper use of `reset...` and of the
    ...
    * integers represented. This method is convenient if we want to save the state
    * for subsequent use.
    * 
