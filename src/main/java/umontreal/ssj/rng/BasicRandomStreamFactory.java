...
/**
 * Represents a basic random stream factory that can constructs new instances of
 * a given @ref RandomStream implementation via the #newInstance method. The
 * class name of the implementation to be used must be passed to the constructor
 * as a `String`, which must be the name of a nullary constructor of a @ref
 * RandomStream object (i.e., a constructor that has no parameters). The streams
 * are constructed by the factory by reflection from this `String`.
 *
...
public class BasicRandomStreamFactory implements RandomStreamFactory {
    ...
    *                                     implementation of @ref RandomStream, or
   ...
   /**
    * Sets the associated random stream class to `rsClass`. The supplied class
    * object must represent an implementation of @ref RandomStream and must provide
   ...
   private void checkRandomStreamClass(final Class rsClass) {
      ...
      try {
         ...
         throw new IllegalArgumentException(
               "The random stream class " + rsClass.getName() + " does not have a " + "nullary public constructor.");
      }
   }
   ...
   public RandomStream newInstance() {
      try {
         ...
         throw new RandomStreamInstantiationException(this,
               "Cannot access constructor for random stream class " + rsClass.getName(), iae);
      } catch (InstantiationException ie) {
         throw new RandomStreamInstantiationException(this,
               "Cannot instantiate random stream class " + rsClass.getName(), ie);
      } catch (Exception e) {
         throw new RandomStreamInstantiationException(this,
               "Exception while calling the nullary constructor for random stream class " + rsClass.getName(), e);
      }
   }
