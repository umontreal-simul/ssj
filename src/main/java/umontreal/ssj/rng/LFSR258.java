...
/**
 * Extends @ref RandomStreamBase using a 64-bit composite linear feedback shift
 * register (LFSR) (or Tausworthe) RNG as defined in @cite rLEC96a,
 * 
 * @cite rTEZ91b&thinsp;. This generator is the `LFSR258` proposed in
 * @cite rLEC99a&thinsp;. It has five components combined by a bitwise xor. Its
 *       period length is @f$\rho\approx2^{258}@f$. The values of @f$V@f$,
 * @f$W@f$ and @f$Z@f$ are @f$2^{100}@f$, @f$2^{100}@f$ and @f$2^{200}@f$
 *         respectively (see @ref RandomStream for their definition). The seed
 *         of the RNG, and the state of a stream at any given step, are
 *         five-dimensional vectors of 64-bit integers. The default initial seed
 *         <sup title="In previous versions, it was (1234567890, 1234567890,
 *         1234567890, 1234567890, 1234567890).">[1]</sup> of the RNG is
 *         (123456789123456789, 123456789123456789, 123456789123456789,
 *         123456789123456789, 123456789123456789). The `nextValue` method
 *         returns numbers with 53 bits of precision. This generator is fast for
 *         64-bit machines.
 *
...
public class LFSR258 extends RandomStreamBase {
   ...
   /**
    * Sets the initial seed for the class `LFSR258` to the five integers of array
    * `seed[0..4]`. This will be the initial state of the next created stream. The
    * default seed
    * <sup title="In previous versions, it was (1234567890, 1234567890, 1234567890,
    * 1234567890, 1234567890).">[2]</sup> for the first stream is
    * (123456789123456789, 123456789123456789, 123456789123456789,
    * 123456789123456789, 123456789123456789). The first, second, third, fourth and
    ...
    * beginning of a stream with the initial seed `seed[0..4]`. The seed must
    * satisfy the same conditions as in `setPackageSeed`. This method only affects
    * the specified stream; the others are not modified, so the beginning of the
    * streams will not be spaced
    * 
    * @f$Z@f$ values apart. For this reason, this method should only be used in
