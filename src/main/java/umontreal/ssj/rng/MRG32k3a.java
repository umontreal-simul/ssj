...
/**
 * Extends the abstract class @ref RandomStreamBase by using as a backbone (or
 * main) generator the combined multiple recursive generator (CMRG) `MRG32k3a`
 * proposed by L’Ecuyer @cite rLEC99b&thinsp;, implemented in 64-bit
 * floating-point arithmetic. This backbone generator has a period length
 * of @f$\rho\approx2^{191}@f$. The values of @f$V@f$, @f$W@f$, and
 * 
 * @f$Z@f$ are @f$2^{51}@f$, @f$2^{76}@f$, and @f$2^{127}@f$, respectively.
 *         (See @ref RandomStream for their definition.) The seed of the RNG,
 ...
 *         seed of the RNG is @f$(12345, 12345, 12345, 12345, 12345, 12345)@f$.
 *
...
public class MRG32k3a extends RandomStreamBase {
    ...
    * 
    * @f$B_g@f$ and @f$C_g@f$ equal to @f$I_g@f$, and sets its antithetic switch to
    *           `false`. The seed @f$I_g@f$ is equal to the initial seed of the
    *           package given by #setPackageSeed(long[]) if this is the first
    *           stream created, otherwise it is @f$Z@f$ steps ahead of that of the
    *           stream most recently created in this class.
    */
   public MRG32k3a() {
      name = null;
      anti = false;
   ...
   /**
    * Constructs a new stream with an identifier `name` (used when printing the
    * stream state).
    * 
   ...
   /**
    * Sets the initial seed for the class `MRG32k3a` to the six integers in the
    * vector `seed[0..5]`. This will be the seed (initial state) of the first
    * stream. If this method is not called, the default initial seed is @f$(12345,
    * 12345, 12345, 12345, 12345, 12345)@f$. If it is called, the first 3 values of
    * the seed must all be less than @f$m_1 = 4294967087@f$, and not all 0; and the
    * last 3 values must all be less than @f$m_2 = 4294944443@f$, and not all 0.
    * 
   ...
   /**
    * Sets the initial seed @f$I_g@f$ of this stream to the vector `seed[0..5]`.
    * This vector must satisfy the same conditions as in `setPackageSeed`. The
    * stream is then reset to this initial seed. The states and seeds of the other
    * streams are not modified. As a result, after calling this method, the initial
    * seeds of the streams are no longer spaced @f$Z@f$ values apart. For this
    * reason, *this method should be used only in very exceptional situations* (I
    * have never used it myself!); proper use of `reset...` and of the stream
   ...
   /**
    * Returns a string containing the name and the current state @f$C_g@f$ of this
    * stream.
    * 
   ...
   /**
    * Returns a string containing the name of this stream and the values of all its
    * internal variables.
    * 
