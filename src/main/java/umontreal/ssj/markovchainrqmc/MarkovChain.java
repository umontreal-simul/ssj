 ...
 * replicates in parallel. In a concrete subclass, it suffices to implement the
 ...
 * getPerformance()} to get things going. One would usually implement subclasses
 * of @ref MarkovChainComparable or
 * 
 * @ref MarkovChainDouble rather than direct subclasses of `MarkovChain`. Some
 *      other methods are then needed. It is also *essential* to override the
 *      method `clone`, if the class contains non primitive objects, in order to
 *      clone these objects.
 *
 *      The methods in this class permit one to simulate the chain over a
 *      certain number of steps via Monte Carlo or randomized quasi-Monte Carlo.
 *      Statistics on the performance measure of the chain are computed during
 *      these simulations.
 *
...
public abstract class MarkovChain implements Cloneable {
   ...
   /**
    * Sets the Markov chain to its (deterministic) initial state and initializes
    * the collectors for the performance measure, ready to start a simulation. Must
    * also set variable `stopped` to false.
    */
   public abstract void initialState();

   /**
    * Simulates one more step of the chain, from its current state, using `stream`
    * for the randomness. If the chain stops, this method must set variable
    * `stopped` to true.
    */
   public abstract void nextStep(RandomStream stream);

   /**
    * Returns the performance measure (total or average cost or gain) so far, for
   ...
   /**
    * Starts a new simulation run and simulates `numSteps` steps of the Markov
    * chain or until the chain stops, using the given `stream`.
    */
   ...
   /**
    * Starts a new simulation run and simulates until the stopping time is reached,
    * using the given <tt>stream</tt>. Same as {@link #simulSteps()
   ...
   /**
    * Performs `n` simulation runs of the chain, for `numSteps` steps per run,
    * using the given `stream`. The statistics on the performance for the `n` runs
    * are placed in `statRuns`.
    */
   ...
   /**
    * Same as #simulRuns, except that the stream is first reset to its initial seed
    * and then reset to the first substream at the beginning and to the next
    * substream after each run.
    */
   ...
   /**
    * Perform n simulation runs of the chain, each for numSteps steps, and returns
    * average.
    */
   ...
   /**
    * Perform n runs and compute the average, reapeat m times and return the stats
    * in t.
    */
   ...
   /**
    * Performs `m` independent replicates of @f$n@f$ simulation runs of the chain
    * using a RQMC point set, each time storing the average of the performance over
    * the @f$n@f$ chains. @f$n@f$ is the number of points in RQMC point set `p`.
    * Each run goes for `numSteps` steps. For each replicate, the point set `p` is
    * randomized using `rand`, an iterator is created, and each run uses a
    * different substream of this iterator (i.e., a different point). The
    * statistics on the performance for the `m` independent replications are placed
    * in `statReps`.
    */
   ...
   /**
    * Same as #simulRunsWithSubstreams but also returns the results as a formatted
    * string.
    */
   ...
   /**
    * Similar to #simulRQMCFormat, but also gives the variance improvement factor
    * with respect to MC. Assuming that `varMC` gives the variance per run for MC.
    */
   public String testImprovementRQMCFormat(PointSet p, int m, int numSteps, PointSetRandomization rand, double varMC,
         Tally statReps) {
      // Removed next line because numSteps may be infinite!
      // p.randomize (0, numSteps * dimPerStep, noise);
      StringBuffer sb = new StringBuffer(simulRQMCFormat(p, m, numSteps, rand, statReps));
   ...
   /**
    * Returns a string containing the mean, the variance, and a 90% confidence
    * interval for `stat`.
    */
   ...
   /**
    * Returns a string containing the mean, the variance multiplied by `numPoints`,
    * and a 90% confidence interval for `stat`.
    */
   public String formatResultsRQMC(Tally stat, int numPoints) {
      StringBuffer sb = new StringBuffer(" Average value             = ");
      sb.append(PrintfFormat.format(12, 9, 5, stat.average()) + PrintfFormat.NEWLINE);
      sb.append(" Variance * numPoints      = ");
      sb.append(PrintfFormat.format(12, 9, 5, numPoints * stat.variance()) + PrintfFormat.NEWLINE);
      sb.append(stat.formatCIStudent(0.9, 7));
      return sb.toString();
   }
