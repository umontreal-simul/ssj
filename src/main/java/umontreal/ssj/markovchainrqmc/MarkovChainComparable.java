 ...
 * 
 * @ref umontreal.ssj.util.MultiDimComparable interface. A list or array of
 *      `MarkovChainComparable` objects can be sorted according to their states
 *      at a given step, using a @ref umontreal.ssj.util.MultiDimComparator and
 *      an external sorting method.
 *
 ...
 *      is considered smaller than, equal to, or greater than the other chain
 *      `m` in dimension `i`. The meaning of smaller, equal or larger is very
 *      flexible as well as the meaning of dimension.
 *
 *      Concrete subclasses must implement `compareTo` and must initialize the
