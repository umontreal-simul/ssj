 ...
 * @f$\ell@f$-dimensional state. At each step, the @f$n@f$ states are sorted in
 *                        some way and then advance to the next state by using
 *                        the RQMC points. If @f$\ell=1@f$, we just une an
 *                        ordinary sort. If @f$\ell> 1@f$, there are two
 *                        possibilities. The first one is to define a mapping
 *                        from the state space to the real numbers, called a
 *                        sorting function, and sort according to the value of
 *                        this sorting function. We end up sorting
 *                        in @f$\ell’=1@f$ dimensions. The second possibility
 *                        is to use a multivariate sort of the type
 * @ref umontreal.ssj.util.multidimsort.MultiDimSort, which in general can
 *      sort @ref umontreal.ssj.util.multidimsort.MultiDimComparable objects.
 *      Special cases of these multivariate sorts are of type
 * @ref umontreal.ssj.util.multidimsort.MultiDimSort01. Some of them effectively
 *      map the chain states to the one-dimensional interval
 * @f$(0,1)@f$. For example, the sorts based on a Hilbert curve, such as
 * @ref umontreal.ssj.util.multidimsort.HilbertCurveSort, do that. In that case,
 *      we end up again sorting in @f$\ell’=1@f$ dimensions. Otherwise, we
 *      sort in @f$\ell’ \leq \ell@f$ dimensions.
 *
 ...
 *      used to sort the points in the same way as the states of the chains, to
 *      match each point with a state and vice-versa.
 * 
 *      When @f$\ell’=1@f$, we are also allowed to use
 *      only @f$d@f$-dimensional RQMC points exactly as in @ref
 *      ArrayOfDoubleChains. That is, the points are assumed to be already
 *      sorted by a virtual coordinate which is not stored explicitly, and can
 *      be assumed to be @f$i/n@f$ for state @f$i@f$. In this case, we can take
 *      for example the first @f$n@f$ points of a Sobol’ sequence, or a rank-1
 *      lattice for which the first coordinate is dropped. To choose this
 *      option, we must pass `sortCoordPts=0` when running the Array-RQMC
 *      simulations. Otherwise, the points will be sorted by their first
 *      `sortCoordPts` coordinates, where `sortCoordPts` should be the value
 *      of @f$\ell’@f$. When `sortCoordPts > 0`, the current implementation
 *      also imposes that the point set is a @ref
 *      umontreal.ssj.hups.CachedPointSet, and the RQMC points are sorted at
 *      each step. This constraint can be satisfied by using a
 * @ref umontreal.ssj.hups.SortedAndCutPointSet.
 * 
 *      At each step the RQMC points are randomized using a @ref
 *      umontreal.ssj.hups.PointSetRandomization. There are types of point sets
 *      for which only the last @f$d@f$ coordinates have to be randomized (e.g.,
 *      digital nets and lattice rules) and others for which all @f$\ell’ +
...
public class ArrayOfComparableChains<T extends MarkovChainComparable> {
   ...
   /**
    * Same as above, but produces a plot of the mean and the variance as a function
    * of the step.
    * 
    * @param rqmcPts
    * @param sort
