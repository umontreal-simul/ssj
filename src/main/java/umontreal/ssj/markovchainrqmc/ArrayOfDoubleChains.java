 ...
 *      one step. The performance measure is assumed to be additive over all
 *      steps of all copies of the chain. The sum is maintained in a *single*
 *      accumulator for all copies of the chain, updated at each step of each
 *      copy.
 * 
 *      Another difference with @ref ArrayOfComparableChains is that here, no
 *      coordinate of the RQMC points is used for the sort. The points are
 *      assumed to be sorted correctly but not by their first coordinate. It is
 *      assumed that the points are sorted by a virtual (implicit) extra
 *      coordinate which is @f$i/n@f$ for point @f$i@f$, so the first coordinate
 *      must *not* be @f$i/n@f$.
 *
...
public class ArrayOfDoubleChains extends ArrayOfComparableChains {
   ...
   /**
    * Same as {@link #ArrayOfDoubleChains(MarkovChainDouble,PointSetRandomization)
   ...
   /**
    * Creates the vector of states for <tt>n</tt> copies of the base chain.
    */
   ...
   /**
    * Simulate one step for the `n` copies of the base chain, assuming that we are
    * at step `step`. The points are randomized before the simulation using the
    * stored
    * 
    * @ref umontreal.ssj.hups.PointSetRandomization. The dimension of `p` must be
    *      at least as large as the number of uniforms required to simulate one
    *      step of the chain. Returns true if and only if all chains have stopped.
   ...
   /**
    * Simulates the @f$n@f$ copies of the chain, <tt>numSteps</tt> steps for each
    * copy, using point set <tt>p</tt>, where @f$n@f$ is the current number of
    * copies of the chain and is *assumed* to equal the number of points in
    * <tt>p</tt>. At each step, the points are randomized using the stored
    * 
    * @ref umontreal.ssj.hups.PointSetRandomization. The dimension of <tt>p</tt>
    *      must be at least as large as the number of uniforms required to simulate
    *      one step of the chain. That is, no coordinate is used for the sort, so
    *      the user must be careful that the points of <tt>p</tt> are *not* sorted
    *      by their first coordinate. Returns the average performance per run.
    */
