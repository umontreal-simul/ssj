...
/**
 * This class is custom-made for the `CallableBond` example. It generates an
 * Ornstein-Uhlenbeck process as in @ref
 * umontreal.ssj.stochprocess.OrnsteinUhlenbeckProcess, but it also computes the
 * integral of the process, $\int_{t_0}^{t} X(\tau) d\tau$. The 2 x 2
 * correlation matrix for the process R(t) and its integral is decomposed with
 * PCA.
 */
public class OrnsteinUhlenbeckWithIntegratedProcess extends OrnsteinUhlenbeckProcess {
   ...
   /**
    * Sets up various constants and decomposes the correlation matrix at each time
    * step.
    */
   @Override
    ...
    * step a stochastic variable that corresponds to the integral of the process
    * over the time interval: $\int_{t_{i-1}}^{t_{0}} X(t) dt$. Those integrated
    * values are not returned by this method, but are stored in memory and can be
    * retrieved with {@link #getIntegratedPath() getIntegratedPath()}.
    * <p>
    * Two random uniforms (from the inner RandomStream) are required to generate
    * each time step instead of just one for the parent class.
    * 
   ...
   /**
    * {@code generatePath} must be called before this method is called since it
    * depends on the underlying {@code path}.
    *
    * @param couponTimes The times at which the coupons are disbursed, which are
    *                    later (by noticePeriod) than the time at which the
    *                    decisions are taken (the times used to generate the values
    *                    in this class). {@code couponTimes[0]} is meaningless (no
    *                    coupon). The order should correspond to the observation
    *                    times of the process. If {@code couponTimes} has more
    *                    elements than the number of time steps of the process, the
    *                    expectation of those additional terms is computed relative
    *                    to the last time step of the process.
    * 
    * @return $E[exp(-\int_{\tau_m}^{_0} R(t) dt) | R(\tau_m),\int_{\tau_m}^{_0}
    *         R(t) dt) ]$ where $\tau_m$ is one of the {@code observationTimes} of
    *         this {@code StochasticProcess} and $t_m$ is the corresponding
    *         {@code couponTimes}. $t_m$ should be larger than $\tau_m$ by one
    *         notice period. The first element of {@code couponTimes} and the
    *         returned array are meaningless; the returned value at {@code [0]} is
    *         {@code 0.0}.
    */
   ...
   /**
    * This method is independent of the observation times of the object's instance,
    * but it is dependent on the other parameters of the process. It returns an
    * analytic discount, which is the averaging discount at each time step..
    * 
    * @param times The times for which we want the total discounting (back to start
    *              time).
    * @return $E[exp(-\int_{t_0}^{t_m} R(t) dt) | R(t_0)]$ where $t_m$ is one of
    *         the observation times. The first element of the returned array is
    *         1.0.
    */
