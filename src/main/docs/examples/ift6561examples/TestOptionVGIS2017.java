 ...
 * @author P. L'Ecuyer
 *
 *         European option under VG process, simulated via difference of two
 *         gamma variables. Implemements standard MC, IS with exponential
 *         twisting, and IS + twisting + conditioning. All with either MC or
 *         RQMC.
 */

public class TestOptionVGIS2017 {
   ...
   double dim = 2;
   double mu = -0.1; // Mean BM
   double sigma = 0.15; // Volatility of BM
   double theta = mu;
   double nu = 0.2; // Variance parameter of gamma process
   double omega;
   double r = 0.05; // Interest rate
   double K = 52.0; // Strike
   double s0 = 50.0; // Initial value of VG
   ...
   public TestOptionVGIS2017(double mu, double sigma, double nu, double r, double K, double s0, double T,
         double theta0) {
   ...
   public class OptionVGIS2 extends OptionVG1 {
      ...
      public void simulate(RandomStream stream) {
         ...
         double L = factL2 * Math.exp(theta0 * gneg) * distPos.barF(gneg + cis2);
         value = (L * discount * (s0 * Math.exp(r + omega + gpos - gneg) - K));
         if (value < 0.0)
            value = 0.0;
      }
   ...
   public static void main(String[] args) {
      ...
      double K = 70.0; // Strike
      // double theta0 = 0.0; // IS twisting parameter 17.32 25.56
      // double theta0 = 17.32; // for K = 140
      double theta0 = 12.40; // for K = 70

      // K = 90; theta0 = 17.57; // for K = 90

      TestOptionVGIS2017 test = new TestOptionVGIS2017(mu, sigma, nu, r, K, s0, T, theta0);
...
//		int numSkipReg = 0;
      ...
      DigitalNetBase2 pSobol = (new SobolSequence(14, 31, dim - 1)).toNetShiftCj(); // 2^{14} points.
      ...
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vg1, pSobol, randLMS, m, statValue);
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vgis1, pSobol, randLMS, m, statValue);
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vgis2, pSobol, randLMS, m, statValue);

      System.out.println("------------------------------------\n" + "Sobol points with NUS. \n ");
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vg1, pSobolCached, randNUS, m, statValue);
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vgis1, pSobolCached, randNUS, m, statValue);
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vgis2, pSobolCached, randNUS, m, statValue);

      System.out.println("------------------------------------\n" + "Lattice + bakerS. \n ");
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vg1, pKorBaker, randShift, m, statValue);
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vgis1, pKorBaker, randShift, m, statValue);
      TestOptionVGIS2017.simulReplicatesRQMCReport(test.vgis2, pKorBaker, randShift, m, statValue);
