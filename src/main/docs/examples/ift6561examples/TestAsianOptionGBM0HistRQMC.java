...
//		int n = 65536, k = 16;
		...
		TallyStore statValuePosRQMC = new TallyStore(
				"Stats on positive discounted payoffs only, RQMC", n);
		...
		System.out.println("Total CPU time:      " + timer.format() + "\n");
		fractionZero = 1.0 - (double) statValuePos.numberObs()
				/ (double) n;
		System.out.printf("Proportion of zero payoffs: %12.6f%n", fractionZero);
		
	    hist = new HistogramChart(
				"Distribution of positive discounted payoff RQMC", "Payoff",
				"Frequency", statValuePosRQMC.getArray(), statValuePosRQMC.numberObs());
