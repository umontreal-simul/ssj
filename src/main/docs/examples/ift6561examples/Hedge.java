...
public class Hedge {
   ...
   public Hedge(double r, double sigma, double strike, double s0, int s, double[] zeta) {
      this.r = r;
      this.sigma = sigma;
      this.strike = strike;
      this.s = s;
   ...
   public static double getExpectedPayoff(double r, double sigma, double strike, double s0, double time) {
      if (time <= 0)
   ...
   public static double getDeltaExpectedPayoff(double r, double sigma, double strike, double s0, double time) {
      if (time <= 0) {
         if (s0 < strike)
            return 0;
         return 1;
   ...
   public void simulateRuns(int n, RandomStream stream) {
      ...
      System.out.printf("Expected payoff: %f%n", getExpectedPayoff(r, sigma, strike, Math.exp(logS[0]), zeta[s]));

      double[][] data = new double[][] { statPayoff.getArray(), statPortfolio.getArray() };
      new ScatterChart("Hedging of an option at " + (zeta.length - 1) + " times", "g(s(T))", "p(T)", data).view(800,
            600);
      new HistogramChart("Distribution of the loss, for " + (zeta.length - 1) + " hedging points", "g(s(T)) - p(T)",
            "Count", statLoss.getDoubleArrayList()).view(800, 600);
   }
