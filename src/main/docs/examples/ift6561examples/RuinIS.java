...
public class RuinIS {
   ...
   public RuinIS(double c, int n) {
      ...
      genSizes = new RandomVariateGen(new MRG32k3a(), new ExponentialDist(betaIs));
      for (int i = 0; i < n; i++) {
         statIs.add(simulRuin(c));
      }
      System.out.println(" lambda = " + PrintfFormat.format(8, 3, 1, lambda));
      System.out.println(" beta   = " + PrintfFormat.format(8, 3, 1, beta));
      System.out.println(" c      = " + PrintfFormat.format(8, 3, 1, c));
