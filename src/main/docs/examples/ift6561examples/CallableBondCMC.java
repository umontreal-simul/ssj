...
/**
 * Estimates the value of a callable bond where the coupons are paid at the same
 * time as in {@code CallableBond}, but where there is a single call back
 * opportunity at 10.0054 years (the 11th coupon).
 */
public class CallableBondCMC extends CallableBond {
   /**
    * The number of coupons after the last decision (excluding the coupon right
    * after that decision).
    */
   int nPostDecisionCoupons;
   /**
    * All coupon times after the decision (includes time 0.0 and the coupon right
    * after the decision).
    */
   double[] postDecisionPaymentTimes;
   /**
    * As above, but in the case when the bond is called back, such that only the
    * times right after the last decision are needed.
    */
   ...
   /**
    * {@code resetNextSubstream} is called after each simulation such that common
   ...
   public static void main(String[] args) {
      ...
      System.out.println(MonteCarloExperiment.simulateRunsDefaultReportStudent(bond, n, new MRG32k3a(), statValue));
      ...
      System.out.printf("%n*** variance reduction factor from CMC = %.1f%n%n%n",
            (statValue.variance() / tallyCMC.variance()));
