...
public class TestAsianOptionGBM2QMC {
   ...
   public void experOneProcessRQMC(int m, RandomStream noise) {
      ...
      if (asian.getNumObsTimes() <= 2) {
         RQMCExperiment.simulReplicatesRQMCDefaultReportCompare(asian, pStrat, rShift, m, statRQMC, varianceMC,
               secondsMC);
      }
      RQMCExperiment.simulReplicatesRQMCDefaultReportCompare(asian, pLCG, rShift, m, statRQMC, varianceMC, secondsMC);
      // RQMCExperiment.simulateRQMCDefaultReportCompare(asian, pLCGBaker,
      // rShift, m, noise, statRQMC, varianceMC, secondsMC);
      RQMCExperiment.simulReplicatesRQMCDefaultReportCompare(asian, pSobol, dShift, m, statRQMC, varianceMC, secondsMC);
      Chrono timer = new Chrono();
      RQMCExperiment.simulReplicatesRQMC(asian, pLCG0, noShift, m, statRQMC);
      System.out.println(pLCG0.toString());
      System.out.println("Total CPU time:      " + timer.format() + "\n");
      System.out.printf("Average (deterministic): %10.6g%n%n", statRQMC.average());
   }
   ...
   public static void main(String[] args) {
      ...
      Chrono timer = new Chrono();
      MonteCarloExperiment.simulateRunsDefaultReportStudent(asian, n, noise, statValueMC, 0.95, 4, timer);
